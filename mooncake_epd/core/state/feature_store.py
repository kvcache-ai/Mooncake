    ...
    def from_bundle(
        ...
        *,
        ...
        if bundle.grid_thw is not None:
            # grid_thw is tiny semantic routing/model metadata rather than a
            # bandwidth-bearing hidden state. Carry its authoritative values
            # in the control-plane descriptor so direct consumers can rebuild
            # it locally instead of trusting a separate 24-byte DMA. The
            # TensorSpec remains in the ABI for shape/dtype validation and for
            # backwards compatibility with older producers.
            metadata["grid_thw_values"] = (
                bundle.grid_thw.detach().to(device="cpu").tolist()
            )
        return cls(
