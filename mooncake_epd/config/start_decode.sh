...
CUDA_VISIBLE_DEVICES=4 vllm serve "${MOONCAKE_EPD_MODEL_PATH:-/data01/LWX/Qwen3-VL-8B-Instruct}" --port 8200 --tensor-parallel-size 1 --max-model-len 4096 --gpu-memory-utilization 0.65 --kv-transfer-config '{"kv_connector":"MooncakeConnector","kv_role":"kv_consumer","engine_id":"epd-decode","kv_connector_module_path":"mooncake_epd.core.control.vllm_mooncake_connector","kv_connector_extra_config":{"mooncake_protocol":"tcp","num_workers":4,"layered_kv_transfer":true,"layers_per_group":4,"group_delay_ms":0.0,"max_group_bytes":0,"transport_backend":"mooncake_engine_direct"}}'
