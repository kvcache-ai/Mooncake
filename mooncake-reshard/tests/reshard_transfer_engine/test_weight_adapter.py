...
def test_weight_reader_and_sink_accept_only_operation_scoped_peers() -> None:
    source_placement, source_bindings = placement_and_bindings("source", 0x1000)
    target_placement, target_bindings = placement_and_bindings("target", 0x3000)
    plan = bind_logical_transfer_plan(
    ...
    read_engine = RecordingEngine()
    read_receipts = MooncakeTransferEngineReader(read_engine).execute(
        plan,
        source_placement,
        (source,),
        target_placement,
        ...
        source_allocation_guards=allocation_guards_for_bindings((source,)),
        target_allocation_guards=allocation_guards_for_bindings((target,)),
    )

    write_engine = RecordingEngine()
    write_receipts = MooncakeTransferEngineSink(write_engine).execute(
        plan,
        source_placement,
        source,
        target_placement,
    ...
    assert [
        (receipt.target_worker_id, receipt.nbytes) for receipt in read_receipts
    ] == [("target-d0-p0-e0-t0", 8)]
    assert [
        (receipt.source_worker_id, receipt.nbytes) for receipt in write_receipts
    ] == [("source-d0-p0-e0-t0", 8)]
    assert len(read_engine.calls) == 1
    assert len(write_engine.calls) == 1
