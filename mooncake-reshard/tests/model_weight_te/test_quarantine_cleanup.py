...
def test_te_pending_transfer_cannot_drain_before_resource_handoff(
    ...
    original_seal = pending_owner._seal_pending_resource_handoff
    seal_entered = threading.Event()
    allow_seal = threading.Event()
    pending_ids = []

    def block_resource_handoff_seal(pending_transfer_id: str) -> None:
        pending_ids.append(pending_transfer_id)
        seal_entered.set()
        assert allow_seal.wait(timeout=2)
        original_seal(pending_transfer_id)

    pending_owner._seal_pending_resource_handoff = block_resource_handoff_seal
    execution_errors = []
    ...
    worker.start()
    assert seal_entered.wait(timeout=2)
