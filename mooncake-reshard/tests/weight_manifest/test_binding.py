...
def test_runtime_binding_inventory_retains_owner() -> None:
    ...
    inventory = {
        ...
        "participant_id": placement.parts[0].participant_id,
        "instance_id": "instance",
        "generation": 7,
        "lease_id": "lease-7",
        "fragments": (runtime_inventory_tensor(),),
    }
    owner = object()

    binding = WeightRuntimeBindingManifest.from_runtime_inventory(
        inventory,
...
def test_runtime_binding_inventory_requires_stride_proof() -> None:
    placement = placement_manifest()
    inventory = {
        "model_id": MODEL_ID,
        "revision": REVISION,
        ...
        "fragments": (
            {
                key: value
                for key, value in runtime_inventory_tensor().items()
                if key != "stride"
            },
        ),
    }
    ...
    with pytest.raises(ValueError, match="stride"):
        WeightRuntimeBindingManifest.from_runtime_inventory(inventory)


def test_runtime_binding_inventory_does_not_trust_contiguous_flag() -> None:
    placement = placement_manifest()
    fragment = runtime_inventory_tensor(
        stride=(1, 4),
        is_contiguous=True,
    )
    inventory = {
    ...
    binding = WeightRuntimeBindingManifest.from_runtime_inventory(inventory)

    with pytest.raises(ValueError, match="stride"):
        validate_runtime_binding(placement, binding)


def test_runtime_binding_inventory_does_not_trust_runtime_itemsize() -> None:
    placement = placement_manifest()
    fragment = runtime_inventory_tensor(
        itemsize=1,
        stride=(8, 2),
...
def test_runtime_binding_inventory_rejects_fragment_generation_mismatch() -> None:
    ...
    inventory = {
        ...
        "generation": 7,
        "lease_id": "lease-7",
        "fragments": (runtime_inventory_tensor(lease_generation=6),),
    }
...
def test_runtime_binding_inventory_rejects_unsafe_views(
    ...
    placement = placement_manifest()
    fragment = runtime_inventory_tensor(**{field: value})
    inventory = {
        "model_id": MODEL_ID,
        "revision": REVISION,
        "placement_id": placement.placement_id,
...
def test_runtime_binding_inventory_requires_normalized_storage_address() -> None:
    placement = placement_manifest()
    fragment = runtime_inventory_tensor(
        address=0x1008,
        storage_offset=4,
    ...
    with pytest.raises(ValueError, match="storage_address"):
        WeightRuntimeBindingManifest.from_runtime_inventory(inventory)

    fragment["storage_address"] = 0x1000
    fragment["storage_nbytes"] = 64
    binding = WeightRuntimeBindingManifest.from_runtime_inventory(
        inventory,
    )

    assert binding.fragments[0].address == 0x1008
    assert binding.fragments[0].storage_address == 0x1000
    assert binding.fragments[0].storage_offset_bytes == 8
    assert binding.fragments[0].storage_nbytes == 64


def test_runtime_binding_inventory_rejects_misnormalized_view_address() -> None:
    placement = placement_manifest()
    inventory = {
        "model_id": MODEL_ID,
        "revision": REVISION,
    ...
    with pytest.raises(ValueError, match="storage_address plus"):
        WeightRuntimeBindingManifest.from_runtime_inventory(inventory)


def test_runtime_binding_inventory_rejects_view_outside_storage() -> None:
    placement = placement_manifest()
    inventory = {
        "model_id": MODEL_ID,
        "revision": REVISION,
