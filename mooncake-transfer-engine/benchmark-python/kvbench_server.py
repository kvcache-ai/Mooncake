...
def run_server_mooncake(buf_size: int, num_gpus: int, zmq_port: int):
    te = TransferEngine()
    ...
    for dev in range(num_gpus):
        ...
        print(f"[SERVER/RDMA] cuda:{dev} @ {addr:#x}")
...
def run_server_nixl(buf_size: int, num_gpus: int, zmq_port: int):
    ...
    for dev in range(num_gpus):
        ...
        buf = torch.zeros(buf_size // 4, dtype=torch.float32, device=f"cuda:{dev}")
        addr = buf.data_ptr()
        reg_descs = agent.get_reg_descs([(addr, buf_size, 0, "cuda:" + str(dev))], "DRAM")
        ...
        buf_addrs.append(addr)
        print(f"[SERVER/NIXL] cuda:{dev} @ {addr:#x}")
    ...
    print(f"[SERVER/NIXL] export meta & xfer_descs: GPUs={num_gpus}, buf={buf_size}")
    send_handshake(info, port=zmq_port)
...
def main():
    ...
    if a.backend == "mooncake":
        run_server_mooncake(a.buf_size, a.num_gpus, a.zmq_port)
    else:
        run_server_nixl(a.buf_size, a.num_gpus, a.zmq_port)
