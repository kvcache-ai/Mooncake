...
static void *allocateMemoryPool(size_t size, int socket_id) {
	void *start_addr;
	start_addr = mmap((void *)BASE_ADDRESS_HINT, size, PROT_READ | PROT_WRITE, MAP_ANON | MAP_PRIVATE, -1, 0);
...
int target() {
	...
	engine->init(FLAGS_metadata_server, FLAGS_local_server_name.c_str(), connectable_name.c_str(), 12345);
