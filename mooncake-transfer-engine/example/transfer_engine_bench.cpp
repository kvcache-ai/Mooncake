...
static void checkCudaError(cudaError_t result, const char *message) {
	if (result != cudaSuccess) {
		LOG(ERROR) << message << " (Error code: " << result << " - " << cudaGetErrorString(result) << ")" << std::endl;
		exit(EXIT_FAILURE);
	}
}
#endif
...
DEFINE_string(local_server_name, mooncake::getHostname(), "Local server name for segment discovery");
...
static void freeMemoryPool(void *addr, size_t size) {
	...
	checkCudaError(cudaPointerGetAttributes(&attributes, addr), "Failed to get pointer attributes");
...
static inline std::string calculateRate(uint64_t data_bytes, double duration) {
	...
	if (!RATE_UNIT_MP.count(FLAGS_report_unit)) {
		LOG(WARNING) << "Invalid flag: report_unit only support "
		             ...
		             << FLAGS_report_unit << " . Now use GB(default) as report_unit";
...
Status initiatorWorker(TransferEngine *engine, SegmentID segment_id, int thread_id, void *addr) {
...
int initiator() {
	...
	engine->init(FLAGS_metadata_server, FLAGS_local_server_name.c_str(), hostname_port.first.c_str(),
	...
	for (int i = 0; i < buffer_num; ++i) {
		...
		int rc = engine->registerLocalMemory(addr[i], FLAGS_buffer_size, name_prefix + std::to_string(i));
		LOG_ASSERT(!rc);
	}
#else
	for (int i = 0; i < buffer_num; ++i) {
		addr[i] = allocateMemoryPool(FLAGS_buffer_size, i, false);
		int rc = engine->registerLocalMemory(addr[i], FLAGS_buffer_size, "cpu:" + std::to_string(i));
		LOG_ASSERT(!rc);
	}
#endif
	...
	for (int i = 0; i < FLAGS_threads; ++i)
		workers[i] = std::thread(initiatorWorker, engine.get(), segment_id, i, addr[i % buffer_num]);
	...
	LOG(INFO) << "Test completed: duration " << std::fixed << std::setprecision(2) << duration << ", batch count "
	          << batch_count << ", throughput "
	          << calculateRate(batch_count * FLAGS_batch_size * FLAGS_block_size, duration);
