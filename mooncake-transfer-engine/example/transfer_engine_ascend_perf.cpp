...
int initiator() {
	...
	engine->init(FLAGS_metadata_server, FLAGS_local_server_name_new.c_str(), hostname_port.first.c_str(),
	...
	ret = engine->registerLocalMemory(tmp_devAddr, FLAGS_block_size, "npu:" + std::to_string(g_deviceId));
	...
	for (uint32_t i = 0; i < FLAGS_block_iteration; i++) {
		...
		ret = engine->registerLocalMemory(devAddr, FLAGS_batch_size * block_size, "npu:" + std::to_string(g_deviceId));
		if (ret) {
	...
	for (uint32_t i = 0; i < FLAGS_block_iteration; i++) {
		...
		LOG(INFO) << "Test completed: duration " << duration << "us, block size " << block_size / 1024
		          << "KB, total size " << FLAGS_batch_size * block_size / 1024 << "KB , throughput "
