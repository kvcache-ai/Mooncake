...
DEFINE_string(local_server_name, "10.20.130.154:12345", "Local server name for segment discovery");
...
int initiator() {
	...
	std::string FLAGS_local_server_name_npu =
	    hostname_port.first + ":" + std::to_string(hostname_port.second) + ":npu_" + std::to_string(g_deviceId);
	engine->init(FLAGS_metadata_server, FLAGS_local_server_name_npu.c_str(), hostname_port.first.c_str(),
	...
	ret = engine->registerLocalMemory(devAddr, g_TotalSize, "npu:" + std::to_string(g_deviceId));
	if (ret) {
	...
	ret = engine->registerLocalMemory(devAddr2, g_TotalSize, "npu:" + std::to_string(g_deviceId));
	if (ret) {
		LOG(ERROR) << "Failed to registerLocalMemory, ret: " << ret;
		return ret;
	}
	...
	LOG(INFO) << "Test completed: duration " << duration << "us, batch count " << FLAGS_batch_size * FLAGS_block_size
	          << ", throughput " << calculateRate(FLAGS_batch_size * FLAGS_block_size, duration);
...
int target() {
	...
	ret = engine->registerLocalMemory(devAddr, g_TotalSize * FLAGS_recv_num, "npu:" + std::to_string(g_deviceId));
	if (ret) {
		LOG(ERROR) << "Failed to registerLocalMemory, ret: " << ret;
		return ret;
	}
	...
	ret = engine->registerLocalMemory(devAddr2, g_TotalSize * FLAGS_recv_num, "npu:" + std::to_string(g_deviceId));
	if (ret) {
		LOG(ERROR) << "Failed to registerLocalMemory, ret: " << ret;
		return ret;
	}
