...
extern "C" {
    void *my_malloc(ssize_t size, int device, cudaStream_t stream) {
        ...
        if (result != CUDA_SUCCESS) {
            std::cerr << "NvlinkTransport: cuDeviceGet failed: " << result << "\n";
            return nullptr;
        }
        ...
        result = cuDeviceGetAttribute(
            ...
            currentDev);
        if (result != CUDA_SUCCESS) {
            std::cerr << "NvlinkTransport: cuDeviceGetAttribute failed: "
                    << result;
            return nullptr;
        }
	    ...
	    if (result != CUDA_SUCCESS) {
            std::cerr << "NvlinkTransport: cuMemGetAllocationGranularity failed: "
                    << result;
            return nullptr;
        ...
        if (result != CUDA_SUCCESS) {
            std::cerr << "NvlinkTransport: cuMemCreate failed: " << result;
            return nullptr;
        }
        result = cuMemAddressReserve((CUdeviceptr *)&ptr, size, granularity, 0, 0);
        if (result != CUDA_SUCCESS) {
            std::cerr << "NvlinkTransport: cuMemAddressReserve failed: " << result;
            cuMemRelease(handle);
            return nullptr;
        }
        result = cuMemMap((CUdeviceptr)ptr, size, 0, handle, 0);
        if (result != CUDA_SUCCESS) {
            std::cerr << "NvlinkTransport: cuMemMap failed: " << result;
        ...
        if (result != CUDA_SUCCESS) {
            std::cerr << "NvlinkTransport: cuMemSetAccess failed: " << result;
    ...
    void my_free(void *ptr, ssize_t ssize, int device, cudaStream_t stream) {
        ...
        if (result != CUDA_SUCCESS) {
            std::cerr << "NvlinkTransport: cuMemRetainAllocationHandle failed: "
                      << result << "\n";
            return;
        }
