// EP IBGDA device context and operations.
//
...
// in ep_ibgda_write_rdma_atomic_add_wqe).
...
__device__ __forceinline__ mlx5gda_qp_devctx* ep_ibgda_channel(
    const EpIbgdaContext& ctx, int channel, int dst_rank, int qps_per_rank) {
    int qp_idx = dst_rank * qps_per_rank + (channel % qps_per_rank);
    return ctx.qp_devctxs + qp_idx;
}

__device__ __forceinline__ void ep_ibgda_lock(mlx5gda_qp_devctx* qp) {
    uint32_t old;
    do {
...
__device__ __forceinline__ void ep_ibgda_unlock(mlx5gda_qp_devctx* qp) {
    ep_st_release_u32(&qp->mutex, 0u);
}

__device__ __forceinline__ void ep_ibgda_poll_cq(mlx5gda_qp_devctx* qp,
                                                 uint16_t expect) {
    ...
    while (static_cast<int16_t>(wq_tail - expect) <= 0) {
        ...
        wq_tail = ep_bswap16(cq_be) + 1;
    }
    if (wq_tail != qp->wq_tail) qp->wq_tail = wq_tail;
}

__device__ __forceinline__ void ep_ibgda_post_send_db(mlx5gda_qp_devctx* qp) {
    uint32_t num_posted = static_cast<uint32_t>(qp->wq_head);
    ep_st_release_u32(reinterpret_cast<uint32_t*>(&qp->dbr->send_counter),
                      ep_bswap32(num_posted));
    auto* last_wqe = qp->wq + ((num_posted - 1) & qp->wqeid_mask);
    ep_st_release_u64(reinterpret_cast<uint64_t*>(qp->bf + qp->bf_offset),
...
__device__ __forceinline__ void ep_ibgda_write_rdma_write_wqe(
    ...
    wqe->ctrl = {};
    wqe->ctrl.qpn_ds = ep_bswap32((qp->qpn << 8) | 3);
    wqe->ctrl.fm_ce_se = MLX5_WQE_CTRL_CQ_UPDATE;
    wqe->ctrl.opmod_idx_opcode = ep_bswap32(
        (static_cast<uint32_t>(qp->wq_head) << 8) | MLX5_OPCODE_RDMA_WRITE);

    wqe->raddr.raddr = ep_bswap64(raddr);
    wqe->raddr.rkey = rkey;
    wqe->raddr.reserved = 0;

    wqe->data.byte_count = ep_bswap32(bytes);
    wqe->data.lkey = lkey;
    wqe->data.addr = ep_bswap64(laddr);

    ++qp->wq_head;
}

// Issue an RDMA ATOMIC FETCH-AND-ADD WQE (32-bit add_data in big-endian).
__device__ __forceinline__ void ep_ibgda_write_rdma_atomic_add_wqe(
    ...
    wqe->ctrl = {};
    wqe->ctrl.qpn_ds = ep_bswap32((qp->qpn << 8) | 4);
    wqe->ctrl.fm_ce_se = MLX5_WQE_CTRL_CQ_UPDATE;
    wqe->ctrl.opmod_idx_opcode = ep_bswap32(
    ...
    atomic_seg->add_data = ep_bswap32(static_cast<uint32_t>(value));
    atomic_seg->field_boundary = 0;
    atomic_seg->compare = 0;

    wqe->data.byte_count = ep_bswap32(8);  // atomic always 8 bytes
    wqe->data.lkey = lkey;
    wqe->data.addr = ep_bswap64(laddr);
...
__device__ __forceinline__ void ep_ibgda_put(const EpIbgdaContext& ctx,
                                             ...
                                             uint32_t nbytes) {
#ifndef MOONCAKE_EP_USE_MUSA
    auto* qp = ep_ibgda_channel(ctx, channel, dst_rank, qps_per_rank);
    ep_ibgda_lock(qp);
    ep_ibgda_poll_cq(qp, qp->wq_head);
    ep_ibgda_write_rdma_write_wqe(qp, reinterpret_cast<uint64_t>(send_ptr),
                                  ep_bswap32(ctx.rkeys[src_rank]), recv_raddr,
                                  ep_bswap32(ctx.rkeys[dst_rank]), nbytes);
    ep_ibgda_post_send_db(qp);
    ep_ibgda_unlock(qp);
#endif
}
...
__device__ __forceinline__ void ep_ibgda_red_add(
    const EpIbgdaContext& ctx, int channel, int dst_rank, int src_rank,
    int qps_per_rank,
    ...
    ep_ibgda_lock(qp);
    ep_ibgda_poll_cq(qp, qp->wq_head);
    ep_ibgda_write_rdma_atomic_add_wqe(qp, value, laddr,
                                       ep_bswap32(ctx.rkeys[src_rank]),
                                       recv_raddr,
                                       ep_bswap32(ctx.rkeys[dst_rank]));
    ep_ibgda_post_send_db(qp);
    ep_ibgda_unlock(qp);
#endif
}
