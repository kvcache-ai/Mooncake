...
public:
	...
	enum TransferStatusEnum { WAITING, PENDING, INVALID, CANCELED, COMPLETED, TIMEOUT, FAILED };
...
public:
	...
	virtual Status submitTransfer(BatchID batch_id, const std::vector<TransferRequest> &entries) = 0;

	virtual Status submitTransferTask(const std::vector<TransferRequest *> &request_list,
	                                  const std::vector<TransferTask *> &task_list) {
		return Status::NotImplemented("Transport::submitTransferTask is not implemented");
	}
	...
	/// progress.
	virtual Status getTransferStatus(BatchID batch_id, size_t task_id, TransferStatus &status) = 0;
...
protected:
	virtual int install(std::string &local_server_name, std::shared_ptr<TransferMetadata> meta,
...
private:
	virtual int registerLocalMemory(void *addr, size_t length, const std::string &location, bool remote_accessible,
