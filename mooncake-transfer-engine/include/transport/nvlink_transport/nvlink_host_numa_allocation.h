...
class NvlinkHostNumaAllocation {
   public:
    ...
    // The output must be empty. On failure, a nonempty output owns incomplete
    // rollback state: do not use/register it; retain it and retry Release().
    ...
    // Caller serializes owner operations and completes transport unregister
    // before release. On failure retain the owner and retry; destruction is
    // best-effort only. Completed release stages are not repeated.
   ...
   private:
    ...
    // Classify and pin atomically; only kNotOwned may use legacy registration.
    static OwnedRangeResult AcquireOwnedRange(void* base, size_t length,
                                              DriverApi* api);
    static bool ReleaseOwnedRange(void* base, size_t length);
    static bool IsOwnedRangeBase(void* base);
