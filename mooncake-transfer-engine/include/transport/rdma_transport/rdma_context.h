...
public:
	...
	int construct(size_t num_cq_list = 1, size_t num_comp_channels = 1, uint8_t port = 1, int gid_index = 0,
	              size_t max_cqe = 4096, int max_endpoints = 256);
...
public:
	...
	int activeSpeed() const {
		return active_speed_;
	}
...
private:
	...
	int getBestGidIndex(const std::string &device_name, struct ibv_context *context, ibv_port_attr &port_attr,
	                    uint8_t port);
