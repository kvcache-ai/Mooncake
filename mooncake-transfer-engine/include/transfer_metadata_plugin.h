...
struct MetadataStoragePlugin {
	static std::shared_ptr<MetadataStoragePlugin> Create(const std::string &conn_string);
...
struct HandShakePlugin {
	static std::shared_ptr<HandShakePlugin> Create(const std::string &conn_string);
	...
	using OnReceiveCallBack = std::function<int(const Json::Value &, Json::Value &)>;
	...
	virtual int send(std::string ip_or_host_name, uint16_t rpc_port, const Json::Value &local, Json::Value &peer) = 0;
