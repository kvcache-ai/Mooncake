...
static inline std::string getCurrentDateTime() {
	...
	oss << std::put_time(&local_time, "%Y-%m-%d %H:%M:%S") << "." << std::setw(6) << std::setfill('0')
	    << micros.count();
	return oss.str();
}
...
static inline std::pair<std::string, uint16_t> parseHostNameWithPort(const std::string &server_name) {
	...
	if (val <= 0 || val > 65535)
		LOG(WARNING) << "Illegal port number in " << server_name << ". Use default port " << port << " instead";
	else
		port = (uint16_t)val;
	return std::make_pair(trimmed_server_name, port);
}

static inline std::pair<std::string, uint16_t> parseHostNameWithPortAscend(const std::string &server_name,
                                                                           int *device_id) {
	uint16_t port = getDefaultHandshakePort();
	auto pos = server_name.find(':');
	if (pos == server_name.npos) {
...
static inline ssize_t writeFully(int fd, const void *buf, size_t len) {
	...
	while (nbytes) {
		...
		if (rc < 0 && (errno == EAGAIN || errno == EINTR))
			...
			LOG(WARNING) << "Socket write incompleted: expected " << len << " bytes, actual " << len - nbytes
			             << " bytes";
			return len - nbytes;
		}
		pos += rc;
...
static inline ssize_t readFully(int fd, void *buf, size_t len) {
	...
	while (nbytes) {
		...
		if (rc < 0 && (errno == EAGAIN || errno == EINTR))
			...
			LOG(WARNING) << "Socket read incompleted: expected " << len << " bytes, actual " << len - nbytes
			             << " bytes";
			return len - nbytes;
		}
		pos += rc;
...
static inline int writeString(int fd, const HandShakeRequestType type, const std::string &str) {
	uint8_t byte = static_cast<uint8_t>(type);
	// LOG(INFO) << "writeString: type " << (int)byte << ", str(" << str.size()
	//           << "): " << str;
	uint64_t length = str.size() + (type == HandShakeRequestType::OldProtocol ? 0 : sizeof(byte));
...
static inline std::pair<HandShakeRequestType, std::string> readString(int fd) {
	...
	if (n != (ssize_t)length) {
		LOG(ERROR) << "readString: unexpected length, got: " << n << ", expected: " << length;
		return {type, ""};
	}
