...
#ifndef TENT_DEVICE_P2P_OPS_CUH_
#define TENT_DEVICE_P2P_OPS_CUH_
...
// populated by the platform backend (cuda_ops.cuh / musa_ops.cuh).
// This avoids platform-specific ASM in the communication layer.
// ---------------------------------------------------------------------------

__device__ __forceinline__ void p2p_put(DeviceOps* dops,
...
__device__ __forceinline__ void p2p_signal(DeviceOps* dops,
                                           const P2PDeviceContext& ctx,
                                           int dst_rank,
                                           const void* local_base,
...
__device__ __forceinline__ void p2p_wait_signal(DeviceOps* dops,
                                                void* sig,
                                                uint64_t expected) {
...
__device__ __forceinline__ void p2p_wait_signal_32(DeviceOps* dops,
                                                   void* sig,
                                                   uint32_t expected) {
    dops->spin_wait_eq(sig, expected);
}

__device__ __forceinline__ void p2p_signal_add(DeviceOps* dops,
                                               const P2PDeviceContext& ctx,
                                               int dst_rank,
                                               const void* local_base,
...
__device__ __forceinline__ void p2p_flush(DeviceOps* dops) {
    dops->fence_acq_rel();
}
...
#endif  // TENT_DEVICE_P2P_OPS_CUH_
