...
class CoroRpcAgent {
   ...
   public:
    ...
    // Same as callAsync(), but retries once on a transport-level RPC failure.
    // Only for idempotent RPCs: a failure may arrive after the peer already ran
    // the handler, so a retry can run it twice.
