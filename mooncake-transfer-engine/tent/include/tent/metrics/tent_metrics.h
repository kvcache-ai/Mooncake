...
// Can be set via CMake option TENT_METRICS_ENABLED or define TENT_METRICS_ENABLED=0/1
// Default is OFF (0) to match CMake default: option(TENT_METRICS_ENABLED ... OFF)
...
// Estimated buffer size for Prometheus metrics serialization (pre-allocation optimization)
...
private:
    ...
    ylt::metric::counter_t read_bytes_total_{"tent_read_bytes_total", "Total bytes read via TENT"};
    ylt::metric::counter_t write_bytes_total_{"tent_write_bytes_total", "Total bytes written via TENT"};
    ylt::metric::counter_t read_requests_total_{"tent_read_requests_total", "Total read requests via TENT"};
    ylt::metric::counter_t write_requests_total_{"tent_write_requests_total", "Total write requests via TENT"};
    ylt::metric::counter_t read_failures_total_{"tent_read_failures_total", "Total read failures via TENT"};
    ylt::metric::counter_t write_failures_total_{"tent_write_failures_total", "Total write failures via TENT"};
    
    // Histograms - stored as pointers for unified management
    std::vector<ylt::metric::histogram_t*> histograms_;
    // Store bucket boundaries separately since ylt histogram doesn't expose them publicly
    ...
    static inline const std::vector<double> kLatencyBuckets{100, 500, 1000, 5000, 10000, 50000, 100000, 500000, 1000000};
    ylt::metric::histogram_t read_latency_{"tent_read_latency_us", "Read latency distribution in microseconds", kLatencyBuckets};
    ylt::metric::histogram_t write_latency_{"tent_write_latency_us", "Write latency distribution in microseconds", kLatencyBuckets};
    // Size histograms for request size distribution (in bytes)
    // Default buckets: 1KB, 4KB, 16KB, 64KB, 256KB, 1MB, 4MB, 16MB, 64MB, 256MB, 1GB
    static inline const std::vector<double> kSizeBuckets{1024, 4096, 16384, 65536, 262144, 1048576, 4194304, 16777216, 67108864, 268435456, 1073741824};
    ylt::metric::histogram_t read_size_{"tent_read_size_bytes", "Read request size distribution in bytes", kSizeBuckets};
    ylt::metric::histogram_t write_size_{"tent_write_size_bytes", "Write request size distribution in bytes", kSizeBuckets};
...
public:
    ...
    ScopedLatencyRecorder(OperationType type, size_t bytes)
        ...
        // Only record start time if metrics are enabled (avoid clock overhead when disabled)
        if (enabled_) {
    ...
    ~ScopedLatencyRecorder() {
        if (!enabled_ || failed_) return;  // Skip if disabled or already marked as failed
...
#define TENT_RECORD_READ_COMPLETED(bytes, latency) \
    do { \
        if (::mooncake::tent::TentMetrics::isEnabled()) { \
            ::mooncake::tent::TentMetrics::instance().recordReadCompleted(bytes, latency); \
        } \
    } while(0)
...
#define TENT_RECORD_WRITE_COMPLETED(bytes, latency) \
    do { \
        if (::mooncake::tent::TentMetrics::isEnabled()) { \
            ::mooncake::tent::TentMetrics::instance().recordWriteCompleted(bytes, latency); \
        } \
    } while(0)
...
#define TENT_RECORD_WRITE_FAILED(bytes) \
    do { \
        if (::mooncake::tent::TentMetrics::isEnabled()) { \
            ::mooncake::tent::TentMetrics::instance().recordWriteFailed(bytes); \
        } \
    } while(0)

// RAII macro for automatic latency measurement
#define TENT_SCOPED_READ_LATENCY(bytes) \
    ::mooncake::tent::ScopedLatencyRecorder _tent_latency_recorder_(::mooncake::tent::ScopedLatencyRecorder::OperationType::Read, bytes)

#define TENT_SCOPED_WRITE_LATENCY(bytes) \
    ::mooncake::tent::ScopedLatencyRecorder _tent_latency_recorder_(::mooncake::tent::ScopedLatencyRecorder::OperationType::Write, bytes)
