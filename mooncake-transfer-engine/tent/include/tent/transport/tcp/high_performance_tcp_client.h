...
class HighPerformanceTcpClient {
 public:
  struct Config { uint64_t max_transfer_bytes{1ULL<<30}; size_t chunk_size{1ULL<<20}; uint64_t connect_timeout_ms{2000}; };
  explicit HighPerformanceTcpClient(Config config):config_(config){}
  Status transfer(const std::string& host, uint16_t port, uint64_t registration_id,
                  uint64_t remote_addr, void* local_addr, uint64_t length,
