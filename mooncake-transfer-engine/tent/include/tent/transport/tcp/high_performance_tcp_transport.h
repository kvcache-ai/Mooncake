...
struct HighPerformanceTcpParams { std::string bind_address, advertise_address; uint16_t port{0}; size_t worker_count{16}, queue_capacity_per_worker{256}, connections_per_peer{4}; uint64_t max_outstanding_tasks{4096},max_outstanding_bytes{1ULL<<32},max_transfer_bytes{1ULL<<30}; size_t chunk_size{1ULL<<20}; uint64_t connect_timeout_ms{2000},progress_timeout_ms{30000}; };
...
class HighPerformanceTcpTransport final:public Transport{
 public:
  HighPerformanceTcpTransport(); ~HighPerformanceTcpTransport() override;
 ...
 private:
  Status resolve(const Request&,HighPerformanceTcpEndpointAttr*,HighPerformanceTcpBufferAttr*); void finish(const std::shared_ptr<HighPerformanceTcpTask>&,Status); Status parseParams(const std::shared_ptr<Config>&); std::string makeIncarnation() const;
