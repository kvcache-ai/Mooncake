...
struct HighPerformanceTcpTask {
    Request request;
    std::atomic<TransferStatusEnum> status{PENDING};
    std::atomic<size_t> bytes{0};
    std::atomic<bool> completion_claimed{false};
    BatchID progress_batch_id{};
    std::function<void(BatchID)> notify;
    HighPerformanceTcpBufferRegistry::Lease lease;
};
struct HighPerformanceTcpSubBatch : Transport::SubBatch {
    std::vector<std::shared_ptr<HighPerformanceTcpTask>> tasks;
    size_t max_size{};
...
class HighPerformanceTcpTransport final : public Transport {
   public:
    ...
    Status addMemoryBuffer(BufferDesc&, const MemoryOptions&) override;
    Status removeMemoryBuffer(BufferDesc&) override;
    ...
    Status receiveNotification(std::vector<Notification>&) override;
    const char* getName() const override { return "tcp_high_performance"; }

   private:
    Status resolve(const Request&, HighPerformanceTcpEndpointAttr*,
                   HighPerformanceTcpBufferAttr*);
    void finish(const std::shared_ptr<HighPerformanceTcpTask>&, Status);
    std::string makeIncarnation() const;
    bool installed_{false};
    std::atomic<bool> stopping_{false};
    std::string local_segment_name_, incarnation_;
    ...
    std::atomic<uint64_t> next_request_id_{1}, outstanding_tasks_{0},
