...
void RunWriteThenReadAcrossProcesses(size_t task_count) {
            ...
            if (!server.available()) {
                ...
                if (!registered.ok()) {
                    ...
                    if (write(ready_pipe[1], &length, sizeof(length)) !=
                        ...
                        char stop = 0;
                        (void)ReadExactly(stop_pipe[0], &stop, 1);
                    }
                    if (!server
    ...
    if (!ReadExactly(ready_pipe[0], &segment_length, sizeof(segment_length))) {
        close(ready_pipe[0]);
        const int status = child_guard.finish();
        FAIL() << "HP TCP server initialization failed, child status "
               << status;
    }
    std::string server_segment(segment_length, '\0');
    if (!ReadExactly(ready_pipe[0], server_segment.data(), segment_length)) {
        close(ready_pipe[0]);
        const int status = child_guard.finish();
        FAIL() << "Failed to read the HP TCP server segment, child status "
               << status;
    }
    close(ready_pipe[0]);
