...
TEST(HighPerformanceTcpTransportTest,
     PreSendConnectFailureNeedsEndpointRefresh) {
    std::vector<uint8_t> remote_storage(64, 0xA5);
    std::vector<uint8_t> local_storage(remote_storage.size(), 0);

    auto params = MakeParams();
    params.worker_count = 1;
    params.connections_per_peer = 1;

    auto server_metadata = MakeLocalMetadata();
    uint16_t rpc_port = 0;
    ASSERT_TRUE(server_metadata->start(rpc_port).ok());
    std::string server_name = "127.0.0.1:" + std::to_string(rpc_port);
    ASSERT_TRUE(server_metadata->segmentManager()
                    .updateLocal([&](SegmentDesc& segment) -> Status {
                        segment.name = server_name;
                        segment.rpc_server_addr = server_name;
    ...
    HighPerformanceTcpTransport server(params);
    ASSERT_TRUE(
        server.install(server_name, server_metadata, nullptr, nullptr).ok());
    BufferDesc remote_desc;
    remote_desc.addr = reinterpret_cast<uint64_t>(remote_storage.data());
    remote_desc.length = remote_storage.size();
    remote_desc.location = "cpu:0";
    MemoryOptions remote_options;
    remote_options.type = HP_TCP;
    remote_options.perm = kGlobalReadWrite;
    ...
    ASSERT_TRUE(DecodeHighPerformanceTcpEndpointAttr(
                    ...
                    &fresh_endpoint)
                    .ok());
    ASSERT_EQ(fresh_endpoint.endpoints.size(), 1U);
    const uint16_t fresh_port = fresh_endpoint.endpoints[0].port;
    asio::io_context io;
    asio::ip::tcp::socket stale_socket(io);
    stale_socket.open(asio::ip::tcp::v4());
    stale_socket.bind({asio::ip::make_address("127.0.0.1"), 0});
    const uint16_t stale_port = stale_socket.local_endpoint().port();

    auto client_metadata = MakeLocalMetadata();
    HighPerformanceTcpTransport client(params);
    std::string client_name = "hp_transport_client_presend_refresh";
    ASSERT_TRUE(
    ...
    SegmentDescRef cached_stale;
    ASSERT_TRUE(client_metadata->segmentManager()
                    .getRemoteCached(cached_stale, target)
                    .ok());
    auto& cached_endpoint_blob =
        std::get<MemorySegmentDesc>(cached_stale->detail)
            .transport_attrs.at(static_cast<int>(HP_TCP));
    auto stale_endpoint = fresh_endpoint;
    stale_endpoint.endpoints[0].port = stale_port;
    ASSERT_TRUE(EncodeHighPerformanceTcpEndpointAttr(stale_endpoint,
                                                     &cached_endpoint_blob)
                    .ok());

    BufferDesc local_desc;
    local_desc.addr = reinterpret_cast<uint64_t>(local_storage.data());
    local_desc.length = local_storage.size();
    ...
    ASSERT_TRUE(client_metadata->segmentManager()
                    ...
                    .ok());
    HighPerformanceTcpEndpointAttr refreshed_endpoint;
    ASSERT_TRUE(DecodeHighPerformanceTcpEndpointAttr(
                    std::get<MemorySegmentDesc>(refreshed->detail)
                        .transport_attrs.at(static_cast<int>(HP_TCP)),
                    &refreshed_endpoint)
                    .ok());
    ASSERT_EQ(refreshed_endpoint.endpoints.size(), 1U);
    EXPECT_EQ(refreshed_endpoint.endpoints[0].port, fresh_port);
