...
TEST(HighPerformanceTcpTransportTest, SliceFailureCancelsSiblingRead) {
    constexpr size_t kLength = 4ULL << 20;
    asio::io_context peer_io;
    asio::ip::tcp::acceptor stalled_acceptor(
        peer_io, {asio::ip::make_address("127.0.0.1"), 0});
    stalled_acceptor.non_blocking(true);
    asio::ip::tcp::acceptor failed_acceptor(peer_io);
    failed_acceptor.open(asio::ip::tcp::v4());
    failed_acceptor.bind({asio::ip::make_address("127.0.0.1"), 0});
    const uint16_t failed_port = failed_acceptor.local_endpoint().port();

    bool sibling_connected = false;
    bool sibling_closed = false;

    auto remote_metadata = MakeLocalMetadata();
    uint16_t rpc_port = 0;
    ASSERT_TRUE(remote_metadata->start(rpc_port).ok());
    const std::string remote_name = "127.0.0.1:" + std::to_string(rpc_port);

    HighPerformanceTcpEndpointAttr endpoint_attr;
    endpoint_attr.incarnation = "00112233445566778899aabbccddeeff";
    endpoint_attr.endpoints = {
        {"127.0.0.1", failed_port},
        {"127.0.0.1", stalled_acceptor.local_endpoint().port()}};
    endpoint_attr.max_transfer_bytes = kLength;
    std::string encoded_endpoint;
    ASSERT_TRUE(
        EncodeHighPerformanceTcpEndpointAttr(endpoint_attr, &encoded_endpoint)
            .ok());

    BufferDesc remote_desc;
    remote_desc.addr = 0x100000;
    remote_desc.length = kLength;
    remote_desc.location = "cpu:0";
    remote_desc.transports.push_back(TransportType::HP_TCP);
    ASSERT_TRUE(EncodeHighPerformanceTcpBufferAttr(
                    {1, "global_read_write"},
                    &remote_desc.transport_attrs[TransportType::HP_TCP])
                    .ok());
    ASSERT_TRUE(
        remote_metadata->segmentManager()
            .updateLocal([&](SegmentDesc& segment) -> Status {
                segment.name = remote_name;
                segment.rpc_server_addr = remote_name;
                auto& memory = std::get<MemorySegmentDesc>(segment.detail);
                memory
                    .transport_attrs[static_cast<int>(TransportType::HP_TCP)] =
                    encoded_endpoint;
                memory.buffers = {remote_desc};
                return Status::OK();
            })
            .ok());
    ASSERT_TRUE(remote_metadata->segmentManager().synchronizeLocal().ok());
    ...
    SegmentID target = 0;
    ASSERT_TRUE(
        client_metadata->segmentManager().openRemote(target, remote_name).ok());

    std::vector<uint8_t> local(kLength);
    BufferDesc local_desc;
    local_desc.addr = reinterpret_cast<uint64_t>(local.data());
    local_desc.length = local.size();
    local_desc.location = "cpu:0";
    ...
    std::thread stalled_peer([&] {
        ...
        asio::read(socket, asio::buffer(request), error);
        if (error) {
            sibling_closed = true;
            return;
        }
        uint8_t unexpected = 0;
        (void)socket.read_some(asio::buffer(&unexpected, 1), error);
        sibling_closed = static_cast<bool>(error);
    });

    const Status submitted = client.submitTransferTasks(batch, {request});

    TransferStatus transfer_status{PENDING, 0};
    Status first_result = submitted;
    if (submitted.ok()) {
        first_result = WaitForTransportResult(client, batch, transfer_status);
    }
    const TransferStatusEnum first_state = transfer_status.s;
    if (first_state == PENDING) {
        (void)client.cancelTransferTask(batch, 0);
    }
    const Status quiesced = client.quiesce();
    stalled_peer.join();

    EXPECT_TRUE(first_result.ok()) << first_result.ToString();
    EXPECT_EQ(first_state, FAILED);
    EXPECT_TRUE(!sibling_connected || sibling_closed);
