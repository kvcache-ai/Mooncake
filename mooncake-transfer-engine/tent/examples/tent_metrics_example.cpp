 ...
 * - Compile-time disable: Build with -DTENT_METRICS_ENABLED=OFF for zero overhead
...
int main(int argc, char* argv[]) {
    ...
    config.report_interval_seconds = 0;  // Disable periodic logging for this example
    
    std::cout << "\nConfiguration:" << std::endl;
    std::cout << "  - HTTP Server: " << config.http_host << ":" << config.http_port << std::endl;
    std::cout << "  - Prometheus: " << (config.enable_prometheus ? "enabled" : "disabled") << std::endl;
    std::cout << "  - Runtime metrics: " << (config.enabled ? "enabled" : "disabled") << std::endl;
    
    // 2. Initialize metrics system (this starts the HTTP server)
    auto status = TentMetrics::instance().initialize(config);
    if (!status.ok()) {
        std::cerr << "Failed to initialize metrics: " << status.ToString() << std::endl;
        return 1;
    }
    
    std::cout << "\nHTTP Server started. Available endpoints:" << std::endl;
    std::cout << "  - http://localhost:" << config.http_port << "/metrics        (Prometheus format)" << std::endl;
    std::cout << "  - http://localhost:" << config.http_port << "/metrics/summary (Human readable)" << std::endl;
    std::cout << "  - http://localhost:" << config.http_port << "/metrics/json    (JSON format)" << std::endl;
    std::cout << "  - http://localhost:" << config.http_port << "/health          (Health check)" << std::endl;
    
    // 3. Simulate transfer operations with manual latency recording
    std::cout << "\n--- Manual Latency Recording ---" << std::endl;
    std::cout << "Simulating transfer operations with explicit latency values..." << std::endl;
    ...
    for (int i = 0; i < 10; ++i) {
        ...
        std::cout << "  Iteration " << (i + 1) << ": Read " << read_bytes / 1024 / 1024 
                  << " MB (latency: " << read_latency * 1000 << "ms)"
                  << ", Write " << write_bytes / 1024 << " KB (latency: " << write_latency * 1000 << "ms)" << std::endl;
        
        std::this_thread::sleep_for(std::chrono::milliseconds(100));
    }
    
    // 4. Demonstrate ScopedLatencyRecorder for automatic latency measurement
    std::cout << "\n--- Automatic Latency Recording (ScopedLatencyRecorder) ---" << std::endl;
    std::cout << "Using RAII-style automatic latency measurement..." << std::endl;
    ...
    // Example 1: Using TENT_SCOPED_READ_LATENCY macro
    {
        ...
        std::cout << "  Scoped read: 2 MB with ~5ms simulated work" << std::endl;
    ...
    // Example 2: Using TENT_SCOPED_WRITE_LATENCY macro
    {
        ...
        std::cout << "  Scoped write: 1 MB with ~3ms simulated work" << std::endl;
    }  // Latency automatically recorded when scope ends
    
    // Example 3: Using ScopedLatencyRecorder directly with failure handling
    {
        ScopedLatencyRecorder recorder(ScopedLatencyRecorder::OperationType::Read, 512 * 1024);
        std::this_thread::sleep_for(std::chrono::milliseconds(2));
        // Simulate a failure condition
        bool operation_failed = true;  // Simulated failure
        if (operation_failed) {
            recorder.markFailed();
            std::cout << "  Scoped read with failure: 512 KB marked as failed" << std::endl;
        }
    }
    ...
    for (int i = 0; i < 5; ++i) {
        ...
        std::cout << "    Scoped read " << (i + 1) << ": " << 256 * (i + 1) << " KB with ~" << (1 + i) << "ms work" << std::endl;
    }
    
    // 5. Demonstrate runtime enable/disable switch (only if not started with --disabled)
    std::cout << "\n--- Runtime Enable/Disable Switch ---" << std::endl;
    std::cout << "Current state: " << (TentMetrics::isEnabled() ? "enabled" : "disabled") << std::endl;
    ...
    if (!start_disabled) {
        ...
        for (int i = 0; i < 1000; ++i) {
            TENT_RECORD_READ_COMPLETED(1024, 0.001);  // These are no-ops when disabled
        }
        std::cout << "  1000 record calls completed (no-op, metrics disabled)" << std::endl;
        ...
        std::cout << "  Skipping enable/disable demo (started with --disabled)" << std::endl;
    }
    ...
    std::cout << "\n=== Prometheus Metrics (includes latency histograms) ===" << std::endl;
    std::string prometheus_metrics = TentMetrics::instance().getPrometheusMetrics();
    ...
    if (server_mode) {
        ...
        std::cout << "  curl http://localhost:" << config.http_port << "/metrics          # Prometheus format" << std::endl;
        std::cout << "  curl http://localhost:" << config.http_port << "/metrics/summary  # Human readable summary" << std::endl;
        std::cout << "  curl http://localhost:" << config.http_port << "/metrics/json     # JSON format" << std::endl;
        std::cout << "  curl http://localhost:" << config.http_port << "/health           # Health check" << std::endl;
