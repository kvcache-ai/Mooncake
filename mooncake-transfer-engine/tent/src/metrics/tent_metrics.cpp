...
Status TentMetrics::initialize(const MetricsConfig& config) {
    ...
    // Note: config latency_buckets are in seconds, convert to microseconds for histogram
    if (!config_.latency_buckets.empty()) {
        ...
        read_latency_ = ylt::metric::histogram_t("tent_read_latency_us", 
                                               "Read latency distribution in microseconds", 
                                               latency_buckets_us);
        write_latency_ = ylt::metric::histogram_t("tent_write_latency_us", 
    ...
    if (!config_.size_buckets.empty()) {
        read_size_ = ylt::metric::histogram_t("tent_read_size_bytes",
                                             "Read request size distribution in bytes",
                                             config_.size_buckets);
        write_size_ = ylt::metric::histogram_t("tent_write_size_bytes",
    ...
    LOG(INFO) << "TENT metrics initialized successfully, HTTP server listening on "
...
void TentMetrics::initHttpServer() {
    ...
    http_server_ = std::make_unique<coro_http_server>(config_.http_server_threads, config_.http_port);
    ...
    http_server_->set_http_handler<GET>(
        "/metrics/summary", [this](coro_http_request& req, coro_http_response& resp) {
            std::string summary = getSummaryString();
    ...
    http_server_->set_http_handler<GET>(
        "/metrics/json", [this](coro_http_request& req, coro_http_response& resp) {
...
std::string TentMetrics::getSummaryString() {
    ...
    auto formatBytes = [](double bytes) -> std::string {
        ...
        if (bytes >= 1e12) s << bytes / 1e12 << " TB";
        else if (bytes >= 1e9) s << bytes / 1e9 << " GB";
        else if (bytes >= 1e6) s << bytes / 1e6 << " MB";
        else if (bytes >= 1e3) s << bytes / 1e3 << " KB";
        else s << bytes << " B";
        return s.str();
    };
...
Status TentMetrics::initialize(const MetricsConfig& config) {
    config_ = config;
    initialized_ = true;
    LOG(INFO) << "TENT metrics disabled at compile time (TENT_METRICS_ENABLED=0)";
    return Status::OK();
}
