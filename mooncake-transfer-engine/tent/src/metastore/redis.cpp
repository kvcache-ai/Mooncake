...
Status RedisMetaStore::connect(const std::string &endpoint,
    ...
    // Input validation
    if (endpoint.empty()) {
        return Status::MetadataError(std::string("Redis endpoint cannot be empty") + LOC_MARK);
    }
    ...
    if (db_index > REDIS_MAX_DB_INDEX) {
        return Status::MetadataError(std::string("Redis database index out of range (0-255)") + LOC_MARK);
    }
    ...
    if (client_->err) {
        std::string message = "Redis connection failed: " + std::string(client_->errstr);
    ...
    if (!password.empty()) {
        ...
        redisReply *reply = (redisReply *)redisCommand(client_, "AUTH %b", 
                                                       password.data(), password.size());
        RedisReplyGuard reply_guard(reply);
        
        if (!reply || reply->type == REDIS_REPLY_ERROR) {
            // Don't leak specific error details that might contain sensitive info
    ...
    if (db_index != REDIS_DEFAULT_DB_INDEX) {
        redisReply *reply = (redisReply *)redisCommand(client_, "SELECT %d", db_index);
        RedisReplyGuard reply_guard(reply);
        
        if (!reply || reply->type == REDIS_REPLY_ERROR) {
            std::string message = "Redis failed to select database " + std::to_string(db_index);
            cleanupFailedConnection();
            return Status::MetadataError(message + LOC_MARK);
        }
    }
...
Status RedisMetaStore::handleRedisReply(redisReply* reply, const std::string& operation) const {
    if (!reply) {
        return Status::MetadataError("Redis " + operation + " failed: connection error" + LOC_MARK);
    }
    
    if (reply->type == REDIS_REPLY_ERROR) {
        // Filter out potentially sensitive information from Redis error messages
        ...
        LOG(WARNING) << "Redis operation failed - " << operation << ": " << error_msg;
...
Status RedisMetaStore::get(const std::string &key, std::string &value) {
    ...
    redisReply *resp = (redisReply *)redisCommand(client_, "GET %b", 
                                                  key.data(), key.size());
...
Status RedisMetaStore::set(const std::string &key, const std::string &value) {
    ...
    // Use binary-safe formatting to prevent command injection
    redisReply *resp = (redisReply *)redisCommand(client_, "SET %b %b",
                                                  key.data(), key.size(),
...
Status RedisMetaStore::remove(const std::string &key) {
    ...
    // Use binary-safe formatting to prevent command injection
    redisReply *resp = (redisReply *)redisCommand(client_, "DEL %b", 
                                                  key.data(), key.size());
    RedisReplyGuard reply_guard(resp);
