...
namespace mooncake {
MultiTransport::MultiTransport(std::shared_ptr<TransferMetadata> metadata, std::string &local_server_name)
...
Status MultiTransport::submitTransfer(BatchID batch_id, const std::vector<TransferRequest> &entries) {
	auto &batch_desc = *((BatchDesc *)(batch_id));
	...
	for (auto &request : entries) {
		...
		++task_id;
		submit_tasks[transport].request_list.push_back((TransferRequest *)&request);
		submit_tasks[transport].task_list.push_back(&task);
	}
	Status overall_status = Status::OK();
	for (auto &entry : submit_tasks) {
		auto status = entry.first->submitTransferTask(entry.second.request_list, entry.second.task_list);
		if (!status.ok()) {
...
Status MultiTransport::getTransferStatus(BatchID batch_id, size_t task_id, TransferStatus &status) {
	auto &batch_desc = *((BatchDesc *)(batch_id));
	const size_t task_count = batch_desc.task_list.size();
	...
	if (success_slice_count + failed_slice_count == task.slice_count) {
		...
		if (globalConfig().slice_timeout > 0) {
			...
			const int64_t kPacketDeliveryTimeout = globalConfig().slice_timeout * 1000000000;
...
Status MultiTransport::getBatchTransferStatus(BatchID batch_id, TransferStatus &status) {
	...
	status.s = (success_count == task_count) ? Transport::TransferStatusEnum::COMPLETED
	                                         : Transport::TransferStatusEnum::WAITING;
	return Status::OK();
}

Transport *MultiTransport::installTransport(const std::string &proto, std::shared_ptr<Topology> topo) {
	Transport *transport = nullptr;
...
Status MultiTransport::selectTransport(const TransferRequest &entry, Transport *&transport) {
	...
	if (!target_segment_desc) {
		return Status::InvalidArgument("Invalid target segment ID " + std::to_string(entry.target_id));
	}
