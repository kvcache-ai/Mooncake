...
int RdmaTransport::install(std::string &local_server_name, std::shared_ptr<TransferMetadata> meta,
...
int RdmaTransport::registerLocalMemory(void *addr, size_t length, const std::string &name, bool remote_accessible,
	...
	if (name == kWildcardLocation) {
		const std::vector<MemoryLocationEntry> entries = getMemoryLocation(addr, length);
		if (entries.empty())
			return -1;
...
int RdmaTransport::registerLocalMemoryBatch(const std::vector<RdmaTransport::BufferEntry> &buffer_list,
	...
	for (size_t i = 0; i < buffer_list.size(); ++i) {
		if (results[i].get()) {
			LOG(WARNING) << "RdmaTransport: Failed to register memory: addr " << buffer_list[i].addr << " length "
...
int RdmaTransport::unregisterLocalMemoryBatch(const std::vector<void *> &addr_list) {
	...
	for (auto &addr : addr_list) {
		results.emplace_back(
		    std::async(std::launch::async, [this, addr]() -> int { return unregisterLocalMemory(addr, false); }));
	}

	for (size_t i = 0; i < addr_list.size(); ++i) {
		if (results[i].get())
			LOG(WARNING) << "RdmaTransport: Failed to unregister memory: addr " << addr_list[i];
	}

	return metadata_->updateLocalSegmentDesc();
}

Status RdmaTransport::submitTransfer(BatchID batch_id, const std::vector<TransferRequest> &entries) {
	auto &batch_desc = *((BatchDesc *)(batch_id));
	if (batch_desc.task_list.size() + entries.size() > batch_desc.batch_size) {
		LOG(ERROR) << "RdmaTransport: Exceed the limitation of current batch's "
		              "capacity";
		return Status::InvalidArgument("RdmaTransport: Exceed the limitation of capacity, batch id: " +
	...
	for (auto &request : entries) {
		...
		for (uint64_t offset = 0; offset < request.length; offset += kBlockSize) {
			...
			while (retry_cnt < kMaxRetryCount) {
				if (selectDevice(local_segment_desc.get(), (uint64_t)slice->source_addr, slice->length, buffer_id,
				                 device_id, retry_cnt++))
					continue;
			...
			if (device_id < 0) {
				...
				for (auto &entry : slices_to_post)
					for (auto s : entry.second)
						delete s;
				LOG(ERROR) << "RdmaTransport: Address not registered by any device(s) " << source_addr;
				return Status::AddressNotRegistered("RdmaTransport: not registered by any device(s), "
				                                    "address: " +
...
Status RdmaTransport::submitTransferTask(const std::vector<TransferRequest *> &request_list,
	...
	for (size_t index = 0; index < request_list.size(); ++index) {
		...
		for (uint64_t offset = 0; offset < request.length; offset += kBlockSize) {
			...
			if (!found_device) {
				...
				LOG(ERROR) << "Memory region not registered by any active device(s): " << source_addr;
				return Status::AddressNotRegistered("Memory region not registered by any active device(s): " +
				                                    std::to_string(reinterpret_cast<uintptr_t>(source_addr)));
			}
...
Status RdmaTransport::getTransferStatus(BatchID batch_id, std::vector<TransferStatus> &status) {
	auto &batch_desc = *((BatchDesc *)(batch_id));
...
Status RdmaTransport::getTransferStatus(BatchID batch_id, size_t task_id, TransferStatus &status) {
	auto &batch_desc = *((BatchDesc *)(batch_id));
	const size_t task_count = batch_desc.task_list.size();
	if (task_id >= task_count) {
		return Status::InvalidArgument("RdmaTransport::getTransportStatus invalid argument, batch id: " +
		                               std::to_string(batch_id));
	}
	auto &task = batch_desc.task_list[task_id];
...
#ifdef CONFIG_ERDMA
	if (context->deleteEndpoint(peer_desc.local_nic_path))
		return ERR_ENDPOINT;
#endif
...
int RdmaTransport::initializeRdmaResources() {
	...
	for (auto &device_name : hca_list) {
		...
		int ret = context->construct(config.num_cq_per_ctx, config.num_comp_channels_per_ctx, config.port,
...
int RdmaTransport::startHandshakeDaemon(std::string &local_server_name) {
	return metadata_->startHandshakeDaemon(
	    std::bind(&RdmaTransport::onSetupRdmaConnections, this, std::placeholders::_1, std::placeholders::_2),
...
int RdmaTransport::selectDevice(SegmentDesc *desc, uint64_t offset, size_t length, int &buffer_id, int &device_id,
                                int retry_count) {
	if (!desc)
