...
MsgChannel::MsgChannel(RdmaTransport &transport, RdmaContext &context,
                       std::string peer_server_name)
    : transport_(transport),
      context_(context),
      peer_server_name_(std::move(peer_server_name)) {
    max_pending_sends_ = globalConfig().rdma_msg_pool_base;
}
...
int MsgChannel::createResources() {
    ...
    size_t slots = cfg.rdma_msg_pool_base;
    size_t slot_size = cfg.rdma_msg_slot_size;
    max_pending_sends_ = slots;
    if (slots == 0 || slot_size < kMsgHeaderSize + 64) {
    ...
    attr.cap.max_send_wr = static_cast<uint32_t>(slots);
    attr.cap.max_recv_wr = static_cast<uint32_t>(slots);
    ...
    inflight_slots_.assign(slots, -1);
    if (repostAllRecvs()) {
        destroyResources();
        return ERR_ENDPOINT;
    }
...
int MsgChannel::repostAllRecvs() {
    for (size_t i = 0; i < pool_.slotCount(); ++i) {
...
int MsgChannel::connectActive() {
    ...
    max_pending_sends_ =
        std::min(max_pending_sends_,
                 peer_desc.msg_rq_depth
                     ? static_cast<size_t>(peer_desc.msg_rq_depth)
                     : max_pending_sends_);
    std::lock_guard<std::mutex> lock(resource_mutex_);
...
int MsgChannel::postSend(const MsgHeader &hdr, const void *payload,
    ...
    if (pending_sends_ >= max_pending_sends_) return ERR_TOO_MANY_REQUESTS;

    int slot = pool_.acquireSendSlot();
    if (slot < 0) return ERR_TOO_MANY_REQUESTS;
...
void MsgChannel::dispatchRecv(size_t idx, size_t byte_len) {
    ...
    if (expects_payload && kMsgHeaderSize + hdr.length > byte_len) {
        LOG(ERROR) << "MsgChannel: truncated payload from " << peer_server_name_;
        return;
    }
    ...
    transport_.onMsgReceived(peer_server_name_, hdr, payload);
}
