...
namespace mooncake {
std::shared_ptr<RdmaEndPoint> FIFOEndpointStore::getEndpoint(const std::string &peer_nic_path) {
...
std::shared_ptr<RdmaEndPoint> FIFOEndpointStore::insertEndpoint(const std::string &peer_nic_path,
                                                                RdmaContext *context) {
	...
	int ret =
	    endpoint->construct(context->cq(), config.num_qp_per_ep, config.max_sge, config.max_wr, config.max_inline);
	if (ret)
		return nullptr;
...
std::shared_ptr<RdmaEndPoint> SIEVEEndpointStore::getEndpoint(const std::string &peer_nic_path) {
	RWSpinlock::ReadGuard guard(endpoint_map_lock_);
	auto iter = endpoint_map_.find(peer_nic_path);
	if (iter != endpoint_map_.end()) {
		iter->second.second.store(true, std::memory_order_relaxed); // This is safe within read lock
...
std::shared_ptr<RdmaEndPoint> SIEVEEndpointStore::insertEndpoint(const std::string &peer_nic_path,
                                                                 RdmaContext *context) {
	RWSpinlock::WriteGuard guard(endpoint_map_lock_);
	if (endpoint_map_.find(peer_nic_path) != endpoint_map_.end()) {
		LOG(INFO) << "Endpoint " << peer_nic_path << " already exists in SIEVEEndpointStore";
