...
int WorkerPool::submitPostSend(
    ...
    if (submitted_slice_count == 0 &&
        ...
        if (markContextFailure(all_rails_failed_peers,
...
bool WorkerPool::markContextFailure(
    const std::unordered_set<std::string> &failed_peers, int min_peers,
    ...
    auto rec = health_tracker_.recordFailure(
        failed_peers, kContextFailureThreshold, min_peers,
...
void WorkerPool::handleLocalFailure(const std::string &peer_nic_path,
    ...
    bool context_disabled = markContextFailure(
        {peer_server}, 1, "local completion-failure batches");
