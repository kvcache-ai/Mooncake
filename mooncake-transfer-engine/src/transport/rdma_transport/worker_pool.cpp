...
WorkerPool::WorkerPool(RdmaContext &context, int numa_socket_id)
    : context_(context), numa_socket_id_(numa_socket_id), workers_running_(true), suspended_flag_(0),
      redispatch_counter_(0), submitted_slice_count_(0), processed_slice_count_(0) {
...
int WorkerPool::submitPostSend(const std::vector<Transport::Slice *> &slice_list) {
	...
	for (auto &slice : slice_list) {
		...
		if (!segment_desc_map.count(target_id)) {
			...
			if (!segment_desc_map[target_id]) {
				segment_desc_map.clear();
				LOG(ERROR) << "Cannot get target segment description #" << target_id;
	...
	for (auto &slice : slice_list) {
		...
		if (RdmaTransport::selectDevice(peer_segment_desc.get(), slice->rdma.dest_addr, slice->length, buffer_id,
		                                device_id)) {
			...
			if (RdmaTransport::selectDevice(peer_segment_desc.get(), slice->rdma.dest_addr, slice->length, buffer_id,
			                                device_id)) {
				slice->markFailed();
				context_.engine().meta()->dumpMetadataContent(peer_segment_desc->name, slice->rdma.dest_addr,
				                                              slice->length);
				continue;
			}
		}
	...
	for (int shard_id = 0; shard_id < kShardCount; ++shard_id) {
		...
		slice_queue_count_[shard_id].fetch_add(slice_list_map[shard_id].size(), std::memory_order_relaxed);
...
void WorkerPool::performPostSend(int thread_id) {
	...
	for (auto &entry : local_slice_queue) {
		...
		if (!endpoint->connected() && endpoint->setupConnectionsByActive()) {
			LOG(ERROR) << "Worker: Cannot make connection for endpoint: " << entry.first << ", mark it inactive";
			for (auto &slice : entry.second)
				failed_slice_list.push_back(slice);
			endpoint->set_active(false);
			failed_nr_polls++;
			if (context_.active() && failed_nr_polls > 32 && !success_nr_polls) {
				LOG(WARNING) << "Failed to establish peer endpoints in local RNIC " << context_.nicPath()
				             << ", mark it inactive";
	...
	for (int cq_index = thread_id; cq_index < context_.cqCount(); cq_index += kTransferWorkerCount) {
		...
		for (int i = 0; i < nr_poll; ++i) {
			...
			if (wc[i].status != IBV_WC_SUCCESS) {
				...
				if (wc[i].status != IBV_WC_WR_FLUSH_ERR || show_work_request_flushed_error)
					LOG(ERROR) << "Worker: Process failed for slice (opcode: " << slice->opcode
					           << ", source_addr: " << slice->source_addr << ", length: " << slice->length
					           << ", dest_addr: " << (void *)slice->rdma.dest_addr
					           << ", local_nic: " << context_.deviceName() << ", peer_nic: " << slice->peer_nic_path
					           << ", dest_rkey: " << slice->rdma.dest_rkey << ", retry_cnt: " << slice->rdma.retry_cnt
					           << "): " << ibv_wc_status_str(wc[i].status);
				failed_nr_polls++;
				if (context_.active() && failed_nr_polls > 32 && !success_nr_polls) {
					LOG(WARNING) << "Too many errors found in local RNIC " << context_.nicPath()
					             << ", mark it inactive";
					context_.set_active(false);
				}
...
void WorkerPool::redispatch(std::vector<Transport::Slice *> &slice_list, int thread_id) {
	...
	for (auto &slice : slice_list) {
		if (slice->rdma.retry_cnt >= slice->rdma.max_retry_cnt) {
			...
			int buffer_id, device_id;
			if (!peer_segment_desc ||
			    RdmaTransport::selectDevice(peer_segment_desc.get(), slice->rdma.dest_addr, slice->length, buffer_id,
			                                device_id, slice->rdma.retry_cnt)) {
				slice->markFailed();
				processed_slice_count_++;
				continue;
			}
...
int WorkerPool::doProcessContextEvents() {
	...
	if (ibv_get_async_event(context_.context(), &event) < 0)
		return ERR_CONTEXT;
	LOG(WARNING) << "Worker: Received context async event " << ibv_event_type_str(event.event_type) << " for context "
	             << context_.deviceName();
	if (event.event_type == IBV_EVENT_QP_FATAL) {
		auto endpoint = (RdmaEndPoint *)event.element.qp->qp_context;
		endpoint->set_active(false);
	} else if (event.event_type == IBV_EVENT_DEVICE_FATAL || event.event_type == IBV_EVENT_CQ_ERR ||
	           event.event_type == IBV_EVENT_WQ_FATAL || event.event_type == IBV_EVENT_PORT_ERR ||
