...
static int initServerNetSocket(RankInfo *local_rank_info) {
	RETRY_CALL(HcclNetInit(NICDeployment::NIC_DEPLOYMENT_DEVICE, local_rank_info->devicePhyId,
	...
	RETRY_CALL(hrtRaGetSingleSocketVnicIpInfo(local_rank_info->devicePhyId, DeviceIdType::DEVICE_ID_TYPE_PHY_ID,
	                                          local_rank_info->devicePhyId, localVnicIp),
	           "hrtRaGetSingleSocketVnicIpInfo failed");

	RETRY_CALL(HcclNetOpenDev(&vnicNetDevCtx_, NicType::VNIC_TYPE, local_rank_info->devicePhyId,
...
static int initControlSocket(RankInfo *local_rank_info) {
	...
	LOG(INFO) << "initControlSocket successful, Server listening on host port" << ntohs(g_server_addr_.sin_port)
	          << "..."
	          << "g_server_socket_" << g_server_socket_;
...
int initTransportMem(RankInfo *local_rank_info) {
	...
	LOG(INFO) << "initTransportMem local_rank_info rankId: " << local_rank_info->rankId
	          << ", serverIdx: " << local_rank_info->serverIdx << ", deviceLogicId: " << local_rank_info->deviceLogicId
	          << ", devicePhyId: " << local_rank_info->devicePhyId
	          << ", deviceIp: " << inet_ntoa(local_rank_info->deviceIp)
	          << ", devicePort: " << local_rank_info->devicePort << ", hostIp: " << inet_ntoa(local_rank_info->hostIp)
	          << ", hostPort: " << local_rank_info->hostPort << ", pid: " << local_rank_info->pid;
...
int controlInfoSend(RankInfo *local_rank_info, RankInfo *remote_rank_info) {
	...
	LOG(INFO) << "transportMemTask local_rank_info rankId: " << local_rank_info->rankId
	          << ", serverIdx: " << local_rank_info->serverIdx << ", deviceLogicId: " << local_rank_info->deviceLogicId
	          << ", devicePhyId: " << local_rank_info->devicePhyId
	          << ", deviceIp: " << inet_ntoa(local_rank_info->deviceIp)
	          << ", devicePort: " << local_rank_info->devicePort << ", hostIp: " << inet_ntoa(local_rank_info->hostIp)
	          << ", hostPort: " << local_rank_info->hostPort << ", pid: " << local_rank_info->pid;
	LOG(INFO) << "transportMemTask remote_rank_info rankId: " << remote_rank_info->rankId
	          ...
	          << ", devicePort: " << remote_rank_info->devicePort << ", hostIp: " << inet_ntoa(remote_rank_info->hostIp)
	          << ", hostPort: " << remote_rank_info->hostPort << ", pid: " << remote_rank_info->pid;
	...
	int client_socket = connectToTarget(inet_ntoa(remote_rank_info->hostIp), remote_rank_info->hostPort);
	if (client_socket < 0) {
...
int createClientSocket(std::shared_ptr<hccl::HcclSocket> &hccl_socket, RankInfo *local_rank_info,
                       RankInfo *remote_rank_info, bool is_cross_hccs, std::string tag) {
	int ret = 0;
	hccl::HcclIpAddress rempoteDevIp;
	std::string key_str = inet_ntoa(remote_rank_info->hostIp) + std::to_string(remote_rank_info->devicePhyId);
	std::string baseTag_ =
	    inet_ntoa(local_rank_info->hostIp) + std::to_string(local_rank_info->devicePhyId) + key_str + tag;
	if (!is_cross_hccs) {
		...
		ret = hrtRaGetSingleSocketVnicIpInfo(local_rank_info->devicePhyId, DeviceIdType::DEVICE_ID_TYPE_PHY_ID,
		...
		hccl_socket =
		    std::make_shared<hccl::HcclSocket>(baseTag_, vnicNetDevCtx_, rempoteDevIp, remote_rank_info->devicePort,
		                                       hccl::HcclSocketRole::SOCKET_ROLE_CLIENT);
	} else {
		rempoteDevIp = hccl::HcclIpAddress(remote_rank_info->deviceIp);
		hccl_socket =
		    std::make_shared<hccl::HcclSocket>(baseTag_, nicNetDevCtx_, rempoteDevIp, remote_rank_info->devicePort,
		                                       hccl::HcclSocketRole::SOCKET_ROLE_CLIENT);
	}
	...
	if (ret) {
		...
		LOG(ERROR) << "client hccl_socket init failed, target rank_id:" << remote_rank_info->devicePhyId
		           << ",local rank_id:" << local_rank_info->devicePhyId << ", rempoteDevIp:" << deviceIp
		           << ", remote port:" << remote_rank_info->devicePort << ", ret: " << ret;
		return ret;
	}
	...
	if (ret) {
		char deviceIp[64];
		inet_ntop(AF_INET, &rempoteDevIp, deviceIp, sizeof(deviceIp));
		LOG(ERROR) << "client hccl_socket Connect failed, target rank_id:" << remote_rank_info->devicePhyId
		           << ",local rank_id:" << local_rank_info->devicePhyId << ", rempoteDevIp:" << deviceIp
		           << ", remote port:" << remote_rank_info->devicePort << ", ret: " << ret;
		return ret;
	}
...
int createTransportMem(RankInfo *local_rank_info, RankInfo *remote_rank_info,
	...
	if (is_cross_hccs) {
		transport_mem = hccl::TransportMem::Create(hccl::TransportMem::TpType::ROCE, notifyPool_, nicNetDevCtx_,
		                                           dispatcher_, attrInfo);
	} else {
		transport_mem = hccl::TransportMem::Create(hccl::TransportMem::TpType::IPC, notifyPool_, vnicNetDevCtx_,
		                                           dispatcher_, attrInfo);
	}
	...
	if (ret) {
		...
		LOG(ERROR) << "client SetDataSocket failed, target rank_id:" << remote_rank_info->devicePhyId
		           << ",local rank_id:" << local_rank_info->devicePhyId << ", rempoteDevIp:" << deviceIp
		           << ", remote port:" << remote_rank_info->devicePort << ", ret: " << ret;
		return ret;
	}
	...
	if (ret) {
		char deviceIp[64];
		inet_ntop(AF_INET, &remote_rank_info->deviceIp, deviceIp, sizeof(deviceIp));
		LOG(ERROR) << "client SetSocket failed, target rank_id:" << remote_rank_info->devicePhyId
		           << ",local rank_id:" << local_rank_info->devicePhyId << ", rempoteDevIp:" << deviceIp
		           << ", remote port:" << remote_rank_info->devicePort << ", ret: " << ret;
		return ret;
	}
	...
	if (ret) {
		char deviceIp[64];
		inet_ntop(AF_INET, &remote_rank_info->deviceIp, deviceIp, sizeof(deviceIp));
		LOG(ERROR) << "client Connect failed, target rank_id:" << remote_rank_info->devicePhyId
		           << ",local rank_id:" << local_rank_info->devicePhyId << ", rempoteDevIp:" << deviceIp
		           << ", remote port:" << remote_rank_info->devicePort << ", ret: " << ret;
		return ret;
	}
	...
	for (size_t i = 0; i < m_num; ++i) {
		...
		if (ret != 0 && ret != 20) {
			LOG(ERROR) << "HcclMemReg failed, ret:" << ret << " addr: " << g_localMemAddr[i]
			           << " len: " << g_localMemLen[i];
			return ret;
		}
		char *desc = nullptr;
		...
		if (ret) {
			LOG(ERROR) << "HcclMemExport failed, ret:" << ret << " addr: " << g_localMemAddr[i]
			           << " len: " << g_localMemLen[i];
			return ret;
		}
		...
		if (memcpy_s(rmaMemDescs[i].memDesc, hccl::TRANSPORT_EMD_ESC_SIZE, desc, desc_len + 1) != EOK) {
			LOG(ERROR) << "memcpy_s failed, ret:" << ret << " addr: " << g_localMemAddr[i]
			           << " len: " << g_localMemLen[i];
			return -1;
		}
		...
		if (!is_cross_hccs) {
			...
			if (ret) {
				LOG(ERROR) << "HcclMemGrant failed, ret:" << ret << " addr: " << g_localMemAddr[i]
				           << " len: " << g_localMemLen[i];
				;
				return ret;
			}
	...
	for (uint32_t i = 0; i < m_num; ++i) {
		...
		if (ret) {
			LOG(ERROR) << "transport_mem->EnableMemAccess failed, ret:" << ret << " i:" << i
			           << "local_rank: " << local_rank_info->devicePhyId
			           << "remote_rank: " << remote_rank_info->devicePhyId;
			return ret;
		}
...
int transportMemTask(RankInfo *local_rank_info, RankInfo *remote_rank_info, int op_code, uint64_t offset,
                     uint64_t req_len, void *local_mem, aclrtStream stream) {
	int ret = 0;
	// 1. Check if a connection has been established with the peer, and send
	// local information to the peer
	std::string key_str = inet_ntoa(remote_rank_info->hostIp) + std::to_string(remote_rank_info->devicePhyId);
	...
	// 2. Check if transport_mem has been established with the peer, and send
	// local information to the peer.
	std::shared_ptr<hccl::TransportMem> transport_mem {};
	...
	if (op_code == WRITE) {
		...
		if (ret) {
			LOG(ERROR) << "transport_mem Read failed, localMem.addr: " << local_mem << "local_mem.size: " << req_len
			           << ", remoteMem.addr: " << remoteMem.addr << ", remoteMem.size: " << req_len << ", ret: " << ret
			           << "retry failed..";
			return ret;
		}
	} else {
	...
	if (ret) {
		LOG(ERROR) << "aclrtSynchronizeStream failed, localMem.addr: " << local_mem << "local_mem.size: " << req_len
		           << ", remoteMem.addr: " << remoteMem.addr << ", remoteMem.size: " << req_len << ", ret: " << ret;
		return ret;
	}
	...
	if (printEnabled()) {
		...
		LOG(INFO) << "pid: " << pid << "; "
		          << "thread submit one block size: " << req_len;
		LOG(INFO) << "pid: " << pid << "; "
		          << "thread call write/read spent: " << duration_call.count() << "us";
		LOG(INFO) << "pid: " << pid << "; "
		          << "thread sync stream spent: " << duration_sync.count() << "us";
	} else {
		(void)start;
		(void)mid;
		(void)stop;
...
int acceptSocket(std::shared_ptr<hccl::HcclSocket> &hccl_socket, RankInfo *local_rank_info,
                 RankControlInfo remote_control_info, std::string baseTag_, hccl::HcclIpAddress rempoteDevIp,
                 bool is_cross_hccs) {
	int ret = 0;
...
int transportMemAccept(RankInfo *local_rank_info) {
	...
	LOG(INFO) << "Received remote_control_info, deviceLogicId: " << remote_control_info.deviceLogicId
	...
	if (iter != target_key_to_transport_mem_map_.end()) {
		LOG(WARNING) << "A duplicate connection request from the same remote endpoint "
	...
	if (!is_cross_hccs) {
		...
		ret = acceptSocket(hccl_ctrl_socket, local_rank_info, remote_control_info, baseTag_ + "ctrl", rempoteDevIp,
		                   is_cross_hccs);
		if (ret) {
		...
		ret = acceptSocket(hccl_data_socket, local_rank_info, remote_control_info, baseTag_ + "data", rempoteDevIp,
		                   is_cross_hccs);
		if (ret) {
	...
	if (ret) {
		char deviceIp[64];
		inet_ntop(AF_INET, &rempoteDevIp, deviceIp, sizeof(deviceIp));
		LOG(ERROR) << "client SetDataSocket failed, target rank_id:" << remote_control_info.devicePhyId
		           << ",local rank_id:" << local_rank_info->devicePhyId << ", rempoteDevIp:" << deviceIp
		           << ", ret: " << ret;
		return ret;
	}
	...
	if (ret) {
		char deviceIp[64];
		inet_ntop(AF_INET, &rempoteDevIp, deviceIp, sizeof(deviceIp));
		LOG(ERROR) << "client Connect failed, target rank_id:" << remote_control_info.devicePhyId
		           << ",local rank_id:" << local_rank_info->devicePhyId << ", rempoteDevIp:" << deviceIp
		           << ", ret: " << ret;
		return ret;
	}
