...
std::vector<std::shared_ptr<hccl::HcclSocket>> g_connect_socket;
std::vector<std::shared_ptr<hccl::HcclSocket>> g_accept_socket;
...
int createTransportMem(RankInfo *local_rank_info, RankInfo *remote_rank_info, std::shared_ptr<hccl::TransportMem>& transport_mem) {
    ...
    g_connect_socket.push_back(hccl_socket);
    ...
    for (size_t i = 0; i < m_num; ++i) {
        hccl::TransportMem::RmaMem localRmaMem = {hccl::RmaMemType::DEVICE, g_localMemAddr[i], (uint64_t)g_localMemLen[i]};
        ret = transport_mem->RegMem(localRmaMem, rmaMemDescs[i]);
        if (ret) {
            LOG(ERROR) << "transport_mem->RegMem failed, ret:" << ret << " addr: " << g_localMemAddr[i] << " len: " << (uint64_t)g_localMemLen[i];
            return ret;
        }
    }
                    ...
                    uint64_t req_len, void *local_mem, aclrtStream stream)
{
    ...
    if (op_code == WRITE){
        ...
        if (ret) {
            LOG(WARNING) << "transport_mem Write failed, localMem.addr: " 
                        << local_mem << "local_mem.size: " << req_len
                        << ", remoteMem.addr: "  << remoteMem.addr 
                        << ", remoteMem.size: " << req_len
                        << ", ret: " << ret
                        << "retry connect to remote..";
            // 重试应对对端重启
            ret = controlInfoSend(local_rank_info, remote_rank_info);
            if (ret) {
                LOG(ERROR) << "controlInfoSend failed, ret: " << ret;
            ...
            ret = transport_mem->Write(localMem, remoteMem, stream);
            if (ret) {
        ...
        if (ret) {
            LOG(WARNING) << "transport_mem Read failed, localMem.addr: " 
                        << local_mem << "local_mem.size: " << req_len
                        << ", remoteMem.addr: "  << remoteMem.addr 
                        << ", remoteMem.size: " << req_len
                        << ", ret: " << ret
...
int transportMemAccept(RankInfo *local_rank_info) {
    ...
    // 根据host_ip+device_id，查找是否存在相同的点对点TransportMem
    ...
    g_accept_socket.push_back(hccl_socket);
    ...
    for (size_t i = 0; i < m_num; ++i) {
        ...
        ret = transport_mem->RegMem(localRmaMem, rmaMemDescs[i]);
        if (ret) {
            LOG(ERROR) << "transport_mem->RegMem failed, ret:" << ret;
            return ret;
        }
    }
    hccl::TransportMem::RmaMemDescs localRmaMemDescs;
