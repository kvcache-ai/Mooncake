...
void HcclTransport::initiatorLoop(int deviceLogicId, int selfIdx) {
	...
	while (running_) {
		...
		ret = transportMemTask(&local_rank_info_, &remote_rank_info_, slice->opcode, slice->hccl.dest_addr,
...
int HcclTransport::initPdThread() {
	...
	LOG(INFO) << "HcclTransport: initPdThread, pid: " << pid << ";"
	          << "init " << THREAD_NUM << " initiator threads and accept threads, deviceLogicId: " << deviceLogicId;
	return 0;
}

// Get HostIp\Port\DevicePhyId
int HcclTransport::getDevIdAndIpPortFromServerName(std::string &identifier, std::string &hostIp, int &port,
                                                   int &npuId) {
		...
		if (line.rfind("address_", 0) == 0) {
			...
			if (equal_pos != std::string::npos) {
				...
				if (key == std::to_string(devicePhyId)) {
					...
					deviceIp.erase(deviceIp.begin(), std::find_if(deviceIp.begin(), deviceIp.end(),
					                                              [](unsigned char c) { return !std::isspace(c); }));
					deviceIp.erase(std::find_if(deviceIp.rbegin(), deviceIp.rend(),
					                            [](unsigned char c) { return !std::isspace(c); })
					                   .base(),
					               deviceIp.end());
					...
					LOG(INFO) << "rankInfoParse Success, hostIp: " << hostIp << " rankId: " << local_rank_info_.rankId
					          ...
					          << " devicePort: " << local_rank_info_.devicePort << " deviceIp: " << deviceIp
					          << " pid:" << local_rank_info_.pid;
...
int HcclTransport::install(std::string &local_server_name, std::shared_ptr<TransferMetadata> meta,
	...
	LOG(INFO) << "HcclTransport: begin to install transport, local devicePhyId: " << devicePhyId
...
Status HcclTransport::submitTransfer(BatchID batch_id, const std::vector<TransferRequest> &entries) {
	auto &batch_desc = *((BatchDesc *)(batch_id));
	if (batch_desc.task_list.size() + entries.size() > batch_desc.batch_size) {
		LOG(ERROR) << "HcclTransport: Exceed the limitation of current batch's "
		              "capacity";
		return Status::InvalidArgument("HcclTransport: Exceed the limitation of capacity, batch id: " +
...
Status HcclTransport::submitTransferTask(const std::vector<TransferRequest *> &request_list,
...
Status HcclTransport::getTransferStatus(BatchID batch_id, size_t task_id, TransferStatus &status) {
	auto &batch_desc = *((BatchDesc *)(batch_id));
	const size_t task_count = batch_desc.task_list.size();
	if (task_id >= task_count) {
		return Status::InvalidArgument("HcclTransport::getTransportStatus invalid argument, batch id: " +
		                               std::to_string(batch_id));
	}
...
int HcclTransport::registerLocalMemoryBatch(const std::vector<Transport::BufferEntry> &buffer_list,
...
int HcclTransport::unregisterLocalMemoryBatch(const std::vector<void *> &addr_list) {
	for (auto &addr : addr_list)
		unregisterLocalMemory(addr, -1);
	return metadata_->updateLocalSegmentDesc();
}
