...
void HeterogeneousTcpTransport::transferLoop() {
    ...
    while (running_) {
        ...
        if (transferQueues_.empty()) {
            transfer_cond_.wait(
                lock, [this] { return !transferQueues_.empty() || !running_; });
            if (transferQueues_.empty()) {
                continue;
            }
            ...
            ret = aclrtMemcpyAsync(static_cast<char*>(hostAddr_) + offset_,
                                   ...
                                   total_length, ACL_MEMCPY_DEVICE_TO_HOST,
                                   stream);
            if (ret) {
                LOG(ERROR)
                    << "HeterogeneousTcpTransport: aclrtMemcpyAsync dtoh "
                       "error, ret: "
            ...
            if (ret) {
                LOG(ERROR)
                    << "HeterogeneousTcpTransport: aclrtSynchronizeStream "
                       "error, ret: "
                    << ret;
                transfer_counter_.fetch_add(1);
                continue;
            ...
            if (!s.ok()) {
                LOG(ERROR) << "HeterogeneousTcpTransport: Tcp "
    ...
    int HeterogeneousTcpTransport::install(
        ...
        if (ret) {
            LOG(ERROR)
                << "HeterogeneousTcpTransport: aclrtGetDevice failed, ret: "
                << ret;
            return ret;
        }
        ...
        if (ret) {
            LOG(ERROR) << "HeterogeneousTcpTransport::TcpTransport install "
                          "error, ret: "
                       << ret;
            return ret;
        }
    ...
    int HeterogeneousTcpTransport::registerLocalMemory(
        void* addr, size_t length, const std::string& name,
        ...
        if (attributes.location.type != 1) {
            ret = transport_->registerLocalMemory(addr, length, "cpu", true,
                                                  true);
            if (ret) {
                LOG(ERROR) << "heterogeneous tcp transport registerLocalMemory "
                              "error, ret: "
                           << ret;
                return ret;
            }
    ...
    int HeterogeneousTcpTransport::unregisterLocalMemory(void* addr,
        ...
        if (attributes.location.type != 1) {
            ...
            if (ret) {
                LOG(ERROR) << "heterogeneous tcp transport "
                              "unregisterLocalMemory error, ret: "
                           << ret;
                return ret;
            }
        }
    ...
    int HeterogeneousTcpTransport::registerLocalMemoryBatch(
        ...
        for (auto& buffer : buffer_list) {
            ...
            if (ret) {
                LOG(ERROR)
                    << "HeterogeneousTcpTransport registerLocalMemoryBatch "
                       "error, ret: "
                    << ret;
                return ret;
            }
    ...
    int HeterogeneousTcpTransport::createStream() {
        if (firstSubmit_) {
            ...
            if (ret) {
                LOG(ERROR) << "HeterogeneousTcpTransport: aclrtCreateStream "
                              "error, ret: "
                           << ret;
                return ret;
            }
    ...
    Status HeterogeneousTcpTransport::submitTransferTask(
        ...
        if (ret) {
            LOG(ERROR) << "tcp aclrtPointerGetAttributes error, ret: " << ret;
            return Status::InvalidArgument(
                "HeterogeneousTcpTransport: aclrtPointerGetAttributes error");
        }
        ...
        int usedHugeDevNum = 0;
        {
            ...
            while (index < task_list.size()) {
                ...
                std::unique_lock<std::mutex> lock(transfer_mutex_);
                transferQueues_.emplace(std::move(subTasks), total_length,
                                        devId_);
                lock.unlock();
                transfer_cond_.notify_one();
                subTasks.clear();
