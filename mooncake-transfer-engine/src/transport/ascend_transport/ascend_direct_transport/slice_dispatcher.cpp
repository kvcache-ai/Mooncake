...
void DefaultSliceDispatcher::enqueue(
    ...
    // processSliceList derives the ADXL direction from the first slice, so the
    // opcode has to be part of the key: a submission mixing reads and writes to
    // one segment would otherwise transfer part of it the wrong way.
    std::map<
