...
struct Session : public std::enable_shared_from_this<Session> {
	...
	void initiate(void *buffer, uint64_t dest_addr, size_t size, TransferRequest::OpCode opcode) {
		session_mutex_.lock();
...
private:
	void writeHeader() {
		...
		asio::async_write(socket_, asio::buffer(&header_, sizeof(SessionHeader)),
		                  [this, self](const asio::error_code &ec, std::size_t len) {
			                  if (ec || len != sizeof(SessionHeader)) {
				                  LOG(ERROR) << "Session::writeHeader failed. Error: " << ec.message()
				                             << " (value: " << ec.value() << ")"
				                             << ", bytes written: " << len << ", expected: " << sizeof(SessionHeader);
				                  if (on_finalize_)
	...
	void readHeader() {
		// LOG(INFO) << "readHeader";
		auto self(shared_from_this());
		asio::async_read(socket_, asio::buffer(&header_, sizeof(SessionHeader)),
		                 [this, self](const asio::error_code &ec, std::size_t len) {
			                 if (ec || len != sizeof(SessionHeader)) {
				                 LOG(ERROR) << "Session::readHeader failed. Error: " << ec.message()
				                            << " (value: " << ec.value() << ")"
				                            << ", bytes read: " << len << ", expected: " << sizeof(SessionHeader);
				                 if (on_finalize_)
					                 on_finalize_(TransferStatusEnum::FAILED);
				                 session_mutex_.unlock();
				                 return;
	...
	void writeBody() {
		...
		if (buffer_size == 0) {
			if (on_finalize_)
				on_finalize_(TransferStatusEnum::COMPLETED);
			session_mutex_.unlock();
			return;
		}

		asio::async_write(socket_, asio::buffer(addr + total_transferred_bytes_, buffer_size),
		                  [this, addr, self](const asio::error_code &ec, std::size_t transferred_bytes) {
			                  if (ec) {
				                  LOG(ERROR) << "Session::writeBody failed. Error: " << ec.message()
				                             << " (value: " << ec.value() << ")"
				                             << ", total_transferred_bytes_: " << total_transferred_bytes_
	...
	void readBody() {
		...
		asio::async_read(socket_, asio::buffer(addr + total_transferred_bytes_, buffer_size),
		                 [this, addr, self](const asio::error_code &ec, std::size_t transferred_bytes) {
			                 if (ec) {
				                 LOG(ERROR) << "Session::readBody failed. Error: " << ec.message()
				                            << " (value: " << ec.value() << ")"
				                            << ", total_transferred_bytes_: " << total_transferred_bytes_
				                            << ", current transferred_bytes: " << transferred_bytes;
				                 if (on_finalize_)
...
struct TcpContext {
	TcpContext(short port) : acceptor(io_context, asio::ip::tcp::endpoint(asio::ip::tcp::v4(), port)) {
	}
...
int TcpTransport::install(std::string &local_server_name, std::shared_ptr<TransferMetadata> meta,
...
int TcpTransport::registerLocalMemoryBatch(const std::vector<Transport::BufferEntry> &buffer_list,
...
int TcpTransport::unregisterLocalMemoryBatch(const std::vector<void *> &addr_list) {
	for (auto &addr : addr_list)
		unregisterLocalMemory(addr, false);
	return metadata_->updateLocalSegmentDesc();
}

Status TcpTransport::getTransferStatus(BatchID batch_id, size_t task_id, TransferStatus &status) {
	auto &batch_desc = *((BatchDesc *)(batch_id));
	const size_t task_count = batch_desc.task_list.size();
	if (task_id >= task_count) {
		return Status::InvalidArgument("TcpTransport::getTransportStatus invalid argument, batch id: " +
...
Status TcpTransport::submitTransfer(BatchID batch_id, const std::vector<TransferRequest> &entries) {
	auto &batch_desc = *((BatchDesc *)(batch_id));
	if (batch_desc.task_list.size() + entries.size() > batch_desc.batch_size) {
		LOG(ERROR) << "TcpTransport: Exceed the limitation of current batch's "
		              "capacity";
		return Status::InvalidArgument("TcpTransport: Exceed the limitation of capacity, batch id: " +
		                               std::to_string(batch_id));
	}
...
Status TcpTransport::submitTransferTask(const std::vector<TransferRequest *> &request_list,
...
void TcpTransport::startTransfer(Slice *slice) {
	try {
		...
		auto endpoint_iterator =
		    resolver.resolve(asio::ip::tcp::v4(), meta_entry.ip_or_host_name, std::to_string(desc->tcp_data_port));
		...
		session->initiate(slice->source_addr, slice->tcp.dest_addr, slice->length, slice->opcode);
	} catch (std::exception &e) {
		LOG(ERROR) << "TcpTransport::startTransfer encountered an ASIO "
		              "exception. Slice details - source_addr: "
		           << slice->source_addr << ", length: " << slice->length << ", opcode: " << (int)slice->opcode
