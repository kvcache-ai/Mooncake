...
void CoroRPCInterface::setDataReceiveCallback(pybind11::function callback) {
    ...
    if (impl_->communicator) {
        ...
        impl_->communicator->setDataReceiveCallback(
            [interface_ptr](const std::string& source,
...
void CoroRPCInterface::handleIncomingData(const std::string& source,
                                          const std::string& data) {
...
pybind11::object CoroRPCInterface::ReceivedTensor::rebuildTensorInternal()
    const {
    std::cout << "DEBUG: Starting rebuildTensorInternal" << std::endl;
    std::cout << "DEBUG: Data size: " << data.size() << " bytes" << std::endl;
    std::cout << "DEBUG: TensorMetadata size: " << sizeof(TensorMetadata)
              << " bytes" << std::endl;
    ...
    std::cout << "DEBUG: Extracted metadata - dtype: " << metadata.dtype
              << ", ndim: " << metadata.ndim << std::endl;
    ...
    std::cout << "DEBUG: Element size: " << element_size << " bytes"
              << std::endl;
    ...
    for (int i = 0; i < metadata.ndim; i++) {
        ...
        std::cout << "DEBUG: Shape[" << i << "] = " << metadata.shape[i]
                  << std::endl;
    }
    ...
    std::cout << "DEBUG: Expected data size: " << expected_data_size << " bytes"
              << std::endl;
    std::cout << "DEBUG: Actual data size: " << actual_data_size << " bytes"
              << std::endl;
    ...
    std::cout << "DEBUG: Data pointer: "
              << static_cast<const void*>(tensor_data) << std::endl;
    std::cout << "DEBUG: Base data pointer: "
              << static_cast<const void*>(data.data()) << std::endl;
    ...
    try {
        pybind11::object numpy_array =
            create_numpy_array_from_data(tensor_data, dtype_enum, tensor_shape);
        std::cout << "DEBUG: Successfully created numpy array" << std::endl;

        // Convert to PyTorch tensor
        std::cout << "DEBUG: About to convert to PyTorch tensor..."
                  << std::endl;
        ...
        std::cout << "DEBUG: Error in tensor creation: " << e.what()
                  << std::endl;
        throw;
    }
}
