...
bool NvlinkHostNumaAllocation::ReleaseOwnedRange(void* base, size_t length) {
    auto& registry = OwnedRanges();
    std::lock_guard<std::mutex> lock(registry.mutex);
    auto entry = registry.ranges.find(base);
    if (entry == registry.ranges.end() || entry->second.length != length ||
        entry->second.registration_pins == 0)
        return false;
    --entry->second.registration_pins;
    return true;
}

NvlinkHostNumaAllocation::OwnedRangeResult
NvlinkHostNumaAllocation::AcquireOwnedRange(void* base, size_t length,
                                            DriverApi* api) {
    const uintptr_t begin = reinterpret_cast<uintptr_t>(base);
    if (!api || !base || !length ||
        length > std::numeric_limits<uintptr_t>::max() - begin)
        return OwnedRangeResult::kInvalid;
    auto& registry = OwnedRanges();
    std::lock_guard<std::mutex> lock(registry.mutex);
    auto entry = registry.ranges.find(base);
    if (entry != registry.ranges.end()) {
        if (entry->second.length != length) return OwnedRangeResult::kInvalid;
        if (entry->second.releasing) return OwnedRangeResult::kUnavailable;
        if (entry->second.registration_pins ==
            std::numeric_limits<size_t>::max())
            return OwnedRangeResult::kError;
        try {
        ...
        ++entry->second.registration_pins;
        return OwnedRangeResult::kPinned;
    }
    ...
    for (const auto& [owned_base, entry] : registry.ranges) {
        ...
        if (begin < owned_end && owned_begin < end)
            return OwnedRangeResult::kInvalid;
    }
    return OwnedRangeResult::kNotOwned;
}

bool NvlinkHostNumaAllocation::IsOwnedRangeBase(void* base) {
    auto& registry = OwnedRanges();
    std::lock_guard<std::mutex> lock(registry.mutex);
    return registry.ranges.count(base) != 0;
}
...
Status NvlinkHostNumaAllocation::CreateWithDriverApi(
    ...
    std::unique_ptr<NvlinkHostNumaAllocation>& allocation) {
    if (allocation)
        return Status::InvalidArgument("allocation output must be empty");
...
Status NvlinkHostNumaAllocation::Release() {
    ...
    auto& registry = OwnedRanges();
    std::unique_lock<std::mutex> registry_lock(registry.mutex, std::defer_lock);
    if (owned_range_registered_) {
        registry_lock.lock();
        auto entry = registry.ranges.find(base_);
        if (entry == registry.ranges.end() || entry->second.length != length_)
            return Status::Memory("HOST_NUMA owned-range provenance missing");
        if (entry->second.registration_pins != 0)
            return Status::Memory("HOST_NUMA allocation is still registered");
    ...
    if (owned_range_registered_) {
        // Keep address-free and provenance removal atomic to new owners that
        // may reserve the same VA. A later handle-release retry owns no VA.
        registry.ranges.erase(reinterpret_cast<void*>(ptr));
        owned_range_registered_ = false;
    }
    if (registry_lock.owns_lock()) registry_lock.unlock();
    if (handle_owned_) {
        const auto handle =
            static_cast<CUmemGenericAllocationHandle>(allocation_handle_);
        // Project policy treats a non-success result as retaining ownership,
        // so a later close can retry the same release stage.
        CUresult result = driver_api_.mem_release(handle);
        if (result != CUDA_SUCCESS) return cudaFailure("cuMemRelease", result);
        handle_owned_ = false;
...
#endif  // MOONCAKE_NVLINK_HOST_NUMA_ENABLED
