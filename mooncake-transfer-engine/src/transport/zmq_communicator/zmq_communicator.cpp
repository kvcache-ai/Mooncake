...
bool ZmqCommunicator::initialize(const ZmqConfig& config) {
    ...
    bool use_rdma = (protocol && std::string_view(protocol) == "rdma") || config.enable_rdma;
    ...
    //     LOG(INFO) << "ZMQ Communicator using RDMA transport";
    // } else {
        LOG(INFO) << "ZMQ Communicator using TCP transport (RDMA not yet configured)";
    // }
    ...
    LOG(INFO) << "ZMQ Communicator initialized with pool_size=" << config.pool_size;
    return true;
}
...
std::shared_ptr<BasePattern> ZmqCommunicator::createPattern(
    ...
    switch (type) {
        ...
        case ZmqSocketType::REP:
            return std::make_shared<ReqRepPattern>(client_pools_, server, false);
        case ZmqSocketType::PUB:
            return std::make_shared<PubSubPattern>(client_pools_, server, true);
        case ZmqSocketType::SUB:
            return std::make_shared<PubSubPattern>(client_pools_, server, false);
        case ZmqSocketType::PUSH:
            return std::make_shared<PushPullPattern>(client_pools_, server, true);
        case ZmqSocketType::PULL:
            return std::make_shared<PushPullPattern>(client_pools_, server, false);
...
coro_rpc::coro_rpc_server* ZmqCommunicator::getOrCreateServer(
    ...
    // const char* protocol = std::getenv("MC_RPC_PROTOCOL");
    // if ((protocol && std::string_view(protocol) == "rdma") || config_.enable_rdma) {
    //     try {
    //         server->init_ibv();
...
async_simple::coro::Lazy<RpcResult> ZmqCommunicator::sendDataAsync(
    ...
    SocketInfo* info;
    {
        ...
        if (!info || !info->pattern) {
            LOG(ERROR) << "Socket " << socket_id << " not found or pattern not created";
            co_return RpcResult{-1, "Invalid socket"};
        }
    }
    
    std::string endpoint = target_endpoint.value_or("");
    auto result = co_await info->pattern->sendAsync(endpoint, data, data_size, topic);
    co_return result;
}
...
async_simple::coro::Lazy<int> ZmqCommunicator::sendTensorAsync(
    ...
    std::string endpoint = target_endpoint.value_or("");
    auto result = co_await info->pattern->sendTensorAsync(endpoint, tensor, topic);
    co_return result;
}
...
void ZmqCommunicator::sendReply(int socket_id, const void* data, size_t data_size) {
    std::lock_guard lock(sockets_mutex_);
