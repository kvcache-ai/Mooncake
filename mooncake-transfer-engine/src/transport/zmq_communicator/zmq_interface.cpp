...
void ZmqInterface::shutdown() {
    impl_->communicator->shutdown();
}
...
TensorInfo ZmqInterface::extractTensor(pybind11::handle tensor) {
    ...
    pybind11::tuple shape_tuple = tensor_obj.attr("shape").cast<pybind11::tuple>();
...
pybind11::object ZmqInterface::request(int socket_id, pybind11::handle data) {
    ...
    auto result = async_simple::coro::syncAwait(
        impl_->communicator->sendDataAsync(socket_id, data_str.data(), data_str.size())
    );
...
pybind11::object ZmqInterface::requestAsync(int socket_id, pybind11::handle data,
    ...
    auto coro_lambda = [communicator, socket_id, data_str, future_obj, loop]()
        ...
        try {
            ...
            auto callback = [future_obj, loop, result]() {
                pybind11::gil_scoped_acquire acquire;
                if (result.code == 0) {
                    future_obj.attr("set_result")(pybind11::bytes(result.response_data));
                } else {
            ...
            auto callback = [future_obj, loop, e]() {
                pybind11::gil_scoped_acquire acquire;
                auto exc = pybind11::module_::import("builtins").attr("RuntimeError")(
                    pybind11::str(std::string("Exception: ") + e.what())
                );
                future_obj.attr("set_exception")(exc);
            };
            auto py_callback = pybind11::cpp_function(callback);
...
int ZmqInterface::publish(int socket_id, const std::string& topic,
    ...
    pybind11::gil_scoped_release release;
    auto result = async_simple::coro::syncAwait(
        impl_->communicator->sendDataAsync(socket_id, data_str.data(),
                                          data_str.size(), topic)
    );
...
pybind11::object ZmqInterface::publishAsync(int socket_id, const std::string& topic,
                                             pybind11::handle data,
                                             pybind11::handle loop) {
    pybind11::gil_scoped_acquire acquire;
    ...
    auto coro_lambda = [communicator, socket_id, data_str, topic, future_obj, loop]()
        -> async_simple::coro::Lazy<void> {
        try {
            auto result = co_await communicator->sendDataAsync(
...
void ZmqInterface::setSubscribeCallback(int socket_id, pybind11::function callback) {
    pybind11::gil_scoped_acquire acquire;
    impl_->data_callbacks[socket_id] = callback;
    
    auto interface_ptr = this;
    impl_->communicator->setReceiveCallback(socket_id,
        [interface_ptr, socket_id](std::string_view source, std::string_view data,
...
void ZmqInterface::setReceiveCallback(int socket_id, pybind11::function callback) {
    setSubscribeCallback(socket_id, callback);  // Reuse the same logic
}
...
pybind11::object ZmqInterface::sendTensorAsync(int socket_id, pybind11::handle tensor,
                                                pybind11::handle loop) {
    pybind11::gil_scoped_acquire acquire;
    ...
    auto coro_lambda = [communicator, socket_id, tensor_info, future_obj, loop, tensor_obj]()
        -> async_simple::coro::Lazy<void> {
        try {
            auto result = co_await communicator->sendTensorAsync(socket_id, tensor_info);
...
void ZmqInterface::setTensorReceiveCallback(int socket_id, pybind11::function callback) {
    pybind11::gil_scoped_acquire acquire;
    impl_->tensor_callbacks[socket_id] = callback;
    
    auto interface_ptr = this;
    impl_->communicator->setTensorReceiveCallback(socket_id,
        [interface_ptr, socket_id](std::string_view source, const TensorInfo& tensor,
                                   const std::optional<std::string>& topic) {
            pybind11::gil_scoped_acquire acquire;
...
void bind_zmq_interface(pybind11::module_& m) {
    ...
    py::class_<ZmqInterface>(m, "ZmqInterface")
        ...
        .def("set_tensor_receive_callback", &ZmqInterface::setTensorReceiveCallback);
}
