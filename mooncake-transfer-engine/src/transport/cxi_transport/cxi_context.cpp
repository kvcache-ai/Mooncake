...
int CxiContext::construct(size_t num_cq_list, size_t max_cqe,
    ...
    // IMPORTANT: FI_MR_ENDPOINT must be specified when using CXI, otherwise
    // fi_getinfo returns ENODATA CXI does not support FI_MR_VIRT_ADDR, so
    ...
    // Get fabric info
    int ret = fi_getinfo(fi_version(), nullptr, nullptr, 0, hints_, &fi_info_);
    if (ret) {
    ...
    if (ret) {
        LOG(ERROR) << "CxiContext::construct: buildSharedEndpoint failed for "
                   << device_name_;
        return ret;
    }
...
int CxiContext::registerMemoryRegionInternal(void* addr, size_t length,
    ...
    if (length > (size_t)globalConfig().max_mr_size) {
        ...
        LOG(ERROR)
            << "shared ep must be built before registering memory regions!";
    }
    ...
    if (cuda_ret == cudaSuccess && attributes.type == cudaMemoryTypeDevice) {
        ...
        cudaSetDevice(device_ordinal);  // if by any chance we are setting a
    ...
    if (ret) {
        LOG(ERROR) << "fi_mr_bind failed for " << addr << ": "
                   << fi_strerror(-ret);
        return ERR_CONTEXT;
    }
    ret = fi_mr_enable(mrMeta.mr);
    if (ret) {
        LOG(ERROR) << "fi_mr_enable failed for " << addr << ": "
                   << fi_strerror(-ret);
        return ERR_CONTEXT;
    }
