...
TransferMetadata::TransferMetadata(const std::string &conn_string) {
	...
	if (!handshake_plugin_) {
		LOG(ERROR) << "Unable to create metadata handshake plugin with conn string: " << conn_string;
	}
	...
	if (!storage_plugin_) {
		LOG(ERROR) << "Unable to create metadata storage plugin with conn string " << conn_string;
	}
}
...
int TransferMetadata::receivePeerNotify(const Json::Value &peer_json, Json::Value &local_json) {
...
int TransferMetadata::encodeSegmentDesc(const SegmentDesc &desc, Json::Value &segmentJSON) {
	...
	if (segmentJSON["protocol"] == "rdma") {
		...
		LOG(ERROR) << "Unsupported segment descriptor for register, name " << desc.name << " protocol "
		           << desc.protocol;
		return ERR_METADATA;
	}
	return 0;
...
int TransferMetadata::updateSegmentDesc(const std::string &segment_name, const SegmentDesc &desc) {
	...
	if (!storage_plugin_->set(getFullMetadataKey(segment_name), segmentJSON)) {
		LOG(ERROR) << "Failed to register segment descriptor, name " << desc.name << " protocol " << desc.protocol;
		return ERR_METADATA;
	}
...
int TransferMetadata::removeSegmentDesc(const std::string &segment_name) {
	...
	if (!storage_plugin_->remove(getFullMetadataKey(segment_name))) {
		LOG(ERROR) << "Failed to unregister segment descriptor, name " << segment_name;
		return ERR_METADATA;
	}
	return 0;
}

std::shared_ptr<TransferMetadata::SegmentDesc> TransferMetadata::decodeSegmentDesc(Json::Value &segmentJSON,
	...
	if (desc->protocol == "rdma") {
		for (const auto &deviceJSON : segmentJSON["devices"]) {
			...
			if (device.name.empty() || device.gid.empty()) {
				LOG(WARNING) << "Corrupted segment descriptor, name " << segment_name << " protocol " << desc->protocol;
				return nullptr;
			}
		...
		for (const auto &bufferJSON : segmentJSON["buffers"]) {
			...
			if (buffer.name.empty() || !buffer.addr || !buffer.length || buffer.rkey.empty() ||
		...
		int ret = desc->topology.parse(segmentJSON["priority_matrix"].toStyledString());
		if (ret) {
			LOG(WARNING) << "Corrupted segment descriptor, name " << segment_name << " protocol " << desc->protocol;
		}
		...
		for (const auto &bufferJSON : segmentJSON["buffers"]) {
			...
			if (buffer.name.empty() || !buffer.addr || !buffer.length || buffer.shm_name.empty()) {
				LOG(WARNING) << "Corrupted segment descriptor, name " << segment_name << " protocol " << desc->protocol
				             << "buffer name " << buffer.name << "buffer addr " << buffer.addr << "buffer length "
				             << buffer.length << "buffer shm_name " << buffer.shm_name;
				return nullptr;
			}
			desc->buffers.push_back(buffer);
		}
		...
		LOG(ERROR) << "Unsupported segment descriptor, name " << segment_name << " protocol " << desc->protocol;
		return nullptr;
	}
	return desc;
}

int TransferMetadata::receivePeerMetadata(const Json::Value &peer_json, Json::Value &local_json) {
...
std::shared_ptr<TransferMetadata::SegmentDesc> TransferMetadata::getSegmentDesc(const std::string &segment_name) {
...
int TransferMetadata::syncSegmentCache(const std::string &segment_name) {
	...
	for (auto &entry : segment_id_to_desc_map_) {
		...
		if (segment_desc)
			...
			LOG(WARNING) << "segment " << entry.second->name << " is now invalid";
	}
	return 0;
}

std::shared_ptr<TransferMetadata::SegmentDesc> TransferMetadata::getSegmentDescByName(const std::string &segment_name,
                                                                                      bool force_update) {
...
std::shared_ptr<TransferMetadata::SegmentDesc> TransferMetadata::getSegmentDescByID(SegmentID segment_id,
                                                                                    bool force_update) {
	if (segment_id != LOCAL_SEGMENT_ID && (!globalConfig().metacache || force_update)) {
		RWSpinlock::WriteGuard guard(segment_lock_);
		if (!segment_id_to_desc_map_.count(segment_id))
			return nullptr;
		auto segment_desc = getSegmentDesc(segment_id_to_desc_map_[segment_id]->name);
		if (!segment_desc)
			return nullptr;
		segment_id_to_desc_map_[segment_id] = segment_desc;
...
int TransferMetadata::addLocalSegment(SegmentID segment_id, const std::string &segment_name,
...
int TransferMetadata::removeLocalMemoryBuffer(void *addr, bool update_metadata) {
	bool addr_exist = false;
	{
		...
		*new_segment_desc = *segment_desc;
		segment_desc = new_segment_desc;
		for (auto iter = segment_desc->buffers.begin(); iter != segment_desc->buffers.end(); ++iter) {
...
int TransferMetadata::addRpcMetaEntry(const std::string &server_name, RpcMetaDesc &desc) {
	...
	if (p2p_handshake_mode_) {
		...
		handshake_plugin_->registerOnMetadataCallBack(
		    [this](const Json::Value &peer, Json::Value &local) -> int { return receivePeerMetadata(peer, local); });
...
int TransferMetadata::getRpcMetaEntry(const std::string &server_name, RpcMetaDesc &desc) {
	{
...
int TransferMetadata::startHandshakeDaemon(OnReceiveHandShake on_receive_handshake, uint16_t listen_port, int sockfd) {
	...
	handshake_plugin_->registerOnConnectionCallBack(
	    [on_receive_handshake](const Json::Value &peer, Json::Value &local) -> int {
	...
	handshake_plugin_->registerOnNotifyCallBack(
	    [this](const Json::Value &peer, Json::Value &local) -> int { return receivePeerNotify(peer, local); });
	return 0;
}

int TransferMetadata::sendHandshake(const std::string &peer_server_name, const HandShakeDesc &local_desc,
	...
	Json::Value peer;
	int ret = handshake_plugin_->send(peer_location.ip_or_host_name, peer_location.rpc_port, local, peer);
	if (ret)
		return ret;
	TransferHandshakeUtil::decode(peer, peer_desc);
	if (!peer_desc.reply_msg.empty()) {
		LOG(ERROR) << "Handshake rejected by " << peer_server_name << ": " << peer_desc.reply_msg;
		return ERR_METADATA;
	}
	return 0;
}

int TransferMetadata::sendNotify(const std::string &peer_server_name, const NotifyDesc &local_desc,
	...
	Json::Value peer;
	int ret = handshake_plugin_->sendNotify(peer_location.ip_or_host_name, peer_location.rpc_port, local, peer);
	if (ret)
		return ret;
	TransferNotifyUtil::decode(peer, peer_desc);
	if (peer_desc.notify_msg.empty()) {
		LOG(ERROR) << "Notify rejected by " << peer_server_name << ": " << peer_desc.notify_msg;
		return ERR_METADATA;
	}
	return 0;
}
