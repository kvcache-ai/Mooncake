...
int TransferMetadata::encodeSegmentDesc(const SegmentDesc &desc,
    ...
    // Encode protocols as JSON array
    Json::Value protocolsJSON(Json::arrayValue);
    for (const auto &protocol : desc.protocols) {
        protocolsJSON.append(protocol);
    }
    segmentJSON["protocols"] = protocolsJSON;

    // Check if at least one supported protocol is present (early validation)
    bool has_rdma = desc.hasProtocol("rdma");
    bool has_tcp = desc.hasProtocol("tcp");
    bool has_ascend = desc.hasProtocol("ascend");
    bool has_nvlink = desc.hasProtocol("nvlink");
    bool has_cxl = desc.hasProtocol("cxl");

    if (!has_rdma && !has_tcp && !has_ascend && !has_nvlink && !has_cxl) {
        LOG(ERROR) << "No supported protocol found, failed to encode segment "
                      "descriptor, name "
                   << desc.name;
        return ERR_METADATA;
    }
    ...
    // Prepare common buffer descriptions first
    ...
    for (const auto &buffer : desc.buffers) {
        ...
        // Let each protocol fill its own specific fields
        if (has_rdma) {
    ...
    // Encode RDMA-specific fields if RDMA protocol is present
    if (has_rdma) {
    ...
    // Encode Ascend-specific fields if Ascend protocol is present
    if (has_ascend) {
        Json::Value devicesJSON(Json::arrayValue);
        for (const auto &device : desc.devices) {
            Json::Value deviceJSON;
    ...
    // Encode CXL-specific fields if CXL protocol is present
    if (has_cxl) {
...
TransferMetadata::decodeSegmentDesc(Json::Value &segmentJSON,
    ...
    // Decode protocols from JSON array
    if (segmentJSON.isMember("protocols")) {
        for (const auto &protocolJSON : segmentJSON["protocols"]) {
            desc->protocols.push_back(protocolJSON.asString());
        }
    } else {
        LOG(ERROR) << "No protocols field found in segment descriptor, name "
                   << segment_name;
        return nullptr;
    }

    // Check if at least one supported protocol is present
    bool has_rdma = desc->hasProtocol("rdma");
    bool has_tcp = desc->hasProtocol("tcp");
    bool has_ascend = desc->hasProtocol("ascend");
    bool has_nvlink = desc->hasProtocol("nvlink");
    ...
    if (!has_rdma && !has_tcp && !has_ascend && !has_nvlink && !has_cxl &&
        !desc->hasProtocol("nvmeof")) {
        LOG(ERROR) << "No supported protocol found, failed to decode segment "
                      "descriptor, name "
                   << segment_name;
        return nullptr;
    }
    ...
    // Parse RDMA-specific fields if RDMA protocol is present
    if (has_rdma) {
            ...
            for (const auto &deviceJSON : segmentJSON["devices"]) {
                ...
                if (device.name.empty() || device.gid.empty()) {
                    LOG(WARNING) << "Corrupted segment descriptor, name "
                                 << segment_name << " protocol rdma";
                    LOG(WARNING)
                        << "segmentJSON: " << segmentJSON.toStyledString();
                    return nullptr;
                }
        ...
        if (segmentJSON.isMember("priority_matrix")) {
    ...
    // Parse Ascend-specific fields if Ascend protocol is present
    if (has_ascend) {
        if (segmentJSON.isMember("devices")) {
            for (const auto &deviceJSON : segmentJSON["devices"]) {
                DeviceDesc device;
        ...
        if (segmentJSON.isMember("rank_info")) {
    ...
    // Parse CXL-specific fields if CXL protocol is present
    if (has_cxl) {
        if (segmentJSON.isMember("cxl_name")) {
            desc->cxl_name = segmentJSON["cxl_name"].asString();
        }
        if (segmentJSON.isMember("cxl_base_addr")) {
            desc->cxl_base_addr = segmentJSON["cxl_base_addr"].asUInt64();
        }
    }

    // Parse buffers once for all protocols with integrated validation
    if (segmentJSON.isMember("buffers")) {
        for (const auto &bufferJSON : segmentJSON["buffers"]) {
            ...
            // Validate common buffer fields first
            if (buffer.name.empty() || !buffer.length) {
                LOG(WARNING) << "Corrupted segment descriptor, name "
                             << segment_name << " invalid buffer fields";
                return nullptr;
            }

            // Parse and validate protocol-specific buffer fields
            if (has_rdma) {
                ...
                if (!buffer.addr || buffer.rkey.empty() ||
            ...
            } else if (has_tcp || has_ascend) {
                if (!buffer.addr) {
                    LOG(WARNING)
                        << "Corrupted segment descriptor, name " << segment_name
                        << " protocol " << (has_tcp ? "tcp" : "ascend");
                    return nullptr;
                }
            } else if (has_nvlink) {
                ...
                if (!buffer.addr || buffer.shm_name.empty()) {
                    LOG(WARNING)
                        << "Corrupted segment descriptor, name " << segment_name
                        << " protocol nvlink"
                ...
                // CXL only needs name and length, no additional validation
                // needed
            } else if (desc->hasProtocol("nvmeof")) {
                // NVMe-oF uses a different buffer structure
                NVMeoFBufferDesc nvmeof_buffer;
                nvmeof_buffer.file_path = bufferJSON["file_path"].asString();
                nvmeof_buffer.length = bufferJSON["length"].asUInt64();
                const Json::Value &local_path_map =
                    bufferJSON["local_path_map"];
                for (const auto &key : local_path_map.getMemberNames()) {
                    nvmeof_buffer.local_path_map[key] =
                        local_path_map[key].asString();
                }
                desc->nvmeof_buffers.push_back(nvmeof_buffer);
                continue;  // Skip adding to regular buffers for NVMe-oF
            }
...
bool TransferMetadata::hasLocalSegment(const std::string &segment_name) {
    RWSpinlock::ReadGuard guard(segment_lock_);
    return segment_name_to_id_map_.count(segment_name) > 0;
}

std::shared_ptr<TransferMetadata::SegmentDesc>
TransferMetadata::getLocalSegmentDesc(const std::string &segment_name) {
    RWSpinlock::ReadGuard guard(segment_lock_);
    if (segment_name_to_id_map_.count(segment_name)) {
        return segment_id_to_desc_map_[segment_name_to_id_map_[segment_name]];
    }
    return nullptr;
}
