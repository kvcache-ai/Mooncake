...
int TransferEngine::init(const std::string &metadata_conn_string, const std::string &local_server_name,
                         const std::string &ip_or_host_name, uint64_t rpc_port) {
	LOG(INFO) << "Transfer Engine starting. Server: " << local_server_name << ", Metadata: " << metadata_conn_string
	          << ", ip_or_host_name: " << ip_or_host_name << ", rpc_port: " << rpc_port;

	local_server_name_ = local_server_name;
	TransferMetadata::RpcMetaDesc desc;
	std::string rpc_binding_method;

	if (getenv("MC_LEGACY_RPC_PORT_BINDING") || metadata_conn_string == P2PHANDSHAKE) {
		rpc_binding_method = "legacy/P2P";
#ifdef USE_ASCEND
		int device_id = -1;
		auto [host_name, port] = parseHostNameWithPortAscend(local_server_name, &device_id);
		LOG(INFO) << "Transfer Engine parseHostNameWithPortAscend. Server: " << host_name << " port: " << port
		          << " device_id: " << device_id;
#else
	...
	LOG(INFO) << "Transfer Engine RPC using " << rpc_binding_method << ", listening on " << desc.ip_or_host_name << ":"
	          << desc.rpc_port;
	...
	if (auto_discover_) {
		...
		if (getenv("MC_CUSTOM_TOPO_JSON")) {
			...
			if (!topo_json.empty()) {
				...
				LOG(WARNING) << "Failed to load custom topology from " << path << ", falling back to auto-detect.";
				local_topology_->discover(filter_);
			}
		} else {
			local_topology_->discover(filter_);
		}
		LOG(INFO) << "Topology discovery complete. Found " << local_topology_->getHcaList().size() << " HCAs.";
...
// Only for testing
Transport *TransferEngine::installTransport(const std::string &proto, void **args) {
	...
	for (auto &entry : local_memory_regions_) {
		int ret = transport->registerLocalMemory(entry.addr, entry.length, entry.location, entry.remote_accessible);
		if (ret < 0)
			return nullptr;
	}
	return transport;
...
std::string TransferEngine::getLocalIpAndPort() {
	return metadata_->localRpcMeta().ip_or_host_name + ":" + std::to_string(metadata_->localRpcMeta().rpc_port);
}
...
int TransferEngine::sendNotify(SegmentID target_id, TransferMetadata::NotifyDesc notify_msg) {
...
Transport::SegmentHandle TransferEngine::openSegment(const std::string &segment_name) {
...
int TransferEngine::registerLocalMemory(void *addr, size_t length, const std::string &location, bool remote_accessible,
	...
	for (auto transport : multi_transports_->listTransports()) {
		int ret = transport->registerLocalMemory(addr, length, location, remote_accessible, update_metadata);
		if (ret < 0)
			return ret;
	}
...
int TransferEngine::registerLocalMemoryBatch(const std::vector<BufferEntry> &buffer_list, const std::string &location) {
	...
	std::unique_lock<std::shared_mutex> lock(mutex_);
	for (auto &buffer : buffer_list) {
		local_memory_regions_.push_back({buffer.addr, buffer.length, location, true});
	}
	return 0;
}
...
void TransferEngine::InitializeMetricsConfig() {
	...
	if (metric_env) {
		...
		metrics_enabled_ = (value == "1" || value == "true" || value == "yes" || value == "on");
	}
	...
	if (interval_env) {
		try {
			...
			if (interval > 0) {
				...
				LOG(INFO) << "Metrics reporting interval set to " << metrics_interval_seconds_ << " seconds";
			} else {
				LOG(WARNING) << "Invalid MC_TE_METRIC_INTERVAL_SECONDS value: " << interval_env
				             << ", must be positive. Using default: " << metrics_interval_seconds_;
			}
		} catch (const std::exception &e) {
			LOG(WARNING) << "Failed to parse MC_TE_METRIC_INTERVAL_SECONDS: " << interval_env
...
void TransferEngine::StartMetricsReportingThread() {
	...
	if (!metrics_enabled_) {
		LOG(INFO) << "Metrics reporting is disabled (set MC_TE_METRIC=1 to enable)";
		return;
	}

	should_stop_metrics_thread_ = false;
	metrics_reporting_thread_ = std::thread([this]() {
		LOG(INFO) << "Metrics reporting thread started (interval: " << metrics_interval_seconds_ << "s)";
		...
		while (!should_stop_metrics_thread_) {
			...
			for (uint64_t i = 0; i < metrics_interval_seconds_ && !should_stop_metrics_thread_; ++i) {
			...
			auto bytes_transferred_in_interval = transferred_bytes_counter_.value();
			transferred_bytes_counter_.reset(); // Reset counter for the next interval
			...
			double throughput_megabytes_per_second =
			    static_cast<double>(bytes_transferred_in_interval) / (metrics_interval_seconds_ * kBytesPerMegabyte);

			LOG(INFO) << "[Metrics] Transfer Engine Throughput: " << std::fixed << std::setprecision(2)
			          << throughput_megabytes_per_second << " MB/s (over last " << metrics_interval_seconds_ << "s)";
		}
