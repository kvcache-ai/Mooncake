...
bool TransferEngine::isLocalMemoryRegistered(const void* addr) {
    return impl_->isLocalMemoryRegistered(addr);
}
...
bool TransferEngine::isLocalMemoryRegistered(const void* addr) {
    if (!use_tent_) return impl_->isLocalMemoryRegistered(addr);
