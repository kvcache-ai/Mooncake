...
uintptr_t alignPage(uintptr_t address) {
...
std::string genGpuNodeName(int node) {
	if (node >= 0)
		return "cuda:" + std::to_string(node);
	return kWildcardLocation;
}
...
const std::vector<MemoryLocationEntry> getMemoryLocation(void *start, size_t len) {
	...
	if (result != cudaSuccess) {
		LOG(ERROR) << "cudaPointerGetAttributes failed (Error code: " << result << " - " << cudaGetErrorString(result)
		           << ")" << std::endl;
	...
	if (attributes.type == cudaMemoryTypeDevice) {
		entries.push_back({(uint64_t)start, len, genGpuNodeName(attributes.device)});
		return entries;
	}
#endif
	...
	if (rc != 0) {
		PLOG(WARNING) << "Failed to get NUMA node, addr: " << start << ", len: " << len;
		entries.push_back({(uint64_t)start, len, kWildcardLocation});
		free(pages);
		free(status);
		return entries;
	...
	for (long long i = 1; i < n; i++) {
		if (status[i] != node) {
			...
			entries.push_back({start_addr, size_t(new_start_addr - start_addr), genCpuNodeName(node)});
			start_addr = new_start_addr;
			node = status[i];
		}
	}
	entries.push_back({start_addr, (uint64_t)start + len - start_addr, genCpuNodeName(node)});
	free(pages);
	free(status);
	return entries;
}
