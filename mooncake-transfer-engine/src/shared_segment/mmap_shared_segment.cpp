...
Status HostRegister(void* addr, uint64_t size, int32_t device_id,
    ...
    // A portable registration is visible from every context, so which device is
    // current does not matter and none of it has to be switched.
    (void)device_id;
    (void)ascend_dev_ptr;
    auto err = cudaHostRegister(addr, size, cudaHostRegisterPortable);
    if (err != cudaSuccess) {
        return Status::Memory(
