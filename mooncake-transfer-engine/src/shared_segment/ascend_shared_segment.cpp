...
// ExportToShareableHandle is available on that runtime.
bool IsAdxlExportShareableHandleAvailable() {
    if (&adxl::AdxlEngine::MallocMem == nullptr ||
        &adxl::AdxlEngine::FreeMem == nullptr ||
        &adxl::AdxlEngine::ExportToShareableHandle == nullptr) {
        return false;
    }
    return adxl::IsAdxlFeatureSupported(adxl::EXPORT_SHAREABLE_HANDLE);
}
