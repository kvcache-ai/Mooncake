...
void loadGlobalConfig(GlobalConfig &config) {
	...
	if (num_cq_per_ctx_env) {
		...
		if (val > 0 && val < 256)
			...
			LOG(WARNING) << "Ignore value from environment variable MC_NUM_CQ_PER_CTX";
	}
	...
	if (gid_index_env) {
		...
		if (val >= 0 && val < 256)
			...
			LOG(WARNING) << "Ignore value from environment variable MC_GID_INDEX";
	}
	...
	if (max_cqe_per_ctx_env) {
		...
		if (val > 0 && val <= UINT16_MAX)
			...
			LOG(WARNING) << "Ignore value from environment variable MC_MAX_CQE_PER_CTX";
	}
	...
	if (max_ep_per_ctx_env) {
		...
		if (val > 0 && val <= UINT16_MAX)
			...
			LOG(WARNING) << "Ignore value from environment variable MC_MAX_EP_PER_CTX";
	}
	...
	if (num_qp_per_ep_env) {
		...
		if (val > 0 && val < 256)
			...
			LOG(WARNING) << "Ignore value from environment variable MC_NUM_QP_PER_EP";
	}
	...
	if (slice_timeout_env) {
		...
		if (val > 0 && val < 65536)
			...
			LOG(WARNING) << "Ignore value from environment variable MC_SLICE_TIMEOUT";
	}
	...
	if (log_dir_path) {
		...
		if (opendir(log_dir_path) == NULL) {
			LOG(WARNING) << "Path [" << log_dir_path << "] is not a valid directory path. Still logging to stderr.";
		} else if (access(log_dir_path, W_OK) != 0) {
			LOG(WARNING) << "Path [" << log_dir_path << "] is not a permitted directory path for the current user. \
...
void updateGlobalConfig(ibv_device_attr &device_attr) {
	...
	if (config.max_ep_per_ctx * config.num_qp_per_ep > (size_t)device_attr.max_qp)
...
void dumpGlobalConfig() {
	...
	LOG(INFO) << "num_comp_channels_per_ctx = " << config.num_comp_channels_per_ctx;
