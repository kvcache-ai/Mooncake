...
void TransferMetadata::SegmentDesc::dump() const {
	...
	for (auto &device : devices) {
		LOG(INFO) << "    device name " << device.name << ", lid " << device.lid << ", " << device.gid;
	}
	LOG(INFO) << "  buffers: ";
	for (auto &buffer : buffers) {
		LOG(INFO) << "    buffer type " << buffer.name << ", address " << (void *)buffer.addr << "--"
...
void TransferMetadata::dumpMetadataContent(const std::string &segment_name, uint64_t offset, uint64_t length) {
	...
	if (current_ts - last_ts > kMinDisplayThreshold || globalConfig().trace) {
		LOG(INFO) << "Failed to get segment descriptor for segment " << segment_name << " address " << (void *)offset
		          << "--" << (void *)(offset + length);
	...
	if (rpc_meta_locked)
		rpc_meta_lock_.unlockShared();
	if (segment_locked)
		segment_lock_.unlockShared();
}
...
void TransferMetadata::dumpMetadataContentUnlocked() {
	...
	for (auto &entry : segment_id_to_desc_map_) {
		...
		if (!desc) {
			...
			LOG(INFO) << "segment id: " << entry.first << ", ref object " << &desc;
			desc->dump();
		}
	}
	LOG(INFO) << "=== Local RPC Route ===";
	LOG(INFO) << "location: " << local_rpc_meta_.ip_or_host_name << ":" << local_rpc_meta_.rpc_port;
	LOG(INFO) << "=== Remote RPC Routes ===";
	for (auto &entry : rpc_meta_map_) {
		LOG(INFO) << "segment name: " << entry.first << ", location: " << entry.second.ip_or_host_name << ":"
