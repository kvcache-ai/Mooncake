...
struct RedisStoragePlugin : public MetadataStoragePlugin {
	RedisStoragePlugin(const std::string &metadata_uri) : client_(nullptr), metadata_uri_(metadata_uri) {
		auto hostname_port = parseHostNameWithPort(metadata_uri);
		client_ = redisConnect(hostname_port.first.c_str(), hostname_port.second);
		if (!client_) {
			LOG(ERROR) << "RedisStoragePlugin: unable to connect " << metadata_uri_;
			return;
		}
		if (client_->err) {
			LOG(ERROR) << "RedisStoragePlugin: unable to connect " << metadata_uri_ << ": " << client_->errstr;
			redisFree(client_);
			client_ = nullptr;
			return;
		}
	...
	RedisStoragePlugin(const std::string &metadata_uri, const std::string &password, const uint8_t &db_index)
		...
		if (!password.empty()) {
			auto *reply = static_cast<redisReply *>(redisCommand(client_, "AUTH %s", password.c_str()));
			if (!reply || reply->type == REDIS_REPLY_ERROR) {
				LOG(ERROR) << "RedisStoragePlugin: authentication failed for " << metadata_uri_;
		...
		if (db_index != 0) {
			auto *reply = static_cast<redisReply *>(redisCommand(client_, "SELECT %d", db_index));
			if (!reply || reply->type == REDIS_REPLY_ERROR) {
				LOG(ERROR) << "RedisStoragePlugin: failed to select database " << (int)db_index << " for "
				           << metadata_uri_;
				freeReplyObject(reply);
				redisFree(client_);
				client_ = nullptr;
	...
	virtual bool get(const std::string &key, Json::Value &value) {
		...
		if (!resp) {
			LOG(ERROR) << "RedisStoragePlugin: unable to get " << key << " from " << metadata_uri_;
			return false;
		}
		if (!resp->str) {
			LOG(ERROR) << "RedisStoragePlugin: unable to get " << key << " from " << metadata_uri_;
	...
	virtual bool set(const std::string &key, const Json::Value &value) {
		...
		if (!resp) {
			LOG(ERROR) << "RedisStoragePlugin: unable to put " << key << " from " << metadata_uri_;
			return false;
		}
		freeReplyObject(resp);
		return true;
	...
	virtual bool remove(const std::string &key) {
		...
		if (!resp) {
			LOG(ERROR) << "RedisStoragePlugin: unable to remove " << key << " from " << metadata_uri_;
			return false;
		}
		freeReplyObject(resp);
		return true;
...
struct HTTPStoragePlugin : public MetadataStoragePlugin {
	HTTPStoragePlugin(const std::string &metadata_uri) : client_(nullptr), metadata_uri_(metadata_uri) {
	...
	virtual bool get(const std::string &key, Json::Value &value) {
		...
		if (res != CURLE_OK) {
			LOG(ERROR) << "Error from http client, GET " << url << " error: " << curl_easy_strerror(res);
			return false;
		}
		...
		if (responseCode != 200) {
			LOG(ERROR) << "Unexpected code in http response, GET " << url << " response code: " << responseCode
	...
	virtual bool set(const std::string &key, const Json::Value &value) {
		...
		if (res != CURLE_OK) {
			LOG(ERROR) << "Error from http client, PUT " << url << " error: " << curl_easy_strerror(res);
			return false;
		}
		...
		curl_easy_getinfo(client_, CURLINFO_RESPONSE_CODE, &responseCode);
		if (responseCode != 200) {
			LOG(ERROR) << "Unexpected code in http response, PUT " << url << " response code: " << responseCode
			           << " response body: " << readBuffer;
			return false;
		}
	...
	virtual bool remove(const std::string &key) {
		...
		CURLcode res = curl_easy_perform(client_);
		if (res != CURLE_OK) {
			LOG(ERROR) << "Error from http client, DELETE " << url << " error: " << curl_easy_strerror(res);
			return false;
		}
		...
		curl_easy_getinfo(client_, CURLINFO_RESPONSE_CODE, &responseCode);
		if (responseCode != 200) {
			LOG(ERROR) << "Unexpected code in http response, DELETE " << url << " response code: " << responseCode
			           << " response body: " << readBuffer;
			return false;
		}
		return true;
...
struct EtcdStoragePlugin : public MetadataStoragePlugin {
	EtcdStoragePlugin(const std::string &metadata_uri) : client_(metadata_uri), metadata_uri_(metadata_uri) {
	}
	...
	virtual bool get(const std::string &key, Json::Value &value) {
		...
		if (!resp.is_ok()) {
			LOG(ERROR) << "EtcdStoragePlugin: unable to get " << key << " from " << metadata_uri_ << ": "
			           << resp.error_message();
			return false;
		}
	...
	virtual bool set(const std::string &key, const Json::Value &value) {
		...
		if (!resp.is_ok()) {
			LOG(ERROR) << "EtcdStoragePlugin: unable to set " << key << " from " << metadata_uri_ << ": "
			           << resp.error_message();
			return false;
		}
		return true;
	...
	virtual bool remove(const std::string &key) {
		...
		if (!resp.is_ok()) {
			LOG(ERROR) << "EtcdStoragePlugin: unable to delete " << key << " from " << metadata_uri_ << ": "
			           << resp.error_message();
			return false;
		}
		return true;
...
#else
struct EtcdStoragePlugin : public MetadataStoragePlugin {
	EtcdStoragePlugin(const std::string &metadata_uri) : metadata_uri_(metadata_uri) {
		auto ret = NewEtcdClient((char *)metadata_uri_.c_str(), &err_msg_);
		if (ret) {
			LOG(ERROR) << "EtcdStoragePlugin: unable to connect " << metadata_uri_ << ": " << err_msg_;
	...
	virtual bool get(const std::string &key, Json::Value &value) {
		...
		if (ret) {
			LOG(ERROR) << "EtcdStoragePlugin: unable to get " << key << " in " << metadata_uri_ << ": " << err_msg_;
			// free the memory for storing error message
			free(err_msg_);
			err_msg_ = nullptr;
			return false;
	...
	virtual bool set(const std::string &key, const Json::Value &value) {
		Json::FastWriter writer;
		const std::string json_file = writer.write(value);
		auto ret = EtcdPutWrapper((char *)key.c_str(), (char *)json_file.c_str(), &err_msg_);
		if (ret) {
			LOG(ERROR) << "EtcdStoragePlugin: unable to set " << key << " in " << metadata_uri_ << ": " << err_msg_;
			// free the memory for storing error message
			free(err_msg_);
			err_msg_ = nullptr;
			return false;
	...
	virtual bool remove(const std::string &key) {
		...
		if (ret) {
			LOG(ERROR) << "EtcdStoragePlugin: unable to remove " << key << " in " << metadata_uri_ << ": " << err_msg_;
			// free the memory for storing error message
			free(err_msg_);
			err_msg_ = nullptr;
			return false;
...
std::pair<std::string, std::string> parseConnectionString(const std::string &conn_string) {
...
std::shared_ptr<MetadataStoragePlugin> MetadataStoragePlugin::Create(const std::string &conn_string) {
		...
		if (db_index_str) {
			try {
				...
				if (index >= 0 && index <= 255) {
					...
					LOG(WARNING) << "Invalid Redis DB index: " << index << ", using default 0";
				}
			} catch (const std::exception &e) {
				LOG(WARNING) << "Failed to parse MC_REDIS_DB_INDEX: " << e.what() << ", using default 0";
			}
		}

		return std::make_shared<RedisStoragePlugin>(parsed_conn_string.second, password_str, db_index);
	}
#endif // USE_REDIS

#ifdef USE_HTTP
	if (parsed_conn_string.first == "http" || parsed_conn_string.first == "https") {
		return std::make_shared<HTTPStoragePlugin>(conn_string); // including prefix
	}
#endif // USE_HTTP

	LOG(FATAL) << "Unable to find metadata storage plugin " << parsed_conn_string.first
...
static inline const std::string getNetworkAddress(struct sockaddr *addr) {
	if (addr->sa_family == AF_INET) {
		...
		if (inet_ntop(addr->sa_family, &(sock_addr->sin_addr), ip, INET_ADDRSTRLEN) != NULL)
			return std::string(ip) + ":" + std::to_string(ntohs(sock_addr->sin_port));
		...
		if (inet_ntop(addr->sa_family, &(sock_addr->sin6_addr), ip, INET6_ADDRSTRLEN) != NULL)
			return std::string(ip) + ":" + std::to_string(ntohs(sock_addr->sin6_port));
	}
...
struct SocketHandShakePlugin : public HandShakePlugin {
	...
	virtual int startDaemon(uint16_t listen_port, int sockfd) {
		...
		if (sockfd >= 0) {
			...
			listen_fd_ = socket(globalConfig().use_ipv6 ? AF_INET6 : AF_INET, SOCK_STREAM, 0);
			if (listen_fd_ < 0) {
			...
			timeout.tv_sec = 1;
			timeout.tv_usec = 0;
			if (setsockopt(listen_fd_, SOL_SOCKET, SO_RCVTIMEO, &timeout, sizeof(timeout))) {
			...
			if (globalConfig().use_ipv6) {
				...
				if (bind(listen_fd_, (sockaddr *)&bind_address, sizeof(sockaddr_in6)) < 0) {
				...
				if (bind(listen_fd_, (sockaddr *)&bind_address, sizeof(sockaddr_in)) < 0) {
					PLOG(ERROR) << "SocketHandShakePlugin: bind (port " << listen_port << ")";
					closeListen();
					return ERR_SOCKET;
				}
			...
			while (listener_running_) {
				...
				if (!reader.parse(json_str, peer)) {
					LOG(ERROR) << "SocketHandShakePlugin: failed to receive "
					           ...
					           << reader.getFormattedErrorMessages() << ", json string length: " << json_str.size()
				...
				if (type == HandShakeRequestType::Connection || type == HandShakeRequestType::OldProtocol) {
				...
				if (rc > 0) {
					LOG(ERROR) << "Unexpected socket read result: " << rc << ", byte: " << int(byte);
				} else if (rc < 0) {
	...
	virtual int sendNotify(std::string ip_or_host_name, uint16_t rpc_port, const Json::Value &local,
	                       Json::Value &peer) {
		...
		if (getaddrinfo(ip_or_host_name.c_str(), service, &hints, &result)) {
			PLOG(ERROR) << "SocketHandShakePlugin: failed to get IP address of peer "
			               "server "
	...
	virtual int send(std::string ip_or_host_name, uint16_t rpc_port, const Json::Value &local, Json::Value &peer) {
		struct addrinfo hints;
		struct addrinfo *result, *rp;
		memset(&hints, 0, sizeof(hints));
		hints.ai_family = AF_INET;
	...
	int doConnect(struct addrinfo *addr, int &conn_fd) {
		...
		if (connect(conn_fd, addr->ai_addr, addr->ai_addrlen)) {
			PLOG(ERROR) << "SocketHandShakePlugin: connect()" << getNetworkAddress(addr->ai_addr);
			close(conn_fd);
			return ERR_SOCKET;
		}
	...
	int doSend(struct addrinfo *addr, const Json::Value &local, Json::Value &peer) {
		...
		ret = writeString(conn_fd, HandShakeRequestType::Connection, Json::FastWriter {}.write(local));
		if (ret) {
			LOG(ERROR) << "SocketHandShakePlugin: failed to send handshake message: "
			              "malformed json format, check tcp connection";
			close(conn_fd);
			return ret;
		}
		...
		if (type != HandShakeRequestType::Connection) {
			LOG(ERROR) << "SocketHandShakePlugin: unexpected handshake message type";
			close(conn_fd);
			return ERR_SOCKET;
		}
	...
	virtual int exchangeMetadata(std::string ip_or_host_name, uint16_t rpc_port, const Json::Value &local_metadata,
	...
	int doSendNotify(struct addrinfo *addr, const Json::Value &local_notify, Json::Value &peer_notify) {
		int conn_fd = -1;
		int ret = doConnect(addr, conn_fd);
		if (ret) {
			return ret;
		...
		ret = writeString(conn_fd, HandShakeRequestType::Notify, Json::FastWriter {}.write(local_notify));
		if (ret) {
			LOG(ERROR) << "SocketHandShakePlugin: failed to send metadata message: "
			              "malformed json format, check tcp connection";
			close(conn_fd);
			return ret;
		}
	...
	int doSendMetadata(struct addrinfo *addr, const Json::Value &local_metadata, Json::Value &peer_metadata) {
		int conn_fd = -1;
		int ret = doConnect(addr, conn_fd);
		if (ret) {
			return ret;
		...
		ret = writeString(conn_fd, HandShakeRequestType::Metadata, Json::FastWriter {}.write(local_metadata));
		if (ret) {
			LOG(ERROR) << "SocketHandShakePlugin: failed to send metadata message: "
			              "malformed json format, check tcp connection";
			close(conn_fd);
...
std::shared_ptr<HandShakePlugin> HandShakePlugin::Create(const std::string &conn_string) {
	...
	for (ifa = ifaddr; ifa != nullptr; ifa = ifa->ifa_next) {
		...
		if (ifa->ifa_addr->sa_family == family) {
			...
			if (!(ifa->ifa_flags & IFF_UP) || !(ifa->ifa_flags & IFF_RUNNING)) {
				LOG(INFO) << "Skipping interface " << ifa->ifa_name << " (not UP or not RUNNING)";
				continue;
			}

			char host[NI_MAXHOST];
			if (getnameinfo(ifa->ifa_addr, use_ipv6 ? sizeof(struct sockaddr_in6) : sizeof(struct sockaddr_in), host,
			                NI_MAXHOST, nullptr, 0, NI_NUMERICHOST) == 0) {
				LOG(INFO) << "Found active interface " << ifa->ifa_name << " with IP " << host;
				ips.push_back(host);
			}
		}
	}
