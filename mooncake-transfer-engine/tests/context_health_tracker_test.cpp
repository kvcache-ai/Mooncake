...
TEST(ContextHealthTracker, SinglePeerStreakNeverTrips) {
    ...
    for (int i = 0; i < 100; ++i) {
        auto rec = t.recordFailure(peers({"storeA"}), kThreshold, kMinPeers);
        EXPECT_FALSE(rec.tripped_now) << "iteration " << i;
    }
    EXPECT_FALSE(t.tripped());
    EXPECT_EQ(t.streak(), 100);
    EXPECT_EQ(t.distinctPeerCount(), 1u);
}
...
TEST(ContextHealthTracker, MultiPeerStreakTripsAtThreshold) {
    ...
    auto rec = t.recordFailure(peers({"storeB"}), kThreshold, kMinPeers);
...
TEST(ContextHealthTracker, TripsOnlyOnceAndStragglersDontRetrip) {
    ...
    auto t = makeTracker(clk);
    for (int i = 0; i < kThreshold - 1; ++i)
        t.recordFailure(peers({"a"}), kThreshold, kMinPeers);
    EXPECT_TRUE(
        t.recordFailure(peers({"b"}), kThreshold, kMinPeers).tripped_now);
    ...
    clk->now = 999;
    for (int i = 0; i < 10; ++i) {
        auto rec = t.recordFailure(peers({"a", "b"}), kThreshold, kMinPeers);
        EXPECT_FALSE(rec.tripped_now);
    }
...
TEST(ContextHealthTracker, LateSecondPeerTripsPastThreshold) {
    ...
    auto t = makeTracker(clk);
    for (int i = 0; i < 40; ++i) {
        auto rec = t.recordFailure(peers({"a"}), kThreshold, kMinPeers);
        EXPECT_FALSE(rec.tripped_now);
    }
    auto rec = t.recordFailure(peers({"b"}), kThreshold, kMinPeers);
    EXPECT_TRUE(rec.tripped_now);
...
TEST(ContextHealthTracker, MinPeersOneIsLegacyBehavior) {
    ...
    auto t = makeTracker(clk);
    for (int i = 0; i < kThreshold - 1; ++i)
        EXPECT_FALSE(t.recordFailure(peers({"a"}), kThreshold, 1).tripped_now);
    EXPECT_TRUE(t.recordFailure(peers({"a"}), kThreshold, 1).tripped_now);
}

TEST(ContextHealthTracker, SuccessResetsStreakAndPeers) {
    auto clk = std::make_shared<FakeClock>();
    auto t = makeTracker(clk);
    for (int i = 0; i < kThreshold - 1; ++i)
        t.recordFailure(peers({"a", "b"}), kThreshold, kMinPeers);
    t.recordSuccess();
    EXPECT_EQ(t.streak(), 0);
    EXPECT_EQ(t.distinctPeerCount(), 0u);
    ...
    EXPECT_TRUE(
        t.recordFailure(peers({"a", "b"}), kThreshold, kMinPeers).tripped_now);
}
...
TEST(ContextHealthTracker, CallbacksRunOnTransitions) {
    ...
    for (int i = 0; i < kThreshold - 1; ++i)
        t.recordFailure(peers({"a", "b"}), kThreshold, kMinPeers, deactivate);
            ...
            threads.emplace_back([&t, &deactivate] {
                for (int k = 0; k < 500; ++k)
                    (void)t.recordFailure(
                        {std::string("peer") + std::to_string(k % 3)}, 4, 1,
                        deactivate);
            });
    ...
    for (int i = 0; i < 4; ++i)
        threads.emplace_back([&t, i] {
            ...
            for (int k = 0; k < kIters; ++k)
                (void)t.recordFailure(ps, kThreshold, kMinPeers);
        });
    for (int i = 0; i < 4; ++i)
