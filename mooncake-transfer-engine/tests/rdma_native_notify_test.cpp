...
class RdmaContextTestPeer {
   public:
    ...
    static ibv_cq *cq(RdmaContext &context) { return context.notify_cq_; }
};
class RdmaNotificationTestPeer {
   public:
    ...
    static void pauseNotifications(TransferEngine &engine) {
        auto *rdma = transport(engine);
        ASSERT_NE(rdma, nullptr);
        rdma->notify_running_.store(false, std::memory_order_release);
        if (rdma->notify_worker_.joinable()) rdma->notify_worker_.join();
    }
};
}  // namespace mooncake
...
TEST_F(NativeNotifyTest, RetirementWakesSenderWaitingForSlot) {
    ...
    RdmaNotificationTestPeer::pauseNotifications(*sender_);
...
TEST_F(NativeNotifyTest, SharedCqRoutesConcurrentPeers) {
    auto context = RdmaNotificationTestPeer::transport(*receiver_)
                       ->getContextList()
                       .front();
    // The CQ exists before the first peer and stays fixed as peers connect.
    auto *cq = RdmaContextTestPeer::cq(*context);
    ASSERT_NE(cq, nullptr);
    ASSERT_GE(cq->cqe, globalConfig().max_cqe);
    const int capacity = cq->cqe;
