...
class FakeRedisServer {
   public:
    ...
    // Makes the next N data-plane replies (GET/SET/DEL) be RESP errors, e.g.
    // "-READONLY You can't write against a read only replica.\r\n". AUTH and
    // SELECT are left alone so the plugin can still connect.
    void ForceErrorReplies(int count,
                           const std::string &message = "READONLY forced") {
        std::lock_guard<std::mutex> lock(mutex_);
        error_replies_remaining_ = count;
   ...
   private:
    ...
    std::string Dispatch(const std::vector<std::string> &argv) {
        ...
        if (command == "GET" || command == "SET" || command == "DEL") {
            if (error_replies_remaining_ > 0) {
                --error_replies_remaining_;
    ...
    int error_replies_remaining_ = 0;
...
// #4107: a REDIS_REPLY_ERROR means the server answered and rejected the
// command (READONLY after failover, OOM, MISCONF, NOAUTH). That must surface as
// failure — and must not be retried, because reconnecting cannot fix a
// logical rejection from a healthy socket.
TEST_F(RedisStoragePluginTest, ErrorReplyIsReportedAsFailure) {
    FakeRedisServer server;
    server.SetValue("mooncake/node-9", "{\"segment\":\"node-9\"}");
    auto plugin = MetadataStoragePlugin::Create(server.endpoint());
    ASSERT_NE(plugin, nullptr);

    Json::Value read;
    ASSERT_EQ(plugin->getWithStatus("mooncake/node-9", read),
              GetResult::kFound);
    ASSERT_EQ(server.accept_count(), 1);

    server.ForceErrorReplies(4,
                             "READONLY You can't write against a read "
                             "only replica");

    Json::Value value;
    value["segment"] = "node-9-write";
    EXPECT_FALSE(plugin->set("mooncake/node-9", value));
    EXPECT_FALSE(plugin->get("mooncake/node-9", read));
    EXPECT_EQ(plugin->getWithStatus("mooncake/node-9", read),
              GetResult::kUnavailable);
    EXPECT_FALSE(plugin->remove("mooncake/node-9"));

    // A logical error reply must not tear down and rebuild the connection.
    EXPECT_EQ(server.accept_count(), 1);

    // Once the forced errors are exhausted the same connection works again.
    EXPECT_TRUE(plugin->set("mooncake/node-9", value));
    EXPECT_EQ(server.accept_count(), 1);
}
