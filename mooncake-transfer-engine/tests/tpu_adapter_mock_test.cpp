...
TEST(TpuMockAdapter, MemcpyAsyncFallsBackToSync) {
    ...
    memset(src, 0x55, 16);
    EXPECT_EQ(cudaMemcpyAsync(dst, src, 16, cudaMemcpyDeviceToDevice, nullptr),
              cudaSuccess);
...
TEST(TpuMockAdapter, MemGetHandleForAddressRange) {
    ...
    int handle = 0;
    EXPECT_EQ(cuMemGetHandleForAddressRange(
                  &handle, reinterpret_cast<CUdeviceptr>(ptr), 4096,
