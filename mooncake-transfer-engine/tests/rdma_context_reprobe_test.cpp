...
class RdmaContextTestPeer {
   public:
    static void seedAutoGidState(RdmaContext &context, ibv_context *verbs_ctx,
                                 uint8_t port, uint16_t lid,
                                 const ibv_gid &gid, int gid_index) {
...
constexpr std::array<uint8_t, 16> kCurrentGid = {0x00, 0x00, 0x00, 0x00,
                                                  0x00, 0x00, 0x00, 0x00,
                                                  0x00, 0x00, 0x00, 0x00,
                                                  0x00, 0x00, 0x00, 0x11};
class RdmaContextReprobeTest : public ::testing::Test {
   protected:
    void SetUp() override {
        ...
        local_desc->devices.push_back({"synthetic0", 23, formatGid(kCurrentGid),
                                       ""});
        ASSERT_EQ(metadata_->addLocalSegment(
                      LOCAL_SEGMENT_ID, "local-rdma-segment",
                      std::move(local_desc)),
                  0);
...
TEST_F(RdmaContextReprobeTest,
    ...
    bool changed =
        context_->reprobeAutoGid({formatGid(kCurrentGid), 9}, {});
