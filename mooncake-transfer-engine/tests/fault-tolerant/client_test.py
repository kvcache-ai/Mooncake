...
def main():
    ...
    client_engine = MooncakeTransferEngine(
        hostname="localhost:10011", gpu_id=0, ib_device=None  # Using GPU 0  # No specific IB device
    )
    ...
    while True:
        ret = client_engine.transfer_sync(
            server_session_id, client_ptr, server_ptr, min(client_len, server_len)  # Transfer minimum of both lengths
        )
