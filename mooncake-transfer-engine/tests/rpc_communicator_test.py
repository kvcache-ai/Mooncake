...
def run_client(target_url, num_threads=8, data_size_mb=1):
    ...
    print(
        f"Starting client, connecting to {target_url} with {num_threads} threads, {data_size_mb}MB data packets"
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Mooncake Communication Bandwidth Test Tool"
    )
    parser.add_argument(
        "mode", choices=["server", "client"], help="Run mode: server or client"
    )
    parser.add_argument(
        "--url", default="127.0.0.1:9004", help="URL address (default: 127.0.0.1:9004)"
    )
    parser.add_argument(
        "--threads", type=int, default=8, help="Number of client threads (default: 8)"
    )
    parser.add_argument(
        "--data-size", type=int, default=1, help="Data packet size in MB (default: 1)"
    )
    ...
    if args.mode == "server":
        ...
        bind_url = (
            f"0.0.0.0:{args.url.split(':')[-1]}"
            if ":" in args.url
            else f"0.0.0.0:{args.url}"
        )
