...
TEST_F(CxiTransportUnitTest, TestInit) {
    ...
    engine = std::make_unique<TransferEngine>(false, std::vector<std::string>{device_filter});
    ...
    rc = engine->init(metadata_server_, local_server_name_,
                            hp.first.c_str(), hp.second);
    EXPECT_EQ(rc, 0) << "engine->init failed";
    xport = engine->installTransport("cxi", nullptr);
    EXPECT_EQ(xport, nullptr) << "installTrasport(\"cxi\") should fail for invalid_device";
...
TEST_F(CxiTransportUnitTest, TestAllocate) {
    ...
    int retcode = engine->registerLocalMemory((void*) dummy_buffer.data(), dummy_buffer_size, "invalid_device:0");
    EXPECT_EQ(retcode, ERR_DEVICE_NOT_FOUND) << "invalid device memory should not be registered!";

    
    retcode = engine->registerLocalMemory((void*) buffer.data(), buffer_size, "cpu:0");
    EXPECT_EQ(retcode, 0) << "unable to register memory";

    retcode = engine->registerLocalMemory((void*) (buffer.data() + (buffer_size >> 1)), buffer_size >> 1, "cpu:1");
    EXPECT_EQ(retcode, ERR_ADDRESS_OVERLAPPED) << "should not be able to register overlapping MRs!";
