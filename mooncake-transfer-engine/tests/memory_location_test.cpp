...
TEST(MemoryLocationTest, MallocMultipleNodes) {
	...
	void *pages[3] = {addr, (void *)((uint64_t)addr + 4096), (void *)((uint64_t)addr + 4096 * 9)};
	...
	if (nodea == nodeb) {
		...
		EXPECT_EQ(entries[1].start, reinterpret_cast<uint64_t>(addr) + 4096 * 2);
		EXPECT_EQ(entries[1].location, locationa);
		EXPECT_EQ(entries[1].len, static_cast<size_t>(4096 * 7));

		// check the third memory location
		EXPECT_EQ(entries[2].start, reinterpret_cast<uint64_t>(addr) + 4096 * 9);
