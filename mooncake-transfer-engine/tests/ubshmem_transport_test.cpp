...
#define MNNVL_PROTOCOL "ubshmem"

DEFINE_string(metadata_server, "http://127.0.0.1:8080/metadata", "Metadata server address");
...
static void* allocateAclBuffer(size_t size, int npu_id) {
    ...
    if (!checkAcl(aclrtGetDevice(&npu_id), "UBShmemTransport: aclrtGetDevice failed")) {
        return nullptr;
    }
    ...
    if (!checkAcl(result, "UBShmemTransport: Failed to allocate specific device memory")) {
        return nullptr;
    }

    result = aclrtReserveMemAddress(&ptr, size, 0, nullptr, 0);
...
static void* allocateAclBufferHost(size_t size, int npu_id) {
    ...
    auto result = aclrtMallocPhysical(&handle, size, &prop, 0);
    if (!checkAcl(result, "UBShmemTransport: Failed to allocate specific host memory")) {
        return nullptr;
    }

    result = aclrtReserveMemAddress(&ptr, size, 0, nullptr, 1);
    if (!checkAcl(result, "UBShmemTransport: aclrtReserveMemAddress failed")) {
        (void)aclrtFreePhysical(handle);
        return nullptr;
    }
...
static void clientThread(int npu_id, const std::string& metadataServer,
    ...
    int rc = client_engine->registerLocalMemory(client_buffer, kDataLength * 2,
                                                "npu:" + std::to_string(npu_id));
    ASSERT_EQ(rc, 0);

    auto server_segment_id = client_engine->openSegment(segmentId);
    auto segment_desc = client_engine->getMetadata()->getSegmentDescByID(server_segment_id);
    // Write: client -> server
    {
        ...
        checkAclError(aclrtMemcpy(client_buffer, kDataLength, host_data.data(), kDataLength,
    ...
    checkAclError(
        aclrtMemcpy(host_check.data(), kDataLength, (char*)client_buffer + kDataLength,
                   kDataLength, ACL_MEMCPY_DEVICE_TO_HOST),
...
TEST(UBShmemTransportTest, WriteAndReadCrossNPU) {
    ...
    // Start server thread
    std::thread serverThreadObj(serverThread, FLAGS_server_npu_id, FLAGS_metadata_server,
                               FLAGS_local_server_name,
                               std::ref(serverReadyPromise),
                               std::ref(testCompleteFuture));

    // Start client thread
    std::thread clientThreadObj(clientThread, FLAGS_client_npu_id, FLAGS_metadata_server,
                               FLAGS_local_server_name,
