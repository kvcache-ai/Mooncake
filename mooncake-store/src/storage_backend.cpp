...
ErrorCode StorageBackend::StoreObject(const ObjectKey &key, const std::vector<Slice> &slices) {
	...
	if (ret != static_cast<ssize_t>(slices_total_size)) {
		LOG(INFO) << "Write size mismatch for: " << path << ", expected: " << slices_total_size << ", got: " << ret;
		return ErrorCode::FILE_WRITE_FAIL;
	}
	// TODO: Determine whether the data has been completely and correctly
	// written. If the write operation fails, the corresponding file should be
	// deleted to prevent incomplete data from being found in subsequent get
	// operations. Alternatively, a marking method can be used to record that
...
ErrorCode StorageBackend::StoreObject(const ObjectKey &key, const std::string &str) {
	std::string path = ResolvePath(key);
	...
	if (ret != static_cast<ssize_t>(file_total_size)) {
		LOG(INFO) << "Write size mismatch for: " << path << ", expected: " << file_total_size << ", got: " << ret;
...
ErrorCode StorageBackend::LoadObject(const ObjectKey &key, std::vector<Slice> &slices) {
	std::string path = ResolvePath(key);
	return LoadObjectInPath(path, slices);
}

ErrorCode StorageBackend::LoadObject(const ObjectKey &key, std::vector<Slice> &slices, std::string &path) {
	return LoadObjectInPath(path, slices);
}

ErrorCode StorageBackend::LoadObjectInPath(const std::string &path, std::vector<Slice> &slices) {
	...
	if (ret != static_cast<ssize_t>(slices_total_size)) {
		LOG(INFO) << "Read size mismatch for: " << path << ", expected: " << slices_total_size << ", got: " << ret;
...
ErrorCode StorageBackend::LoadObject(const ObjectKey &key, std::string &str) {
	std::string path = ResolvePath(key);
	FILE *file = fopen(path.c_str(), "rb");
	...
	if (ret != static_cast<ssize_t>(file_total_size)) {
		LOG(INFO) << "Read size mismatch for: " << path << ", expected: " << file_total_size << ", got: " << ret;
...
std::unordered_map<ObjectKey, Replica::Descriptor> StorageBackend::BatchQueryKey(const std::vector<ObjectKey> &keys) {
	namespace fs = std::filesystem;
...
std::string StorageBackend::SanitizeKey(const ObjectKey &key) const {
	...
	for (char c : key) {
		...
		sanitized_key.push_back(kInvalidChars.find(c) != std::string_view::npos ? '_' : c);
	}
