...
KeyRetentionMetric::KeyRetentionMetric(
    ...
    : anchor_(anchor),
      ...
      removed_age(prefix + "_removed_age_seconds",
                  ...
                  p2p::metric_util::LifetimeBuckets(), labels),
...
void KeyRetentionMetric::serialize(std::string& str) {
    ...
    p2p::metric_util::SerializeBucketHistogram(
        ...
        labels_, p2p::metric_util::LifetimeBuckets(), live_age_buckets);
    ...
    p2p::metric_util::SerializeBucketHistogram(
        ...
        labels_, p2p::metric_util::LifetimeBuckets(), all_buckets);
}
...
std::string KeyRetentionMetric::summary_metrics() {
    ...
    const std::vector<double>& boundaries = p2p::metric_util::LifetimeBuckets();
    ...
    const std::vector<int64_t> live_q = p2p::metric_util::InterpolateQuantiles(
        boundaries, live_age_buckets, kQuantiles);
    const std::vector<int64_t> removed_q =
        p2p::metric_util::InterpolateQuantiles(boundaries, removed_buckets,
                                               kQuantiles);
    const std::vector<int64_t> all_q = p2p::metric_util::InterpolateQuantiles(
        boundaries, all_buckets, kQuantiles);
