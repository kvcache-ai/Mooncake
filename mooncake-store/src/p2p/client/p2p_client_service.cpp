...
ErrorCode P2PClientService::ConnectToMaster(
    ...
    if (IsHAMode(master_server_entry)) {
        ...
        auto conn = master_client_.Connect(master_address);
        if (!conn) {
        ...
        auto conn = master_client_.Connect(master_server_entry);
        if (!conn) {
            return conn.error();
        }
        return ErrorCode::OK;
...
P2PClientService::InnerRegisterClient() {
    LOG(INFO) << "InnerRegisterClient called, client_id=" << client_id_
              << ", registered=" << registered_.load();
    ...
    LOG(INFO) << "InnerRegisterClient: RPC result: "
              << (register_result ? "OK" : toString(register_result.error()));
    if (!register_result) {
...
P2PClientService::RegisterClient() {
    ...
    if (!guard.is_valid()) {
        LOG(WARNING) << "inflight guard invalid, running="
                     << inflight_tracker_.is_running();
...
tl::expected<void, ErrorCode> P2PClientService::InnerUnregisterClient() {
    ...
    tl::expected<void, ErrorCode> ret;
...
bool P2PClientService::ReconnectToMaster(
    ...
    if (is_ha_mode) {
        ...
        auto conn = master_client_.Connect(current_master_address);
        if (!conn) {
...
void P2PClientService::HeartbeatTryRegister() {
    MutexLocker lk(&registration_mutex_, /*lock_now=*/false);
    if (!lk.TryLock()) {
        LOG(INFO) << "Skip heartbeat-driven register: op in progress, client_id="
                  << client_id_;
        return;
    }
    InflightTracker::Guard guard = AcquireInflightGuard();
...
async_simple::coro::Lazy<ErrorCode> P2PClientService::RunForwardReadOnRoute(
    ...
    if (!tr) {
        ...
        if (metrics_) {
            auto rollback_elapsed = std::chrono::duration_cast<std::chrono::microseconds>(
                std::chrono::steady_clock::now() - rollback_start).count();
            if (cleanup_unpin) {
...
void P2PClientService::RegisterHttpMethods() {
    ...
    http_server_->set_http_handler<GET>(
        "/get", [this](coro_http_request& req, coro_http_response& resp) {
            ...
            auto result = Get(key, client_buffer_allocator_, {});
            if (!result) {
    ...
    http_server_->set_http_handler<POST>(
        "/put", [this](coro_http_request& req, coro_http_response& resp) {
            ...
            auto alloc_result = client_buffer_allocator_->allocate(body.size());
            if (!alloc_result) {
                resp.set_status_and_content(status_type::service_unavailable,
    ...
    http_server_->set_http_handler<POST>(
        "/register", [this](coro_http_request& req, coro_http_response& resp) {
            ...
            if (!result) {
                LOG(ERROR) << "/register failed: " << result.error()
                           << ", running=" << inflight_tracker_.is_running()
                           << ", registered=" << registered_.load();
                resp.set_status_and_content(
                    ErrorToHttpStatus(result.error()),
                    std::string(toString(result.error())));
            ...
            LOG(INFO) << "/register succeeded, view_version="
                      << result.value().view_version;
            resp.set_status_and_content(status_type::ok, "OK");
        });
