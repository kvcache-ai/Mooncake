...
tl::expected<void, ErrorCode> StorageTier::Init(
    ...
    try {
        ...
        // Recreate the directory that CleanStoragePath removed, so Init()
        // can scan it without hitting a "No such file or directory" error.
        {
            auto fs_config = FileStorageConfig::FromEnvironment();
            LOG(INFO) << "StorageTier::Init recreating directory: "
                      << fs_config.storage_filepath;
            std::filesystem::create_directories(fs_config.storage_filepath);
        }
