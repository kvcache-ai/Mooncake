...
// --- Singleton Instance ---
P2PHAMetricManager& P2PHAMetricManager::instance() {
    static P2PHAMetricManager static_instance;
    return static_instance;
}

// --- Constructor ---
P2PHAMetricManager::P2PHAMetricManager()
    ...
    : oplog_last_sequence_id_("ha_oplog_last_sequence_id",
      ...
      oplog_pending_entries_(
          ...
          "Number of out-of-order entries waiting in P2POpLogApplierBase"),
...
void P2PHAMetricManager::set_oplog_last_sequence_id(int64_t seq_id) {
    oplog_last_sequence_id_.update(seq_id);
}

int64_t P2PHAMetricManager::get_oplog_last_sequence_id() {
    return static_cast<int64_t>(oplog_last_sequence_id_.value());
}

void P2PHAMetricManager::set_oplog_applied_sequence_id(int64_t seq_id) {
    oplog_applied_sequence_id_.update(seq_id);
}

int64_t P2PHAMetricManager::get_oplog_applied_sequence_id() {
    return static_cast<int64_t>(oplog_applied_sequence_id_.value());
}

void P2PHAMetricManager::set_oplog_standby_lag(int64_t lag) {
    oplog_standby_lag_.update(lag);
}

int64_t P2PHAMetricManager::get_oplog_standby_lag() {
    return static_cast<int64_t>(oplog_standby_lag_.value());
}

void P2PHAMetricManager::set_oplog_pending_entries(int64_t count) {
    oplog_pending_entries_.update(count);
}

int64_t P2PHAMetricManager::get_oplog_pending_entries() {
    return static_cast<int64_t>(oplog_pending_entries_.value());
}

void P2PHAMetricManager::set_oplog_async_queue_size(int64_t size) {
    oplog_async_queue_size_.update(size);
}

int64_t P2PHAMetricManager::get_oplog_async_queue_size() {
    return static_cast<int64_t>(oplog_async_queue_size_.value());
}

void P2PHAMetricManager::set_election_is_leader(bool value) {
    election_is_leader_.update(value ? 1 : 0);
}

void P2PHAMetricManager::set_oplog_async_workers_running(bool value) {
    oplog_async_workers_running_.update(value ? 1 : 0);
}

void P2PHAMetricManager::set_standby_degraded(bool value) {
    standby_degraded_.update(value ? 1 : 0);
}

void P2PHAMetricManager::set_primary_degraded(bool value) {
    primary_degraded_.update(value ? 1 : 0);
}

void P2PHAMetricManager::set_oplog_last_successful_poll_timestamp_ms(
    int64_t timestamp_ms) {
    oplog_last_successful_poll_timestamp_ms_.update(timestamp_ms);
}

void P2PHAMetricManager::set_p2p_snapshot_bootstrap_baseline_sequence_id(
    int64_t seq_id) {
    p2p_snapshot_bootstrap_baseline_sequence_id_.update(seq_id);
}

void P2PHAMetricManager::set_p2p_bootstrap_catchup_target_sequence_id(
    int64_t seq_id) {
...
void P2PHAMetricManager::inc_oplog_skipped_entries(int64_t val) {
    oplog_skipped_entries_total_.inc(val);
}

int64_t P2PHAMetricManager::get_oplog_skipped_entries_total() {
    return static_cast<int64_t>(oplog_skipped_entries_total_.value());
}

void P2PHAMetricManager::inc_oplog_checksum_failures(int64_t val) {
    oplog_checksum_failures_total_.inc(val);
}

int64_t P2PHAMetricManager::get_oplog_checksum_failures_total() {
    return static_cast<int64_t>(oplog_checksum_failures_total_.value());
}

void P2PHAMetricManager::inc_oplog_gap_resolve_attempts(int64_t val) {
    oplog_gap_resolve_attempts_total_.inc(val);
}

int64_t P2PHAMetricManager::get_oplog_gap_resolve_attempts_total() {
    return static_cast<int64_t>(oplog_gap_resolve_attempts_total_.value());
}

void P2PHAMetricManager::inc_oplog_gap_resolve_success(int64_t val) {
    oplog_gap_resolve_success_total_.inc(val);
}

int64_t P2PHAMetricManager::get_oplog_gap_resolve_success_total() {
    return static_cast<int64_t>(oplog_gap_resolve_success_total_.value());
}

void P2PHAMetricManager::inc_oplog_write_failures(int64_t val) {
    oplog_write_failures_total_.inc(val);
}

int64_t P2PHAMetricManager::get_oplog_write_failures_total() {
    return static_cast<int64_t>(oplog_write_failures_total_.value());
}

void P2PHAMetricManager::inc_oplog_write_retries(int64_t val) {
    oplog_write_retries_total_.inc(val);
}

int64_t P2PHAMetricManager::get_oplog_write_retries_total() {
    return static_cast<int64_t>(oplog_write_retries_total_.value());
}

void P2PHAMetricManager::inc_election_attempts(int64_t val) {
    election_attempts_total_.inc(val);
}
void P2PHAMetricManager::inc_election_failures(int64_t val) {
    election_failures_total_.inc(val);
}
void P2PHAMetricManager::inc_election_leadership_lost(int64_t val) {
    election_leadership_lost_total_.inc(val);
}
void P2PHAMetricManager::inc_election_reconnects(int64_t val) {
    election_reconnects_total_.inc(val);
}
void P2PHAMetricManager::inc_election_watch_failures(int64_t val) {
    election_watch_failures_total_.inc(val);
}
void P2PHAMetricManager::inc_election_polling_fallbacks(int64_t val) {
    election_polling_fallbacks_total_.inc(val);
}
void P2PHAMetricManager::inc_oplog_best_effort_dropped(int64_t val) {
    oplog_best_effort_dropped_total_.inc(val);
}
void P2PHAMetricManager::inc_oplog_queue_rejected(int64_t val) {
    oplog_queue_rejected_total_.inc(val);
}
void P2PHAMetricManager::inc_oplog_queue_bypassed(int64_t val) {
    oplog_queue_bypassed_total_.inc(val);
}
void P2PHAMetricManager::inc_oplog_sync_wait_timeouts(int64_t val) {
    oplog_sync_wait_timeouts_total_.inc(val);
}
void P2PHAMetricManager::inc_oplog_read_failures(int64_t val) {
    oplog_read_failures_total_.inc(val);
}
void P2PHAMetricManager::inc_oplog_reader_reconnect_attempts(int64_t val) {
    oplog_reader_reconnect_attempts_total_.inc(val);
}
void P2PHAMetricManager::inc_oplog_reader_reconnect_failures(int64_t val) {
    oplog_reader_reconnect_failures_total_.inc(val);
}
void P2PHAMetricManager::inc_oplog_reader_reconnects(int64_t val) {
    oplog_reader_reconnects_total_.inc(val);
}
void P2PHAMetricManager::inc_oplog_apply_failures(int64_t val) {
    oplog_apply_failures_total_.inc(val);
}
void P2PHAMetricManager::inc_oplog_best_effort_apply_skipped(int64_t val) {
    oplog_best_effort_apply_skipped_total_.inc(val);
}
void P2PHAMetricManager::inc_oplog_confirmed_holes(int64_t val) {
    oplog_confirmed_holes_total_.inc(val);
}
void P2PHAMetricManager::inc_force_promotions(int64_t val) {
    force_promotions_total_.inc(val);
}
void P2PHAMetricManager::inc_promotion_catchup_incomplete(int64_t val) {
    promotion_catchup_incomplete_total_.inc(val);
}
void P2PHAMetricManager::inc_promotion_restore_failures(int64_t val) {
    promotion_restore_failures_total_.inc(val);
}
void P2PHAMetricManager::inc_promotion_skipped_replicas(int64_t val) {
    promotion_skipped_replicas_total_.inc(val);
}
void P2PHAMetricManager::inc_promotion_skipped_objects(int64_t val) {
    promotion_skipped_objects_total_.inc(val);
}

void P2PHAMetricManager::inc_p2p_snapshot_bootstrap_success(int64_t val) {
    p2p_snapshot_bootstrap_success_total_.inc(val);
}

void P2PHAMetricManager::inc_p2p_snapshot_bootstrap_failures(int64_t val) {
    p2p_snapshot_bootstrap_failures_total_.inc(val);
}

void P2PHAMetricManager::inc_p2p_snapshot_resync_success(int64_t val) {
    p2p_snapshot_resync_success_total_.inc(val);
}

void P2PHAMetricManager::inc_p2p_snapshot_resync_failures(int64_t val) {
    p2p_snapshot_resync_failures_total_.inc(val);
}

void P2PHAMetricManager::inc_oplog_watch_disconnections(int64_t val) {
    oplog_watch_disconnections_total_.inc(val);
}

int64_t P2PHAMetricManager::get_oplog_watch_disconnections_total() {
    return static_cast<int64_t>(oplog_watch_disconnections_total_.value());
}

void P2PHAMetricManager::inc_oplog_applied_entries(int64_t val) {
    oplog_applied_entries_total_.inc(val);
}

int64_t P2PHAMetricManager::get_oplog_applied_entries_total() {
    return static_cast<int64_t>(oplog_applied_entries_total_.value());
}

void P2PHAMetricManager::inc_oplog_dropped_put_end(int64_t val) {
    oplog_dropped_put_end_total_.inc(val);
}

int64_t P2PHAMetricManager::get_oplog_dropped_put_end_total() {
    return static_cast<int64_t>(oplog_dropped_put_end_total_.value());
}

void P2PHAMetricManager::inc_oplog_batch_commits(int64_t val) {
    oplog_batch_commits_total_.inc(val);
}

void P2PHAMetricManager::inc_oplog_sync_batch_commits(int64_t val) {
    oplog_sync_batch_commits_total_.inc(val);
}

int64_t P2PHAMetricManager::get_oplog_batch_commits_total() {
    return static_cast<int64_t>(oplog_batch_commits_total_.value());
}

int64_t P2PHAMetricManager::get_oplog_sync_batch_commits_total() {
    return static_cast<int64_t>(oplog_sync_batch_commits_total_.value());
}

// ========== Latency Histograms ==========

void P2PHAMetricManager::observe_oplog_write_latency_us(int64_t latency_us) {
    oplog_write_latency_us_.observe(latency_us);
}

void P2PHAMetricManager::observe_election_duration_ms(int64_t duration_ms) {
    election_duration_ms_.observe(duration_ms);
}

void P2PHAMetricManager::observe_oplog_apply_latency_us(int64_t latency_us) {
    oplog_apply_latency_us_.observe(latency_us);
}

// ========== State Machine Metrics ==========

void P2PHAMetricManager::set_standby_state(int64_t state_value) {
    standby_state_.update(state_value);
}

int64_t P2PHAMetricManager::get_standby_state() {
    return static_cast<int64_t>(standby_state_.value());
}

void P2PHAMetricManager::inc_state_transitions(int64_t val) {
    state_transitions_total_.inc(val);
}

int64_t P2PHAMetricManager::get_state_transitions_total() {
    return static_cast<int64_t>(state_transitions_total_.value());
}

// ========== Serialization ==========

std::string P2PHAMetricManager::serialize_metrics() {
    std::stringstream ss;
...
std::string P2PHAMetricManager::get_summary_string() {
    std::stringstream ss;
