...
tl::expected<bool, ErrorCode> P2PMasterClient::ExistKey(
    std::string_view object_key, std::string ctx_attachment) {
    ScopedVLogTimer timer(1, "P2PMasterClient::ExistKey");
    timer.LogRequest("object_key=", object_key);
    auto result =
        ctx_attachment.empty()
            ? invoke_rpc<&P2PMasterRpcService::ExistKey, bool>(object_key)
            : invoke_rpc_with_context<
                  &P2PMasterRpcService::ExistKey_with_context, bool>(
                  std::move(ctx_attachment), object_key);
    ...
    auto result =
        ctx_attachment.empty()
            ...
            : invoke_batch_rpc_with_context<
                  &P2PMasterRpcService::BatchExistKey_with_context, bool>(
                  std::move(ctx_attachment), object_keys.size(), object_keys);
    timer.LogResponse("result=", result.size(), " keys");
    return result;
}

tl::expected<std::vector<P2PRouteDescriptor>, ErrorCode>
P2PMasterClient::GetReadRoute(std::string_view key,
    ...
    auto result =
        ctx_attachment.empty()
            ...
            : invoke_rpc_with_context<
                  &P2PMasterRpcService::GetReadRoute_with_context,
                  std::vector<P2PRouteDescriptor>>(
...
async_simple::coro::Lazy<
    tl::expected<std::vector<P2PRouteDescriptor>, ErrorCode>>
P2PMasterClient::AsyncGetReadRoute(std::string_view key,
                                   const P2PReadRouteConfig& config,
                                   std::string ctx_attachment) {
    if (ctx_attachment.empty()) {
        co_return co_await invoke_rpc_async<&P2PMasterRpcService::GetReadRoute,
                                            std::vector<P2PRouteDescriptor>>(
            P2PGetReadRouteRequest{.key = key, .config = config});
    }
    co_return co_await invoke_rpc_async_with_context<
        &P2PMasterRpcService::GetReadRoute_with_context,
        std::vector<P2PRouteDescriptor>>(
        std::move(ctx_attachment),
        P2PGetReadRouteRequest{.key = key, .config = config});
...
P2PMasterClient::BatchGetReadRoute(const std::vector<std::string_view>& keys,
                                   const P2PReadRouteConfig& config,
                                   std::string ctx_attachment) {
    ...
    auto result =
        ctx_attachment.empty()
            ...
            : invoke_rpc_with_context<
                  &P2PMasterRpcService::BatchGetReadRoute_with_context,
    ...
    for (size_t i = 0; i < keys.size(); ++i) {
        if (result->error_codes[i] == ErrorCode::OK) {
            ...
            response.emplace_back(tl::make_unexpected(result->error_codes[i]));
        }
    }
...
P2PMasterClient::GetReadRouteByRegex(std::string_view regex,
    ...
    auto result =
        ctx_attachment.empty()
            ? invoke_rpc<&P2PMasterRpcService::GetReadRouteByRegex,
...
tl::expected<P2PClientStatus, ErrorCode> P2PMasterClient::QueryClientStatus(
    const UUID& client_id) {
    ScopedVLogTimer timer(1, "P2PMasterClient::QueryClientStatus");
    timer.LogRequest("client_id=", client_id);
    auto result =
        invoke_rpc<&P2PMasterRpcService::QueryClientStatus, P2PClientStatus>(
            client_id);
    timer.LogResponseExpected(result);
    return result;
}
...
tl::expected<ViewVersionId, ErrorCode> P2PMasterClient::RegisterClient(
    const P2PRegisterClientRequest& req) {
    ScopedVLogTimer timer(1, "P2PMasterClient::RegisterClient");
    timer.LogRequest("client_id=", client_id_,
                     ", segments_count=", req.segments.size());
    auto result =
        invoke_rpc<&P2PMasterRpcService::RegisterClient, ViewVersionId>(req);
    timer.LogResponseExpected(result);
    return result;
}

tl::expected<ViewVersionId, ErrorCode> P2PMasterClient::UnregisterClient(
    const UUID& client_id) {
    ScopedVLogTimer timer(1, "P2PMasterClient::UnregisterClient");
    timer.LogRequest("client_id=", client_id_);
    auto result =
        invoke_rpc<&P2PMasterRpcService::UnregisterClient, ViewVersionId>(
            client_id);
    timer.LogResponseExpected(result);
    return result;
}
...
P2PMasterClient::GetWriteRoute(const P2PGetWriteRouteRequest& req,
    ...
    auto result = ctx_attachment.empty()
                      ...
                      : invoke_rpc_with_context<
                            &P2PMasterRpcService::GetWriteRoute_with_context,
                            std::vector<P2PWriteCandidate>>(
                            std::move(ctx_attachment), req);
    timer.LogResponseExpected(result);
    return result;
}
    ...
    auto result =
        ctx_attachment.empty()
            ...
            : invoke_rpc_with_context<
                  ...
                  P2PBatchGetWriteRouteResponse>(std::move(ctx_attachment),
                                                 req);
    timer.LogResponseExpected(result);
    return result;
}
...
tl::expected<void, ErrorCode> P2PMasterClient::PublishRoute(
    const P2PPublishRouteRequest& req, std::string ctx_attachment) {
    ...
    auto result =
        ctx_attachment.empty()
            ...
            : invoke_rpc_with_context<
                  &P2PMasterRpcService::PublishRoute_with_context, void>(
                  std::move(ctx_attachment), req);
    timer.LogResponseExpected(result);
    return result;
}
...
tl::expected<void, ErrorCode> P2PMasterClient::WithdrawRoute(
    const P2PWithdrawRouteRequest& req, std::string ctx_attachment) {
    ...
    auto result =
        ctx_attachment.empty()
            ...
            : invoke_rpc_with_context<
                  &P2PMasterRpcService::WithdrawRoute_with_context, void>(
                  std::move(ctx_attachment), req);
    timer.LogResponseExpected(result);
    return result;
}
...
std::vector<tl::expected<void, ErrorCode>> P2PMasterClient::BatchWithdrawRoute(
    const P2PBatchWithdrawRouteRequest& req, std::string ctx_attachment) {
    ...
    auto result =
        ctx_attachment.empty()
            ...
            : invoke_batch_rpc_with_context<
                  &P2PMasterRpcService::BatchWithdrawRoute_with_context, void>(
                  std::move(ctx_attachment), req.segment_ids.size(), req);
