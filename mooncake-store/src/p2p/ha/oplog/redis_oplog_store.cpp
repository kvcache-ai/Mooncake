...
ErrorCode RedisOpLogStore::Init() {
    ...
    P2PHAMetricManager::instance().set_oplog_last_sequence_id(
...
ErrorCode RedisOpLogStore::EnqueueWrite(const OpLogEntry& entry, bool sync) {
        ...
        if (existing != inflight_writes_.end()) {
            ...
            if (!sync && inflight_writes_.size() >= async_queue_max_entries_) {
                ...
                if (async_queue_overflow_mode_ ==
                    OpLogAsyncQueueOverflowMode::BYPASS) {
                    P2PHAMetricManager::instance().inc_oplog_queue_bypassed();
                } else {
                    P2PHAMetricManager::instance().inc_oplog_queue_rejected();
                }
            ...
            P2PHAMetricManager::instance().set_oplog_async_queue_size(
    ...
    if (!completed) {
        P2PHAMetricManager::instance().inc_oplog_sync_wait_timeouts();
...
void RedisOpLogStore::StartAsyncWorkers() {
    ...
    P2PHAMetricManager::instance().set_oplog_async_workers_running(true);
}
...
void RedisOpLogStore::StopAsyncWorkers() {
    ...
    P2PHAMetricManager::instance().set_oplog_async_workers_running(false);
...
void RedisOpLogStore::AsyncWriteLoop(size_t worker_id) {
    ...
    while (async_running_.load()) {
        ...
        ErrorCode err = ctx ? PersistEntryNoLatest(ctx, pending->entry)
                            : ErrorCode::INTERNAL_ERROR;
        P2PHAMetricManager::instance().observe_oplog_write_latency_us(
            std::chrono::duration_cast<std::chrono::microseconds>(
                std::chrono::steady_clock::now() - write_start)
                .count());
        if (err != ErrorCode::OK) {
            P2PHAMetricManager::instance().inc_oplog_write_failures();
            ...
            size_t attempts;
            {
                ...
                if (IsBestEffortP2POpLog(pending->entry.op_type) &&
                    ...
                    pending->done = true;
                    pending->result = ErrorCode::INTERNAL_ERROR;
                    P2PHAMetricManager::instance().inc_oplog_best_effort_dropped();
                    P2PHAMetricManager::instance().set_oplog_async_queue_size(
                        static_cast<int64_t>(inflight_writes_.size()));
                    sync_cv_.notify_all();
            ...
            LOG(WARNING) << "RedisOpLogStore: async write failed, retrying"
                         ...
                         << ", attempts=" << attempts
                         << ", error=" << toString(err);
            P2PHAMetricManager::instance().inc_oplog_write_retries();
            std::this_thread::sleep_for(
                std::chrono::milliseconds(kAsyncRetryDelayMs));
            {
                std::lock_guard<std::mutex> lock(async_mutex_);
...
ErrorCode RedisOpLogStore::GetLatestSequenceId(uint64_t& sequence_id) {
    ...
    if (reply->type == REDIS_REPLY_NIL) {
        sequence_id = 0;
        P2PHAMetricManager::instance().set_oplog_last_sequence_id(0);
        return ErrorCode::OK;
    }
    if (reply->type != REDIS_REPLY_STRING ||
