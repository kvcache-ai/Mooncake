...
ErrorCode P2PHotStandbyService::Promote(bool force) {
    ...
    if (force_apply_failure) {
        P2PHAMetricManager::instance().inc_force_promotions();
    ...
    auto gaps = force_apply_failure
                    ? P2POpLogApplierBase::GapResolveResult{}
...
ErrorCode P2PHotStandbyService::FinalCatchUpForPromotionLocked(
    ...
    for (; batch_count < kMaxCatchUpBatches; ++batch_count) {
        ...
        if (read_err != ErrorCode::OK) {
            P2PHAMetricManager::instance().inc_promotion_catchup_incomplete();
...
ErrorCode P2PHotStandbyService::BootstrapFromSnapshotSources(
    ...
    for (const auto& endpoint : endpoints) {
        ...
        if (err == ErrorCode::OK) {
            ...
            P2PHAMetricManager::instance().inc_p2p_snapshot_bootstrap_success();
            P2PHAMetricManager::instance()
    ...
    P2PHAMetricManager::instance().inc_p2p_snapshot_bootstrap_failures();
    return ErrorCode::INTERNAL_ERROR;
}
...
ErrorCode P2PHotStandbyService::ResyncFromSnapshotLocked() {
    ...
    if (err != ErrorCode::OK) {
        LOG(ERROR) << "P2PHotStandbyService: snapshot resync bootstrap failed"
                   << ", cluster_id=" << config_.cluster_id
                   << ", error=" << toString(err);
        P2PHAMetricManager::instance().inc_p2p_snapshot_resync_failures();
        return err;
    }
    ...
    P2PHAMetricManager::instance().set_p2p_bootstrap_catchup_target_sequence_id(
    ...
    P2PHAMetricManager::instance().inc_p2p_snapshot_resync_success();
        ...
        while (!recovery_stopping_) {
            ...
            if (err == ErrorCode::OK) {
                if (!attempted_snapshot_resync) {
                    P2PHAMetricManager::instance().inc_oplog_reader_reconnects();
                    LOG(INFO)
