    ...
    while (eviction_running_) {
        ...
        if (used_ratio > eviction_high_watermark_ratio_ ||
            ...
            double evict_ratio_lowerbound =
                std::max(evict_ratio_target * 0.5, // 閼峰啿鐨憰浣圭獝濮规媽绻栨稊鍫濐樋
...
void MasterService::BatchEvict(double evict_ratio_target,
    ...
    auto try_evict_or_offload =
        ...
        // LOCAL_DISK replica already exists 閳?safe to delete MEMORY immediately
        ...
        if (queued) {
            ...
            // (data survives via the pinned replica 閳?disk). Evict them now to
        ...
        // skip this cycle 閳?the outer eviction loop will retry after the
    ...
    // TODO: 闂呮劕鎯堟稉鈧稉顏勪海鐠佹拝绱濋崡铏閺堝』hard閻ㄥ嫭鏆熼幑顔煎瀻鐢啯鐦潏鍐ㄦ綆閸栤偓
    // 閹碘偓娴犮儰绮犲В蹇庨嚋shard娑擃厽绐愬Ч鎵祲閸氬本鐦笟瀣畱閺佺増宓侀敍鍫㈡倞閹磭濮搁崘鍏哥瑓閿?
    ...
    if (evicted_count > 0 || released_discarded_cnt > 0 ||
        ...
        MasterMetricManager::instance().inc_eviction_success(evicted_count,
                                                             total_freed_size);
        MasterMetricManager::instance().inc_mem_eviction_success(evicted_count,
                                                             total_freed_size);
    } else {
        if (object_count == 0) {
...
void MasterService::NoFBatchEvict(double evict_ratio_target,
    ...
    for (size_t i = 0; i < metadata_shards_.size(); i++) {
        MetadataShardAccessorRW shard(this,
                                      (start_idx + i) % metadata_shards_.size());
        DiscardExpiredProcessingReplicas(shard, now);
        object_count += shard->metadata.size();
    ...
    while (nof_heartbeat_running_) {
        ...
        std::optional<MountedNoFSegmentSnapshot> probe_target;
        {
            ...
            if (!ok_segments.empty()) {
                ...
                for (size_t offset = 0; offset < ok_segments.size();
                     ++offset) {
        ...
        if (probe_success) {
            ...
            auto success_time = std::chrono::steady_clock::now();
            {
                std::lock_guard<std::mutex> lock(nof_heartbeat_mutex_);
                auto it =
                    nof_heartbeat_states_.find(probe_target->segment_id);
                if (it != nof_heartbeat_states_.end()) {
