...
auto MasterService::AddReplica(const UUID& client_id, const std::string& key,
    ...
    std::shared_lock<std::shared_mutex> shared_lock(snapshot_mutex_);
    {
        ...
        replica.bind_local_disk_lifetime(segment_it->second->lifetime);
    }
    ...
    try {
...

        {
            ...
            std::vector<std::pair<Segment, UUID>> unready_segments;
            {
                ScopedSegmentAccess segment_access =
                    segment_manager_.getSegmentAccess();
                segment_access.GetUnreadySegments(unready_segments);
            }
        ...
        {
            // LOCAL_DISK-only clients are absent from GetAllSegments(). Start
            // their restored lifetime grace period too, so ClientMonitor
            // invalidates them if they do not reconnect after restore.
