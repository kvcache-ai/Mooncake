...
MasterService::MasterService(const MasterServiceConfig& config)
    ...
    // Create metadata backend - all metadata operations will be delegated
    backend_ = std::make_unique<NativeMetadataBackend>(this);
...
auto MasterService::UnmountSegment(const UUID& segment_id,
                                   const UUID& client_id)
    -> tl::expected<void, ErrorCode> {
    return backend_->UnmountSegment(segment_id, client_id);
}
...
auto MasterService::BatchQueryIp(const std::vector<UUID>& client_ids)
    -> tl::expected<
        ...
        ErrorCode> {
    auto result = backend_->BatchQueryIp(client_ids);
    if (!result) return tl::unexpected(result.error());
    std::unordered_map<UUID, std::vector<std::string>, boost::hash<UUID>> converted(
...
auto MasterService::BatchReplicaClear(
    ...
    -> tl::expected<std::vector<std::string>, ErrorCode> {
    return backend_->BatchReplicaClear(object_keys, client_id, segment_name);
}
...
auto MasterService::GetReplicaListByRegex(const std::string& regex_pattern,
    ...
    -> tl::expected<
        ...
        ErrorCode> {
    return backend_->GetReplicaListByRegex(regex_pattern, tenant_id);
}

auto MasterService::GetReplicaList(const std::string& key,
                                   const std::string& tenant_id)
    -> tl::expected<GetReplicaListResponse, ErrorCode> {
    return backend_->GetReplicaList(key, tenant_id);
}
...
auto MasterService::PutStart(const UUID& client_id, const std::string& key,
    ...
    -> tl::expected<std::vector<Replica::Descriptor>, ErrorCode> {
    return backend_->PutStart(client_id, key, tenant_id, slice_length, config);
}
...
auto MasterService::PutEnd(const UUID& client_id, const std::string& key,
    ...
    -> tl::expected<void, ErrorCode> {
    return backend_->PutEnd(client_id, key, tenant_id, replica_type);
}

auto MasterService::AddReplica(const UUID& client_id, const std::string& key,
                               const std::string& tenant_id, Replica& replica)
    -> tl::expected<void, ErrorCode> {
    return backend_->AddReplica(client_id, key, tenant_id, replica);
}
...
auto MasterService::PutRevoke(const UUID& client_id, const std::string& key,
                              ...
                              ReplicaType replica_type)
    -> tl::expected<void, ErrorCode> {
    return backend_->PutRevoke(client_id, key, tenant_id, replica_type);
}
...
std::vector<tl::expected<void, ErrorCode>> MasterService::BatchPutEnd(
    ...
    return backend_->BatchPutEnd(client_id, keys, tenant_id, replica_type);
}

std::vector<tl::expected<void, ErrorCode>> MasterService::BatchPutRevoke(
    const UUID& client_id, const std::vector<std::string>& keys,
    const std::string& tenant_id, ReplicaType replica_type) {
    return backend_->BatchPutRevoke(client_id, keys, tenant_id, replica_type);
}
...
auto MasterService::UpsertStart(const UUID& client_id, const std::string& key,
                                ...
                                const ReplicateConfig& config)
    -> tl::expected<std::vector<Replica::Descriptor>, ErrorCode> {
    return backend_->UpsertStart(client_id, key, tenant_id, slice_length,
                                 config);
}
...
auto MasterService::UpsertEnd(const UUID& client_id, const std::string& key,
                              ...
                              ReplicaType replica_type)
    -> tl::expected<void, ErrorCode> {
    return backend_->UpsertEnd(client_id, key, tenant_id, replica_type);
}
...
auto MasterService::UpsertRevoke(const UUID& client_id, const std::string& key,
                                 ...
                                 ReplicaType replica_type)
    -> tl::expected<void, ErrorCode> {
    return backend_->UpsertRevoke(client_id, key, tenant_id, replica_type);
}
...
MasterService::BatchUpsertStart(const UUID& client_id,
                                ...
                                const ReplicateConfig& config) {
    return backend_->BatchUpsertStart(client_id, keys, tenant_id, slice_lengths,
                                      config);
}
...
std::vector<tl::expected<void, ErrorCode>> MasterService::BatchUpsertEnd(
    const UUID& client_id, const std::vector<std::string>& keys,
    const std::string& tenant_id) {
    return backend_->BatchUpsertEnd(client_id, keys, tenant_id);
}

std::vector<tl::expected<void, ErrorCode>> MasterService::BatchUpsertRevoke(
    const UUID& client_id, const std::vector<std::string>& keys,
    const std::string& tenant_id) {
    return backend_->BatchUpsertRevoke(client_id, keys, tenant_id);
}
...
auto MasterService::EvictDiskReplica(const UUID& client_id,
                                     ...
                                     ReplicaType replica_type)
    -> tl::expected<void, ErrorCode> {
    return backend_->EvictDiskReplica(client_id, key, tenant_id, replica_type);
}

std::vector<tl::expected<void, ErrorCode>> MasterService::BatchEvictDiskReplica(
    const UUID& client_id, const std::vector<std::string>& keys,
    const std::string& tenant_id, ReplicaType replica_type) {
    return backend_->BatchEvictDiskReplica(client_id, keys, tenant_id,
                                           replica_type);
}
...
tl::expected<CopyStartResponse, ErrorCode> MasterService::CopyStart(
    ...
    return backend_->CopyStart(client_id, key, tenant_id, src_segment,
                               tgt_segments);
}
...
auto MasterService::NotifyOffloadSuccess(
    ...
    -> tl::expected<void, ErrorCode> {
    return backend_->NotifyOffloadSuccess(client_id, tasks, metadatas);
}
...
auto MasterService::PromotionObjectHeartbeat(const UUID& client_id)
    ...
    return backend_->PromotionObjectHeartbeat(client_id);
}
...
auto MasterService::PromotionAllocStart(
    ...
    return backend_->PromotionAllocStart(client_id, key, tenant_id, size,
                                        preferred_segments);
}
...
auto MasterService::NotifyPromotionSuccess(const UUID& client_id,
                                           ...
                                           const std::string& tenant_id)
    -> tl::expected<void, ErrorCode> {
    return backend_->NotifyPromotionSuccess(client_id, key, tenant_id);
}
...
auto MasterService::NotifyPromotionFailure(const UUID& client_id,
                                           ...
                                           const std::string& tenant_id)
    -> tl::expected<void, ErrorCode> {
    return backend_->NotifyPromotionFailure(client_id, key, tenant_id);
}
