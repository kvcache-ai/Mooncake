...
MasterService::LockObjectRouteForTesting(TenantState& tenant_state) const {
...
MasterService::GroupEvictionResult MasterService::EvictGroupOrObject(
    ...
    MetadataShardAccessorRW shard(tenant_handle.get());
    ...
    for (const auto& member_key : member_keys) {
        ...
        // released); the trigger is erased by the caller. gated by !enable_oplog_
        // like the original callback path.
...
void MasterService::FinalizeRemovedReplicasAfterDurable(
    ...
    CancelPromotionTaskForRemovedReplicas(tenant_state, metadata,
                                          erased_replica_ids);
    if (!metadata.IsValid()) {
        EraseMetadata(tenant_state, object_entry, tenant_id, quota_mode, &shard);
        if (tenant_state.Empty()) {
            tenant_directory_.Remove(tenant_id);
        }
    } else {
...
void MasterService::EraseMetadata(
    ...
    EraseMetadata(tenant_state, entry, tenant_id, quota_mode, shard);
}
    ...
    tenant_directory_.Visit(
        [&](const TenantId& tenant_id,
            ...
            for (const auto& entry : objs) {
                ...
                ObjectMetadata& metadata = *entry->metadata();
                auto lease =
                    tenant_state.object_route.LeaseFor(metadata.group_id);
...
size_t MasterService::GetMetadataShardIndex(
    const ObjectMetadata& metadata) const {
    // The soft-pin deadline index keys on the tenant-scoped key (MakeScopedKey),
    // which is unique per
    // tenant, so the former shard index is a constant no-op here.
    return 0;
}
...
void MasterService::CleanupExpiredSoftPins(
    ...
    // Each scoped key belongs to exactly one tenant container, so no shard
...
void MasterService::ClearStaleHandles(
    ...
    tenant_directory_.Visit(
        [&](const TenantId& tenant_id,
            const std::shared_ptr<TenantState>& handle) {
            MetadataShardAccessorRW shard(handle.get());
            auto& tenant_state = *handle;
            // Collect handles, then lock each per-object.
            std::vector<std::shared_ptr<mooncake::tenant::ObjectEntry>> objs;
            handle->VisitObjects(
            ...
            for (const auto& entry : objs) {
                ...
                if (!cleanup_plan.removed_ids.empty()) {
                    ...
                    if (CleanupStaleHandles(key, tenant_id, tenant_state,
                                            metadata, is_stale, &shard)) {
...
auto MasterService::GetAllKeys(const TenantId& tenant_id)
    ...
    if (tenant_handle) {
        ...
        for (const auto& entry : objs) {
            ...
            // Collect-then-process (route released); lock per-object while reading.
            std::shared_lock<std::shared_mutex> entry_lock(entry->mutex);
            const ObjectMetadata& metadata = *entry->metadata();
            if (!HasReadableReplica(metadata)) {
                continue;
...
tl::expected<void, ErrorCode> MasterService::RestoreFromStandbySnapshot(
    ...
    for (auto& object : prepared_objects) {
        ...
        auto tenant_handle =
            GetOrCreateTenantStateHandle(object.tenant_id);
        auto& tenant_state = *tenant_handle;
...
auto MasterService::AllocateAndInsertMetadata(
    ...
    if (deadline_to_index) {
        soft_pin_deadline_index_.Upsert(tenant_id.MakeScopedKey(key),
                                        GetMetadataShardIndex(*entry->metadata()),
                                        *deadline_to_index);
    }
    ...
    auto attempt_once =
        ...
        auto now = std::chrono::system_clock::now();
        {
            ...
            if (entry && entry->has_metadata()) {
                ...
                // just before every EraseMetadata call, which re-acquires it and
                // releases it before the route mutation (the two locks are never
                // held together).
    ...
    auto attempt_once =
            ...
            case_a_committed_soft_pin_timeout;
        {
            ...
            if (object_entry != nullptr && object_entry->has_metadata()) {
                auto cleanup_plan =
                    BuildStaleHandleCleanupPlan(*object_entry->metadata(),
                                                alive_clients);
                if (!cleanup_plan.removed_ids.empty()) {
                    auto persist_result = PersistStaleHandleCleanupForHA(
                ...
                if (has_replacement_charge) {
                    ...
                    if (!transfer_result) {
                        LogTenantQuotaLedgerError(transfer_result,
                                                  ...
                                                  object_id.tenant_id, key);
                        EraseMetadata(tenant_state, new_entry, object_id.tenant_id,
                                      QuotaEraseMode::kFull, &shard);
...
tl::expected<CopyStartResponse, ErrorCode> MasterService::CopyStart(
    ...
    object_entry->replication_task = ReplicationTask{
        client_id, std::chrono::system_clock::now(),
        ReplicationTask::Type::COPY, source->id(), std::move(replica_ids),
        pending_quota_charge, dynamic_replication_lease_id,
        dynamic_replication_version_epoch};
...
long MasterService::RemoveAll(bool force) {
    ...
    // concurrent RemoveAlls must not both count/tear down the same object), so
    // serialize it with an EXCLUSIVE snapshot lock.
...
long MasterService::RemoveAll(const TenantId& tenant_id, bool force) {
    ...
    // snapshot lock so concurrent RemoveAlls do not double-count/tear down.
    std::unique_lock<std::shared_mutex> shared_lock(snapshot_mutex_);
    auto now = std::chrono::system_clock::now();
    const TenantId& normalized_tenant = ResolveRequestTenantId(tenant_id);
...
void MasterService::RunDfsEviction() {
    ...
    while (true) {
        ...
        // tenant's keys live in one tenant container, so there is no per-shard
        ...
        if (!tenant_handle) {
            ...
            // Validate and remove candidates. Take the per-object lock (there
            // the (removed) tenant lock; the object lock is released before
...
auto MasterService::NotifyOffloadSuccess(
    ...
    for (size_t i = 0; i < tasks.size(); ++i) {
        ...
        bool added_new_local_disk_replica = false;
        {
            ...
            // shared-lock section, before the per-object lock, so the check and
...
size_t MasterService::RunPromotionCandidateRetry(size_t max_shards_to_scan) {
    ...
    // shard-based partitioning; the batch size bounds the scan.
    (void)max_shards_to_scan;
...
void MasterService::ClearDynamicReplicationStateForKey(
    ...
    if (entry) {
        ...
        entry->dynamic_replication_leases.clear();
    }
    auto& leases = tenant_state.object_route.dynamic_replication_leases;
        ...
        [&](const TenantId& tenant_id,
            ...
            for (const auto& entry : objs) {
                ...
                if (entry->dynamic_replication_pending &&
                    entry->dynamic_replication_pending
                            ->expire_at_ms_epoch < now_ms) {
            ...
            auto& leases =
                tenant_state.object_route.dynamic_replication_leases;
            for (auto lease_it = leases.begin(); lease_it != leases.end();) {
                if (lease_it->second.expire_at_ms_epoch < now_ms) {
                    lease_it = leases.erase(lease_it);
                } else {
                    ++lease_it;
                }
            }
...
MasterService::SubmitReplicaActionProposalLocked(
    ...
    auto& tenant_state = accessor.GetTenantState();
    auto object_entry = accessor.GetEntry();
    auto& tenant_leases = tenant_state.object_route.dynamic_replication_leases;
    auto lease_it = tenant_leases.find(proposal_id);
    if (lease_it != tenant_leases.end()) {
        if (lease_it->second.expire_at_ms_epoch >= now_ms) {
    ...
    tenant_leases[proposal_id] = lease;
    return lease;
}
...
PromotionQueueResult MasterService::TryPushPromotionQueue(
    ...
    // a few extra tasks slip in, but the per-object lock serializes a given
    ...
    EraseCandidate(tenant_state, key);
    if (object_entry != nullptr) {
        object_entry->promotion_task = PromotionTask{
            .source_id = source->id(),
            .alloc_id = 0,
...
auto MasterService::PromotionAllocStart(
    ...
    // removed or evicted. The per-object lock is held for the rest of this
...
void MasterService::DiscardExpiredProcessingReplicas(
    ...
    auto& tenant_state = shard.get();
    // Collect handles, then lock each per-object (there is no tenant level lock).
    ...
    for (const auto& entry : entries) {
        ...
        const auto ttl = entry->offloading_task->start_time +
                         put_start_release_timeout_sec_;
        if (ttl > now) {
            continue;
        }
    ...
    for (const auto& entry : entries) {
        ...
        const auto ttl = entry->promotion_task->start_time +
                         put_start_release_timeout_sec_;
        if (ttl > now) {
            continue;
        }
        auto source = entry->metadata()->GetReplicaByID(
            entry->promotion_task->source_id);
        if (source != nullptr) {
            source->dec_refcnt();
        }
        ...
        ReleaseTenantQuota(GetBoundTenantQuotaHandle(tenant_state),
                           std::exchange(
                               entry->promotion_task
                                   ->pending_quota_charge_bytes,
                               0));
...
MasterService::EvictTenantMemoryForQuota(const TenantId& tenant_id,
    ...
    auto try_evict_group_or_object =
        ...
        auto evict_one_member =
            ...
            // per-object lock is released, so do not erase here.
            return outcome;
        };
...
void MasterService::BatchEvict(double evict_ratio_target,
    ...
    // single snapshot-consistent walk of all tenants (no shard partitioning).
    std::shared_lock<std::shared_mutex> shared_lock(snapshot_mutex_);
        ...
        tenant_directory_.Visit(
            [&](const TenantId& tenant_id,
                ...
                for (auto& entry : objects) {
                    ...
                    evicted_count++;
                    PublishKvRemovedAfterEvict(key, metadata.size * erased,
...
MasterService::MetadataSerializer::Serialize() {
    ...
    for (const auto& [tenant_id, handle] : tenants_with_metadata) {
        ...
        std::vector<uint8_t> compressed_data =
            zstd_compress(reinterpret_cast<const uint8_t*>(tenant_buffer.data()),
...
MasterService::MetadataSerializer::Deserialize(
    ...
    for (uint32_t i = 0; i < shards_obj->via.map.size; ++i) {
        ...
        // deserialization ignores the shard and re-routes each object to its own
        // tenant, so those snapshots migrate automatically.
...
void MasterService::MetadataSerializer::Reset() {
    service_->soft_pin_deadline_index_.Clear();
    {
        ...
        service_->tenant_directory_.Visit(
            [&](const TenantId& tenant_id, const std::shared_ptr<TenantState>&) {
...
tl::expected<void, SerializationError>
MasterService::MetadataSerializer::DeserializeTenant(const msgpack::object& obj) {
    if (obj.type != msgpack::type::MAP) {
        return tl::make_unexpected(SerializationError(
    ...
    for (uint32_t j = 0; j < metadata_array->via.array.size; ++j) {
        ...
        // TenantDirectory. There is no tenant-level lock; InsertObject takes the
        // route lock itself, and the object is brand-new so no per-object lock
        // is needed.
...
MasterService::ValidateDynamicReplicaPendingForCopyStart(
    ...
    const bool dynamic_copy = dynamic_replication_lease_id != UUID{};
    if (!pending_entry || !pending_entry->dynamic_replication_pending.has_value()) {
        if (dynamic_copy) {
            return tl::make_unexpected(
                ErrorCode::UNAVAILABLE_IN_CURRENT_STATUS);
        }
