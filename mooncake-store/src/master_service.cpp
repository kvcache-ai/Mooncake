...
std::shared_ptr<mooncake::metadata::TenantCatalog>
...
TenantQuotaHandle MasterService::GetBoundTenantQuotaHandle(
    const metadata::TenantCatalog& tenant_state) const {
    if (!enable_multi_tenants_) {
        return nullptr;
    }
...
MasterService::GroupEvictionResult MasterService::EvictGroupOrObject(
    ...
    const std::function<MasterService::EvictMemberOutcome(
        const std::string&, ObjectMetadata&, metadata::TenantCatalog&,
        metadata::TenantCatalog&)>& evict_one_member) {
    ...
    for (const auto& member_key : member_keys) {
        ...
        EvictMemberOutcome member_outcome = evict_one_member(
            member_key, member_metadata, tenant_state, tenant_state);
...
void MasterService::CleanupExpiredSoftPins(
    ...
    for (const auto& entry : expired_entries) {
        ...
        auto object_entry = tenant_handle->Get(key);
        if (!object_entry) {
            continue;
        }
...
tl::expected<void, ErrorCode> MasterService::ClearStaleHandles(
    ...
    catalog_.Visit([&](const TenantId& tenant_id,
        ...
        // Whether this sweep actually dropped a route slot for this tenant.
        // Only then is the route worth rehashing down: a sweep that finds
        // nothing must not pay for it.
        bool erased_in_tenant = false;
        for (const auto& entry : objs) {
        ...
        if (erased_in_tenant) {
            handle->ShrinkRouteTableIfSparse();
        }
    });
    if (first_persist_error) {
...
std::vector<tl::expected<bool, ErrorCode>> MasterService::BatchExistKey(
    ...
    std::shared_lock<std::shared_mutex> shared_lock(snapshot_mutex_);
    auto tenant_handle = catalog_.Lookup(normalized_tenant);
    if (!tenant_handle) {
...
tl::expected<void, ErrorCode> MasterService::RestoreFromStandbyState(
    ...
    const auto restore_chunk =
        ...
        for (const auto& entry : objects) {
            ...
            {
                auto existing_tenant = catalog_.Lookup(tenant_id);
                if (existing_tenant &&
        ...
        for (const auto& object : prepared_objects) {
            auto tenant_handle = catalog_.Lookup(object.tenant_id);
            if (tenant_handle &&
        ...
        for (auto& object : prepared_objects) {
            ...
            auto& tenant_state = *tenant_handle;
            auto entry = std::make_shared<mooncake::metadata::ObjectEntry>(
    ...
    for (const auto& key : object_keys) {
        ...
        if (clear_all_segments) {
            ...
            if (had_completed_disk_on_segment &&
                ...
                tenant_state.OnDiskReplicaRemoved(had_completed_disk_on_segment,
                                                  metadata);
            }
...
MasterService::BatchGetReplicaList(const std::vector<std::string>& keys,
    ...
    if (tenant_handle) {
        ...
        for (size_t original_idx = 0; original_idx < keys.size();
            ...
            const auto object_entry = tenant_state.Get(key);
            if (!object_entry) {
                VLOG(1) << "key=" << key << ", info=object_not_found";
...
auto MasterService::PutStart(const UUID& client_id, const std::string& key,
    ...
    auto admit =
        ...
        auto now = std::chrono::system_clock::now();
        {
            ...
            auto tenant_handle =
                GetOrCreateTenantCatalogHandle(object_id.tenant_id);
            auto& tenant_state = *tenant_handle;
            ...
            if (entry) {
                ...
                if (entry) {
                    ...
                    EraseMetadataLocked(tenant_state, entry,
                                        object_id.tenant_id,
                                        std::move(entry_lock),
                                        QuotaEraseMode::kFull, &tenant_state);
                    entry.reset();
            ...
            if (!entry) {
                return AllocateAndInsertMetadata(
                    tenant_state, client_id, key, slice_length, config,
...
auto MasterService::AddReplicaForRetainedClient(const UUID& client_id,
    ...
    if (!replacing_existing) {
        ...
        auto& tenant_state = accessor.GetTenantCatalog();
        tenant_state.OnDiskReplicaAdded(metadata);
        SyncCacheTotalAccounting(metadata);
            ...
            case_a_committed_soft_pin_timeout;
        {
                        ...
                        return tl::make_unexpected(
                            ErrorCode::OBJECT_ALREADY_EXISTS);
                                   ...
                                   *object_entry, object_entry->metadata(),
                                   retaining_clients, &tenant_state)) {
            ...
            if (!object_entry) {
                ...
                EraseMetadataLocked(tenant_state, object_entry,
                                    ...
                                    QuotaEraseMode::kPreserveOld, &tenant_state,
                                    previous_kv_media);
                ...
                tl::expected<std::vector<Replica::Descriptor>, ErrorCode>
                    allocate_result =
                        replacement_replicas.has_value()
                            ? InsertMetadata(
                                  tenant_state, client_id, key, slice_length,
                ...
                auto new_entry = tenant_state.Get(key);
                if (has_replacement_charge) {
                    if (!new_entry) {
                        auto rollback_result =
                            replacement_charge.ReleaseReplacement(
...
auto MasterService::EvictDiskReplica(const UUID& client_id,
    ...
    if (replica_type == ReplicaType::DISK) {
        ...
        if (had_completed_disk) {
            auto& tenant_state = accessor.GetTenantCatalog();
            tenant_state.OnDiskReplicaRemoved(had_completed_disk, metadata);
        }
    }
    ...
    while (true) {
        ...
        if (!tenant_handle) {
            ...
            for (const size_t i : candidate_indices) {
                const auto& candidate = candidates[i];
                auto object_entry = tenant_state.Get(candidate.key);
                if (!object_entry) {
                    accepted[i] = true;
                    continue;
                }
            ...
            if (accessor.Exists()) {
                ...
                if (object_entry != nullptr &&
                    ...
                    if (!segment_mounted) {
                        ...
                        auto& tenant_state = accessor.GetTenantCatalog();
                        tenant_state.OnDiskReplicaAdded(obj_metadata);
        ...
        catalog_.Visit(
            [&](const TenantId& tenant_id,
                ...
                for (const auto& key : handle->PromotionCandidateKeys()) {
                    ...
                    // Quick pre-filter under the per-object lock to avoid
                    // adding candidates that are obviously ineligible.
...
void MasterService::CleanupExpiredDynamicReplicationState() {
    ...
    catalog_.Visit([&](const TenantId& tenant_id,
                       const std::shared_ptr<metadata::TenantCatalog>& handle) {
        metadata::TenantCatalog& tenant_state = *handle;
        auto objs = handle->SnapshotObjects();
        // (key, generation at detection) — the re-Get below must verify the
...
bool MasterService::HasDynamicReplicationPending(
    metadata::TenantCatalog& tenant_state, const std::string& key) {
    auto entry = tenant_state.Get(key);
    ...
    while (eviction_running_) {
        ...
        if (used_ratio > eviction_high_watermark_ratio_ ||
            ...
            // tasks if we have not done this for a long time.
            {
                ...
                catalog_.Visit(
                    [&](const TenantId& tenant_id,
                        const std::shared_ptr<metadata::TenantCatalog>&
                            handle) {
                        DiscardExpiredProcessingReplicas(*handle, now);
                    });
...
MasterService::RebuildClientLivenessAfterSnapshotRestore() {
    ...
    catalog_.Visit([&](const TenantId&,
        ...
        for (const auto& entry : objs) {
            auto lk = entry->LockShared();
            for (const auto& replica : entry->metadata().GetAllReplicas()) {
                if (replica.is_local_disk_replica()) {
...
MasterService::EvictTenantMemoryForQuota(const TenantId& tenant_id,
    ...
    auto evict_replicas =
        [&, this](metadata::TenantCatalog& tenant_state,
                  ObjectMetadata& metadata,
    ...
    auto try_evict_group_or_object =
        ...
        // membership is read from the tenant's own object_index.
        auto evict_one_member =
            [&, this](const std::string& member_key,
                      ObjectMetadata& member_metadata,
                      metadata::TenantCatalog& state,
                      metadata::TenantCatalog& accessor) -> EvictMemberOutcome {
            (void)accessor;
...
void MasterService::BatchEvict(double evict_ratio_target,
    ...
    auto try_evict_or_offload =
        [&, this](
            const TenantId& tenant_id, const std::string& key,
            ObjectMetadata& metadata, metadata::TenantCatalog& tenant_state,
    ...
    // erase() never returns bucket memory, so a tenant that once held far more
    // objects than it does now would keep its high-water bucket array for the
    // lifetime of the process. Rehash the routes of the tenants that actually
    // evicted this cycle down to their live size. Each call takes only that
    // tenant's route lock, so unrelated tenants keep mutating.
    if (!evicted_tenants.empty()) {
        ...
        for (const auto& tenant_id : evicted_tenants) {
            auto handle = catalog_.Lookup(tenant_id);
            if (handle) {
                handle->ShrinkRouteTableIfSparse();
            }
        }
...
MasterService::MetadataSerializer::Serialize() {
    ...
    std::vector<std::pair<TenantId, std::shared_ptr<metadata::TenantCatalog>>>
...
void MasterService::MetadataSerializer::Reset() {
    ...
    // Group state is single-sourced in each tenant's object_index, so
...
MasterService::MetadataSerializer::SerializeTenant(
    ...
    // for large tenants.
    std::vector<std::shared_ptr<mooncake::metadata::ObjectEntry>> entries;
...
MasterService::ValidateDynamicReplicaPendingForCopyStart(
    ...
    auto pending_entry = tenant_state.Get(key);
    const bool dynamic_copy = dynamic_replication_lease_id != UUID{};
    if (!pending_entry ||
