...
MasterService::MasterService(bool enable_gc, uint64_t default_kv_lease_ttl, uint64_t default_kv_soft_pin_ttl,
    ...
    : allocation_strategy_(std::make_shared<RandomAllocationStrategy>()), enable_gc_(enable_gc),
      default_kv_lease_ttl_(default_kv_lease_ttl), default_kv_soft_pin_ttl_(default_kv_soft_pin_ttl),
      allow_evict_soft_pinned_objects_(allow_evict_soft_pinned_objects), eviction_ratio_(eviction_ratio),
      eviction_high_watermark_ratio_(eviction_high_watermark_ratio), client_live_ttl_sec_(client_live_ttl_sec),
      enable_ha_(enable_ha), cluster_id_(cluster_id) {
...
auto MasterService::MountSegment(const Segment &segment, const UUID &client_id) -> tl::expected<void, ErrorCode> {
	...
	if (enable_ha_) {
		...
		if (!client_ping_queue_.push(pod_client_id)) {
			LOG(ERROR) << "segment_name=" << segment.name << ", error=client_ping_queue_full";
...
auto MasterService::ReMountSegment(const std::vector<Segment> &segments, const UUID &client_id)
	...
	if (ok_client_.contains(client_id)) {
		LOG(WARNING) << "client_id=" << client_id << ", warn=client_already_remounted";
		// Return OK because this is an idempotent operation
		return {};
	}
...
auto MasterService::UnmountSegment(const UUID &segment_id, const UUID &client_id) -> tl::expected<void, ErrorCode> {
	...
	ScopedSegmentAccess segment_access = segment_manager_.getSegmentAccess();
	auto err = segment_access.CommitUnmountSegment(segment_id, client_id, metrics_dec_capacity);
	if (err != ErrorCode::OK) {
		return tl::make_unexpected(err);
	}
	return {};
...
auto MasterService::ExistKey(const std::string &key) -> tl::expected<bool, ErrorCode> {
	...
	if (auto status = metadata.HasDiffRepStatus(ReplicaStatus::COMPLETE)) {
		LOG(WARNING) << "key=" << key << ", status=" << *status << ", error=replica_not_ready";
...
std::vector<tl::expected<bool, ErrorCode>> MasterService::BatchExistKey(const std::vector<std::string> &keys) {
...
auto MasterService::GetAllKeys() -> tl::expected<std::vector<std::string>, ErrorCode> {
...
auto MasterService::GetAllSegments() -> tl::expected<std::vector<std::string>, ErrorCode> {
	ScopedSegmentAccess segment_access = segment_manager_.getSegmentAccess();
...
auto MasterService::QuerySegments(const std::string &segment) -> tl::expected<std::pair<size_t, size_t>, ErrorCode> {
	ScopedSegmentAccess segment_access = segment_manager_.getSegmentAccess();
...
auto MasterService::GetReplicaList(std::string_view key) -> tl::expected<std::vector<Replica::Descriptor>, ErrorCode> {
...
auto MasterService::PutStart(const std::string &key, const std::vector<uint64_t> &slice_lengths,
                             const ReplicateConfig &config)
    -> tl::expected<std::vector<Replica::Descriptor>, ErrorCode> {
	if (config.replica_num == 0 || key.empty() || slice_lengths.empty()) {
		LOG(ERROR) << "key=" << key << ", replica_num=" << config.replica_num
		           << ", slice_count=" << slice_lengths.size() << ", key_size=" << key.size()
	...
	for (size_t i = 0; i < slice_lengths.size(); ++i) {
		if (slice_lengths[i] > kMaxSliceSize) {
			LOG(ERROR) << "key=" << key << ", slice_index=" << i << ", slice_size=" << slice_lengths[i]
	...
	VLOG(1) << "key=" << key << ", value_length=" << total_length << ", slice_count=" << slice_lengths.size()
	...
	if (it != metadata_shards_[shard_idx].metadata.end() && !CleanupStaleHandles(it->second)) {
	...
	replicas.reserve(config.replica_num);
	{
		...
		for (size_t i = 0; i < config.replica_num; ++i) {
			...
			for (size_t j = 0; j < slice_lengths.size(); ++j) {
				...
				auto handle = allocation_strategy_->Allocate(allocators, allocators_by_name, chunk_size, config);

				if (!handle) {
					LOG(ERROR) << "key=" << key << ", replica_id=" << i << ", slice_index=" << j
				...
				VLOG(1) << "key=" << key << ", replica_id=" << i << ", slice_index=" << j << ", handle=" << *handle
	...
	metadata_shards_[shard_idx].metadata.emplace(
	    ...
	    std::forward_as_tuple(total_length, std::move(replicas), config.with_soft_pin));
	return replica_list;
}

auto MasterService::PutEnd(const std::string &key) -> tl::expected<void, ErrorCode> {
	MetadataAccessor accessor(this, key);
	if (!accessor.Exists()) {
...
auto MasterService::PutRevoke(const std::string &key) -> tl::expected<void, ErrorCode> {
	MetadataAccessor accessor(this, key);
	if (!accessor.Exists()) {
	...
	if (auto status = metadata.HasDiffRepStatus(ReplicaStatus::PROCESSING)) {
		LOG(ERROR) << "key=" << key << ", status=" << *status << ", error=invalid_replica_status";
...
std::vector<tl::expected<void, ErrorCode>> MasterService::BatchPutEnd(const std::vector<std::string> &keys) {
...
std::vector<tl::expected<void, ErrorCode>> MasterService::BatchPutRevoke(const std::vector<std::string> &keys) {
	std::vector<tl::expected<void, ErrorCode>> results;
	results.reserve(keys.size());
	for (const auto &key : keys) {
...
auto MasterService::Remove(const std::string &key) -> tl::expected<void, ErrorCode> {
	MetadataAccessor accessor(this, key);
	if (!accessor.Exists()) {
...
long MasterService::RemoveAll() {
	...
	VLOG(1) << "action=remove_all_objects"
	        << ", removed_count=" << removed_count << ", total_freed_size=" << total_freed_size;
	return removed_count;
}

auto MasterService::MarkForGC(const std::string &key, uint64_t delay_ms) -> tl::expected<void, ErrorCode> {
...
auto MasterService::Ping(const UUID &client_id) -> tl::expected<std::pair<ViewVersionId, ClientStatus>, ErrorCode> {
	if (!enable_ha_) {
...
void MasterService::GCThreadFunc() {
	...
	std::priority_queue<GCTask *, std::vector<GCTask *>, GCTaskComparator> local_pq;
	...
	while (gc_running_) {
		...
		while (!local_pq.empty()) {
			...
			if (!result && result.error() != ErrorCode::OBJECT_NOT_FOUND &&
				...
				LOG(WARNING) << "key=" << task->key << ", error=gc_remove_failed, error_code=" << result.error();
			}
			delete task;
		}
		double used_ratio = MasterMetricManager::instance().get_global_used_ratio();
		if (used_ratio > eviction_high_watermark_ratio_ || (need_eviction_ && eviction_ratio_ > 0.0)) {
			double evict_ratio_target =
			    std::max(eviction_ratio_, used_ratio - eviction_high_watermark_ratio_ + eviction_ratio_);
			double evict_ratio_lowerbound =
			    std::max(evict_ratio_target * 0.5, used_ratio - eviction_high_watermark_ratio_);
...
void MasterService::BatchEvict(double evict_ratio_target, double evict_ratio_lowerbound) {
	if (evict_ratio_target < evict_ratio_lowerbound) {
		LOG(ERROR) << "evict_ratio_target=" << evict_ratio_target
		           << ", evict_ratio_lowerbound=" << evict_ratio_lowerbound << ", error=invalid_params";
	...
	for (size_t i = 0; i < metadata_shards_.size(); i++) {
		...
		for (auto it = shard.metadata.begin(); it != shard.metadata.end(); it++) {
			...
			if (!it->second.IsLeaseExpired(now) || it->second.HasDiffRepStatus(ReplicaStatus::COMPLETE)) {
				continue;
			}
		...
		if (ideal_evict_num > 0 && !candidates.empty()) {
			...
			std::nth_element(candidates.begin(), candidates.begin() + (evict_num - 1), candidates.end());
			...
			while (it != shard.metadata.end()) {
				...
				// pass
				if (!it->second.IsLeaseExpired(now) || it->second.IsSoftPinned(now) ||
	...
	long target_evict_num = std::ceil(object_count * evict_ratio_lowerbound) - evicted_count;
	// The actual number of objects we can evict in the second pass
	target_evict_num = std::min(target_evict_num, (long)no_pin_objects.size() + (long)soft_pin_objects.size());
	...
	if (target_evict_num > 0) {
		...
		if (target_evict_num <= static_cast<long>(no_pin_objects.size())) {
			...
			std::nth_element(no_pin_objects.begin(), no_pin_objects.begin() + (target_evict_num - 1),
			                 no_pin_objects.end());
			...
			for (size_t i = 0; i < metadata_shards_.size() && target_evict_num > 0; i++) {
				...
				while (it != shard.metadata.end() && target_evict_num > 0) {
					if (it->second.lease_timeout <= target_timeout && !it->second.IsSoftPinned(now) &&
					    !it->second.HasDiffRepStatus(ReplicaStatus::COMPLETE)) {
						// Evict this object
						total_freed_size += it->second.size * it->second.replicas.size();
						it = shard.metadata.erase(it);
			...
			const long soft_pin_evict_num = target_evict_num - static_cast<long>(no_pin_objects.size());
			// For soft pin objects, prioritize to evict the ones with smaller
			// lease timeout.
			std::nth_element(soft_pin_objects.begin(), soft_pin_objects.begin() + (soft_pin_evict_num - 1),
			...
			for (size_t i = 0; i < metadata_shards_.size() && target_evict_num > 0; i++) {
				...
				while (it != shard.metadata.end() && target_evict_num > 0) {
					...
					if (!it->second.IsSoftPinned(now) || it->second.lease_timeout <= soft_target_timeout) {
						total_freed_size += it->second.size * it->second.replicas.size();
						it = shard.metadata.erase(it);
						evicted_count++;
						target_evict_num--;
			...
			LOG(ERROR) << "Error in second pass eviction: target_evict_num=" << target_evict_num
			           << ", no_pin_objects.size()=" << no_pin_objects.size()
			           << ", soft_pin_objects.size()=" << soft_pin_objects.size() << ", evicted_count=" << evicted_count
			           << ", object_count=" << object_count << ", evict_ratio_target=" << evict_ratio_target
	...
	VLOG(1) << "action=evict_objects"
	        << ", evicted_count=" << evicted_count << ", total_freed_size=" << total_freed_size;
}
		...
		if (!expired_clients.empty()) {
			...
			std::vector<std::string> segment_names;
			{
				...
				for (auto &client_id : expired_clients) {
					...
					for (auto &seg : segments) {
						...
						if (segment_access.PrepareUnmountSegment(seg.id, metrics_dec_capacity) == ErrorCode::OK) {
							unmount_segments.push_back(seg.id);
							dec_capacities.push_back(metrics_dec_capacity);
							client_ids.push_back(client_id);
							segment_names.push_back(seg.name);
						} else {
							LOG(ERROR) << "client_id=" << client_id << ", segment_name=" << seg.name
							           << ", "
			...
			if (!unmount_segments.empty()) {
				...
				for (size_t i = 0; i < unmount_segments.size(); i++) {
					...
					LOG(INFO) << "client_id=" << client_ids[i] << ", segment_name=" << segment_names[i]
					          << ", action=unmount_expired_segment";
				}
			}
		}

		std::this_thread::sleep_for(std::chrono::milliseconds(kClientMonitorSleepMs));
	}
}
