...
DeserializeStandbyObjectMetadata(
    const msgpack::object& object, const SegmentPool::ReadAccess& segment_view,
...
class CatalogBackedSnapshotProvider final : public SnapshotProvider {
   public:
    ...
    tl::expected<std::optional<LoadedSnapshot>, ErrorCode> LoadLatestSnapshot(
        ...
        auto drivers = CreateRegionDrivers(driver_config);
        if (!drivers) return tl::make_unexpected(drivers.error());
        SegmentPool segment_pool(std::move(*drivers));
        auto deserialize_segments =
            ha::StoreResourceSnapshotCodec::Decode(segments_content);
        ...
        auto restored = segment_pool.RestoreSnapshot(
            std::move(deserialize_segments->segment_pool), false);
        if (!restored) return tl::make_unexpected(restored.error());
