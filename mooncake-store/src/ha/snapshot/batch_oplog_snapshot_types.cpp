...
void EncodeIdentity(const SnapshotMetadata& metadata, Json::Value* root) {
    (*root)["schema_version"] =
        static_cast<Json::UInt>(metadata.schema_version);
...
bool DecodeIdentity(const Json::Value& root, SnapshotMetadata* metadata,
                    std::string* reason) {
    return GetUInt32(root, "schema_version", &metadata->schema_version,
                     reason) &&
...
bool DecodeBatchOpLogSnapshotDescriptor(
    ...
    if (descriptor == nullptr) {
        SetReason(reason, "descriptor output is null");
        return false;
    }
    Json::Value root;
...
bool DecodeBatchOpLogSnapshotManifest(std::string_view value,
    ...
    if (manifest == nullptr) {
        SetReason(reason, "manifest output is null");
        return false;
    }
    Json::Value root;
