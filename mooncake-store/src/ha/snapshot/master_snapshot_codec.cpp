...
MasterSnapshotStateView::MasterSnapshotStateView(
    MasterService& master_service, SegmentPool& segment_pool,
    LocalSsdManager& local_ssd_manager, ClientTaskManager& task_manager)
    : master_service(master_service),
      segment_pool(segment_pool),
      local_ssd_manager(local_ssd_manager),
...
MasterSnapshotCodec::EncodeSegments(const SegmentPool& segment_pool,
    ...
    auto snapshot = segment_pool.CaptureSnapshot();
    if (!snapshot) {
        return tl::unexpected(SerializationError(
            snapshot.error(), "failed to capture segment pool snapshot"));
    }
    return StoreResourceSnapshotCodec::Encode(
        *snapshot, local_ssd_manager.ExportPersistedState());
}

tl::expected<void, SerializationError> MasterSnapshotCodec::DecodeSegments(
    MasterService* master_service, const std::vector<uint8_t>& data) const {
    auto snapshot = StoreResourceSnapshotCodec::Decode(data);
    if (!snapshot) {
    ...
    if (!restored) {
        return tl::unexpected(SerializationError(
            restored.error(), "failed to restore segment pool snapshot"));
    }
    master_service->local_ssd_manager_.RestorePersistedState(
        std::move(snapshot->local_ssd));
    return {};
}
