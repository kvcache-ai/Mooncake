...
Client::Client(const std::string &local_hostname, const std::string &metadata_connstring,
               const std::string &storage_root_dir)
    : local_hostname_(local_hostname), metadata_connstring_(metadata_connstring), storage_root_dir_(storage_root_dir),
...
Client::~Client() {
	...
	for (auto &segment : segments_to_unmount) {
		auto result = UnmountSegment(reinterpret_cast<void *>(segment.base), segment.size);
		if (!result) {
...
ErrorCode Client::InitTransferEngine(const std::string &local_hostname, const std::string &metadata_connstring,
	...
	int rc = transfer_engine_.init(metadata_connstring, local_hostname, hostname, port);
	...
	if (protocol == "rdma") {
		...
		try {
			...
			LOG(ERROR) << "tcp_transport_install_failed error_message=\"" << e.what() << "\"";
	...
	transfer_submitter_ = std::make_unique<TransferSubmitter>(transfer_engine_, local_hostname, storage_backend_);

	return ErrorCode::OK;
}

std::optional<std::shared_ptr<Client>> Client::Create(const std::string &local_hostname,
                                                      const std::string &metadata_connstring,
                                                      const std::string &protocol, void **protocol_args,
                                                      const std::string &master_server_entry) {
	// If MOONCAKE_STORAGE_ROOT_DIR is set, use it as the storage root directory
	std::string storage_root_dir =
	    std::getenv("MOONCAKE_STORAGE_ROOT_DIR") ? std::getenv("MOONCAKE_STORAGE_ROOT_DIR") : "";

	auto client = std::shared_ptr<Client>(new Client(local_hostname, metadata_connstring, storage_root_dir));
	...
	err = client->InitTransferEngine(local_hostname, metadata_connstring, protocol, protocol_args);
	if (err != ErrorCode::OK) {
...
tl::expected<void, ErrorCode> Client::Get(const std::string &object_key, std::vector<Slice> &slices) {
...
Client::BatchGet(const std::vector<std::string> &object_keys,
	...
	for (size_t i = 0; i < batched_query_results.size(); ++i) {
		if (batched_query_results[i]) {
			...
			results.emplace_back(tl::unexpected(batched_query_results[i].error()));
		}
	}
	...
	if (!valid_keys.empty()) {
		...
		auto valid_results = BatchGet(valid_keys, valid_replica_lists, valid_slices);
...
tl::expected<std::vector<Replica::Descriptor>, ErrorCode> Client::Query(const std::string &object_key) {
...
Client::BatchQuery(const std::vector<std::string> &object_keys) {
	...
	if (response.size() != object_keys.size()) {
		LOG(ERROR) << "BatchQuery response size mismatch. Expected: " << object_keys.size()
		           << ", Got: " << response.size();
...
Client::BatchGet(const std::vector<std::string> &object_keys,
	...
	if (replica_lists.size() != object_keys.size()) {
		LOG(ERROR) << "Replica lists size (" << replica_lists.size() << ") doesn't match object keys size ("
		           << object_keys.size() << ")";
		std::vector<tl::expected<void, ErrorCode>> results;
		results.reserve(object_keys.size());
		for (size_t i = 0; i < object_keys.size(); ++i) {
	...
	std::vector<std::tuple<size_t, std::string, TransferFuture>> pending_transfers;
	...
	for (size_t i = 0; i < object_keys.size(); ++i) {
		...
		auto future = transfer_submitter_->submit(replica, slices_it->second, TransferRequest::READ);
		if (!future) {
		...
		VLOG(1) << "Submitted transfer for key " << key << " using strategy: " << static_cast<int>(future->strategy());
...
tl::expected<void, ErrorCode> Client::Put(const ObjectKey &key, std::vector<Slice> &slices,
...
enum class PutOperationState { PENDING, MASTER_FAILED, TRANSFER_FAILED, FINALIZE_FAILED, SUCCESS };

class PutOperation {
public:
	PutOperation(std::string_view k, const std::vector<Slice> &s) : key(k), slices(s) {
	...
	void SetError(ErrorCode error, const std::string &context = "") {
		...
		if (!context.empty()) {
			failure_context = toString(error) + ": " + context + "; " + failure_context.value_or("");
		}
		...
		LOG(WARNING) << "Put operation failed for key " << key
		             << ", context: " << failure_context.value_or("unknown error");
	}

	bool IsResolved() const {
		return state != PutOperationState::PENDING;
	}
...
std::vector<PutOperation> Client::CreatePutOperations(const std::vector<ObjectKey> &keys,
...
void Client::StartBatchPut(std::vector<PutOperation> &ops, const ReplicateConfig &config) {
	...
	auto start_responses = master_client_.BatchPutStart(keys, slice_lengths, config);

	// Ensure response size matches request size
	if (start_responses.size() != ops.size()) {
		LOG(ERROR) << "BatchPutStart response size mismatch: expected " << ops.size() << ", got "
		           << start_responses.size();
		for (auto &op : ops) {
			op.SetError(ErrorCode::RPC_FAIL, "BatchPutStart response size mismatch");
		}
		return;
	}
	...
	for (size_t i = 0; i < ops.size(); ++i) {
		if (!start_responses[i]) {
			...
			VLOG(1) << "Successfully started put for key " << ops[i].key << " with " << ops[i].replicas.size()
			        << " replicas";
		}
	}
}
...
void Client::SubmitTransfers(std::vector<PutOperation> &ops) {
	...
	for (auto &op : ops) {
		...
		if (op.replicas.empty()) {
			op.SetError(ErrorCode::INTERNAL_ERROR, "No replicas available for transfer");
			continue;
		}
		...
		for (size_t replica_idx = 0; replica_idx < op.replicas.size(); ++replica_idx) {
			const auto &replica = op.replicas[replica_idx];

			auto submit_result = transfer_submitter_->submit(replica, op.slices, TransferRequest::WRITE);

			if (!submit_result) {
				failure_context = "Failed to submit transfer for replica " + std::to_string(replica_idx);
		...
		if (!all_transfers_submitted) {
			LOG(ERROR) << "Transfer submission failed for key " << op.key << ": " << failure_context;
			op.SetError(ErrorCode::TRANSFER_FAIL, failure_context);
			op.pending_transfers.clear();
		} else {
			VLOG(1) << "Successfully submitted " << op.pending_transfers.size() << " transfers for key " << op.key;
		}
	}
}
...
void Client::WaitForTransfers(std::vector<PutOperation> &ops) {
	for (auto &op : ops) {
		...
		if (all_transfers_succeeded) {
			VLOG(1) << "All transfers completed successfully for key " << op.key;
			// Transfer phase successful - continue to finalization
			// Note: Don't mark as SUCCESS yet, need to complete finalization
		} else {
			std::string error_context = "Transfer " + std::to_string(failed_transfer_idx) + " failed";
			LOG(ERROR) << "Transfer failed for key " << op.key << ": " << toString(first_error) << " (" << error_context
			           << ")";
...
void Client::FinalizeBatchPut(std::vector<PutOperation> &ops) {
	...
	for (size_t i = 0; i < ops.size(); ++i) {
		...
		// finalization
		if (!op.IsResolved() && !op.replicas.empty() && !op.pending_transfers.empty()) {
			// Transfers completed, needs BatchPutEnd
			successful_keys.emplace_back(op.key);
			successful_indices.emplace_back(i);
		} else if (op.state != PutOperationState::PENDING && !op.replicas.empty()) {
	...
	if (!successful_keys.empty()) {
		...
		if (end_responses.size() != successful_keys.size()) {
			LOG(ERROR) << "BatchPutEnd response size mismatch: expected " << successful_keys.size() << ", got "
			           << end_responses.size();
			for (size_t idx : successful_indices) {
				ops[idx].SetError(ErrorCode::RPC_FAIL, "BatchPutEnd response size mismatch");
			}
		} else {
			...
			for (size_t i = 0; i < end_responses.size(); ++i) {
				...
				if (!end_responses[i]) {
					LOG(ERROR) << "Failed to finalize put for key " << successful_keys[i] << ": "
					           << toString(end_responses[i].error());
					ops[op_idx].SetError(end_responses[i].error(), "BatchPutEnd failed");
				} else {
					// Operation fully successful
					ops[op_idx].SetSuccess();
					VLOG(1) << "Successfully completed put for key " << successful_keys[i];
				}
			}
		}
	}
	...
	if (!failed_keys.empty()) {
		...
		if (revoke_responses.size() != failed_keys.size()) {
			LOG(ERROR) << "BatchPutRevoke response size mismatch: expected " << failed_keys.size() << ", got "
			           << revoke_responses.size();
			// Mark all failed operations with revoke RPC failure
			for (size_t idx : failed_indices) {
				ops[idx].SetError(ErrorCode::RPC_FAIL, "BatchPutRevoke response size mismatch");
			}
		} else {
			...
			for (size_t i = 0; i < revoke_responses.size(); ++i) {
				...
				if (!revoke_responses[i]) {
					LOG(ERROR) << "Failed to revoke put for key " << failed_keys[i] << ": "
					           << toString(revoke_responses[i].error());
					// Preserve original error but note revoke failure in
					// context
					std::string original_context = ops[op_idx].failure_context.value_or("unknown error");
					ops[op_idx].failure_context = original_context + "; revoke also failed";
				} else {
					LOG(INFO) << "Successfully revoked failed put for key " << failed_keys[i];
				}
			}
		}
	}
	...
	for (auto &op : ops) {
		if (!op.IsResolved()) {
			op.SetError(ErrorCode::INTERNAL_ERROR, "Operation not resolved after finalization");
			LOG(ERROR) << "Operation for key " << op.key << " was not properly resolved";
		}
	}
}

std::vector<tl::expected<void, ErrorCode>> Client::CollectResults(const std::vector<PutOperation> &ops) {
	std::vector<tl::expected<void, ErrorCode>> results;
	...
	for (const auto &op : ops) {
		...
		if (!op.result.has_value()) {
			...
			LOG(ERROR) << "Operation for key " << op.key << " failed: " << toString(op.result.error())
			           << (op.failure_context ? (" (" + *op.failure_context + ")") : "");
		} else {
			VLOG(1) << "Operation for key " << op.key << " completed successfully";
		}
	}
...
std::vector<tl::expected<void, ErrorCode>> Client::BatchPut(const std::vector<ObjectKey> &keys,
...
tl::expected<void, ErrorCode> Client::MountSegment(const void *buffer, size_t size) {
	if (buffer == nullptr || size == 0 || reinterpret_cast<uintptr_t>(buffer) % facebook::cachelib::Slab::kSize ||
	    size % facebook::cachelib::Slab::kSize) {
		LOG(ERROR) << "buffer=" << buffer << " or size=" << size << " is not aligned to "
	...
	for (auto &it : mounted_segments_) {
		...
		if (std::max(l1, l2) < std::min(r1, r2)) {
			LOG(ERROR) << "segment_overlaps base1=" << mtseg.base << " size1=" << mtseg.size << " base2=" << buffer
			           << " size2=" << size;
			return tl::unexpected(ErrorCode::INVALID_PARAMS);
		}
	}

	int rc = transfer_engine_.registerLocalMemory((void *)buffer, size, kWildcardLocation, true, true);
	if (rc != 0) {
		LOG(ERROR) << "register_local_memory_failed base=" << buffer << " size=" << size << ", error=" << rc;
		return tl::unexpected(ErrorCode::INVALID_PARAMS);
	}

	Segment segment(generate_uuid(), local_hostname_, reinterpret_cast<uintptr_t>(buffer), size);

	auto mount_result = master_client_.MountSegment(segment, client_id_);
	if (!mount_result) {
		ErrorCode err = mount_result.error();
		LOG(ERROR) << "mount_segment_to_master_failed base=" << buffer << " size=" << size << ", error=" << err;
		return tl::unexpected(err);
	}
...
tl::expected<void, ErrorCode> Client::UnmountSegment(const void *buffer, size_t size) {
	std::lock_guard<std::mutex> lock(mounted_segments_mutex_);
	auto segment = mounted_segments_.end();

	for (auto it = mounted_segments_.begin(); it != mounted_segments_.end(); ++it) {
		if (it->second.base == reinterpret_cast<uintptr_t>(buffer) && it->second.size == size) {
			segment = it;
			break;
		}
	}
	...
	auto unmount_result = master_client_.UnmountSegment(segment->second.id, client_id_);
	if (!unmount_result) {
		ErrorCode err = unmount_result.error();
		LOG(ERROR) << "Failed to unmount segment from master: " << toString(err);
		return tl::unexpected(err);
	}

	int rc = transfer_engine_.unregisterLocalMemory(reinterpret_cast<void *>(segment->second.base));
	if (rc != 0) {
...
tl::expected<void, ErrorCode> Client::RegisterLocalMemory(void *addr, size_t length, const std::string &location,
                                                          bool remote_accessible, bool update_metadata) {
	if (this->transfer_engine_.registerLocalMemory(addr, length, location, remote_accessible, update_metadata) != 0) {
		return tl::unexpected(ErrorCode::INVALID_PARAMS);
	}
	return {};
}

tl::expected<void, ErrorCode> Client::unregisterLocalMemory(void *addr, bool update_metadata) {
	if (this->transfer_engine_.unregisterLocalMemory(addr, update_metadata) != 0) {
		return tl::unexpected(ErrorCode::INVALID_PARAMS);
	}
	return {};
}
...
std::vector<tl::expected<bool, ErrorCode>> Client::BatchIsExist(const std::vector<std::string> &keys) {
	auto response = master_client_.BatchExistKey(keys);

	// Check if we got the expected number of responses
	if (response.size() != keys.size()) {
		LOG(ERROR) << "BatchExistKey response size mismatch. Expected: " << keys.size() << ", Got: " << response.size();
		// Return vector of RPC_FAIL errors
...
void Client::PrepareStorageBackend(const std::string &storage_root_dir, const std::string &fsdir) {
	// Initialize storage backend
...
ErrorCode Client::GetFromLocalFile(const std::string &object_key, std::vector<Slice> &slices,
                                   std::vector<Replica::Descriptor> &replicas) {
	if (!storage_backend_) {
...
void Client::PutToLocalFile(const std::string &key, std::vector<Slice> &slices) {
	if (!storage_backend_)
		return;
	...
	write_thread_pool_.enqueue(
	    [backend = storage_backend_, key, value = std::move(value)] { backend->StoreObject(key, value); });
}

ErrorCode Client::TransferData(const Replica::Descriptor &replica_descriptor, std::vector<Slice> &slices,
...
ErrorCode Client::TransferWrite(const Replica::Descriptor &replica_descriptor, std::vector<Slice> &slices) {
	return TransferData(replica_descriptor, slices, TransferRequest::WRITE);
}

ErrorCode Client::TransferRead(const Replica::Descriptor &replica_descriptor, std::vector<Slice> &slices) {
	size_t total_size = 0;
...
void Client::PingThreadFunc() {
	...
	auto remount_segment = [this]() {
		...
		auto remount_result = master_client_.ReMountSegment(segments, client_id_);
		if (!remount_result) {
	...
	while (ping_running_) {
		...
		if (remount_segment_future.valid() &&
		    remount_segment_future.wait_for(std::chrono::seconds(0)) == std::future_status::ready) {
		...
		if (ping_result) {
			...
			if (client_status == ClientStatus::NEED_REMOUNT && !remount_segment_future.valid()) {
				// Ensure at most one remount segment thread is running
				remount_segment_future = std::async(std::launch::async, remount_segment);
			}
