...
std::vector<tl::expected<void, ErrorCode>> Client::BatchGet(
    ...
    if (globalConfig().alloc_same_node_first) {
        return BatchGetWhenSameNodeFirst(object_keys, replica_lists, slices);
    }
...
void Client::StartBatchPutWhenSameNodeFirst(std::vector<PutOperation>& ops,
    ...
    VLOG(1) << "Put operation for key " << key << " len:" << all_slice_len
        ...
        for (size_t i = 0; i < keys.size(); ++i) {
            if (start_responses[i]) {
                ...
                auto seg = ops[i + 1]
                               ...
                               .transport_endpoint_;
                VLOG(1) << "Put operation for key " << ops[i+1].key
...
std::vector<tl::expected<void, ErrorCode>> Client::BatchGetWhenSameNodeFirst(
    ...
    for (size_t i = 0; i < object_keys.size(); ++i) {
        ...
        auto& buffer_descriptor =
            memory_descriptor.buffer_descriptors[0];
        auto seg = buffer_descriptor.transport_endpoint_;
        if (seg_to_op_map.find( seg) == seg_to_op_map.end()) {
            seg_to_op_map.emplace(
                seg, BatchGetOperation{
                         {replica}, {slices_it->second}, {i}, {}});
        }
        seg_to_op_map.at(seg).batched_slices.emplace_back(std::move(slices_it->second));
    ...
    for (auto &seg_to_op : seg_to_op_map) {
        ...
        if (!future) {
            for (auto index : op.key_indexes) {
                ...
                LOG(ERROR)
                    << "Failed to submit transfer operation for key: "
                    << object_keys[index];
            }
            continue;
        }
