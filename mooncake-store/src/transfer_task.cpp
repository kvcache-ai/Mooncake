...
void SpdkNofWorkerPool::workerThread(int work_idx) {
                ...
                if (it == seg_to_qos.end()) {
                    ...
                    if (IsSpdkNofDebugEnabled()) {
                        LOG(INFO) << "nof_qos_create worker_idx=" << work_idx
                                  ...
                                  << " blocks_per_chunk=" << seg_to_qos[task->seg_handle]->blocks_per_chunk
                                  << " inflight_blocks_limit="
                                  << seg_to_qos[task->seg_handle]->inflight_blocks_limit;
                    }
                } else {
    ...
    // seg_to_qos will automatically clean up SpdkNofQos objects using unique_ptr
...
std::optional<TransferFuture> TransferSubmitter::submitSpdkNofOperation(
    ...
    if (handle.transport_endpoint_.empty() || handle.size_ < size) {
        LOG(ERROR) << "Invalid NoF request endpoint="
                   << handle.transport_endpoint_ << ", buffer_size="
                   << handle.size_ << ", request_size=" << size;
        return std::nullopt;
    }
    ...
    if (block_size == INVALID_BLOCK_SIZE ||
        ...
        LOG(ERROR) << "NoF request offset=" << handle.buffer_address_
                   << ", size=" << size << " is not aligned to block size "
                   << block_size;
        return std::nullopt;
    }
