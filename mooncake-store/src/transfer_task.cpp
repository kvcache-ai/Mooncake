...
// ============================================================================
// to fully utilize the available ssd bandwidth, we use a default of 8 worker
// threads.
...
FilereadWorkerPool::FilereadWorkerPool(std::shared_ptr<StorageBackend> &backend) : shutdown_(false) {
	VLOG(1) << "Creating FilereadWorkerPool with " << kDefaultFilereadWorkers << " workers";
...
void FilereadWorkerPool::submitTask(FilereadTask task) {
	{
		std::lock_guard<std::mutex> lock(queue_mutex_);
		if (shutdown_.load()) {
			LOG(WARNING) << "Attempting to submit task to shutdown FilereadWorkerPool";
...
void FilereadWorkerPool::workerThread() {
	...
	while (true) {
		...
		// Wait for task or shutdown signal
		{
			...
			queue_cv_.wait(lock, [this] { return shutdown_.load() || !task_queue_.empty(); });
		...
		if (task.state) {
			try {
				...
				if (error_code == ErrorCode::OK) {
					VLOG(2) << "Fileread task completed successfully with " << task.file_path;
...
MemcpyWorkerPool::MemcpyWorkerPool() : shutdown_(false) {
	VLOG(1) << "Creating MemcpyWorkerPool with " << kDefaultMemcpyWorkers << " workers";
...
void MemcpyWorkerPool::submitTask(MemcpyTask task) {
	{
		std::lock_guard<std::mutex> lock(queue_mutex_);
		if (shutdown_.load()) {
			LOG(WARNING) << "Attempting to submit task to shutdown MemcpyWorkerPool";
			task.state->set_completed(ErrorCode::TRANSFER_FAIL);
			return;
		}
		task_queue_.push(std::move(task));
	...
	while (true) {
		...
		if (task.state) {
			try {
				...
				VLOG(2) << "Memcpy task completed successfully with " << task.operations.size() << " operations";
				task.state->set_completed(ErrorCode::OK);
			} catch (const std::exception &e) {
...
void TransferEngineOperationState::check_task_status() {
	...
	for (size_t i = 0; i < batch_size_; ++i) {
		...
		if (!s.ok()) {
			LOG(ERROR) << "Failed to get transfer status for batch " << batch_id_ << " task " << i << " with error "
			           << s.message();
		...
		case TransferStatusEnum::INVALID:
			LOG(ERROR) << "Transfer failed for batch " << batch_id_ << " task " << i << " with status "
	...
	if (has_failure) {
		VLOG(1) << "Setting batch " << batch_id_ << " result to TRANSFER_FAIL due to task failures";
		set_result_internal(ErrorCode::TRANSFER_FAIL);
		return;
	}
...
void TransferEngineOperationState::set_result_internal(ErrorCode error_code) {
	if (result_.has_value()) {
		LOG(ERROR) << "Attempting to set result multiple times for batch " << batch_id_
	...
	VLOG(1) << "Setting transfer result for batch " << batch_id_ << " to " << static_cast<int>(error_code);
...
void TransferEngineOperationState::wait_for_completion() {
	...
	while (true) {
		if (getCurrentTimeInNano() - start_ts > timeout_seconds * kOneSecondInNano) {
			LOG(ERROR) << "Failed to complete transfers after " << timeout_seconds << " seconds for batch "
			           << batch_id_;
			set_result_internal(ErrorCode::TRANSFER_FAIL);
			return;
		}
		...
		// Continue polling
		VLOG(1) << "Transfer engine operation still pending for batch " << batch_id_;
	}
}
...
TransferFuture::TransferFuture(std::shared_ptr<OperationState> state) : state_(std::move(state)) {
	CHECK(state_) << "TransferFuture requires valid state";
}

bool TransferFuture::isReady() const {
...
TransferSubmitter::TransferSubmitter(TransferEngine &engine, const std::string &local_hostname,
                                     std::shared_ptr<StorageBackend> &backend)
    : engine_(engine), local_hostname_(local_hostname), memcpy_pool_(std::make_unique<MemcpyWorkerPool>()),
	...
	if (env_value == nullptr) {
		...
		if (env_str == "false" || env_str == "0" || env_str == "no" || env_str == "off") {
			memcpy_enabled_ = false;
		} else if (env_str == "true" || env_str == "1" || env_str == "yes" || env_str == "on") {
			memcpy_enabled_ = true;
		} else {
			LOG(WARNING) << "Invalid value for MC_STORE_MEMCPY: " << env_str << ", defaulting to enabled";
			memcpy_enabled_ = true;
		}
	}

	VLOG(1) << "TransferSubmitter initialized with memcpy_enabled=" << memcpy_enabled_;
}

std::optional<TransferFuture> TransferSubmitter::submit(const Replica::Descriptor &replica, std::vector<Slice> &slices,
...
TransferSubmitter::submitMemcpyOperation(const std::vector<AllocatedBuffer::Descriptor> &handles,
	...
	VLOG(1) << "Memcpy transfer submitted to worker pool with " << handles.size() << " operations";

	return TransferFuture(state);
}

std::optional<TransferFuture>
TransferSubmitter::submitTransferEngineOperation(const std::vector<AllocatedBuffer::Descriptor> &handles,
...
std::optional<TransferFuture> TransferSubmitter::submitFileReadOperation(const Replica::Descriptor &replica,
                                                                         std::vector<Slice> &slices,
                                                                         Transport::TransferRequest::OpCode op_code) {
...
TransferStrategy TransferSubmitter::selectStrategy(const std::vector<AllocatedBuffer::Descriptor> &handles,
...
bool TransferSubmitter::isLocalTransfer(const std::vector<AllocatedBuffer::Descriptor> &handles) const {
	return std::all_of(handles.begin(), handles.end(),
	                   [this](const auto &handle) { return handle.segment_name_ == local_hostname_; });
}

bool TransferSubmitter::validateTransferParams(const std::vector<AllocatedBuffer::Descriptor> &handles,
                                               const std::vector<Slice> &slices) const {
	if (handles.empty()) {
	...
	if (handles.size() > slices.size()) {
		LOG(ERROR) << "invalid_partition_count handles_size=" << handles.size() << " slices_size=" << slices.size();
		return false;
	}

	for (size_t i = 0; i < handles.size(); ++i) {
		if (handles[i].size_ != slices[i].size) {
			LOG(ERROR) << "Size of replica partition " << i << " (" << handles[i].size_
