...
DistributedObjectStore::~DistributedObjectStore() {
	if (client_ && segment_ptr_) {
		...
		ErrorCode rc = client_->UnmountSegment(local_hostname, (void *)segment_ptr_);
		if (rc != ErrorCode::OK) {
			LOG(ERROR) << "Failed to unmount segment in destructor: " << toString(rc);
		}
	}
}
...
int DistributedObjectStore::setup(const std::string &local_hostname, const std::string &metadata_server,
	...
	client_->Init(local_hostname, metadata_server, protocol, args, master_server_addr);

	client_buffer_allocator_ = std::make_unique<SimpleAllocator>(local_buffer_size);
	ErrorCode rc = client_->RegisterLocalMemory(client_buffer_allocator_->getBase(), local_buffer_size,
	                                            kWildcardLocation, false, false);
	segment_ptr_ = (uint64_t)allocate_buffer_allocator_memory(global_segment_size);
	if (segment_ptr_ == 0) {
		LOG(ERROR) << "Failed to allocate segment memory";
		return 1;
	}
	rc = client_->MountSegment(local_hostname, (void *)segment_ptr_, global_segment_size);
	if (rc != ErrorCode::OK) {
...
int DistributedObjectStore::allocateSlices(std::vector<mooncake::Slice> &slices,
	...
	length = 0;
	if (!object_info.replica_list_size())
		return -1;
...
int DistributedObjectStore::put(const std::string &key, const std::string &value) {
