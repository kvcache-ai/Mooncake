...
class NoFAllocationCandidate final : public AllocationCandidate {
   public:
    explicit NoFAllocationCandidate(
...
NoFSegmentManager::~NoFSegmentManager() {
    ...
    for (const auto& name : names) {
        if (metrics.get_segment_total_nof_capacity(name) == 0) {
            metrics.remove_nof_segment_metrics(name);
        }
    }
}
...
ErrorCode ScopedNoFSegmentWriteAccess::MountSegment(const NoFSegment& segment,
    ...
    if (segment.id == UUID{0, 0} || segment.name.empty() || segment.size == 0) {
        return ErrorCode::INVALID_PARAMS;
    }
    const auto existing =
        nof_segment_manager_->mounted_segments_.find(segment.id);
    if (existing != nof_segment_manager_->mounted_segments_.end()) {
        const auto& mounted = existing->second;
        if (mounted.status != SegmentStatus::OK) {
            return ErrorCode::UNAVAILABLE_IN_CURRENT_STATUS;
        }
        return mounted.client_id == client_id &&
                       mounted.segment.name == segment.name &&
                       mounted.segment.base == segment.base &&
                       mounted.segment.size == segment.size &&
    ...
    auto created = CreateBufferAllocator(
        nof_segment_manager_->memory_allocator_, segment.name, segment.base,
        segment.size, segment.te_endpoint, ReplicaType::NOF_SSD);
    ...
    auto candidate = std::make_unique<NoFAllocationCandidate>(allocator);
    auto [mounted, inserted] = nof_segment_manager_->mounted_segments_.emplace(
        segment.id,
        MountedNoFSegment{segment, client_id, SegmentStatus::OK,
                          std::move(allocator), std::move(candidate)});
    DCHECK(inserted);
    const bool added = nof_segment_manager_->placement_index_.AddCandidate(
        segment.name, *mounted->second.candidate, {});
    DCHECK(added);
