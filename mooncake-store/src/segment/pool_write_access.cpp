...
bool ContainsTarget(const PlacementIndex& index, std::string_view name,
                    const AllocationTarget* target) {
    auto* group = index.GetView().Find(name);
    return group && std::find(group->targets.begin(), group->targets.end(),
                              target) != group->targets.end();
}

bool ContainsHostRegion(const HostRegionIndex& index, const Segment& segment) {
    auto host = index.find(segment.host_id);
    if (host == index.end()) {
        return false;
    }
    auto group = host->second.find(segment.name);
    return group != host->second.end() && group->second.contains(segment.id);
}
...
ScopedSegmentPoolWriteAccess::PrepareUnmount(const UUID& segment_id,
    ...
    const SegmentStatus previous_status = mounted->second.status;
    const bool was_active = resource->active;
    const bool was_placed =
        ContainsTarget(segment_pool_->placement_index_,
                       mounted->second.segment.name, &resource->target);
    const bool was_host_indexed = ContainsHostRegion(
        segment_pool_->catalog_.regions_by_host_, mounted->second.segment);
    ...
    return RegionUnmountTxn(std::move(segment), client_id, previous_status,
                            was_active, was_placed, was_host_indexed);
}
...
ErrorCode ScopedSegmentPoolWriteAccess::RollbackUnmount(
    ...
    if (transaction.was_active_ && !resource->active &&
        !driver->Reactivate(segment_id)) {
        return ErrorCode::INTERNAL_ERROR;
    }
    if (transaction.was_placed_) {
        const bool added = segment_pool_->placement_index_.AddTarget(
            mounted->second.segment.name, &resource->target);
        DCHECK(added);
    }
    if (transaction.was_host_indexed_) {
        AddHostRegion(segment_pool_->catalog_.regions_by_host_,
                      mounted->second.segment);
    }
    mounted->second.status = transaction.previous_status_;
