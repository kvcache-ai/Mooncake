...
ErrorCode EtcdHelper::ConnectToEtcdStoreClient(const std::string &etcd_endpoints) {
	...
	if (etcd_connected_) {
		if (connected_endpoints_ != etcd_endpoints) {
			LOG(ERROR) << "Etcd client already connected to " << connected_endpoints_ << ", while trying to connect to "
			           << etcd_endpoints;
...
ErrorCode EtcdHelper::CreateWithLease(const char *key, const size_t key_size, const char *value,
                                      const size_t value_size, EtcdLeaseId lease_id, EtcdRevisionId &revision_id) {
	char *err_msg = nullptr;
	int ret = EtcdStoreCreateWithLeaseWrapper((char *)key, (int)key_size, (char *)value, (int)value_size, lease_id,
	                                          &revision_id, &err_msg);
	if (ret == -2) {
		VLOG(1) << "key=" << std::string(key, key_size) << ", lease_id=" << lease_id << ", error=" << err_msg;
		free(err_msg);
		return ErrorCode::ETCD_TRANSACTION_FAIL;
	} else if (ret != 0) {
		LOG(ERROR) << "key=" << std::string(key, key_size) << ", lease_id=" << lease_id << ", error=" << err_msg;
		free(err_msg);
		return ErrorCode::ETCD_OPERATION_ERROR;
	} else {
		return ErrorCode::OK;
...
ErrorCode EtcdHelper::WatchUntilDeleted(const char *key, const size_t key_size) {
	char *err_msg = nullptr;
	int err_code = EtcdStoreWatchUntilDeletedWrapper((char *)key, (int)key_size, &err_msg);
	if (err_code != 0) {
		LOG(ERROR) << "key=" << std::string(key, key_size) << ", error=" << err_msg;
		free(err_msg);
		if (err_code == -2) {
