...
tl::expected<void, ErrorCode> DistributedStorageBackend::Init() {
    ...
    if (distributed_config_.enable_health_check) {
        ...
        auto write_result = fs_adapter_->WriteFile(probe_path, std::span<const char>(probe_data.data(), probe_data.size()));
        if (!write_result) {
        ...
        auto read_result = fs_adapter_->ReadFile(
            probe_path, read_buf.data(), read_buf.size());
...
DistributedStorageBackend::IsEnableOffloading() {
    return true;
}
...
std::string DistributedStorageBackend::GetObjectPath(
    ...
    std::string bucket =
        fmt::format("{:02x}", hash % hash_bucket_count_);
    std::string safe_key = EscapeFilename(key);
    return root_dir_ + "/" + bucket + "/" + safe_key;
}

std::string DistributedStorageBackend::EscapeFilename(
    const std::string& key) {
    std::string result;
