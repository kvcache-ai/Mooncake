...
bool MatchesBucketDataFileName(const std::string& path, int64_t bucket_id) {
    const std::string expected =
        "bucket_" + ImmutableBucketAllocator::FormatBucketId(bucket_id) +
        ".data";
    ...
    if (!IsBucketMode()) {
        for (size_t i = 0; i < requests.size(); ++i) {
            ...
            if (!IsDfsDescriptorRangeValid(desc, distributed_config_)) {
                LOG(ERROR) << "Invalid DFS descriptor range for key "
                           << request.key << ", offset=" << desc.offset
            ...
            for (const auto& slice : request.slices) {
                if ((!slice.ptr && slice.size > 0) ||
                    slice.size > std::numeric_limits<uint64_t>::max() -
                                     total_size) {
                    invalid = true;
                    break;
                }
            ...
            if (invalid || total_size != desc.object_size) {
                LOG(WARNING) << "Invalid DFS write request for key "
                             << request.key << ", expected=" << desc.object_size
                             << ", actual=" << total_size;
                results[i] = tl::make_unexpected(ErrorCode::INVALID_PARAMS);
                continue;
            }
            ...
            if (!write_result) {
                ...
                results[i] = tl::make_unexpected(write_result.error());
                continue;
            }
...
DistributedStorageBackend::PrepareReadTasks(
    ...
    for (size_t i = 0; i < requests.size(); ++i) {
        ...
        task.total_size = request.descriptor.aligned_size;
        ...
        const bool merge_candidate =
            distributed_config_.batch_read_merge_enabled &&
    ...
    while (done < object_size && index < iovs.size()) {
        ...
        auto read_result = task.direct_read
                               ? fs_adapter_->DirectReadAt(
                                     ...
                                     static_cast<int>(pending.size()),
                                     static_cast<int64_t>(entry.value_offset +
                                                          done))
                               : fs_adapter_->ReadAt(
                                     task.target.fd, pending.data(),
                                     static_cast<int>(pending.size()),
        ...
        for (size_t i = 0; i < requests.size(); ++i) {
            ...
            if (!read_result) {
                LOG(WARNING) << "DFS read failed for key " << request.key
                             << ", error=" << read_result.error();
                results[i] = tl::make_unexpected(read_result.error());
                continue;
            }
