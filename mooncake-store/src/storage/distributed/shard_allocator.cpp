...
struct PersistedAllocation {
    std::string key;
    uint64_t allocation_id_high = 0;
    uint64_t allocation_id_low = 0;
...
YLT_REFL(PersistedAllocation, key, allocation_id_high, allocation_id_low,
         raw_offset, offset, object_size, aligned_size, requested_size,
         reserved_bytes);
...
tl::expected<void, ErrorCode> ShardAllocator::Init(
    ...
    std::map<int, uint8_t> existing_sidecars;
    constexpr std::string_view prefix = "dfs_shard_";
    for (const auto& name : *files) {
        std::string_view view(name);
        if (!view.starts_with(prefix)) continue;
        uint8_t sidecar = 0;
        std::string_view suffix;
        if (view.ends_with(".data")) {
            sidecar = 1;
            suffix = ".data";
        } else if (view.ends_with(".meta")) {
            sidecar = 2;
            suffix = ".meta";
        } else if (view.ends_with(".wal")) {
            sidecar = 4;
            suffix = ".wal";
        } else {
            continue;
        ...
        if (digits.size() < 2 || digits.front() < '0' || digits.front() > '9' ||
            ...
            (existing_sidecars[index] & sidecar) != 0) {
            LOG(ERROR) << "Invalid or ambiguous DFS shard filename " << name;
            return tl::make_unexpected(ErrorCode::INVALID_PARAMS);
        }
        existing_sidecars[index] |= sidecar;
        if (sidecar == 1) {
            existing_paths.emplace(index, mount_path_ + "/" + name);
        }
    }
    ...
    discovered_count = 0;
    for (const auto& [index, sidecars] : existing_sidecars) {
        if (sidecars != 7) {
            LOG(ERROR) << "Incomplete DFS allocator sidecar set for shard="
                       << index;
            return tl::make_unexpected(ErrorCode::FILE_READ_FAIL);
        }
        ++discovered_count;
...
ShardAllocator::CreateShard(int shard_idx, const std::string& path,
                            std::vector<std::string>& created_files) {
...
tl::expected<void, ErrorCode> ShardAllocator::CreateFreshShard(
...
tl::expected<void, ErrorCode> ShardAllocator::LoadShard(ShardState& shard,
                                                        int shard_idx) {
...
tl::expected<void, ErrorCode> ShardAllocator::SaveCheckpointLocked(
    ...
    for (const auto& [offset, record] : shard.offset_to_handle) {
        persisted.allocations.push_back(
            {record.key, record.allocation_id.first,
             record.allocation_id.second, record.raw_offset, offset,
             record.object_size, record.aligned_size, record.requested_size,
             record.bytes});
    }
...
tl::expected<void, ErrorCode> ShardAllocator::LoadCheckpointLocked(
    ShardState& shard, int shard_idx) {
    ...
    for (const auto& record : persisted.allocations) {
        ...
        const UUID allocation_id{record.allocation_id_high,
                                 record.allocation_id_low};
        if (record.key.empty() || allocation_id == UUID{} ||
        ...
        if (!shard.offset_to_handle
                 .emplace(
                     ...
                     ShardState::AllocationRecord{
                         record.key, allocation_id, shared, record.raw_offset,
...
tl::expected<void, ErrorCode> ShardAllocator::LoadWalLocked(ShardState& shard,
                                                            int shard_idx) {
        ...
        if (frame.type == kWalAllocate) {
            ...
            if (!shard.offset_to_handle
                     .emplace(record.offset,
                              ShardState::AllocationRecord{
                                  record.key, allocation_id, shared,
                                  record.raw_offset, record.object_size,
                                  record.aligned_size, record.requested_size,
                                  record.reserved_bytes, false})
                     .second) {
                return tl::make_unexpected(ErrorCode::INVALID_REPLICA);
            }
        } else {
...
tl::expected<void, ErrorCode> ShardAllocator::AppendAllocationLocked(
    ShardState& shard, int shard_idx,
    const ShardState::AllocationRecord& record, uint64_t exposed_offset) {
    WalAllocatePayload payload{
        {record.key, record.allocation_id.first, record.allocation_id.second,
         record.raw_offset, exposed_offset, record.object_size,
...
tl::expected<void, ErrorCode> ShardAllocator::AppendReleaseLocked(
    ShardState& shard, int shard_idx,
...
tl::expected<void, ErrorCode> ShardAllocator::CompactShardLocked(
    ShardState& shard, int shard_idx) {
    if (shard.poisoned) {
        return tl::make_unexpected(ErrorCode::DFS_SERVICE_UNAVAILABLE);
    }
...
tl::expected<DistributedFSDescriptor, ErrorCode> ShardAllocator::Allocate(
    ...
    for (size_t attempt = 0; attempt < shards->size(); ++attempt) {
        ...
        auto shared =
            std::make_shared<OffsetAllocationHandle>(std::move(*handle));
        const UUID allocation_id = generate_uuid();
        auto [it, inserted] = shard.offset_to_handle.emplace(
            offset, ShardState::AllocationRecord{
                        key, allocation_id, shared, raw_offset, size,
                        aligned_size, requested_size, reserved, false});
        ...
        return DistributedFSDescriptor{
            ShardDataPath(shard), offset,    size,
...
std::vector<BatchAllocateResult> ShardAllocator::BatchAllocate(
    ...
    for (size_t i = 0; i < requests.size(); ++i) {
        ...
        for (size_t j = i; j > 0; --j) {
            ...
            if (allocated.success) Free(allocated.key, allocated.descriptor);
        }
...
uint64_t ShardAllocator::GetTotalCapacity() const {
    ...
    const auto shards =
        std::atomic_load_explicit(&shards_, std::memory_order_acquire);
    if (!shards) return 0;
    for (const auto& shard : *shards) total += shard->capacity;
    return total;
}

void ShardAllocator::Free(uint64_t offset, uint64_t aligned_size, int shard_idx,
                          const std::string& key, const UUID& allocation_id) {
    const auto shards =
        std::atomic_load_explicit(&shards_, std::memory_order_acquire);
    ...
    if (it == shard.offset_to_handle.end() || it->second.key != key ||
        ...
        it->second.allocation_id != allocation_id) {
        return;
    }
        ...
        for (auto it = shard.offset_to_handle.begin();
            ...
            if (!claimed.contains({shard_idx, it->first})) {
                QueuePendingFreeLocked(shard, it->second.handle,
                                       ...
                                       it->second.bytes, orphan_free_at);
...
void ShardAllocator::QueuePendingFreeLocked(
...
tl::expected<void, ErrorCode> ShardAllocator::CleanupExpiredPendingFreesLocked(
    ShardState& shard, int shard_idx,
...
tl::expected<void, ErrorCode> ShardAllocator::RunMaintenance() {
    const auto shards =
        std::atomic_load_explicit(&shards_, std::memory_order_acquire);
    if (!initialized_.load(std::memory_order_acquire) ||
        !recovery_ready_.load(std::memory_order_acquire)) {
...
tl::expected<void, ErrorCode> ShardAllocator::Checkpoint() {
    const auto shards =
        std::atomic_load_explicit(&shards_, std::memory_order_acquire);
    if (!initialized_.load(std::memory_order_acquire)) {
        return tl::make_unexpected(ErrorCode::DFS_SERVICE_UNAVAILABLE);
...
double ShardAllocator::EffectiveUsageLocked(const ShardState& shard) const {
...
std::string ShardAllocator::ShardDataPath(const ShardState& shard) const {
    return shard.path;
}

std::string ShardAllocator::ShardMetadataPath(const ShardState& shard) const {
    return std::filesystem::path(shard.path)
        .replace_extension(".meta")
        .string();
}

std::string ShardAllocator::ShardWalPath(const ShardState& shard) const {
    return std::filesystem::path(shard.path).replace_extension(".wal").string();
}

std::string ShardAllocator::ConfigurationFingerprint(
...
std::string ShardAllocator::FormatShardIdx(int idx, int shard_count) {
    // New paths stay stable as the shard count grows. Existing paths are
    // discovered and preserved during recovery.
    (void)shard_count;
