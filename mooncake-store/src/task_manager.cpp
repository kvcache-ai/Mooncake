...
UUID ScopedTaskAccess::submit_task(const UUID& client_id, TaskType type, const std::string& payload) {
...
std::vector<Task> ScopedTaskAccess::pop_tasks(const UUID& client_id, size_t batch_size) {
...
std::optional<Task> ScopedTaskAccess::find_task_by_id(const UUID& task_id) const {
    auto it = manager_->all_tasks_.find(task_id);
    if (it != manager_->all_tasks_.end()) {
        return it->second;
    }
...
void ScopedTaskAccess::mark_failed(const UUID& client_id, const UUID& task_id, const std::string& error_message) {
...
void ScopedTaskAccess::mark_success(const UUID& client_id, const UUID& task_id) {
...
void ScopedTaskAccess::update_task_status(const UUID& task_id, TaskStatus status, const std::string& error_message) {
    auto it = manager_->all_tasks_.find(task_id);
    if (it != manager_->all_tasks_.end()) {
