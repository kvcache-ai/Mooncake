...
std::vector<Task> ClientTaskManager::pop_tasks(const std::string& localhost_name, size_t batch_size) {
    ...
    LOG(INFO) << "Popped " << result.size() << " tasks for client " << localhost_name;
...
void ClientTaskManager::update_task_status(const UUID& task_id, TaskStatus status, const std::string& error_message) {
    auto it = all_tasks_.find(task_id);
    if (it != all_tasks_.end()) {
