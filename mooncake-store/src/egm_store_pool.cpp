...
std::string Trim(std::string value) {
    auto not_space = [](unsigned char value) { return !std::isspace(value); };
...
EgmStorePoolResult<bool> ParseBool(const std::string& value) {
    ...
    return Error<bool>("invalid enable_egm_store_pool value: " + value);
}
...
EgmStorePoolResult<std::vector<int>> ParseNodes(const std::string& expression) {
    ...
    while (begin <= expression.size()) {
        ...
        const std::string token = Trim(expression.substr(begin, end - begin));
        if (token.empty() || token.front() == '-') {
            return Error<std::vector<int>>("invalid EGM NUMA node: " + token);
        }
        int node = -1;
        const auto parsed =
            std::from_chars(token.data(), token.data() + token.size(), node);
...
EgmStorePoolResult<EgmStorePoolOptions> ParseEgmStorePoolOptions(
    ...
    if (enabled != config.end()) {
        auto parsed = ParseBool(enabled->second);
    ...
    if (nodes == config.end() || Trim(nodes->second) == "auto") return options;
