...
tl::expected<void, ErrorCode> ClientRpcService::ReadRemoteData(
    ...
    for (const auto& buffer_desc : request.dest_buffers) {
        if (buffer_desc.size == 0 || buffer_desc.addr == 0) {
            LOG(ERROR) << "ReadRemoteData: invalid buffer (zero size or null address)";
            timer.LogResponse("error_code=", ErrorCode::INVALID_PARAMS);
            return tl::make_unexpected(ErrorCode::INVALID_PARAMS);
        }
    }
    ...
    auto result = data_manager_.ReadData(request.key, request.dest_buffers);
...
tl::expected<void, ErrorCode> ClientRpcService::WriteRemoteData(
    ...
    for (const auto& buffer_desc : request.src_buffers) {
        if (buffer_desc.size == 0 || buffer_desc.addr == 0) {
            LOG(ERROR) << "WriteRemoteData: invalid buffer (zero size or null address)";
            timer.LogResponse("error_code=", ErrorCode::INVALID_PARAMS);
            return tl::make_unexpected(ErrorCode::INVALID_PARAMS);
        }
    }
...
std::vector<tl::expected<void, ErrorCode>> ClientRpcService::BatchReadRemoteData(
    ...
    if (request.keys.size() != request.dest_buffers_list.size()) {
        LOG(ERROR) << "BatchReadRemoteData: key count (" << request.keys.size()
                   << ") != buffer list count (" << request.dest_buffers_list.size() << ")";
    ...
    // Process each key sequentially (can be parallelized in future)
    for (size_t i = 0; i < request.keys.size(); ++i) {
...
std::vector<tl::expected<void, ErrorCode>> ClientRpcService::BatchWriteRemoteData(
    ...
    if (request.keys.size() != request.src_buffers_list.size()) {
        LOG(ERROR) << "BatchWriteRemoteData: key count (" << request.keys.size()
                   << ") != buffer list count (" << request.src_buffers_list.size() << ")";
        for (size_t i = 0; i < request.keys.size(); ++i) {
            results.emplace_back(tl::make_unexpected(ErrorCode::INVALID_PARAMS));
        }
        return results;
