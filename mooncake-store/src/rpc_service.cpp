    ...
    if (config.enable_metric_reporting) {
        metric_report_thread_ = std::thread([this]() {
            while (metric_report_running_) {
                ...
                if (result) {
                    LOG(INFO) << "KV Event Publisher Metrics: " << result.value();
                }
...
tl::expected<void, ErrorCode> WrappedMasterService::PutEnd(
    ...
    ReplicaType replica_type, const StoreEventInfo& store_event_info) {
    return execute_rpc(
        "PutEnd",
        [&] { return master_service_.PutEnd(client_id, key, replica_type, store_event_info); },
        [&](auto& timer) {
            timer.LogRequest("client_id=", client_id, ", key=", key,
...
std::vector<tl::expected<void, ErrorCode>> WrappedMasterService::BatchPutEnd(
    ...
    for (const auto& key : keys) {
        ...
        if (auto it = key_event_infos_map.find(key); it != key_event_infos_map.end()) {
            store_event_info = it->second;
        }
        results.emplace_back(
            master_service_.PutEnd(client_id, key, ReplicaType::MEMORY, store_event_info));
    }
