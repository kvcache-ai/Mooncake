...
void WrappedMasterService::init_http_server() {
    ...
    http_server_.set_http_handler<GET>(
        "/batch_query_keys", [&](coro_http_request& req, coro_http_response& resp) {
            ...
            if (keys.empty()) {
                resp.set_status_and_content(
                    ...
                    "{\"success\":false,\"error\":\"No keys provided. Use ?keys=key1,key2,...\"}");
                return;
            }
            ...
            for (size_t i = 0; i < n; ++i) {
                ...
                for (const auto& rep : replicas) {
                    ...
                    for (size_t idx = 0; idx < mem_desc.buffer_descriptors.size(); ++idx) {
                        std::string tmp;
                        struct_json::to_json(mem_desc.buffer_descriptors[idx], tmp);
                        if (!first) ss += ",";
                        ss += tmp;
                        first = false;
                    }
