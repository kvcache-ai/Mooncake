...
std::map<std::string, std::vector<Replica::Descriptor>> DummyClient::batch_get_replica(
    const std::vector<std::string> &keys) {
    std::map<std::string, std::vector<Replica::Descriptor>> replica_list_map = {};
    auto batch_result = invoke_rpc<&RealClient::batch_get_replica, std::map<std::string, std::vector<Replica::Descriptor>>>(keys);
...
std::vector<Replica::Descriptor> DummyClient::get_replica(const std::string &key) {
    std::vector<Replica::Descriptor> replica_list = {};
    auto result = invoke_rpc<&RealClient::get_replica, std::vector<Replica::Descriptor>>(key);
    if (!result.has_value()) {
