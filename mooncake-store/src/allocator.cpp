...
AllocatedBuffer::~AllocatedBuffer() {
	...
	if (alloc) {
		...
		VLOG(1) << "buf_handle_deallocated segment_name=" << segment_name_ << " size=" << size_;
	} else {
...
BufferAllocator::BufferAllocator(std::string segment_name, size_t base, size_t size)
    : segment_name_(segment_name), base_(base), total_size_(size), cur_size_(0) {
	VLOG(1) << "initializing_buffer_allocator segment_name=" << segment_name
	        << " base_address=" << reinterpret_cast<void *>(base) << " size=" << size;
	...
	memory_allocator_ = std::make_unique<facebook::cachelib::MemoryAllocator>(
	    ...
	    reinterpret_cast<void *>(header_region_start_.get()), header_region_size_, reinterpret_cast<void *>(base),
	    size);
...
std::unique_ptr<AllocatedBuffer> BufferAllocator::allocate(size_t size) {
	...
	try {
		...
		if (!buffer) {
			LOG(WARNING) << "allocation_failed size=" << size << " segment=" << segment_name_
	...
	VLOG(1) << "allocation_succeeded size=" << size << " segment=" << segment_name_ << " address=" << buffer;
	cur_size_.fetch_add(size);
	MasterMetricManager::instance().inc_allocated_size(size);
	return std::make_unique<AllocatedBuffer>(shared_from_this(), segment_name_, buffer, size);
}
...
void BufferAllocator::deallocate(AllocatedBuffer *handle) {
	try {
		...
		size_t freed_size = handle->size_; // Store size before handle might become invalid
		cur_size_.fetch_sub(freed_size);
		MasterMetricManager::instance().dec_allocated_size(freed_size);
		VLOG(1) << "deallocation_succeeded address=" << handle->buffer_ptr_ << " size=" << freed_size
...
void SimpleAllocator::deallocate(void *ptr, size_t size) {
	if (!memory_allocator_ || !ptr) {
		LOG(WARNING) << "invalid_deallocation_request allocator=" << (memory_allocator_ ? "valid" : "null")
		             << " ptr=" << (ptr ? "valid" : "null");
		return;
	}
