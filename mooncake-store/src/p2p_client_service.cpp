    ...
    for (auto& candidate : candidates) {
        ...
        if (proxy.client_id == client_id_) {
            ...
            write_ops.push_back(std::make_unique<LocalWriteOp>(
                &*data_manager_, key, &slices, proxy.segment_id));
        } else {
            std::string endpoint =
...
std::unique_ptr<TaskHandle<void>> P2PClientService::LocalWriteOp::Dispatch() {
    ...
    auto handle = data_manager->Put(key, *slices, tier_id);
    if (!handle) {
...
std::unique_ptr<TaskHandle<void>> P2PClientService::RemoteWriteOp::Dispatch() {
    ...
    auto req = std::make_shared<RemoteWriteRequest>(*write_req);
    req->target_tier_id = proxy.segment_id;
    auto cached_proxy = proxy;
    auto* cache = route_cache;

    peer_ptr->AsyncWriteRemoteData(*req).start(
        [promise, req, cached_proxy,
...
std::vector<P2PClientService::ResolvedRoute> P2PClientService::LoadCachedRoutes(
    ...
    if (route_cache_) {
        for (const auto& item : route_cache_->Get(key).items()) {
            ...
            if (item.segment_group_id[0] != '\0') {
                proxy.segment_group_id = item.segment_group_id;
            }
            std::string endpoint =
                proxy.ip_address + ":" + std::to_string(proxy.rpc_port);
...
async_simple::coro::Lazy<void> P2PClientService::RunReadWithRetry(
    ...
    try {
        while (auto route = co_await iter.AsyncNext()) {
            try {
                req->target_tier_id = route->proxy.segment_id;
                req->target_segment_group_id = route->proxy.segment_group_id;
