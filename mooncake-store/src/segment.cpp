...
ErrorCode ScopedSegmentAccess::MountSegment(const Segment &segment, const UUID &client_id) {
	...
	if (exist_segment_it != segment_manager_->mounted_segments_.end()) {
		...
		if (exist_segment.status == SegmentStatus::OK) {
			...
			LOG(ERROR) << "segment_name=" << segment.name << ", error=segment_already_exists_but_not_ok"
	...
	try {
		...
		if (!allocator) {
			LOG(ERROR) << "segment_name=" << segment.name << ", error=failed_to_create_allocator";
			return ErrorCode::INVALID_PARAMS;
		}
	} catch (...) {
		LOG(ERROR) << "segment_name=" << segment.name << ", error=unknown_exception_during_allocator_creation";
		return ErrorCode::INVALID_PARAMS;
	}
	...
	segment_manager_->mounted_segments_[segment.id] = {segment, SegmentStatus::OK, std::move(allocator)};
...
ErrorCode ScopedSegmentAccess::ReMountSegment(const std::vector<Segment> &segments, const UUID &client_id) {
	for (const auto &segment : segments) {
		...
		if (err == ErrorCode::UNAVAILABLE_IN_CURRENT_STATUS || err == ErrorCode::INTERNAL_ERROR) {
			LOG(ERROR) << "segment_name=" << segment.name << ", error=fail_to_remount_segment";
			return err;
		} else if (err == ErrorCode::INVALID_PARAMS) {
			// Ignore INVALID_PARAMS. This error cannot be solved by a new
			// remount request.
			LOG(WARNING) << "segment_name=" << segment.name << ", warn=invalid_params";
			...
			// remount request.
			LOG(ERROR) << "segment_name=" << segment.name << ", error=unexpected_error (" << err << ")";
		}
	}
...
ErrorCode ScopedSegmentAccess::PrepareUnmountSegment(const UUID &segment_id, size_t &metrics_dec_capacity) {
	...
	if (it == segment_manager_->mounted_segments_.end()) {
		LOG(WARNING) << "segment_id=" << segment_id << ", warn=segment_not_found";
		return ErrorCode::SEGMENT_NOT_FOUND;
	}
	if (it->second.status == SegmentStatus::UNMOUNTING) {
		LOG(ERROR) << "segment_id=" << segment_id << ", error=segment_is_unmounting";
		return ErrorCode::UNAVAILABLE_IN_CURRENT_STATUS;
	}
	...
	if (alloc_it != segment_manager_->allocators_.end()) {
		...
		LOG(ERROR) << "segment_name=" << segment.name << ", error=allocator_not_found_in_allocators";
	}
	...
	if (!found_in_allocators_by_name) {
		LOG(ERROR) << "segment_name=" << segment.name << ", error=allocator_not_found_in_allocators_by_name";
	}
...
ErrorCode ScopedSegmentAccess::CommitUnmountSegment(const UUID &segment_id, const UUID &client_id,
	...
	if (!found_in_client_segments) {
		LOG(ERROR) << "segment_id=" << segment_id << ", error=segment_not_found_in_client_segments";
	}
...
ErrorCode ScopedSegmentAccess::GetClientSegments(const UUID &client_id, std::vector<Segment> &segments) const {
...
ErrorCode ScopedSegmentAccess::GetAllSegments(std::vector<std::string> &all_segments) {
	all_segments.clear();
...
ErrorCode ScopedSegmentAccess::QuerySegments(const std::string &segment, size_t &used, size_t &capacity) {
	const auto &allocators = segment_manager_->allocators_by_name_.find(segment);
