...
tl::expected<CacheBlockHandle, ErrorCode> Client::GetZeroCopy(
    ...
    return GetZeroCopy(object_key, query_result.value());
}

tl::expected<CacheBlockHandle, ErrorCode> Client::GetZeroCopy(
    const std::string& object_key, const QueryResult& query_result) {
    // Precondition: hot cache must be enabled
    if (!hot_cache_) {
        return tl::unexpected(ErrorCode::INVALID_PARAMS);
    }
    ...
    // Only memory replicas are supported for zero-copy
    if (!replica.is_memory_replica()) {
        return tl::unexpected(ErrorCode::INVALID_PARAMS);
    }

    auto& mem_desc = replica.get_memory_descriptor();
    size_t object_size = mem_desc.buffer_descriptor.size_;

    // Object must fit in a single cache block
    if (object_size > hot_cache_->GetBlockSize()) {
        return tl::unexpected(ErrorCode::INVALID_PARAMS);
    }

    // --- Cache hit path ---
    HotMemBlock* blk = hot_cache_->GetHotKey(object_key);
    if (blk != nullptr) {
        VLOG(1) << "GetZeroCopy cache hit: key=" << object_key;
        return CacheBlockHandle(hot_cache_, object_key, blk, blk->size);
    }

    // --- Cache miss path ---
    HotMemBlock* free_block = hot_cache_->GetFreeBlock();
    if (!free_block) {
        return tl::unexpected(ErrorCode::NO_AVAILABLE_HANDLE);
    }

    // Build slices pointing into the cache block for TransferRead
    std::vector<Slice> cache_slices;
    cache_slices.emplace_back(Slice{free_block->addr, free_block->size});

    err = TransferRead(replica, cache_slices);
    if (err != ErrorCode::OK) {
        // Return block to pool on failure
        free_block->key_.clear();
        hot_cache_->PutHotKey(free_block);
        LOG(ERROR) << "GetZeroCopy transfer_read_failed key=" << object_key;
        return tl::unexpected(err);
    }
    ...
    // Populate block metadata and insert into LRU with ref_count=1
    free_block->size = object_size;
    free_block->key_ = object_key;

    if (hot_cache_->PutHotKeyWithRef(free_block)) {
        VLOG(1) << "GetZeroCopy cache miss populated: key=" << object_key;
        return CacheBlockHandle(hot_cache_, object_key, free_block, object_size);
    }

    // Race: another thread inserted the same key first.
    // Our block was returned to the pool by PutHotKeyWithRef.
    // Retry via cache hit path.
    blk = hot_cache_->GetHotKey(object_key);
    if (blk != nullptr) {
        VLOG(1) << "GetZeroCopy race retry hit: key=" << object_key;
        return CacheBlockHandle(hot_cache_, object_key, blk, blk->size);
    }

    // Very unlikely: raced AND the winner was immediately evicted
    return tl::unexpected(ErrorCode::NO_AVAILABLE_HANDLE);
}
