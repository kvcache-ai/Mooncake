...
DEFINE_string(conf_path, "mooncake-store/conf/master.yaml", "master service config file path");
...
void InitMasterConf(const mooncake::DefaultConfig& default_config, 
    ...
    default_config.GetUInt32("rpc_thread_num", &master_config.rpc_thread_num, 4);
    default_config.GetString("rpc_address", &master_config.rpc_address, "0.0.0.0");
    ...
    default_config.GetUInt64("default_kv_lease_ttl", &master_config.default_kv_lease_ttl,mooncake::DEFAULT_DEFAULT_KV_LEASE_TTL);
    default_config.GetUInt64("default_kv_soft_pin_ttl", &master_config.default_kv_soft_pin_ttl, mooncake::DEFAULT_KV_SOFT_PIN_TTL_MS);
    default_config.GetBool("allow_evict_soft_pinned_objects",
                            &master_config.allow_evict_soft_pinned_objects, mooncake::DEFAULT_ALLOW_EVICT_SOFT_PINNED_OBJECTS);
    default_config.GetDouble("eviction_ratio", &master_config.eviction_ratio, mooncake::DEFAULT_EVICTION_RATIO);
    default_config.GetDouble("eviction_high_watermark_ratio",
                                &master_config.eviction_high_watermark_ratio, mooncake::DEFAULT_EVICTION_HIGH_WATERMARK_RATIO);
    default_config.GetInt64("client_live_ttl_sec", &master_config.client_live_ttl_sec, mooncake::DEFAULT_CLIENT_LIVE_TTL_SEC);
    ...
    default_config.GetString("cluster_id", &master_config.cluster_id, mooncake::DEFAULT_CLUSTER_ID);
    default_config.GetString("memory_allocator", &master_config.memory_allocator, "offset");
}
...
void LoadConfigFromCmdline(mooncake::MasterConfig& master_config) {
    ...
    if (google::GetCommandLineFlagInfo("rpc_address", &info) && !info.is_default) {
        master_config.rpc_address = FLAGS_rpc_address;
    }
    if (google::GetCommandLineFlagInfo("rpc_conn_timeout_seconds", &info) && !info.is_default) {
        master_config.rpc_conn_timeout_seconds = FLAGS_rpc_conn_timeout_seconds;
    }
    if (google::GetCommandLineFlagInfo("rpc_enable_tcp_no_delay", &info) && !info.is_default) {
        master_config.rpc_enable_tcp_no_delay = FLAGS_rpc_enable_tcp_no_delay;
    }
    if (google::GetCommandLineFlagInfo("enable_gc", &info) && !info.is_default) {
        master_config.enable_gc = FLAGS_enable_gc;
    }
    if (google::GetCommandLineFlagInfo("enable_metric_reporting", &info) && !info.is_default) {
        master_config.enable_metric_reporting = FLAGS_enable_metric_reporting;
    }
    if (google::GetCommandLineFlagInfo("metrics_port", &info) && !info.is_default) {
        master_config.metrics_port = FLAGS_metrics_port;
    }
    if (google::GetCommandLineFlagInfo("default_kv_lease_ttl", &info) && !info.is_default) {
        master_config.default_kv_lease_ttl = FLAGS_default_kv_lease_ttl;
    }
    if (google::GetCommandLineFlagInfo("default_kv_soft_pin_ttl", &info) && !info.is_default) {
        master_config.default_kv_soft_pin_ttl = FLAGS_default_kv_soft_pin_ttl;
    }
    if (google::GetCommandLineFlagInfo("allow_evict_soft_pinned_objects", &info) && !info.is_default) {
        master_config.allow_evict_soft_pinned_objects = FLAGS_allow_evict_soft_pinned_objects;
    }
    if (google::GetCommandLineFlagInfo("eviction_ratio", &info) && !info.is_default) {
        master_config.eviction_ratio = FLAGS_eviction_ratio;
    }
    if (google::GetCommandLineFlagInfo("eviction_high_watermark_ratio", &info) && !info.is_default) {
        master_config.eviction_high_watermark_ratio = FLAGS_eviction_high_watermark_ratio;
    }
    if (google::GetCommandLineFlagInfo("enable_ha", &info) && !info.is_default) {
        master_config.enable_ha = FLAGS_enable_ha;
    }
    if (google::GetCommandLineFlagInfo("etcd_endpoints", &info) && !info.is_default) {
        master_config.etcd_endpoints = FLAGS_etcd_endpoints;
    }
    if (google::GetCommandLineFlagInfo("client_live_ttl_sec", &info) && !info.is_default) {
        master_config.client_live_ttl_sec = FLAGS_client_ttl;
    }
    if (google::GetCommandLineFlagInfo("cluster_id", &info) && !info.is_default) {
        master_config.cluster_id = FLAGS_cluster_id;
    }
    if (google::GetCommandLineFlagInfo("memory_allocator", &info) && !info.is_default) {
...
int main(int argc, char* argv[]) {
    ...
    LOG(INFO) << "Master service started on port " << master_config.rpc_port
              ...
              << ", rpc_conn_timeout_seconds=" << master_config.rpc_conn_timeout_seconds
    ...
    if (master_config.enable_ha) {
        ...
        coro_rpc::coro_rpc_server server(master_config.rpc_thread_num, master_config.rpc_port,
            master_config.rpc_address, std::chrono::seconds(master_config.rpc_conn_timeout_seconds),
        ...
        mooncake::WrappedMasterService wrapped_master_service(
            ...
            master_config.eviction_ratio, master_config.eviction_high_watermark_ratio, version,
            master_config.client_live_ttl_sec, master_config.enable_ha, master_config.cluster_id,
            allocator_type);
