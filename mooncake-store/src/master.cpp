...
DEFINE_int32(port, 50051, "Port for master service to listen on (deprecated, use rpc_port)");
DEFINE_int32(max_threads, 4, "Maximum number of threads to use (deprecated, use rpc_thread_num)");
...
DEFINE_uint64(default_kv_lease_ttl, mooncake::DEFAULT_DEFAULT_KV_LEASE_TTL, "Default lease time for kv objects");
DEFINE_uint64(default_kv_soft_pin_ttl, mooncake::DEFAULT_KV_SOFT_PIN_TTL_MS, "Default soft pin ttl for kv objects");
DEFINE_bool(allow_evict_soft_pinned_objects, mooncake::DEFAULT_ALLOW_EVICT_SOFT_PINNED_OBJECTS,
...
DEFINE_double(eviction_high_watermark_ratio, mooncake::DEFAULT_EVICTION_HIGH_WATERMARK_RATIO,
...
DEFINE_int32(rpc_thread_num, 0,
             ...
             "over max_threads)");
DEFINE_int32(rpc_port, 0, "Port for RPC server to listen on (0 = use port, preferred over port)");
DEFINE_string(rpc_address, "0.0.0.0", "Address for RPC server to bind to, required in HA mode");
DEFINE_int32(rpc_conn_timeout_seconds, 0, "Connection timeout in seconds (0 = no timeout)");
DEFINE_bool(rpc_enable_tcp_no_delay, true, "Enable TCP_NODELAY for RPC connections");
...
int main(int argc, char *argv[]) {
	...
	LOG(INFO) << "Master service started on port " << FLAGS_port << ", enable_gc=" << FLAGS_enable_gc
	          << ", max_threads=" << FLAGS_max_threads << ", enable_metric_reporting=" << FLAGS_enable_metric_reporting
	          << ", metrics_port=" << FLAGS_metrics_port << ", default_kv_lease_ttl=" << FLAGS_default_kv_lease_ttl
	          << ", default_kv_soft_pin_ttl=" << FLAGS_default_kv_soft_pin_ttl
	          << ", allow_evict_soft_pinned_objects=" << FLAGS_allow_evict_soft_pinned_objects
	          << ", eviction_ratio=" << FLAGS_eviction_ratio
	          << ", eviction_high_watermark_ratio=" << FLAGS_eviction_high_watermark_ratio
	          << ", enable_ha=" << FLAGS_enable_ha << ", etcd_endpoints=" << FLAGS_etcd_endpoints
	          << ", client_ttl=" << FLAGS_client_ttl << ", rpc_thread_num=" << FLAGS_rpc_thread_num
	          << ", rpc_port=" << FLAGS_rpc_port << ", rpc_address=" << FLAGS_rpc_address
	          << ", rpc_conn_timeout_seconds=" << FLAGS_rpc_conn_timeout_seconds
	          << ", rpc_enable_tcp_no_delay=" << FLAGS_rpc_enable_tcp_no_delay << ", cluster_id=" << FLAGS_cluster_id;

	int server_thread_num = std::min(FLAGS_max_threads, static_cast<int>(std::thread::hardware_concurrency()));
	...
	if (FLAGS_rpc_port > 0) {
		...
		if (FLAGS_port != 50051) { // 50051 is the default value
			LOG(WARNING) << "Both rpc_port and port are set. "
			             << "Using rpc_port=" << FLAGS_rpc_port << ". Please migrate to use rpc_port only.";
		}
	} else {
		rpc_port = FLAGS_port;
	}
	...
	auto rpc_conn_timeout = std::chrono::seconds(FLAGS_rpc_conn_timeout_seconds);
	...
	if (FLAGS_enable_ha) {
		...
		std::string local_hostname = FLAGS_rpc_address + ":" + std::to_string(rpc_port);

		mooncake::MasterServiceSupervisor supervisor(
		    rpc_port, rpc_thread_num, FLAGS_enable_gc, FLAGS_enable_metric_reporting, FLAGS_metrics_port,
		    FLAGS_default_kv_lease_ttl, FLAGS_default_kv_soft_pin_ttl, FLAGS_allow_evict_soft_pinned_objects,
		    FLAGS_eviction_ratio, FLAGS_eviction_high_watermark_ratio, FLAGS_client_ttl, FLAGS_etcd_endpoints,
		    local_hostname, FLAGS_rpc_address, rpc_conn_timeout, FLAGS_rpc_enable_tcp_no_delay, FLAGS_cluster_id);

		return supervisor.Start();
	} else {
		// version is not used in non-HA mode, just pass a dummy value
		mooncake::ViewVersionId version = 0;
		coro_rpc::coro_rpc_server server(rpc_thread_num, rpc_port, FLAGS_rpc_address, rpc_conn_timeout,
		                                 FLAGS_rpc_enable_tcp_no_delay);
		mooncake::WrappedMasterService wrapped_master_service(
		    FLAGS_enable_gc, FLAGS_default_kv_lease_ttl, FLAGS_default_kv_soft_pin_ttl,
		    FLAGS_allow_evict_soft_pinned_objects, FLAGS_enable_metric_reporting, FLAGS_metrics_port,
		    FLAGS_eviction_ratio, FLAGS_eviction_high_watermark_ratio, version, FLAGS_client_ttl, FLAGS_enable_ha,
		    FLAGS_cluster_id);
