...
std::pair<bool, ErrorCode> DataManager::TransferDataFromRemote(
    ...
    if (!handle) {
        ...
        return {false, ErrorCode::INVALID_PARAMS};
    }
    ...
    if (!validate_result.has_value()) {
        return {false, validate_result.error()};
    }
    ...
    if (!buffer_result.has_value()) {
        return {false, buffer_result.error()};
    }
    ...
    for (const auto& [segment_name, buffer_indices] : segment_buffers) {
        ...
        if (seg == static_cast<uint64_t>(ERR_INVALID_ARGUMENT)) {
            ...
            return {false, ErrorCode::TRANSFER_FAIL};
        }
        ...
        if (!batch_result.has_value()) {
            ...
            return {false, batch_result.error()};
        }
