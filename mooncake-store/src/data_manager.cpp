...
tl::expected<std::unique_ptr<TaskHandle<void>>, ErrorCode> DataManager::Put(
    ...
    switch (local_transfer_config_.mode) {
        case LocalTransferMode::TE:
            return PutViaTe(key, slices, tier_id);
        case LocalTransferMode::MEMCPY:
            return PutViaMemcpy(key, slices, tier_id);
    }
    return tl::unexpected(ErrorCode::INTERNAL_ERROR);
}
...
DataManager::PutViaTe(std::string_view key, std::vector<Slice>& slices,
    ...
    AllocationHandle alloc_handle;
    {
        ...
        auto alloc_result = tiered_backend_->Allocate(total_size, tier_id);
        if (!alloc_result) {
...
tl::expected<ReadTaskHandle, ErrorCode> DataManager::Get(
    ...
    auto handle = GetHandle(key, tier_id, segment_group_id);
    if (!handle) {
...
tl::expected<AllocationHandle, ErrorCode> DataManager::GetHandle(
    ...
    if (segment_group_id.has_value() && !segment_group_id->empty()) {
        return GetFromSegmentGroup(key, *segment_group_id, tier_id);
    }
    return tiered_backend_->Get(key, tier_id);
}

tl::expected<AllocationHandle, ErrorCode> DataManager::GetFromSegmentGroup(
    std::string_view key, const std::string& segment_group_id,
...
tl::expected<void, ErrorCode> DataManager::ReadRemoteData(
    ...
    auto handle_result = GetHandle(key, tier_id, segment_group_id);
