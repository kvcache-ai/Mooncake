...
/**
 * @brief Placement fixture owning OffsetBufferAllocators and stable targets.
 */
class BenchmarkPlacement final {
   public:
    BenchmarkPlacement() = default;
    BenchmarkPlacement(BenchmarkPlacement&& other) noexcept
...
static bool dsaAllocateWithEvict(
    BenchmarkPlacement& placement, PlacementPolicyType policy, size_t size,
    int replica_num, std::vector<std::vector<Replica>>& live, std::mt19937& rng,
    ...
    for (int attempt = 0; attempt <= kDsaMaxRetries; ++attempt) {
        auto result = placement.Allocate(size, replica_num, policy);
        if (result.has_value()) {
...
static SizeClassPrefillStats prefillSizeClassChurn(
    ...
    while (stats.attempts < stats.max_attempts) {
        ...
        auto alloc_result = sizeClassAllocateWithEvict(
            placement, cfg.strategy_type, specs[class_idx].size,
            cfg.replica_num, live_allocations, rng, evict_throwaway,
...
static FillUpResult runDsaBenchmark(const BenchConfig& cfg) {
    ...
    if (warmup_allocs > 0) {
        ...
        auto warmup_run = [&](size_t size,
            ...
            (void)dsaAllocateWithEvict(
                placement, cfg.strategy_type, size, cfg.replica_num, live, rng,
                warmup_evict_throwaway, FLAGS_dsa_evict_ratio);
            ++warmup_count;
        };
    ...
    auto run_one = [&](size_t size,
        ...
        auto t0 = std::chrono::high_resolution_clock::now();
        bool ok = dsaAllocateWithEvict(placement, cfg.strategy_type, size,
                                       cfg.replica_num, live, rng, evict_count,
...
static SizeClassChurnResult runSizeClassChurnBenchmark(const BenchConfig& cfg) {
    ...
    for (int i = 0; i < cfg.num_allocations; ++i) {
        ...
        auto t0 = std::chrono::high_resolution_clock::now();
        auto alloc_result = sizeClassAllocateWithEvict(
            placement, cfg.strategy_type, spec.size, cfg.replica_num,
            live_allocations, rng, evict_count, FLAGS_size_class_evict_ratio);
        auto t1 = std::chrono::high_resolution_clock::now();
