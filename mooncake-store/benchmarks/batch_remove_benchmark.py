...
class BenchmarkDemo:
    ...
    def benchmark(self, key_counts, iterations):
        ...
        for count in key_counts:
            ...
            print(
                f"  Sequential: {seq_mean:.3f}s, Batch: {batch_mean:.3f}s, Speedup: {speedup:.2f}x"
            )
        ...
        for r in results:
            print(
                f"{r['count']:>8} | {r['seq']:>12.3f} | {r['batch']:>12.3f} | {r['speedup']:>8.2f}x"
            )
...
class ComprehensiveTest:
    ...
    def test_mixed_existence(self):
        ...
        for i, key in enumerate(non_existing_keys, start=3):
            self.assert_true(
                results[i] < 0, f"Non-existing key {key} returns negative error code"
            )
    ...
    def test_error_code_format(self):
        ...
        self.assert_true(error_code < 0, f"Error code is negative: {error_code}")
        ...
        self.assert_true(
            error_code in known_error_codes or error_code < 0,
    ...
    def test_large_batch(self):
        ...
        self.assert_eq(success_count, count, f"All {count} keys removed successfully")
        self.assert_true(
            elapsed < 5.0, f"Large batch completes in reasonable time ({elapsed:.2f}s)"
        )
        print(
            f"  Info: Removed {count} keys in {elapsed:.3f}s "
...
def main():
    ...
    parser.add_argument(
        "--master",
        default="localhost:30051",
        help="Master server address (default: localhost:30051)",
    )
    parser.add_argument(
        "--metadata",
        default="http://localhost:30080/metadata",
