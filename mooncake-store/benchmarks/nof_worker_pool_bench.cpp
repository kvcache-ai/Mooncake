...
void ResolveUint64Alias(const char *legacy_name, uint64_t *legacy_value,
    ...
    if (*legacy_value != legacy_default && *legacy_value != alias_value) {
        throw std::invalid_argument(std::string("Conflicting --") + legacy_name +
                                    " and --" + alias_name + " values");
    }
...
void WorkerLoop(BenchThreadContext *thread_context,
                ...
                std::atomic<size_t> *active_threads) {
    uint64_t submit_seq =
        static_cast<uint64_t>(thread_context->thread_index) << 48;
            ...
            for (auto &slot : thread_endpoint.slots) {
                    ...
                    if (result == mooncake::ErrorCode::OK) {
                        const uint64_t latency_ns = static_cast<uint64_t>(
                            std::chrono::duration_cast<std::chrono::nanoseconds>(
                                completion_time - slot.submit_time)
                                .count());
                ...
                if (before_end) {
                    ...
                    SubmitSlot(slot, endpoint, thread_endpoint.endpoint_index,
                               *pool, op_mode, submit_seq++, measure_now, stats);
                    any_active = true;
                }
            }
        }
...
int main(int argc, char **argv) {
    ...
    try {
        ...
        std::vector<BenchThreadContext> thread_contexts(effective_submit_threads);
        ...
        for (auto &thread_context : thread_contexts) {
            for (auto &thread_endpoint : thread_context.endpoints) {
                for (auto &slot : thread_endpoint.slots) {
                    ...
                    slot.buffer =
                        wrapper.Alloc(slot.buffer_size, 0x1000, FLAGS_socket_id);
                    if (!slot.buffer) {
        ...
        while (active_threads.load(std::memory_order_relaxed) > 0) {
            ...
            if (now >= warmup_end) {
                ...
                for (size_t endpoint_index = 0; endpoint_index < endpoints.size();
                    ...
                    total_last.bytes += last_endpoint_snapshots[endpoint_index].bytes;
                }
                ...
                LOG(INFO) << "interval bw="
                          ...
                          << total_view.iops << ", fail_iops="
                          << total_view.fail_iops << ", submitted="
                          << total_current.submitted_ops << ", completed="
                          << total_current.completed_ops << ", failed="
                          << total_current.failed_ops;
                ...
                for (size_t endpoint_index = 0; endpoint_index < endpoints.size();
                    ...
                    LOG(INFO) << "interval endpoint[" << endpoint_index
                              ...
                              << ", iops=" << std::fixed << std::setprecision(2)
                              << endpoint_view.iops << ", fail_iops="
                              << endpoint_view.fail_iops << ", endpoint="
                              << endpoints[endpoint_index].endpoint;
                    last_endpoint_snapshots[endpoint_index] = current;
                }
            }

            next_report = now + std::chrono::milliseconds(FLAGS_report_interval_ms);
        }
