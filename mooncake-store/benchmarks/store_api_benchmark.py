...
def collect_environment_info(git: dict[str, Any] | None = None) -> dict[str, Any]:
    return {
...
class OperationStats:
    """Tracks latency, throughput, failures, and misses for one operation."""
...
class MemoryTracker:
    """Samples process RSS and reports before, after, delta, and peak usage."""
...
class TimeBucketRecorder:
    """Records operation stats into fixed time buckets for trend analysis."""
...
def active_key_add(
    ...
    active_key_indexes: dict[str, int],
    key: str,
    size: int,
) -> None:
    if key in active_key_indexes:
        active_key_remove(active_keys, active_sizes, active_key_indexes, key)
    active_key_indexes[key] = len(active_keys)
    active_keys.append(key)
    active_sizes[key] = size


def active_key_try_reserve(
    active_keys: Sequence[str],
    pending_puts: list[int],
    max_active_keys: int,
) -> bool:
    if max_active_keys <= 0:
        return True
    if len(active_keys) + pending_puts[0] >= max_active_keys:
        return False
    pending_puts[0] += 1
    return True


def active_key_release_reservation(pending_puts: list[int]) -> None:
    if pending_puts[0] > 0:
        pending_puts[0] -= 1
...
def active_key_pick(
    ...
    if not active_keys:
        return "", 0
    key = active_keys[rng.randrange(len(active_keys))]
    return key, active_sizes.get(key, 0)


def active_key_remove(
    active_keys: list[str],
    active_sizes: dict[str, int],
    active_key_indexes: dict[str, int],
    key: str,
) -> tuple[str, int]:
    index = active_key_indexes.pop(key, None)
    size = active_sizes.pop(key, 0)
    if index is None:
        return "", 0

    last_index = len(active_keys) - 1
    if index != last_index:
        last_key = active_keys[last_index]
        active_keys[index] = last_key
        active_key_indexes[last_key] = index
    active_keys.pop()
    return key, size


def active_key_pop_random(
    rng: random.Random,
    active_keys: list[str],
    active_sizes: dict[str, int],
    active_key_indexes: dict[str, int],
    ...
    if not active_keys:
        return "", 0
    key = active_keys[rng.randrange(len(active_keys))]
    return active_key_remove(active_keys, active_sizes, active_key_indexes, key)
...
OPERATION_DISPATCH = {
    "put": (run_put, ("payload",)),
    "get": (run_get, ("value_size",)),
    "remove": (run_remove, ()),
    "upsert": (run_upsert, ("payload",)),
    "is_exist": (run_is_exist, ()),
    "get_size": (run_get_size, ("value_size",)),
    "put_batch": (run_put_batch, ("payload", "batch_size")),
    "get_batch": (run_get_batch, ("value_size", "batch_size")),
    "upsert_batch": (run_upsert_batch, ("payload", "batch_size")),
    "batch_is_exist": (run_batch_is_exist, ("batch_size",)),
    "batch_remove": (run_batch_remove, ("batch_size",)),
    "remove_by_regex": (run_remove_by_regex, ("batch_size",)),
    "remove_all": (run_remove_all, ()),
}


def run_dispatched_operation(
    name: str,
    store: Any,
    keys: Sequence[str],
    payload: bytes,
    ...
    runner, argument_names = OPERATION_DISPATCH[name]
    arguments: list[Any] = [store, keys]
    for argument_name in argument_names:
        if argument_name == "payload":
            arguments.append(payload)
        elif argument_name == "value_size":
            arguments.append(len(payload))
        elif argument_name == "batch_size":
            arguments.append(batch_size)
        else:
            raise ValueError(f"unknown dispatch argument {argument_name!r}")
...
def run_mixed_worker(
    ...
    """Runs one mixed-workload worker until the shared deadline expires."""
    ...
    while time.perf_counter() < deadline:
        ...
        if operation == "put":
            ...
            if args.max_active_keys > 0:
                with key_lock:
                    reserved_slot = active_key_try_reserve(
                        active_keys, pending_puts, args.max_active_keys
                    )
            if args.max_active_keys > 0 and not reserved_slot:
                operation = "get"
            else:
                sequence += 1
                ...
                try:
                    ...
                    if reserved_slot:
                        with key_lock:
                            active_key_release_reservation(pending_puts)
                    raise
                elapsed = time.perf_counter() - start
        ...
        if operation == "get":
            with key_lock:
                key, expected_size = active_key_pick(rng, active_keys, active_sizes)
            if not key:
                continue
            ...
            with key_lock:
                key, _ = active_key_pop_random(
                    rng, active_keys, active_sizes, active_key_indexes
                )
            if not key:
                continue
...
def run_mixed_workload(
    ...
    """Preloads keys, runs mixed workers, and aggregates their results."""
    ...
    active_key_indexes = {key: index for index, key in enumerate(active_keys)}
    pending_puts = [0]
    key_lock = threading.Lock()
...
def run_operation(
    ...
    if concurrent_run:
        ...
        stats = run_dispatched_operation(name, store, keys, payload, args.batch_size)
        extras = {}
...
def run_concurrency_scan(
    ...
    """Runs selected operations across the configured concurrency values."""
...
def load_child_report(output_path: Path) -> tuple[dict[str, Any] | None, str | None]:
    if not output_path.exists():
        return None, "output JSON was not created"
    try:
        raw_report = output_path.read_text()
    except OSError as exc:
        return None, f"failed to read output JSON: {exc}"
    if not raw_report.strip():
        return None, "output JSON is empty"
    try:
        return json.loads(raw_report), None
...
def run_child_benchmark_process(
    ...
    """Runs one child benchmark process and loads its JSON report."""
    ...
    process.wait()
    report, report_error = load_child_report(output_path)
    process_result = {
...
def run_multi_client_mixed(args: argparse.Namespace) -> dict[str, Any]:
    """Runs a single-client baseline and then parallel child Store clients."""
    ...
    try:
        for client_index, process in enumerate(processes):
            process.wait()
            report, report_error = load_child_report(output_paths[client_index])
            clients.append(
                {
                    "client_index": client_index,
                    "returncode": process.returncode,
    ...
    successful_clients = sum(
        ...
        if client["returncode"] == 0 and client["report_error"] is None
    )
    baseline_result = (
...
def compare_reports(
    ...
    """Compares matching metric nodes between baseline and current reports."""
...
def build_report(
    ...
    return {
        ...
        "environment": collect_environment_info(git),
        "config": {
            "operation": args.operation,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run Mooncake Store API benchmarks and stress tests."
    )
    parser.add_argument(
        "--operation",
        default="smoke",
