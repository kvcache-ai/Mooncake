 ...
 * @brief Comprehensive benchmark for storage backends (OffsetAllocator, Bucket, FilePerKey)
 *
 * TESTS AVAILABLE:
 ...
 *   - drop_cache_external: prints instructions, user must run sync+drop manually
 *
 * USAGE EXAMPLES:
 *
 ...
 *   ./storage_backend_bench --backend=offset_allocator --cache_mode=drop_cache_external
 *
...
DEFINE_uint64(num_operations, 1000, "Number of operations per test (default: 1000)");
DEFINE_uint64(warmup_operations, 100, "Number of warmup operations (default: 100)");
DEFINE_uint64(num_threads, 1, "Number of threads for concurrent tests (default: 1)");
DEFINE_string(storage_path, "/tmp/mooncake_bench_data", "Storage directory path");
DEFINE_uint64(capacity_gb, 20, "Storage capacity in GB (default: 20GB)");
DEFINE_bool(run_all, false, "Run full benchmark suite with all parameter combinations");
DEFINE_bool(skip_cleanup, false, "Skip cleanup after benchmark (for debugging)");
DEFINE_string(test, "all", 
              "Test to run: init, offload, load, concurrent_load, exist, mixed_rw, churn, restart, all");

// === PR1: Verification & Correctness ===
DEFINE_bool(verify, true, "Enable data integrity verification (default: true)");
DEFINE_uint64(verify_rate, 1, "Verify 1 in N operations (default: 1 = verify all)");
...
DEFINE_double(fill_ratio, 0.1, "Pre-populate to this fraction of capacity (0.0-1.0)");

// === PR2: Cache Modes ===
DEFINE_string(cache_mode, "buffered",
              "Cache mode: buffered (page cache), direct (O_DIRECT), "
              "fadvise_dontneed (evict after use), drop_cache_external (print instructions)");
DEFINE_bool(flush_between_ops, false, 
            "Call fdatasync after each write (NOT IMPLEMENTED - requires backend Flush API)");
...
DEFINE_double(read_ratio, 0.9, "Fraction of read ops in mixed workload (0.0-1.0)");
DEFINE_double(hotset_ratio, 0.2, "Fraction of keys that are 'hot' (0.0-1.0)");
DEFINE_string(pattern, "uniform", "Key access pattern: sequential, uniform, zipf");
DEFINE_double(zipf_skew, 1.0, "Zipf distribution skew parameter (higher = more skewed)");
DEFINE_uint64(report_interval, 100, "Report stats every N operations in time-series mode");
...
std::string CacheModeToString(CacheMode mode) {
    switch (mode) {
        case CacheMode::BUFFERED: return "buffered (page cache)";
        case CacheMode::DIRECT: return "direct (O_DIRECT)";
        case CacheMode::FADVISE_DONTNEED: return "fadvise_dontneed";
        case CacheMode::DROP_CACHE_EXTERNAL: return "drop_cache_external";
    }
    return "unknown";
}
...
std::string BackendTypeToString(BackendType type) {
    switch (type) {
        case BackendType::OFFSET_ALLOCATOR: return "offset_allocator";
        case BackendType::BUCKET: return "bucket";
        case BackendType::FILE_PER_KEY: return "file_per_key";
    }
    return "unknown";
}
...
static bool DropLinuxPageCacheGlobal() {
    ...
    if (::geteuid() != 0) {
        std::cerr << "[CACHE] Not root (euid=" << ::geteuid()
                  ...
                  << "        Run: sudo -E ./storage_backend_bench ... --cache_mode=drop_cache_external\n";
        return false;
    }
    ...
    ::sync();
    std::cerr << "[CACHE] Dropped Linux global caches via /proc/sys/vm/drop_caches\n";
    return true;
#else
    return false;
#endif
...
struct ThreadStats {
    ...
    void Reserve(size_t expected_ops) {
        latencies.reserve(expected_ops);
    }
...
class BenchmarkStats {
   public:
    ...
    void PrintStatistics(const std::string& test_name) const {
        ...
        double throughput_mbps = (backend_time_sec > 0) 
            ? (static_cast<double>(total_bytes_) / MB) / backend_time_sec 
            : 0;
        double ops_per_sec = (backend_time_sec > 0) 
            ? total_operations_ / backend_time_sec 
            : 0;
        
        // Harness overhead = wall clock - backend time (for single-threaded tests)
        double overhead_pct = (wall_clock_sec > 0 && backend_time_sec > 0)
            ? ((wall_clock_sec - backend_time_sec) / wall_clock_sec) * 100.0
            : 0;

        std::cout << std::fixed << std::setprecision(2);
        std::cout << "\n  Wall clock:     " << wall_clock_sec << " s\n";
        std::cout << "  Backend time:   " << backend_time_sec << " s (sum of latencies)\n";
        
        // Interpretation notes:
        // - Single-threaded: backend_time should be close to wall_clock
        // - Multi-threaded: backend_time = sum of all thread latencies (can exceed wall_clock)
        if (thread_stats_.size() == 1) {
            if (overhead_pct > 5.0) {
                std::cout << "  ⚠️  Harness overhead: " << overhead_pct << "% of wall clock\n";
            }
        } else {
            // Multi-threaded: show aggregate backend CPU time vs wall time
            double parallelism = (wall_clock_sec > 0) ? backend_time_sec / wall_clock_sec : 0;
        ...
        std::cout << "  Total data:     " << (total_bytes_ / GB) << "." 
                  << std::setw(2) << std::setfill('0') << ((total_bytes_ % GB) * 100 / GB) << " GB\n";
        std::cout << "  Throughput:     " << throughput_mbps << " MB/s (backend time)";
   ...
   private:
    double GetPercentile(double percentile) const {
        ...
        return merged_latencies_[lower] * (1.0 - frac) + merged_latencies_[upper] * frac;
    }
    ...
    double total_backend_time_ms_ = 0;  // Sum of all latencies (backend-only time)
...
class BufferPool {
   public:
    ...
    void Init(size_t num_buffers, size_t buffer_size) {
        // Round up to alignment (potential future O_DIRECT / avoid false sharing)
        ...
        for (auto& buf : buffers_) {
            ...
            if (ret != 0 || ptr == nullptr) {
                LOG(FATAL) << "Failed to allocate aligned buffer of size " << buffer_size_;
            }
...
class KeyIndexGenerator {
   public:
    KeyIndexGenerator(size_t num_keys, AccessPattern pattern, double zipf_skew = 1.0,
                     size_t seed = 42)
    ...
    size_t GetForOp(size_t op_index) const {
        switch (pattern_) {
            ...
            case AccessPattern::UNIFORM:
            case AccessPattern::ZIPF:
                // Use op_index as seed for deterministic but "random-looking" access
                return (op_index * 2654435761ULL) % num_keys_;  // Knuth's multiplicative hash
        }
        return op_index % num_keys_;
    }
   ...
   private:
    void InitZipf(double skew) {
        ...
        for (size_t i = 0; i < num_keys_; ++i) {
            cumulative += (1.0 / std::pow(static_cast<double>(i + 1), skew)) / sum;
...
class DataGenerator {
   public:
    ...
    uint64_t FillBuffer(char* buffer, size_t size, size_t key_index) {
        ...
        uint64_t val = key_index * 6364136223846793005ULL + 1442695040888963407ULL;
    ...
    bool VerifyBuffer(const char* buffer, size_t size, size_t key_index) {
        ...
        if (remaining > 0) {
            ...
            uint64_t actual = 0;
            std::memcpy(&actual, buffer + num_words * sizeof(uint64_t), remaining);
            if (actual != expected) {
                return false;
            }
        }
...
std::shared_ptr<mooncake::StorageBackendInterface> CreateBackend(
    ...
    switch (type) {
        case BackendType::OFFSET_ALLOCATOR: {
            config.storage_backend_type = mooncake::StorageBackendType::kOffsetAllocator;
            return std::make_shared<mooncake::OffsetAllocatorStorageBackend>(config);
        }
        case BackendType::BUCKET: {
            ...
            return std::make_shared<mooncake::BucketStorageBackend>(config, bucket_config);
        }
        case BackendType::FILE_PER_KEY: {
            config.storage_backend_type = mooncake::StorageBackendType::kFilePerKey;
            mooncake::FilePerKeyConfig fpk_config;
            fpk_config.fsdir = "file_per_key_bench";
            fpk_config.enable_eviction = false;
            return std::make_shared<mooncake::StorageBackendAdaptor>(config, fpk_config);
        }
    }
    return nullptr;
}
...
void CleanupStoragePath(const std::string& path) {
    if (fs::exists(path)) {
        ...
        if (ec) {
            LOG(WARNING) << "Failed to cleanup " << path << ": " << ec.message();
        }
    }
...
void PrintCacheModeWarning(CacheMode mode) {
    switch (mode) {
        case CacheMode::BUFFERED:
            std::cout << "\n  ⚠️  WARNING: Running in BUFFERED mode (page cache enabled)\n";
            std::cout << "     Results may reflect RAM bandwidth, not SSD performance.\n";
            std::cout << "     Use --cache_mode=drop_cache_external for cold-cache testing.\n\n";
            break;
            
        case CacheMode::DIRECT:
            std::cout << "\n  ❌  DIRECT (O_DIRECT) mode: NOT IMPLEMENTED\n";
            std::cout << "     Behaving like BUFFERED mode. O_DIRECT requires backend changes.\n";
            std::cout << "     To bypass page cache, use --cache_mode=drop_cache_external\n\n";
            break;
            
        case CacheMode::FADVISE_DONTNEED:
            std::cout << "\n  ❌  FADVISE_DONTNEED mode: NOT IMPLEMENTED\n";
            std::cout << "     Behaving like BUFFERED mode. posix_fadvise requires fd access.\n";
            std::cout << "     To bypass page cache, use --cache_mode=drop_cache_external\n\n";
            break;
            
        case CacheMode::DROP_CACHE_EXTERNAL:
            std::cout << "\n  ℹ️  DROP_CACHE_EXTERNAL mode selected.\n";
            std::cout << "     To drop page cache before benchmark, run as root:\n";
...
void PrintBenchmarkConfig(BackendType backend, CacheMode cache_mode) {
    ...
    std::cout << "  Value size:   " << (FLAGS_value_size / KB) << " KB (aligned: " 
              << (AlignUp(FLAGS_value_size, kAlignment) / KB) << " KB)\n";
    std::cout << "  Batch size:   " << FLAGS_batch_size << "\n";
    std::cout << "  Operations:   " << FLAGS_num_operations << " (warmup: " 
    ...
    std::cout << "  Flush writes: " << (FLAGS_flush_between_ops ? "yes (NOT IMPLEMENTED - requires backend API)" : "no") << "\n";
    std::cout << "\n  --- Access Pattern ---\n";
...
// NOTE: op_index should be the measured operation index (starting at 0 after warmup),
// not the total operation index including warmup.
...
double BenchInit(BackendType type, const std::string& storage_path, size_t capacity) {
    ...
    if (!result) {
        LOG(ERROR) << "Init failed with error: " << static_cast<int>(result.error());
        return -1;
    }
...
void BenchBatchOffload(BackendType type, const std::string& storage_path,
    ...
    for (size_t op = 0; op < total_ops; ++op) {
        ...
        for (size_t i = 0; i < batch_size; ++i) {
            ...
            gen.FillBuffer(write_buffers.Get(i), value_size, batch.key_indices[i]);
        }
        
        // Build batch object (allocation here is fine - outside timed region)
        // Note: we clear and rebuild each iteration, but it's not in the timed path
        batch.ClearOffload();
        for (size_t i = 0; i < batch_size; ++i) {
            batch.offload_batch.emplace(
                keys.Get(batch.key_indices[i]),
                std::vector<mooncake::Slice>{{write_buffers.Get(i), value_size}});
        }
    ...
    if (FLAGS_verify) {
        ...
        size_t verify_ops = std::max<size_t>(1, num_operations / FLAGS_verify_rate);
        size_t verify_step_ops = std::max<size_t>(1, num_operations / verify_ops);
        
        // Key range for measured operations (skip warmup)
        size_t measured_key_start = warmup_operations * batch_size;
        size_t measured_key_end = measured_key_start + num_operations * batch_size;
        ...
        for (size_t op_idx = 0; op_idx < verify_ops; ++op_idx) {
            ...
            load_batch.reserve(batch_size);
            for (size_t i = 0; i < batch_size; ++i) {
                load_batch.emplace(keys.Get(base_key + i),
            ...
            if (result) {
                for (size_t i = 0; i < batch_size; ++i) {
                    if (!gen.VerifyBuffer(verify_buffers.Get(i), value_size, base_key + i)) {
                        verification_failures++;
                        if (FLAGS_fail_fast) {
                            LOG(FATAL) << "Write verification failed for key " << (base_key + i)
                ...
                LOG(WARNING) << "Readback failed for batch starting at key " << base_key;
            }
        }
        
        if (verification_failures > 0) {
            LOG(ERROR) << "Write verification: " << verification_failures 
                      << " checksum failures out of " << verified_keys << " verified";
        } else {
            std::cout << "  Write verification passed (" << verified_keys << " keys, "
                      << verify_ops << " ops sampled)\n";
        }
    }
...
void BenchBatchLoad(BackendType type, const std::string& storage_path,
    ...
    if (cache_mode == CacheMode::DROP_CACHE_EXTERNAL) {
        ...
        if (!DropLinuxPageCacheGlobal()) {
            std::cout << "  ⚠️  Cache drop failed (not root?). Results may reflect warm cache.\n";
        }
    }
    ...
    for (size_t op = 0; op < total_ops; ++op) {
        ...
        batch.ClearLoad();
        for (size_t i = 0; i < batch_size; ++i) {
            batch.load_batch.emplace(keys.Get(batch.key_indices[i]), 
        ...
        if (!is_warmup) {
            ...
            if (result) {
                ...
                // Verify based on rate (use measured op index, not total op index)
                ...
                if (ShouldVerify(measured_op, is_warmup)) {
                    for (size_t i = 0; i < batch_size; ++i) {
                        if (!gen.VerifyBuffer(read_buffers.Get(i), value_size, 
                            ...
                            LOG(ERROR) << "Checksum mismatch for key " << batch.key_indices[i];
                            if (FLAGS_fail_fast) {
            ...
            if (result) {
                for (size_t i = 0; i < batch_size; ++i) {
                    if (!gen.VerifyBuffer(read_buffers.Get(i), value_size, 
                                          batch.key_indices[i])) {
                        LOG(ERROR) << "Warmup verification failed for key " << batch.key_indices[i];
                        if (FLAGS_fail_fast) {
...
void BenchConcurrentLoad(BackendType type, const std::string& storage_path,
    ...
    for (size_t key_idx = 0; key_idx < total_keys; key_idx += batch_size) {
        ...
        std::unordered_map<std::string, std::vector<mooncake::Slice>> batch_object;
        batch_object.reserve(batch_size);
        for (size_t i = 0; i < batch_size && (key_idx + i) < total_keys; ++i) {
            std::vector<mooncake::Slice> slices;
            slices.emplace_back(mooncake::Slice{setup_buffers.Get(i), value_size});
        ...
        threads.emplace_back([&, t]() {
            ...
            for (size_t op = 0; op < ops_per_thread; ++op) {
                ...
                for (size_t i = 0; i < batch_size; ++i) {
                    ...
                    load_slices.emplace(keys.Get(key_idx), 
                ...
                if (result) {
                    ...
                    for (size_t i = 0; i < batch_size; ++i) {
                        if (!local_gen.VerifyBuffer(local_buffers.Get(i), value_size,
                                                    batch_key_indices[i])) {
...
void BenchIsExist(BackendType type, const std::string& storage_path,
    ...
    std::cout << "  Pre-populating " << num_keys << " keys for hit testing...\n";
    ...
    std::cout << "    - Keys " << num_keys << ".." << (num_operations-1) << " should MISS\n";
...
void BenchIsExist(BackendType type, const std::string& storage_path,
    ...
    BenchIsExist(type, storage_path, capacity, num_operations, stats, miss_stats);
}
...
void BenchMixedRW(BackendType type, const std::string& storage_path,
                  size_t capacity, size_t value_size, size_t batch_size,
                  size_t num_operations, size_t read_threads, size_t write_threads,
                  BenchmarkStats& read_stats, BenchmarkStats& write_stats) {
    ...
    for (size_t key_idx = 0; key_idx < initial_keys; key_idx += batch_size) {
        for (size_t i = 0; i < batch_size && (key_idx + i) < initial_keys; ++i) {
            gen.FillBuffer(setup_buffers.Get(i), value_size, key_idx + i);
        }
        
        std::unordered_map<std::string, std::vector<mooncake::Slice>> batch_object;
        for (size_t i = 0; i < batch_size && (key_idx + i) < initial_keys; ++i) {
            batch_object[keys.Get(key_idx + i)] = {mooncake::Slice{setup_buffers.Get(i), value_size}};
        }
        backend->BatchOffload(batch_object,
            [](const std::vector<std::string>&, std::vector<mooncake::StorageObjectMetadata>&) {
                return mooncake::ErrorCode::OK;
            });
    }
    ...
    for (size_t t = 0; t < read_threads; ++t) {
        threads.emplace_back([&, t]() {
            ...
            KeyIndexGenerator key_gen(initial_keys, StringToAccessPattern(FLAGS_pattern), 
        ...
        threads.emplace_back([&, t]() {
            ...
            for (size_t op = 0; op < num_operations; ++op) {
                ...
                for (size_t i = 0; i < batch_size; ++i) {
                    ...
                    local_gen.FillBuffer(local_buffers.Get(i), value_size, batch.key_indices[i]);
                }
                ...
                for (size_t i = 0; i < batch_size; ++i) {
                    batch.offload_batch.emplace(
                        ...
                        std::vector<mooncake::Slice>{{local_buffers.Get(i), value_size}});
                }
                
                auto start = std::chrono::steady_clock::now();
                auto result = backend->BatchOffload(batch.offload_batch,
                    [](const std::vector<std::string>&, std::vector<mooncake::StorageObjectMetadata>&) {
                        return mooncake::ErrorCode::OK;
                    });
                auto end = std::chrono::steady_clock::now();
...
void BenchChurn(BackendType type, const std::string& storage_path,
    ...
    for (size_t key_idx = 0; key_idx < num_fill_keys; key_idx += batch_size) {
        for (size_t i = 0; i < batch_size && (key_idx + i) < num_fill_keys; ++i) {
        ...
        backend->BatchOffload(batch.offload_batch,
            [](const std::vector<std::string>&, std::vector<mooncake::StorageObjectMetadata>&) {
                return mooncake::ErrorCode::OK;
            });
    }
    ...
    std::cout << "  Running churn: " << num_operations << " mixed read/overwrite ops...\n";
    ...
    for (size_t op = 0; op < num_operations; ++op) {
        ...
        if (is_read) {
            ...
            for (size_t i = 0; i < batch_size; ++i) {
                batch.key_indices[i] = key_dist(rng);
                gen.FillBuffer(buffers.Get(i), value_size, batch.key_indices[i] + op * 1000);
            }
        ...
        // Time-series reporting (wall-clock for progress; final stats use backend time)
        if (FLAGS_time_series && (op + 1) % FLAGS_report_interval == 0) {
            double wall_elapsed = std::chrono::duration<double>(
                std::chrono::steady_clock::now() - churn_start).count();
            // Sum of latencies for backend time
            double backend_time_sec = std::accumulate(thread_stats.latencies.begin(),
                                                      thread_stats.latencies.end(), 0.0) / 1000.0;
            double backend_throughput = (backend_time_sec > 0) 
                ? static_cast<double>(thread_stats.bytes) / MB / backend_time_sec 
                : 0;
            std::cout << "  [" << (op + 1) << "/" << num_operations << "] "
                      << "wall=" << std::fixed << std::setprecision(1) << wall_elapsed << "s, "
                      << "ops=" << thread_stats.operations << ", "
                      << "throughput=" << std::setprecision(1) << backend_throughput << " MB/s (backend)\n";
        }
    }
...
void BenchRestart(BackendType type, const std::string& storage_path,
    ...
    if (type == BackendType::FILE_PER_KEY) {
        ...
        scanmeta_ms = std::chrono::duration<double, std::milli>(scan_end - scan_start).count();
    }
    
    auto restart_end = std::chrono::steady_clock::now();
    double total_restart_ms = std::chrono::duration<double, std::milli>(restart_end - restart_start).count();
    ...
    double avg_first_load = std::accumulate(first_load_latencies.begin(), 
                                            first_load_latencies.end(), 0.0) / first_load_latencies.size();
...
void RunAllBenchmarks(const std::string& storage_path, size_t capacity) {
    ...
    for (auto backend_type : backends) {
        ...
        for (size_t value_size : {128 * KB, 512 * KB}) {
            for (size_t batch_size : {8, 32, 128}) {
                ...
                results.push_back({
                    backend_type, "BATCH_OFFLOAD", value_size, batch_size, 1,
                    write_stats.GetThroughputMBps(), write_stats.GetP99Latency()
                });

                BenchBatchLoad(backend_type, storage_path, capacity,
                               value_size, batch_size, 500, 50, read_stats);

                results.push_back({
                    backend_type, "BATCH_LOAD", value_size, batch_size, 1,
                    read_stats.GetThroughputMBps(), read_stats.GetP99Latency()
                });
            }
        }
        ...
        for (size_t threads : {4, 8}) {
            ...
            BenchConcurrentLoad(backend_type, storage_path, capacity,
                               128 * KB, 32, 200, threads, concurrent_stats);

            results.push_back({
                backend_type, "CONCURRENT_LOAD", 128 * KB, 32, threads,
                concurrent_stats.GetThroughputMBps(), concurrent_stats.GetP99Latency()
            });
        }
    ...
    std::cout << std::left << std::setw(18) << "Backend"
              << std::setw(18) << "Test"
              << std::setw(12) << "Value(KB)"
              << std::setw(10) << "Batch"
              << std::setw(10) << "Threads"
              << std::setw(15) << "MB/s"
              << std::setw(12) << "P99(ms)"
    ...
    for (const auto& r : results) {
        std::cout << std::left
                  << std::setw(18) << BackendTypeToString(r.backend)
                  << std::setw(18) << r.test_name
                  << std::setw(12) << (r.value_size / KB)
                  << std::setw(10) << r.batch_size
                  << std::setw(10) << r.threads
                  << std::fixed << std::setprecision(2)
                  << std::setw(15) << r.throughput_mbps
                  << std::setw(12) << r.p99_latency_ms
                  << "\n";
    }
    std::cout << std::string(100, '=') << "\n";
}
    ...
    if (FLAGS_run_all) {
        ...
        if (FLAGS_test == "offload" || FLAGS_test == "all") {
            BenchBatchOffload(backend_type, storage_path, capacity,
                              ...
                              FLAGS_num_operations, FLAGS_warmup_operations, stats);
        }
        ...
        if (FLAGS_test == "mixed_rw") {
            ...
            BenchMixedRW(backend_type, storage_path, capacity,
                         FLAGS_value_size, FLAGS_batch_size,
                         FLAGS_num_operations, FLAGS_read_threads, FLAGS_write_threads,
                         read_stats, write_stats);
        }
        
        if (FLAGS_test == "churn") {
            stats.Clear();
            BenchChurn(backend_type, storage_path, capacity,
                       FLAGS_value_size, FLAGS_batch_size,
                       FLAGS_num_operations, FLAGS_fill_ratio, stats);
        }
        
        if (FLAGS_test == "restart") {
            BenchRestart(backend_type, storage_path, capacity,
                         FLAGS_value_size, FLAGS_num_operations);
        }
    }
