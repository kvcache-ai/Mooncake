...
void aligned_free_buffer(void* ptr) {
    free(ptr);
}
...
struct BenchmarkConfig {
    ...
    bool use_direct_io = false;  // Use O_DIRECT with raw syscalls (bypasses StorageFile)
    bool use_uring_direct_io = false;  // Use O_DIRECT with UringFile interface (with copy)
    bool use_uring_direct_io_zero_copy = false;  // Use O_DIRECT with zero-copy aligned interface
    bool use_registered_buffers = false;  // Register buffers with io_uring for optimal performance
    size_t alignment = 4096;  // Alignment for O_DIRECT (4KB)
    size_t chunk_size = 1024 * 1024;  // Chunk size for I/O operations (1MB default)
};
...
public:
    ...
    void Run() {
        ...
        std::cout << "Data size: " << FormatSize(config_.data_size) << std::endl;
        std::cout << "Use io_uring: " << (config_.use_uring ? "Yes" : "No") << std::endl;
        if (config_.use_uring) {
            std::cout << "io_uring queue depth: " << config_.uring_queue_depth << std::endl;
            std::cout << "io_uring O_DIRECT: " << (config_.use_uring_direct_io ? "Yes (with copy)" : "No") << std::endl;
            std::cout << "io_uring O_DIRECT zero-copy: " << (config_.use_uring_direct_io_zero_copy ? "Yes" : "No") << std::endl;
            std::cout << "io_uring registered buffers: " << (config_.use_registered_buffers ? "Yes" : "No") << std::endl;
        }
        std::cout << "Use O_DIRECT (raw syscalls): " << (config_.use_direct_io ? "Yes" : "No") << std::endl;
        if (config_.use_direct_io) {
            std::cout << "Alignment: " << config_.alignment << " bytes" << std::endl;
        }
        std::cout << "Chunk size: " << FormatSize(config_.chunk_size) << std::endl;
        ...
        if (config_.use_direct_io || config_.use_uring_direct_io_zero_copy) {
            actual_size = ((config_.data_size + config_.alignment - 1) / config_.alignment) * config_.alignment;
        ...
        if (config_.use_direct_io || config_.use_uring_direct_io_zero_copy) {
            // Allocate aligned buffer for O_DIRECT (raw syscall or zero-copy mode)
            write_data_ptr = aligned_alloc_buffer(actual_size, config_.alignment);
            if (!write_data_ptr) {
            ...
            // UringFile or PosixFile: use std::vector (UringFile will handle alignment internally)
        ...
        for (int iter = 0; iter < config_.iterations; ++iter) {
            std::cout << "--- Iteration " << (iter + 1) << "/" << config_.iterations << " ---" << std::endl;

            if (config_.use_uring_direct_io_zero_copy) {
                // Zero-copy aligned I/O mode
                auto write_result_aligned = BenchmarkAlignedIO(write_data_ptr, actual_size, true);
                ...
                auto read_result_aligned = BenchmarkAlignedIO(nullptr, actual_size, false);
                ...
                if (config_.verify_data && iter == 0) {
                    bool verified = VerifyData(write_data_ptr, read_result_aligned->second, actual_size);
                    if (!verified) {
                ...
                auto write_result_direct = BenchmarkDirectIO(write_data_ptr, actual_size, true);
                if (!write_result_direct) {
                    std::cerr << "Direct I/O write benchmark failed!" << std::endl;
                    aligned_free_buffer(write_data_ptr);
                    return;
                }
                ...
                auto read_result_direct = BenchmarkDirectIO(nullptr, actual_size, false);
                if (!read_result_direct) {
                    std::cerr << "Direct I/O read benchmark failed!" << std::endl;
                    aligned_free_buffer(write_data_ptr);
                    return;
                }
                ...
                // Verify data consistency (only first iteration)
                if (config_.verify_data && iter == 0) {
                    bool verified = VerifyData(write_data_ptr, read_result_direct->second, actual_size);
                    if (!verified) {
                        std::cerr << "Data verification FAILED!" << std::endl;
                ...
                // Verify data consistency (only first iteration)
                if (config_.verify_data && iter == 0) {
                    bool verified = VerifyData(write_data_ptr, read_result->second, actual_size);
                    if (!verified) {
                        std::cerr << "Data verification FAILED!" << std::endl;
...
private:
    ...
    bool SyncAndDropCache() {
        ...
        drop << "3" << std::flush;
        if (drop.fail()) {
            std::cerr << "[CACHE] Failed to write to /proc/sys/vm/drop_caches\n";
            return false;
        }
        drop.close();
    ...
    std::optional<double> BenchmarkWrite(void* data, size_t size) {
        ...
        if (fd < 0) {
            std::cerr << "Failed to open file for writing: "
                     << config_.file_path << " - " << std::strerror(errno) << std::endl;
            return std::nullopt;
        }
        ...
        auto result = file->write(std::span<const char>(static_cast<const char*>(data), size),
                                 size);
        ...
        double bandwidth_mbps = (bytes_written / (1024.0 * 1024.0)) / duration_s;

        std::cout << std::fixed << std::setprecision(2);
        std::cout << "Bytes written: " << FormatSize(bytes_written) << std::endl;
        std::cout << "Time: " << duration_us << " us (" << duration_s << " s)" << std::endl;
    ...
    std::optional<std::pair<double, void*>> BenchmarkAlignedIO(void* write_data, size_t size, bool is_write) {
        std::cout << "=== Zero-Copy Aligned I/O " << (is_write ? "Write" : "Read") << " Benchmark ===" << std::endl;

        // Open file with O_DIRECT
        int flags = is_write ? (O_WRONLY | O_CREAT | O_TRUNC | O_DIRECT | O_CLOEXEC)
        ...
        if (config_.use_registered_buffers && uring_file) {
            ...
            if (registered) {
                std::cout << "Buffer registration: " << reg_us << " us (excluded from I/O time)" << std::endl;
            } else {
                std::cerr << "Warning: Buffer registration failed, continuing without" << std::endl;
            }
        }
        ...
        if (config_.use_registered_buffers && uring_file && uring_file->is_buffer_registered()) {
            ...
            auto unreg_us = std::chrono::duration_cast<std::chrono::microseconds>(
                unreg_end - unreg_start).count();
            std::cout << "Buffer unregistration: " << unreg_us << " us (excluded from I/O time)" << std::endl;
        }
        ...
        std::cout << std::fixed << std::setprecision(2);
        std::cout << "Bytes " << (is_write ? "written" : "read") << ": " << FormatSize(total_bytes) << std::endl;
        std::cout << "Time: " << duration_us << " us (" << duration_s << " s)" << std::endl;
        std::cout << "Bandwidth: " << bandwidth_mbps << " MB/s" << std::endl;
    ...
    std::optional<std::pair<double, void*>> BenchmarkDirectIO(void* write_data, size_t size, bool is_write) {
        std::cout << "=== Direct I/O " << (is_write ? "Write" : "Read") << " Benchmark ===" << std::endl;

        // Open file with O_DIRECT
        int flags = is_write ? (O_WRONLY | O_CREAT | O_TRUNC | O_DIRECT) : (O_RDONLY | O_DIRECT);
        int fd = open(config_.file_path.c_str(), flags, 0644);
        if (fd < 0) {
            std::cerr << "Failed to open file with O_DIRECT: "
                     << config_.file_path << " - " << std::strerror(errno) << std::endl;
        ...
        while (remaining > 0) {
            ...
            if (is_write) {
                ret = pwrite(fd, static_cast<char*>(buffer) + offset, this_chunk, offset);
            } else {
                ret = pread(fd, static_cast<char*>(buffer) + offset, this_chunk, offset);
            }
    ...
    std::optional<std::pair<double, void*>> BenchmarkRead(size_t size) {
        ...
        if (config_.use_uring_direct_io) {
            flags |= O_DIRECT;
            std::cout << "Using UringFile with O_DIRECT (with copy)" << std::endl;
        }
            ...
            for (size_t i = 0; i < size; ++i) {
                if (exp_data[i] != act_data[i]) {
                    std::cerr << "First mismatch at offset " << i << ": "
                             << "expected 0x" << std::hex << (int)(unsigned char)exp_data[i]
                             << ", got 0x" << (int)(unsigned char)act_data[i] << std::dec
                             << std::endl;
                    break;
                }
            }
...
void PrintUsage(const char* program_name) {
    ...
    std::cout << "  --file <path>        Path to test file (default: /tmp/file_bench_test.dat)" << std::endl;
    std::cout << "  --size <bytes>       Size of test data in bytes (default: 104857600 = 100MB)" << std::endl;
    std::cout << "  --size-mb <MB>       Size of test data in MB (overrides --size)" << std::endl;
    std::cout << "  --chunk-size <KB>    I/O chunk size in KB (default: 1024)" << std::endl;
#ifdef USE_URING
    std::cout << "  --use-uring              Use io_uring for I/O operations" << std::endl;
    std::cout << "  --queue-depth <n>        io_uring queue depth (default: 32)" << std::endl;
    std::cout << "  --uring-direct-io        Enable O_DIRECT in UringFile (with memory copy)" << std::endl;
    std::cout << "  --uring-direct-io-zerocopy  Enable O_DIRECT with zero-copy (requires aligned buffer)" << std::endl;
    std::cout << "  --use-registered-buffers Register buffers with io_uring for optimal performance" << std::endl;
#endif
    std::cout << "  --direct-io              Use O_DIRECT with raw syscalls (bypasses StorageFile)" << std::endl;
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--help") {
            ...
            config.chunk_size = std::stoull(argv[++i]) * 1024;  // Convert KB to bytes
        } else if (arg == "--use-uring") {
#ifdef USE_URING
            config.use_uring = true;
#else
            std::cerr << "Warning: io_uring support not compiled in" << std::endl;
#endif
            ...
            if (!config.use_uring) {
                std::cerr << "Note: --uring-direct-io requires --use-uring, enabling io_uring" << std::endl;
                config.use_uring = true;
            }
#else
            std::cerr << "Warning: io_uring support not compiled in" << std::endl;
            ...
            if (!config.use_uring) {
                std::cerr << "Note: --uring-direct-io-zerocopy requires --use-uring, enabling io_uring" << std::endl;
                config.use_uring = true;
            }
#else
            std::cerr << "Warning: io_uring support not compiled in" << std::endl;
            ...
            if (!config.use_uring) {
                std::cerr << "Note: --use-registered-buffers requires --use-uring, enabling io_uring" << std::endl;
                config.use_uring = true;
            }
            if (!config.use_uring_direct_io_zero_copy) {
                std::cerr << "Note: --use-registered-buffers works best with --uring-direct-io-zerocopy" << std::endl;
            }
#else
            std::cerr << "Warning: io_uring support not compiled in" << std::endl;
#endif
