...
//
//	seed      = H("mooncake-paged-kv" || model)
...
const (
	// DefaultKeyPrefix namespaces these keys so they never collide with keys
	// written by the vLLM or SGLang connectors against the same store.
	DefaultKeyPrefix = "mc/pagedkv"
...
const hashSeedDomain = "mooncake-paged-kv\x00"
...
func (c Config) Pages(tokens []int32) ([]Page, error) {
	...
	pages := make([]Page, NumFullPages(len(tokens), c.PageSize))
	prefixStr := c.keyPrefix()

	// One hasher for the whole chain: a long prompt is many pages, and a fresh
	// sha256.New per page is per-page heap that shows up in time to first token.
	// Tokens go in as fixed-width big-endian uint32 (llama.cpp token ids are
	// int32) so the encoding is unambiguous and stable across platforms.
	hasher := sha256.New()
	digest := sha256.Sum256([]byte(hashSeedDomain + c.Model))
	var token [4]byte
	for i := range pages {
		start := i * c.PageSize
		...
		for _, t := range tokens[start:end] {
			binary.BigEndian.PutUint32(token[:], uint32(t))
			hasher.Write(token[:])
		}
		hasher.Sum(digest[:0])
		key := fmt.Sprintf("%s/%s/%s", prefixStr, c.Model, hex.EncodeToString(digest[:keyHashBytes]))
