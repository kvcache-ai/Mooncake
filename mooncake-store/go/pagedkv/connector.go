...
import (
	...
	"github.com/kvcache-ai/Mooncake/mooncake-store/go/pagedkv/prefix"
)

var (
	// ErrReadNotAllowed is returned by LoadCachedPrefix when the role forbids reads.
	ErrReadNotAllowed = errors.New("pagedkv: role does not permit loading cached KV")
	// ErrWriteNotAllowed is returned by StoreComputedPrefix when the role forbids writes.
	ErrWriteNotAllowed = errors.New("pagedkv: role does not permit publishing KV")
)

// Connector bridges a paged-KV inference runtime to a Mooncake Store cluster for
...
func New(cfg Config) (*Connector, error) {
	...
	if cfg.Model == "" {
		return nil, errors.New("pagedkv: Config.Model must be set")
	}
