 ...
 * @brief RPC service for handling remote data read/write requests from peer clients
 * 
 * This service receives RPC requests from other clients (e.g., Client A) to read/write
 * data stored locally (on Client B). It uses DataManager to access TieredBackend and
 * TransferEngine to perform zero-copy RDMA transfers.
 */
class ClientRpcService {
public:
    /**
     * @brief Constructor
     * @param data_manager Reference to DataManager instance (must outlive this object)
     */
    explicit ClientRpcService(DataManager& data_manager);

    /**
     * @brief Read remote data: Client A requests Client B to read data and transfer to A
     ...
     * @param request BatchRemoteReadRequest containing multiple keys and their destination buffers
     ...
     * @param request BatchRemoteWriteRequest containing multiple keys, source buffers, and tier_ids
     * @return Vector of expected results for each key
     */
