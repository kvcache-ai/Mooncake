...
static constexpr uint64_t DEFAULT_KV_SOFT_PIN_TTL_MS = 30 * 60 * 1000; // 30 minutes
...
inline std::ostream &operator<<(std::ostream &os, const ErrorCode &errorCode) noexcept {
 ...
 */
inline std::ostream &operator<<(std::ostream &os, const BufStatus &status) noexcept {
...
inline std::ostream &operator<<(std::ostream &os, const ReplicaStatus &status) noexcept {
	static const std::unordered_map<ReplicaStatus, std::string_view> status_strings {
	    {ReplicaStatus::UNDEFINED, "UNDEFINED"},   {ReplicaStatus::INITIALIZED, "INITIALIZED"},
	    {ReplicaStatus::PROCESSING, "PROCESSING"}, {ReplicaStatus::COMPLETE, "COMPLETE"},
	    {ReplicaStatus::REMOVED, "REMOVED"},       {ReplicaStatus::FAILED, "FAILED"}};
...
struct ReplicateConfig {
	...
	friend std::ostream &operator<<(std::ostream &os, const ReplicateConfig &config) noexcept {
		return os << "ReplicateConfig: { replica_num: " << config.replica_num
		          << ", with_soft_pin: " << config.with_soft_pin << ", preferred_segment: " << config.preferred_segment
		          << " }";
	}
};
...
public:
	...
	AllocatedBuffer(std::shared_ptr<BufferAllocator> allocator, std::string segment_name, void *buffer_ptr,
	                std::size_t size)
	    : allocator_(std::move(allocator)), segment_name_(std::move(segment_name)), buffer_ptr_(buffer_ptr),
	      size_(size) {
	}
	...
	friend std::ostream &operator<<(std::ostream &os, const AllocatedBuffer &buffer);
...
inline AllocatedBuffer::Descriptor AllocatedBuffer::get_descriptor() const {
	return {segment_name_, static_cast<uint64_t>(size()), reinterpret_cast<uintptr_t>(buffer_ptr_), status};
}

// Define operator<< using public accessors or get_descriptor if appropriate
inline std::ostream &operator<<(std::ostream &os, const AllocatedBuffer &buffer) {
...
public:
	...
	Replica() = default;
	Replica(std::vector<std::unique_ptr<AllocatedBuffer>> buffers, ReplicaStatus status)
	...
	[[nodiscard]] bool has_invalid_handle() const {
		return std::any_of(buffers_.begin(), buffers_.end(), [](const std::unique_ptr<AllocatedBuffer> &buf_ptr) {
...
struct Segment {
	...
	size_t size {0};
	Segment() = default;
	Segment(const UUID &id, const std::string &name, uintptr_t base, size_t size)
...
inline std::ostream &operator<<(std::ostream &os, const ClientStatus &status) noexcept {
	static const std::unordered_map<ClientStatus, std::string_view> status_strings {
	    {ClientStatus::UNDEFINED, "UNDEFINED"}, {ClientStatus::OK, "OK"}, {ClientStatus::NEED_REMOUNT, "NEED_REMOUNT"}};
