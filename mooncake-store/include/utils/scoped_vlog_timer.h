...
public:
	ScopedVLogTimer(int level, std::string_view function_name)
	    : level_(level), function_name_(function_name), active_(VLOG_IS_ON(level_)) {
		if (active_) {
	...
	void LogResponse(Args &&...args) {
		if (active_) {
			...
			VLOG(level_) << function_name_ << " response: " << oss.str() << ", latency=" << latency.count() << "us";
	...
	void LogResponseJson(const T &obj) {
		if (active_) {
			...
			VLOG(level_) << function_name_ << " response: " << json << ", latency=" << latency.count() << "us";
			logged_response_ = true;
		}
	}
	...
	~ScopedVLogTimer() {
		if (active_ && !logged_response_) {
			auto end_time = std::chrono::steady_clock::now();
			auto latency = std::chrono::duration_cast<std::chrono::microseconds>(end_time - start_time_);
			VLOG(level_) << function_name_ << " finished, latency=" << latency.count() << "us";
		}
	}
