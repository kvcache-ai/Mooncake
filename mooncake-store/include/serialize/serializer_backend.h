...
inline SnapshotBackendType ParseSnapshotBackendType(const std::string& type_str) {
    ...
    // Debug logging
    LOG(INFO) << "DEBUG ParseSnapshotBackendType: original=[" << type_str << "] len=" 
              << type_str.length() << ", trimmed=[" << trimmed << "] len=" << trimmed.length();
    
#ifdef STORE_USE_ETCD
    if (trimmed == "etcd" || trimmed == "ETCD") {
        LOG(INFO) << "DEBUG: Returning ETCD backend type";
...
#endif
    LOG(INFO) << "DEBUG: Returning LOCAL_FILE backend type (default)";
