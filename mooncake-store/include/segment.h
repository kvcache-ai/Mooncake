 ...
 */
inline std::ostream &operator<<(std::ostream &os, const SegmentStatus &status) noexcept {
	static const std::unordered_map<SegmentStatus, std::string_view> status_strings {
	    {SegmentStatus::UNDEFINED, "UNDEFINED"}, {SegmentStatus::OK, "OK"}, {SegmentStatus::UNMOUNTING, "UNMOUNTING"}};
...
public:
	 ...
	 */
	ErrorCode GetClientSegments(const UUID &client_id, std::vector<Segment> &segments) const;
...
public:
	explicit ScopedAllocatorAccess(
	    std::unordered_map<std::string, std::vector<std::shared_ptr<BufferAllocator>>> &allocators_by_name,
	    std::vector<std::shared_ptr<BufferAllocator>> &allocators, std::shared_mutex &mutex)
	    : allocators_by_name_(allocators_by_name), allocators_(allocators), lock_(mutex) {
	}

	const std::unordered_map<std::string, std::vector<std::shared_ptr<BufferAllocator>>> &getAllocatorsByName() {
...
private:
	...
	std::unordered_map<UUID, MountedSegment, boost::hash<UUID>> mounted_segments_;   // segment_id -> mounted segment
	std::unordered_map<UUID, std::vector<UUID>, boost::hash<UUID>> client_segments_; // client_id -> segment_ids
