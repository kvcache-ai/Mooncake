...
enum class ClientLivenessObservation {
    ...
    RECOVERED_ACTIVE,
    REJECTED_OFFLINE,
    // The wrapped operation failed its success predicate: the observation
    // was NOT committed (state and last_liveness_at_ are untouched). A
    // failed operation proves nothing about liveness.
...
class ClientLivenessRecord {
   public:
    ...
    // Runs `operation` under the transition lock and commits the liveness
    // observation only when the operation reports success. `operation` must
    // return bool: true commits the observation (refreshing last_liveness_at_
    // and recovering SUSPECTED to ACTIVE), false withholds it so a failed
    // operation cannot revive an expiring client.
