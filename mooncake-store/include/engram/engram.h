 ...
 * gating, and ShortConv. Requires Mooncake Store for forward (no local-only mode).
 */
class Engram {
   public:
    Engram(int layer_id,
     ...
     * @param output [B, L, hc_mult, D] output (pre-allocated, registered buffer)
     * @return 0 on success, negative on error
     */
    int forward(const void* hidden_states,
                size_t hidden_states_size,
   ...
   private:
    ...
    std::unordered_map<int, std::vector<std::vector<int64_t>>> vocab_size_across_layers_;
    ...
    static int64_t find_next_prime(int64_t start, std::unordered_set<int64_t>& seen);
    std::vector<std::vector<std::vector<int64_t>>> hash_(
        const std::vector<std::vector<int64_t>>& input_ids,
        int layer_id) const;

    // Embedding lookup / populate
    int embedding_lookup(const std::vector<std::vector<std::vector<int64_t>>>& hash_ids,
                        void* output_buffer,
                        size_t output_size) const;
    int embedding_populate_from_tensors(const std::vector<void*>& embedding_data,
    ...
    void compute_gates_and_fuse(const float* embeddings,
                                ...
                                int L,
                                float* output) const;
    void apply_short_conv(const float* input, int B, int L, float* output) const;
};
