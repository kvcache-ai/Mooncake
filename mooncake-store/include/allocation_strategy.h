...
public:
	 ...
	 */
	virtual std::unique_ptr<AllocatedBuffer>
	Allocate(const std::vector<std::shared_ptr<BufferAllocator>> &allocators,
	         const std::unordered_map<std::string, std::vector<std::shared_ptr<BufferAllocator>>> &allocators_by_name,
...
public:
	...
	std::unique_ptr<AllocatedBuffer>
	Allocate(const std::vector<std::shared_ptr<BufferAllocator>> &allocators,
	         const std::unordered_map<std::string, std::vector<std::shared_ptr<BufferAllocator>>> &allocators_by_name,
		...
		if (auto preferred_buffer = TryPreferredAllocate(allocators_by_name, objectSize, config)) {
...
private:
	...
	std::unique_ptr<AllocatedBuffer> TryPreferredAllocate(
	    const std::unordered_map<std::string, std::vector<std::shared_ptr<BufferAllocator>>> &allocators,
	 ...
	 */
	std::unique_ptr<AllocatedBuffer> TryRandomAllocate(const std::vector<std::shared_ptr<BufferAllocator>> &allocators,
	                                                   size_t objectSize) {
		...
		for (size_t try_count = 0; try_count < max_tries; ++try_count) {
			...
			std::uniform_int_distribution<size_t> dist(0, allocator_indices.size() - 1);
			...
			if (random_index + 1 != allocator_indices.size()) {
				std::swap(allocator_indices[random_index], allocator_indices[allocator_indices.size() - 1]);
			}
