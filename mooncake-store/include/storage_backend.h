...
public:
	 ...
	 * @param key Object identifier
	 * @return ErrorCode::OK if exists, ErrorCode::FILE_NOT_FOUND if not exists,
	 * other ErrorCode for errors
	 */
...
private:
	...
	ErrorCode LoadObjectInPath(const std::string &path, std::vector<Slice> &slices);
};
