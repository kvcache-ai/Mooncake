...
public:
	 ...
	 */
	static std::optional<std::shared_ptr<Client>>
	Create(const std::string &local_hostname, const std::string &metadata_connstring, const std::string &protocol,
	       void **protocol_args, const std::string &master_server_entry = kDefaultMasterAddress);
	 ...
	 */
	std::vector<tl::expected<void, ErrorCode>> BatchGet(const std::vector<std::string> &object_keys,
	...
	tl::expected<void, ErrorCode> Get(const std::string &object_key,
	                                  const std::vector<Replica::Descriptor> &replica_list, std::vector<Slice> &slices);
	/**
	 * @brief Transfers data using pre-queried object information
	...
	BatchGet(const std::vector<std::string> &object_keys,
	 ...
	 * @return ErrorCode indicating success/failure
	 */
	tl::expected<void, ErrorCode> Put(const ObjectKey &key, std::vector<Slice> &slices, const ReplicateConfig &config);
	 ...
	 * @param config Replication configuration
	 */
	std::vector<tl::expected<void, ErrorCode>> BatchPut(const std::vector<ObjectKey> &keys,
	 ...
	 * @return ErrorCode indicating success/failure
	 */
	tl::expected<void, ErrorCode> RegisterLocalMemory(void *addr, size_t length, const std::string &location,
	 ...
	 * @return ErrorCode indicating success/failure
	 */
	tl::expected<void, ErrorCode> unregisterLocalMemory(void *addr, bool update_metadata = true);
	 ...
	 */
	std::vector<tl::expected<bool, ErrorCode>> BatchIsExist(const std::vector<std::string> &keys);

private:
	/**
	 * @brief Private constructor to enforce creation through Create() method
	 */
	Client(const std::string &local_hostname, const std::string &metadata_connstring,
	...
	ErrorCode InitTransferEngine(const std::string &local_hostname, const std::string &metadata_connstring,
	                             const std::string &protocol, void **protocol_args);
	ErrorCode TransferData(const Replica::Descriptor &replica, std::vector<Slice> &slices,
	                       TransferRequest::OpCode op_code);
	ErrorCode TransferWrite(const Replica::Descriptor &replica, std::vector<Slice> &slices);
	ErrorCode TransferRead(const Replica::Descriptor &replica, std::vector<Slice> &slices);
	...
	ErrorCode GetFromLocalFile(const std::string &object_key, std::vector<Slice> &slices,
	                           std::vector<Replica::Descriptor> &replicas);

	void PutToLocalFile(const std::string &object_key, std::vector<Slice> &slices);
	 ...
	 * replica
	 */
	ErrorCode FindFirstCompleteReplica(const std::vector<Replica::Descriptor> &replica_list,
	                                   Replica::Descriptor &replica);

	/**
	 * @brief Batch put helper methods for structured approach
	 */
	std::vector<PutOperation> CreatePutOperations(const std::vector<ObjectKey> &keys,
