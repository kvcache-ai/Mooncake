...
public:
	...
	MemoryPool(PoolId id, size_t poolSize, SlabAllocator &alloc, const std::set<uint32_t> &allocSizes);

	MemoryPool(const MemoryPool &) = delete;
	MemoryPool &operator=(const MemoryPool &) = delete;

	// returns the poolId of this memory pool.
	PoolId getId() const noexcept {
		return id_;
	}
	...
	bool overLimit() const noexcept {
		auto getCurrentUsedAndAdvisedSize = getCurrentUsedSize() + getPoolAdvisedSize();
	...
	size_t getCurrentAllocSize() const noexcept {
		return currAllocSize_;
	}
	...
	SlabReleaseContext startSlabRelease(
	    ClassId victim, ClassId receiver, SlabReleaseMode mode, const void *hint,
	...
	SlabIterationStatus forEachAllocation(ClassId classId, Slab *slab, AllocTraversalFn &&callback) {
		auto &allocClass = getAllocationClassFor(classId);
		return allocClass.forEachAllocation(slab, std::forward<AllocTraversalFn>(callback));
	}
...
private:
	...
	void releaseSlab(SlabReleaseMode mode, const Slab *slab, ClassId receiverClassId);
