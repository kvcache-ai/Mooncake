...
class MemoryPool {
   public:
    ...
    MemoryPool(PoolId id, size_t poolSize, SlabAllocator& alloc,
    ...
    SlabReleaseContext startSlabRelease(
        ClassId victim, ClassId receiver, SlabReleaseMode mode,
        const void* hint,
    ...
    SlabIterationStatus forEachAllocation(ClassId classId, Slab* slab,
    ...
    const Slab* getSlabForMemory(void* memory) {
        return slabAllocator_.getSlabForMemory(memory);
    }
   ...
   private:
    ...
    void releaseSlab(SlabReleaseMode mode, const Slab* slab,
