...
inline auto do_write(const std::tm &time, const std::locale &loc, char format, char modifier) -> std::string {
	...
	auto result = f.in(mb, str.data(), str.data() + str.size(), from_next, buf, buf + buf_size, to_next);
...
template <typename Char, typename Duration>
constexpr Char formatter<std::chrono::time_point<std::chrono::system_clock, Duration>, Char>::default_specs[];
...
struct chrono_formatter {
	...
	void format_localized(const tm &time, char format, char modifier = 0) {
		if (isnan(val))
			return write_nan();
		const auto &loc = localized ? context.locale().template get<std::locale>() : std::locale::classic();
...
public:
	...
	auto format(weekday wd, format_context &ctx) -> decltype(ctx.out()) {
		...
		const auto &loc = localized ? ctx.locale().template get<std::locale>() : std::locale::classic();
