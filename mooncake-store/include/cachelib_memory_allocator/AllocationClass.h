...
enum class SlabIterationStatus { kFinishedCurrentSlabAndContinue, kSkippedCurrentSlabAndContinue, kAbortIteration };
...
public:
	...
	//        is invalid.
	AllocationClass(ClassId classId, PoolId poolId, uint32_t allocSize, const SlabAllocator &s);
	...
	uint32_t getAllocSize() const noexcept {
	...
	// The callback is executed under the lock, immediately after checking if
	// the alloc has been freed.
	//
	// @param ctx       release context for the slab owning this alloc
	// @param memory    memory to check
	...
	// @throws std::runtime_error  if the slab cannot be found inside
	//         slabReleaseAllocMap_
	void processAllocForRelease(const SlabReleaseContext &ctx, void *memory,
	...
	SlabIterationStatus forEachAllocation(Slab *slab, AllocTraversalFn &&callback) {
		// Take a try_lock on this allocation class beginning any new slab
		// release.
		std::unique_lock<std::mutex> startSlabReleaseLockHolder(startSlabReleaseLock_, std::defer_lock);
		...
		Return allocInfo;
		{
			...
			allocInfo = ([this, slab]() -> Return {
				...
				if (!slabHdr || slabHdr->classId != classId_ || slabHdr->poolId != poolId_ || slabHdr->isAdvised() ||
		...
		for (unsigned int i = 0; i < kForEachAllocPrefetchOffset; i++) {
			prefetchOffsetPtr =
			    reinterpret_cast<void *>(reinterpret_cast<uintptr_t>(prefetchOffsetPtr) + allocationSize_);
		...
		for (unsigned int i = 0; i < allocsPerSlab; ++i) {
			...
			ptr = reinterpret_cast<void *>(reinterpret_cast<uintptr_t>(ptr) + allocationSize_);
		}
	...
	SlabReleaseContext startSlabRelease(
	    SlabReleaseMode mode, const void *hint, SlabReleaseAbortFn shouldAbortFn = []() { return false; });
...
private:
	...
	std::pair<bool, std::vector<void *>> pruneFreeAllocs(
	    const Slab *slab, SlabReleaseAbortFn shouldAbortFn = []() { return false; });
	...
	//         this slab class, or if the slab is not actively being released,
	//         or if the context belongs to a different slab.
	void checkSlabInRelease(const SlabReleaseContext &ctx, const void *memory) const;
	...
	void partitionFreeAllocs(const Slab *slab, FreeList &freeAllocs, FreeList &inSlab, FreeList &notInSlab);
