...
static_assert(std::is_standard_layout<Slab>::value, "Slab is not standard layout");
...
struct CACHELIB_PACKED_ATTR SlabHeader {
	...
	SlabHeader(PoolId pid, ClassId cid, uint32_t size) : poolId(pid), classId(cid), allocSize(size) {
	}
	...
	void setAdvised(bool value) {
		value ? setFlag(SlabHeaderFlag::IS_ADVISED) : unSetFlag(SlabHeaderFlag::IS_ADVISED);
	}
	...
	void setMarkedForRelease(bool value) {
		value ? setFlag(SlabHeaderFlag::IS_MARKED_FOR_RELEASE) : unSetFlag(SlabHeaderFlag::IS_MARKED_FOR_RELEASE);
	}
...
private:
	void setFlag(SlabHeaderFlag flag) noexcept {
		const uint8_t bitmask = static_cast<uint8_t>(1u << static_cast<unsigned int>(flag));
		// FIXME:
		// https://fb.workplace.com/groups/cachelibusers/posts/2345418462311949/
	...
	void unSetFlag(SlabHeaderFlag flag) noexcept {
		const uint8_t bitmask =
		    static_cast<uint8_t>(std::numeric_limits<uint8_t>::max() - (1u << static_cast<unsigned int>(flag)));
...
public:
	...
	SlabReleaseContext(const Slab *slab, PoolId pid, ClassId cid, SlabReleaseMode m)
	...
	SlabReleaseContext(const Slab *slab, PoolId pid, ClassId cid, std::vector<void *> allocations, SlabReleaseMode m)
	    : slab_(slab), pid_(pid), victim_(cid), activeAllocations_(std::move(allocations)), mode_(m) {
	}

	// create a context where the slab is already released.
	//
	// also specify the receiver to receive the slab
	SlabReleaseContext(const Slab *slab, PoolId pid, ClassId victim, ClassId receiver)
	...
	//
	// also specify the receiver to receive the slab.
	SlabReleaseContext(const Slab *slab, PoolId pid, ClassId victim, std::vector<void *> allocations, ClassId receiver)
	    : slab_(slab), pid_(pid), victim_(victim), activeAllocations_(std::move(allocations)), receiver_(receiver),
	...
	bool isReleased() const noexcept {
		return activeAllocations_.empty();
	}
	...
	ClassId getClassId() const noexcept {
		return victim_;
	}

	ClassId getReceiverClassId() const noexcept {
		return receiver_;
	}
	...
	const Slab *getSlab() const noexcept {
		return slab_;
	}

	// @return the mode for this slab release context.
	SlabReleaseMode getMode() const noexcept {
		return mode_;
	}
...
private:
	...
	void setReceiver(ClassId receiver) noexcept {
		receiver_ = receiver;
	}
