...
public:
	...
	// See Feishu document.
	MemoryAllocator(Config config, void *headerMemoryStart, size_t headerMemorySize, void *slabMemoryStart,
	...
	PoolId addPool(std::string name, size_t size, const std::set<uint32_t> &allocSizes = {},
	...
	SlabReleaseContext startSlabRelease(
	    PoolId pid, ClassId victim, ClassId receiver, SlabReleaseMode mode, const void *hint = nullptr,
	...
	FOLLY_ALWAYS_INLINE AllocInfo getAllocInfo(const void *memory) const {
		...
		if (!header) {
			throw std::invalid_argument(fmt::format("invalid header for slab memory addr: {}", memory));
		}
	...
	uint64_t forEachAllocation(AllocTraversalFn &&callback) {
		...
		for (unsigned int idx = 0; idx < slabAllocator_.getNumUsableSlabs(); ++idx) {
			...
			if (poolId == Slab::kInvalidPoolId || classId == Slab::kInvalidClassId || slabHdr->isAdvised() ||
			    slabHdr->isMarkedForRelease()) {
				++slabSkipped;
				continue;
			}
			auto &pool = memoryPoolManager_.getPoolById(poolId);
			auto slabIterationStatus = pool.forEachAllocation(classId, slab, std::forward<AllocTraversalFn>(callback));
			if (slabIterationStatus == SlabIterationStatus::kSkippedCurrentSlabAndContinue) {
				++slabSkipped;
	...
	static std::set<uint32_t> generateAllocSizes(double factor = 1.25, uint32_t maxSize = Slab::kSize,
