...
public:
	// See Feishu document.
	SlabAllocator(void *headerMemoryStart, size_t headerMemorySize, void *slabMemoryStart, size_t slabMemorySize);
	...
	FOLLY_ALWAYS_INLINE SlabHeader *getSlabHeader(const void *memory) const noexcept {
	...
	// true if the slab is a valid allocated slab in the memory belonging to
	// this allocator.
	FOLLY_ALWAYS_INLINE bool isValidSlab(const Slab *slab) const noexcept {
		// suppress TSAN race error, this is harmless because
		// nextSlabAllocation_ cannot go backwards and slab can't become invalid
		// once it is valid folly::annotate_ignore_thread_sanitizer_guard
		// g(__FILE__, __LINE__);
	...
	FOLLY_ALWAYS_INLINE const Slab *getSlabForMemory(const void *memory) const noexcept {
		// returns the closest slab boundary for the memory address.
		return reinterpret_cast<const Slab *>(reinterpret_cast<uintptr_t>(memory) & kAddressMask);
	}
	...
	// returns the slab corresponding to the idx, irrespective of the validity
	// of the memory. The caller can use isValidSlab to check if the returned
	// slab is valid.
...
private:
	// null Slab* presenttation. With 4M Slab size, a valid slab index would
	// never reach 2^16 - 1;
	...
	const Slab *getSlabMemoryEnd() const noexcept {
		return reinterpret_cast<Slab *>(reinterpret_cast<uint8_t *>(slabMemoryStart_) + slabMemorySize_);
	}
	...
	FOLLY_ALWAYS_INLINE SlabHeader *getSlabHeader(unsigned int slabIndex) const noexcept {
	...
	std::array<std::atomic<size_t>, std::numeric_limits<PoolId>::max()> memoryPoolSize_ {{}};
	...
	static_assert((Slab::kSize & (Slab::kSize - 1)) == 0, "Slab size is not power of two");

	// mask for all addresses belonging to slab aligned to Slab::kSize;
	static constexpr uint64_t kAddressMask =
	    std::numeric_limits<uint64_t>::max() - (static_cast<uint64_t>(1) << Slab::kNumSlabBits) + 1;
};
} // namespace cachelib
} // namespace facebook
