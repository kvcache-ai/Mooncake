...
#define CACHELIB_SHM_CERTIFY(Name, size)                                                                               \
	...
	constexpr facebook::cachelib::detail::CheckSize<sizeof(Name)> FB_ANONYMOUS_VARIABLE(checkSize) =                   \
...
struct NumBits {
	...
	static constexpr uint8_t value = static_cast<uint8_t>(sizeof(T) * kBitsPerByte);
