...
public:
	...
	struct Config {
		...
		bool needsThrottling() const noexcept {
			return sleepMs != 0;
		}
	...
	uint64_t numThrottles() const noexcept {
		return throttleCounter_;
	}

	explicit Throttler(Config config, ThrottleCb &&throttleCb = nullptr)
	    : config_(std::move(config)), currWorkStartMs_(util::getCurrentTimeMs()), throttleCb_(std::move(throttleCb)) {
	}
