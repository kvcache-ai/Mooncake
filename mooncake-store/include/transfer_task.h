 ...
 */
inline std::ostream &operator<<(std::ostream &os, const TransferStrategy &strategy) noexcept {
	switch (strategy) {
...
public:
	...
	ErrorCode get_result() const { // lock mutex
		...
		assert(result_.has_value() && "get_result() called on an incomplete or failed-to-set "
		                              "operation state.");
...
public:
	TransferEngineOperationState(TransferEngine &engine, BatchID batch_id, size_t batch_size)
	    : engine_(engine), batch_id_(batch_id), batch_size_(batch_size) {
		CHECK(batch_id_ != Transport::INVALID_BATCH_ID) << "Invalid batch ID for transfer engine operation";
	}
...
struct MemcpyOperation {
	...
	MemcpyOperation(void *d, const void *s, size_t sz) : dest(d), src(s), size(sz) {
	}
};
...
struct MemcpyTask {
	...
	MemcpyTask(std::vector<MemcpyOperation> ops, std::shared_ptr<MemcpyOperationState> s)
...
struct FilereadTask {
	...
	FilereadTask(const std::string &path, size_t size, const std::vector<Slice> &slices_ref,
	             std::shared_ptr<FilereadOperationState> s)
	    : file_path(path), file_size(size), slices(slices_ref), state(std::move(s)) {
	}
};
...
public:
	explicit TransferSubmitter(TransferEngine &engine, const std::string &local_hostname,
	 ...
	 * failure
	 */
	std::optional<TransferFuture> submit(const Replica::Descriptor &replica, std::vector<Slice> &slices,
...
private:
	 ...
	 */
	TransferStrategy selectStrategy(const std::vector<AllocatedBuffer::Descriptor> &handles,
	                                const std::vector<Slice> &slices) const;

	/**
	 * @brief Check if all handles refer to local segments
	 */
	bool isLocalTransfer(const std::vector<AllocatedBuffer::Descriptor> &handles) const;

	/**
	 * @brief Validate transfer parameters
	 */
	bool validateTransferParams(const std::vector<AllocatedBuffer::Descriptor> &handles,
	                            const std::vector<Slice> &slices) const;

	/**
	 * @brief Submit memcpy operation asynchronously
	 */
	std::optional<TransferFuture> submitMemcpyOperation(const std::vector<AllocatedBuffer::Descriptor> &handles,
	                                                    std::vector<Slice> &slices,
	                                                    Transport::TransferRequest::OpCode op_code);
	 ...
	 */
	std::optional<TransferFuture> submitTransferEngineOperation(const std::vector<AllocatedBuffer::Descriptor> &handles,
	                                                            std::vector<Slice> &slices,
	                                                            Transport::TransferRequest::OpCode op_code);

	std::optional<TransferFuture> submitFileReadOperation(const Replica::Descriptor &replica,
	                                                      std::vector<Slice> &slices,
	                                                      Transport::TransferRequest::OpCode op_code);
};
