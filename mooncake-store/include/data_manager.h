...
class DataManager {
   ...
   private:
     ...
     * @return std::pair<bool, ErrorCode>: first=true if all failed (don't commit),
     *         second=error code (OK if all succeeded, first error otherwise)
     */
     ...
     * @return ErrorCode indicating success or failure
     */
    tl::expected<void, ErrorCode> CopyFromDRAMBuffer(
        void* temp_buffer, void* dest_ptr, MemoryType dest_type,
        size_t total_size, TieredBackend* backend);
