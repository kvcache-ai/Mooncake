...
public:
	 ...
	 */
	auto ReMountSegment(const std::vector<Segment> &segments, const UUID &client_id) -> tl::expected<void, ErrorCode>;
	 ...
	 */
	auto UnmountSegment(const UUID &segment_id, const UUID &client_id) -> tl::expected<void, ErrorCode>;
	 ...
	 * @return ErrorCode::OK if exists
	 */
	auto QuerySegments(const std::string &segment) -> tl::expected<std::pair<size_t, size_t>, ErrorCode>;
	 ...
	 * @return ErrorCode::OK on success
	 */
	auto MarkForGC(const std::string &key, uint64_t delay_ms) -> tl::expected<void, ErrorCode>;
	 ...
	 */
	auto PutStart(const std::string &key, const std::vector<uint64_t> &slice_lengths, const ReplicateConfig &config)
	 ...
	 *         ping queue is full
	 */
	auto Ping(const UUID &client_id) -> tl::expected<std::pair<ViewVersionId, ClientStatus>, ErrorCode>;
...
private:
	...
	struct ObjectMetadata {
		...
		ObjectMetadata(size_t value_length, std::vector<Replica> &&reps, bool enable_soft_pin)
		    : replicas(std::move(reps)), size(value_length), lease_timeout(), soft_pin_timeout(std::nullopt) {
		...
		void GrantLease(const uint64_t ttl, const uint64_t soft_ttl) {
			...
			if (soft_pin_timeout) {
				soft_pin_timeout = std::max(*soft_pin_timeout, now + std::chrono::milliseconds(soft_ttl));
			}
		}
		...
		bool IsSoftPinned() const {
			return soft_pin_timeout && std::chrono::steady_clock::now() < *soft_pin_timeout;
		}
	...
	static constexpr uint64_t kGCThreadSleepMs = 10; // 10 ms sleep between GC and eviction checks
	...
	public:
		MetadataAccessor(MasterService *service, const std::string &key)
		    : service_(service), key_(key), shard_idx_(service_->getShardIndex(key)),
	...
	static constexpr uint64_t kClientMonitorSleepMs = 1000; // 1000 ms sleep between client monitor checks
	// boost lockfree queue requires trivial assignment operator
	struct PodUUID {
		uint64_t first;
		uint64_t second;
	};
	static constexpr size_t kClientPingQueueSize = 128 * 1024; // Size of the client ping queue
