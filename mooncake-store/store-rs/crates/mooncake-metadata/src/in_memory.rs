...
impl MetadataBackend for InMemoryMetadataBackend {
    ...
    fn put_cold_tier_device_if_absent(
        ...
        self.in_memory_put_cold_tier_device_if_absent(device)
    }

    fn get_cold_tier_device(&self, device_id: &str) -> Result<Option<ColdTierDeviceRecord>> {
        self.in_memory_get_cold_tier_device(device_id)
    }
    ...
    fn list_cold_tier_devices(
        ...
        self.in_memory_list_cold_tier_devices(filter)
    }
    ...
    fn update_cold_tier_device(
        ...
        self.in_memory_update_cold_tier_device(device_id, update)
    }
    ...
    fn apply_cold_tier_usage_delta(
        ...
        self.in_memory_apply_cold_tier_usage_delta(device_id, delta, updated_at_ms)
    }
    ...
    fn reserve_tenant_quota(
        ...
        if let Some(limit) = request.limit.max_bytes.filter(|_| positive_bytes > 0) {
            let admitted = quota
                .used_bytes
        ...
        if let Some(limit) = request.limit.max_objects.filter(|_| positive_objects > 0) {
...
mod tests {
    ...
    #[test]
    fn tenant_quota_reservation_allows_non_positive_deltas_when_usage_is_already_over_limit() {
        let metadata = InMemoryMetadataBackend::new();
