...
fn bounded_set_members_with_limit(
    ...
    scan_set_members_bounded(connection, key, max_items, MAX_REDIS_SCAN_PAGES)
        .map_err(|error| metadata_error(operation, error))
}

fn scan_set_members_bounded(
    connection: &mut redis::Connection,
    key: &str,
    max_items: usize,
    max_pages: usize,
    ...
    for _ in 0..max_pages {
        let (next, batch): (u64, Vec<String>) = cmd("SSCAN")
            .arg(key)
            .arg(cursor)
            .arg("COUNT")
            .arg(REDIS_SCAN_COUNT)
            .query(connection)?;
        if members.len().saturating_add(batch.len()) > max_items {
            return Err(redis::RedisError::from((
                redis::ErrorKind::TypeError,
                "bounded Redis set listing exceeded its item limit",
            )));
        }
    ...
    Err(redis::RedisError::from((
        redis::ErrorKind::TypeError,
        "bounded Redis set listing exceeded its page limit",
    )))
}
...
fn legacy_tenant_policy_keys(
    ...
    keys.extend(scan_keys_bounded(
        connection,
        &format!("{}/*", root_key),
        MAX_REDIS_LIST_ITEMS.saturating_sub(keys.len()),
        MAX_REDIS_SCAN_PAGES,
    )?);
    keys.sort();
    keys.dedup();
...
impl MetadataBackend for RedisMetadataBackend {
    ...
    fn list_object_routes(&self) -> Result<Vec<ObjectRoute>> {
        ...
        let keys: Vec<String> =
            self.query_readonly("redis list object route index", |connection| {
                scan_set_members_bounded(
                    connection,
                    &index,
                    MAX_REDIS_LIST_ITEMS,
    ...
    fn compare_and_swap_object_route(
        ...
        self.redis_compare_and_swap_object_route(key, expected, next)
    }
    ...
    fn put_cold_tier_device_if_absent(
        ...
        self.redis_put_cold_tier_device_if_absent(device)
    }

    fn get_cold_tier_device(&self, device_id: &str) -> Result<Option<ColdTierDeviceRecord>> {
        self.redis_get_cold_tier_device(device_id)
    }
    ...
    fn list_cold_tier_devices(
        ...
        self.redis_list_cold_tier_devices(filter)
    }
    ...
    fn update_cold_tier_device(
        ...
        self.redis_update_cold_tier_device(device_id, update)
    }
    ...
    fn apply_cold_tier_usage_delta(
        ...
        self.redis_apply_cold_tier_usage_delta(device_id, delta, updated_at_ms)
    }
