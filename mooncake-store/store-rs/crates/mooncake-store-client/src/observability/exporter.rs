use super::registry::{
    ActionResultKey, ColdTierRuntimeKey, CounterSample, GaugeSample, HistogramSample,
    MetadataInflightKey, MetadataOperationKey, MetricsSnapshot, PhaseKey, PhaseResultKey,
    PreferredSegmentSkipKey, ReplicaDistributionKey, RequestBytesKey, RequestInflightKey,
    RequestKey, ResultKey, RuntimeKey, RuntimeStatusKey, TenantKey, TransportBytesKey,
    TransportOperationKey, CHECKSUM_VALIDATION_TOTAL, COLD_TIER_IO_BUCKETS, EVICTION_DURATION,
    EVICTION_TOTAL, HEARTBEAT_CONSECUTIVE_FAILURES, HEARTBEAT_LAST_SUCCESS_MS,
    MEMBERSHIP_REFRESH_DURATION, MEMBERSHIP_REFRESH_TOTAL, METADATA_OPERATION_DURATION,
    METADATA_OPERATION_INFLIGHT, METADATA_OPERATION_TOTAL, OBJECT_ROUTES,
...
pub(crate) fn render_prometheus_metrics(snapshot: &MetricsSnapshot) -> String {
    ...
    super::cold_tier_exporter::render_cold_tier_metrics(&mut output, snapshot);
...
pub(super) fn render_cold_tier_io_histogram(
    output: &mut String,
    metric: &str,
    sample: &HistogramSample<ResultKey>,
) {
...
pub(super) fn render_phase_histogram(
    output: &mut String,
    metric: &str,
    sample: &HistogramSample<PhaseKey>,
) {
    for (le, value) in COLD_TIER_IO_BUCKETS.iter().zip(sample.buckets.iter()) {
        output.push_str(&format!(
...
pub(super) fn for_cold_runtime_gauge(
    output: &mut String,
    metric: &str,
...
pub(super) fn counter_family(output: &mut String, metric: &str, help: &str) {
    output.push_str(&format!(
        "# HELP {metric} {help}\n# TYPE {metric} counter\n"
    ));
}

pub(super) fn gauge_family(output: &mut String, metric: &str, help: &str) {
    output.push_str(&format!("# HELP {metric} {help}\n# TYPE {metric} gauge\n"));
}

pub(super) fn histogram_family(output: &mut String, metric: &str, help: &str) {
    output.push_str(&format!(
        "# HELP {metric} {help}\n# TYPE {metric} histogram\n"
    ));
}

pub(super) fn format_bucket(value: f64) -> String {
    format!("{value:.6}")
}
