...
pub enum ReplicaReadSelector {
    Segment(SegmentName),
    /// Select the cold tier backing replica for a cold read (SSD restore).
    #[allow(dead_code)] // cold restore read path
    ColdBacking,
    /// Explicit owner+segment selection; current production path uses segment-only.
    #[allow(dead_code)] // explicit replica migration
    OwnerAndSegment {
