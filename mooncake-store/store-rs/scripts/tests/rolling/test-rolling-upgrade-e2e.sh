#!/usr/bin/env bash
# ---------------------------------------------------------------------------
# test-rolling-upgrade-e2e.sh
#
# End-to-end rolling upgrade verification for Mooncake Store.
#
# This script:
...
#
# Usage:
#   ./scripts/tests/rolling/test-rolling-upgrade-e2e.sh
#
# Environment:
...
list_upstream_dirs() {
  ...
  if primary_worktree=$(git -C "${REPO_ROOT}" worktree list --porcelain 2>/dev/null \
    | awk '/^worktree / { print substr($0, 10); exit }'); then
...
resolve_upstream_build_dir() {
  local candidates=() candidate upstream_dir
  ...
  while IFS= read -r upstream_dir; do
    ...
    candidates+=("${upstream_dir}/build-rust" "${upstream_dir}/build-wheel-compat")
  ...
  echo "unable to find Mooncake runtime libraries" >&2
  printf '  checked: %s\n' "${candidates[@]}" >&2
  exit 1
}
...
RUN_ID=$(date +%s%N)
KEYSPACE="mc/store-rs/rolling-upgrade-e2e/${RUN_ID}"
TEMP_DIR=$(mktemp -d)
PIDS=()
REDIS_STARTED=0
SOURCE_PATCHED=0
...
echo ""
echo "============================================"
echo "  Rolling Upgrade E2E Test PASSED ✅"
echo "============================================"
