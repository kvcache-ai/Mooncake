...
`.aoneci/ci.yaml` defines two relevant jobs:

- `rust-lint` — runs `cargo fmt --all -- --check` on every push to `main`
  and every merge request targeting `main`.
- `build-and-unit-test` — runs `cargo test --lib -- --test-threads=4`
