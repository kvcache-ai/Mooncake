...
// Minimal MetadataStore implementation for P2POpLogApplierBase
class MinimalMockMetadataStore : public MetadataStore {
   public:
    ...
    size_t GetKeyCountForTenant(const std::string&) const override { return 0; }
    bool PutMetadata(const std::string&, const std::string&, const StandbyObjectMetadata&) override { return true; }
    std::optional<StandbyObjectMetadata> GetMetadata(const std::string&, const std::string&) const override { return std::nullopt; }
    bool Remove(const std::string&, const std::string&) override { return true; }
    bool Exists(const std::string&, const std::string&) const override { return false; }
};
...
class OpLogReplicatorTest : public ::testing::Test {
   protected:
    void SetUp() override {
        ...
        applier_ =
            std::make_unique<P2POpLogApplierBase>(metadata_store_.get(), "test");
    ...
    std::unique_ptr<P2POpLogApplierBase> applier_;
