...
protected:
	...
	std::shared_ptr<BufferAllocator> CreateTestAllocator(const std::string &segment_name, size_t base_offset = 0) {
...
TEST_F(AllocationStrategyTest, EmptyAllocatorsMap) {
	std::unordered_map<std::string, std::vector<std::shared_ptr<BufferAllocator>>> empty_allocators_by_name;
...
TEST_F(AllocationStrategyTest, MultipleAllocatorsRandomSelection) {
	...
	for (const auto &segment : allocated_segments) {
		EXPECT_TRUE(segment == "segment1" || segment == "segment2" || segment == "segment3");
	}
}
