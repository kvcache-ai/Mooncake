...
TEST_F(RealClientTest, TestBatchAndNormalGetAllocatedBufferDesc) {
    // Start in-proc master
    ASSERT_TRUE(master_.Start(InProcMasterConfigBuilder().build()))
        << "Failed to start in-proc master";
    master_address_ = master_.master_address();
    ...
    config.replica_num = 1;
    int put_result = py_client_->put(key, data_span, config);
    // test get_allocated_buffer_desc
    std::vector<Replica::Descriptor> desc =
        py_client_->get_allocated_buffer_desc(key);
    EXPECT_EQ(desc.size(), 1)
        << "get_allocated_buffer_desc should return 1 desc";
    EXPECT_EQ(desc[0].is_memory_replica(), true)
        << "get_allocated_buffer_desc should return memory replica";

    // test batch_get_allocated_buffer_desc
    std::vector<std::string> keys = {key};
    std::map<std::string, std::vector<Replica::Descriptor>> desc_map =
        py_client_->batch_get_allocated_buffer_desc(keys);
    EXPECT_EQ(desc_map[key].size(), 1)
        << "batch_get_allocated_buffer_desc should return 1 desc";
    EXPECT_EQ(desc_map[key][0].is_memory_replica(), true)
        << "batch_get_allocated_buffer_desc should return memory replica";

    // test batch_get_allocated_buffer_desc with error keys
    std::vector<std::string> keys1 = {"test_key_1"};
    std::map<std::string, std::vector<Replica::Descriptor>> desc_map1 =
        py_client_->batch_get_allocated_buffer_desc(keys1);
    EXPECT_EQ(desc_map1.size(), 0)
        << "batch_get_allocated_buffer_desc should return empty map when all "
           "keys is invalid";
}
}  // namespace testing
