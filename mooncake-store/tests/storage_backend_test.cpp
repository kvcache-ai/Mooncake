...
TEST_F(StorageBackendTest,
    ...
    EXPECT_FALSE(fs::exists(ResolvePathFromKey(
...
TEST_F(StorageBackendTest, AdaptorScanMetaRejectsEmbeddedKeyPathMismatch) {
    ...
    const fs::path expected_path =
        ResolvePathFromKey(key, cfg.storage_filepath, backend_config.fsdir);
...
TEST_F(StorageBackendTest, AdaptorScanMetaRejectsUnexpectedLayout) {
    ...
    const fs::path symlink_target = ResolvePathFromKey(
        symlink_key, cfg.storage_filepath, source_config.fsdir);
    const fs::path symlink_path =
        ResolvePathFromKey(symlink_key, cfg.storage_filepath, symlink_fsdir);
