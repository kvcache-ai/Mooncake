    ...
    for (size_t i = 0; i < bucket_sz; i++) {
        ...
        for (size_t j = 0; j < batch_sz; j++) {
            ...
            for (size_t k = 0; k < data_sz; k++) {
                ...
                data_size+=data.size();
                void* buffer =
                    client_buffer_allocator->allocate(data.size());
...
TEST_F(StorageBackendTest, StorageBackendAll) {
    ...
    for (auto test_data_it:test_data) {
        ...
        batche_object.emplace(test_data_it.first, Slice{buffer, test_data_it.second.size()});
    }
    ...
    for (const auto& test_data_it : test_data) {
        auto is_exist_object_result = storage_backend.IsExist(test_data_it.first);
