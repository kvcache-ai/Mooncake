...
protected:
	...
	void SetUp() override {
		...
		for (int i = 0; i < master_num; ++i) {
			masters_.emplace_back(std::make_unique<mooncake::testing::MasterProcessHandler>(
	...
	static void GetLeaderIndex(int &leader_index) {
		...
		ASSERT_FALSE(port_str.empty()) << "Port part should not be empty in master address: " << master_address;
	...
	std::vector<std::unique_ptr<mooncake::testing::MasterProcessHandler>> masters_;
	int leader_index_;
};
...
TEST_F(ChaosTest, LeaderKilledFailover) {
	// Create a client
	std::shared_ptr<ClientTestWrapper> client = CreateClientWrapper("0.0.0.0:" + std::to_string(client_port_base));
