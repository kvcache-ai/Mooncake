...
// Command line flags
USE_engine_flags DEFINE_string(master_server_entry, "localhost:50051", "Master server address");
...
private:
	...
	void HandleMount(std::istringstream &iss) {
		...
		if (it->second.segments.find(segment_name) != it->second.segments.end()) {
			std::cout << "Segment " << segment_name << " already mounted" << std::endl;
			return;
		}
	...
	void HandleSleep(std::istringstream &iss) {
		...
		if (seconds <= 0) {
			std::cout << "Invalid sleep command format. Expected: sleep [seconds]" << std::endl;
			return;
		}
