...
ClientTestWrapper::ClientTestWrapper(std::shared_ptr<Client> client, std::shared_ptr<SimpleAllocator> allocator)
...
ClientTestWrapper::CreateClientWrapper(const std::string &hostname, const std::string &metadata_connstring,
	...
	auto client_opt = Client::Create(hostname, // Local hostname
	                                 metadata_connstring, protocol, args, master_server_entry);

	if (!client_opt.has_value()) {
		return std::nullopt;
	}

	std::shared_ptr<SimpleAllocator> allocator = std::make_shared<SimpleAllocator>(local_buffer_size);
	if (!allocator) {
	...
	auto register_result =
	    client_opt.value()->RegisterLocalMemory(allocator->getBase(), local_buffer_size, "cpu:0", false, false);
	ErrorCode error_code = register_result.has_value() ? ErrorCode::OK : register_result.error();
	if (error_code != ErrorCode::OK) {
		LOG(ERROR) << "register_local_memory_failed base=" << allocator->getBase() << " size=" << local_buffer_size
		           << ", error=" << error_code;
		return std::nullopt;
	}
...
ErrorCode ClientTestWrapper::Mount(const size_t size, void *&buffer) {
	...
	if (error_code != ErrorCode::OK) {
		...
		segments_.emplace(reinterpret_cast<uintptr_t>(buffer), SegmentInfo {buffer, size});
		return ErrorCode::OK;
	}
}
...
ErrorCode ClientTestWrapper::Get(const std::string &key, std::string &value) {
	...
	// Create slices
	std::vector<AllocatedBuffer::Descriptor> &descriptors = replica_list[0].get_memory_descriptor().buffer_descriptors;
...
ErrorCode ClientTestWrapper::Put(const std::string &key, const std::string &value) {
	// Create slices
...
ClientTestWrapper::SliceGuard::SliceGuard(std::vector<AllocatedBuffer::Descriptor> &descriptors,
...
ClientTestWrapper::SliceGuard::SliceGuard(size_t size, std::shared_ptr<SimpleAllocator> allocator)
    : allocator_(allocator) {
	while (size != 0) {
