...
// Command line flags
USE_engine_flags;
DEFINE_string(master_server_entry, "etcd://0.0.0.0:2379", "Master server entry");
...
public:
	void Run() {
		...
		const int kOpProbTotal = FLAGS_put_prob + FLAGS_get_prob + FLAGS_mount_prob + FLAGS_unmount_prob;
...
private:
	bool CreateClient() {
		...
		auto client_opt = ClientTestWrapper::CreateClientWrapper(hostname, FLAGS_engine_meta_url, FLAGS_protocol,
	...
	void Put() {
		...
		if (error_code != ErrorCode::OK) {
			LOG(INFO) << TEST_PUT_FAILURE_STR << " key=" << key << " value=" << value
			          << " error=" << toString(error_code);
		} else {
			LOG(INFO) << TEST_PUT_SUCCESS_STR << " key=" << key << " value=" << value;
		}
	}
	...
	void Get() {
		...
		if (error_code != ErrorCode::OK) {
			LOG(INFO) << TEST_GET_FAILURE_STR << " key=" << key << " error=" << toString(error_code);
			return;
		}

		if (!verify_kv(key, value)) {
			LOG(ERROR) << TEST_ERROR_STR << " key=" << key << " value=" << value;
			return;
		} else {
			LOG(INFO) << TEST_GET_SUCCESS_STR << " key=" << key << " value=" << value;
		}
	}
	...
	void Mount() {
		...
		if (error_code != ErrorCode::OK) {
			LOG(INFO) << TEST_MOUNT_FAILURE_STR << " error=" << toString(error_code);
			return;
		} else {
			segments_.emplace_back(buffer);
			LOG(INFO) << TEST_MOUNT_SUCCESS_STR << " buffer=" << buffer << " size=" << kSegmentSize;
		}
	}
	...
	void Unmount() {
		...
		if (error_code != ErrorCode::OK) {
			LOG(INFO) << TEST_UNMOUNT_FAILURE_STR << " error=" << toString(error_code);
		} else {
