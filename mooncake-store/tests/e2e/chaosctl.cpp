...
void generate_testing_report(int master_num, int client_num, const std::string &out_dir) {
	std::stringstream report;
	std::map<std::string, int> total_counts;

	// Define the output order of TEST_xxx_STR
	std::vector<std::string> test_strings = {
	    mooncake::testing::TEST_ERROR_STR,          mooncake::testing::TEST_PUT_SUCCESS_STR,
	    mooncake::testing::TEST_PUT_FAILURE_STR,    mooncake::testing::TEST_GET_SUCCESS_STR,
	    mooncake::testing::TEST_GET_FAILURE_STR,    mooncake::testing::TEST_MOUNT_SUCCESS_STR,
	    mooncake::testing::TEST_MOUNT_FAILURE_STR,  mooncake::testing::TEST_UNMOUNT_SUCCESS_STR,
	    mooncake::testing::TEST_UNMOUNT_FAILURE_STR};

	// master's log only has the following test strings
	std::vector<std::string> master_test_strings = {mooncake::testing::TEST_ERROR_STR};
	...
	for (int i = 0; i < master_num; ++i) {
		std::string filename = out_dir + "/master_" + std::to_string(i) + ".err";
	...
	for (int i = 0; i < client_num; ++i) {
		std::string filename = out_dir + "/client_" + std::to_string(i) + ".err";
		auto counts = count_test_string(filename);
	...
	if (!FLAGS_skip_run) {
		...
		for (int i = 0; i < master_num; ++i) {
			masters.emplace_back(std::make_unique<mooncake::testing::MasterProcessHandler>(
			    FLAGS_master_path, FLAGS_etcd_endpoints, 50051 + i, i, FLAGS_out_dir));
			masters.back()->start();
		}
		...
		for (int i = 0; i < client_num; ++i) {
			...
			clients.emplace_back(std::make_unique<mooncake::testing::ClientProcessHandler>(
		...
		while (true) {
			...
			if (elapsed_seconds >= FLAGS_run_sec) {
				LOG(INFO) << "Chaos testing completed after " << elapsed_seconds << " seconds";
				break;
			}
