...
TEST_F(E2ERandTest, RandomSequentialDeletePutGet) {
	...
	while (true) {
		...
		for (int i = 0; i < kv_range; ++i) {
			...
			if (clients[rand() % client_num]->Get(key, get_value) == ErrorCode::OK && get_value != value) {
				...
				for (size_t i = 0; i < get_value.size(); i++) {
					if (get_value[i] != value[i]) {
						std::string val_substr = value.substr(i, std::min(10, static_cast<int>(get_value.size() - i)));
						std::string get_substr =
						    get_value.substr(i, std::min(10, static_cast<int>(get_value.size() - i)));
						LOG(ERROR) << "Get value mismatch at position " << i << ": " << get_substr
						           << " != " << val_substr;
