...
MasterProcessHandler::MasterProcessHandler(const std::string &path, const std::string &etcd_endpoints, const int port,
                                           const int index, const std::string &out_dir)
    : master_path_(path), etcd_endpoints_(etcd_endpoints), port_(port), index_(index), out_dir_(out_dir) {
}
...
bool MasterProcessHandler::start() {
	if (master_pid_ != 0) {
		LOG(ERROR) << "[m" << index_ << "] Master process already running with PID: " << master_pid_;
		return false;
	}

	// Create output directory if it doesn't exist
	if (mkdir(out_dir_.c_str(), 0755) != 0 && errno != EEXIST) {
		LOG(ERROR) << "[m" << index_ << "] Failed to create output directory: " << out_dir_
	...
	if (pid == -1) {
		LOG(ERROR) << "[m" << index_ << "] Failed to fork process for master: " << strerror(errno);
		return false;
	}
	...
	if (pid == 0) {
		...
		if (stdout_fd == -1) {
			LOG(ERROR) << "[m" << index_ << "] Failed to open stdout file: " << stdout_file.str()
			           << ", error: " << strerror(errno);
			exit(1);
		}
		...
		if (stderr_fd == -1) {
			LOG(ERROR) << "[m" << index_ << "] Failed to open stderr file: " << stderr_file.str()
			           << ", error: " << strerror(errno);
			close(stdout_fd);
			exit(1);
		}
		...
		if (dup2(stdout_fd, STDOUT_FILENO) == -1) {
			LOG(ERROR) << "[m" << index_ << "] Failed to redirect stdout: " << strerror(errno);
			close(stdout_fd);
			close(stderr_fd);
			exit(1);
		}
		...
		if (dup2(stderr_fd, STDERR_FILENO) == -1) {
			LOG(ERROR) << "[m" << index_ << "] Failed to redirect stderr: " << strerror(errno);
			close(stdout_fd);
			close(stderr_fd);
			exit(1);
		}
		...
		execl(master_path_.c_str(), master_path_.c_str(), "--enable-ha=true",
		      ("--etcd-endpoints=" + etcd_endpoints_).c_str(), rpc_address_arg.c_str(), rpc_port_arg.c_str(), nullptr);

		// If execl returns, it means there was an error
		LOG(ERROR) << "[m" << index_ << "] Master process terminated with error: " << strerror(errno);
		exit(1);
	} else {
		// Parent process - store the PID
		master_pid_ = pid;
		LOG(INFO) << "[m" << index_ << "] Started master process with PID: " << pid;
...
bool MasterProcessHandler::kill() {
	...
	if (::kill(master_pid_, SIGKILL) == 0) {
		LOG(INFO) << "[m" << index_ << "] Force killed master process with PID: " << master_pid_
		          << " (simulating crash)";
		...
		LOG(ERROR) << TEST_ERROR_STR << " [m" << index_ << "] Failed to kill master process with PID: " << master_pid_
		           << ": " << strerror(errno);
		success = false;
	}
...
ClientProcessHandler::ClientProcessHandler(const std::string &path, const int index, const std::string &out_dir,
...
bool ClientProcessHandler::start() {
	if (client_pid_ != 0) {
		LOG(ERROR) << "[c" << index_ << "] Client process already running with PID: " << client_pid_;
		return false;
	}

	// Create output directory if it doesn't exist
	if (mkdir(out_dir_.c_str(), 0755) != 0 && errno != EEXIST) {
		LOG(ERROR) << "[c" << index_ << "] Failed to create output directory: " << out_dir_
		           << ", error: " << strerror(errno);
		return false;
	}
	...
	if (pid == -1) {
		LOG(ERROR) << "[c" << index_ << "] Failed to fork process for client: " << strerror(errno);
		return false;
	}
	...
	if (pid == 0) {
		...
		int stdout_fd = open(stdout_file.str().c_str(), open_flags, 0644);
		if (stdout_fd == -1) {
			LOG(ERROR) << "[c" << index_ << "] Failed to open stdout file: " << stdout_file.str()
			           << ", error: " << strerror(errno);
			exit(1);
		}
		...
		int stderr_fd = open(stderr_file.str().c_str(), open_flags, 0644);
		if (stderr_fd == -1) {
			LOG(ERROR) << "[c" << index_ << "] Failed to open stderr file: " << stderr_file.str()
			           << ", error: " << strerror(errno);
			close(stdout_fd);
			exit(1);
		}
		...
		// Redirect stdout and stderr
		if (dup2(stdout_fd, STDOUT_FILENO) == -1) {
			LOG(ERROR) << "[c" << index_ << "] Failed to redirect stdout: " << strerror(errno);
			close(stdout_fd);
			close(stderr_fd);
			exit(1);
		}
		...
		if (dup2(stderr_fd, STDERR_FILENO) == -1) {
			LOG(ERROR) << "[c" << index_ << "] Failed to redirect stderr: " << strerror(errno);
			close(stdout_fd);
			close(stderr_fd);
			exit(1);
		}
		...
		if (config_.put_prob.has_value()) {
			args.push_back("--put-prob=" + std::to_string(config_.put_prob.value()));
		}
		if (config_.get_prob.has_value()) {
			args.push_back("--get-prob=" + std::to_string(config_.get_prob.value()));
		}
		if (config_.mount_prob.has_value()) {
			args.push_back("--mount-prob=" + std::to_string(config_.mount_prob.value()));
		}
		if (config_.unmount_prob.has_value()) {
			args.push_back("--unmount-prob=" + std::to_string(config_.unmount_prob.value()));
		}
		if (config_.port.has_value()) {
			args.push_back("--port=" + std::to_string(config_.port.value()));
		}
		if (config_.master_server_entry.has_value()) {
			args.push_back("--master-server-entry=" + config_.master_server_entry.value());
		}
		if (config_.engine_meta_url.has_value()) {
			args.push_back("--engine-meta-url=" + config_.engine_meta_url.value());
		}
		...
		LOG(ERROR) << "[c" << index_ << "] Client process terminated with error: " << strerror(errno);
		exit(1);
	} else {
		// Parent process - store the PID
		client_pid_ = pid;
		LOG(INFO) << "[c" << index_ << "] Started client process with PID: " << pid;
		// Mark that this is no longer the first start
		first_start_ = false;
...
bool ClientProcessHandler::kill() {
	...
	if (::kill(client_pid_, SIGKILL) == 0) {
		LOG(INFO) << "[c" << index_ << "] Force killed client process with PID: " << client_pid_
		          << " (simulating crash)";
		...
		LOG(ERROR) << TEST_ERROR_STR << " [c" << index_ << "] Failed to kill client process with PID: " << client_pid_
		           << ": " << strerror(errno);
		success = false;
	}
