...
protected:
	...
	static std::shared_ptr<ClientTestWrapper> CreateClientWrapper(const std::string &host_name) {
		auto client_opt =
		    ClientTestWrapper::CreateClientWrapper(host_name,             // Local hostname
		                                           FLAGS_engine_meta_url, // Metadata connection string
		                                           FLAGS_protocol, FLAGS_device_name, "etcd://" + FLAGS_etcd_endpoints);
...
TEST_F(ChaosRandTest, RandomMasterCrashWithSmallValue) {
	...
	static_assert(segment_size >= kv_range * 128 * 2, "The segment size is too small, the eviction may happen");
	...
	for (int i = 0; i < master_num; ++i) {
		masters.emplace_back(std::make_unique<mooncake::testing::MasterProcessHandler>(
		    FLAGS_master_path, FLAGS_etcd_endpoints, master_port_base + i, i, FLAGS_out_dir));
	...
	for (int i = 0; i < client_num; ++i) {
		clients.emplace_back(CreateClientWrapper("0.0.0.0:" + std::to_string(client_port_base + i)));
	...
	while (true) {
		...
		for (int key_i = 0; key_i < kv_range; ++key_i) {
			...
			if (err == ErrorCode::OK || err == ErrorCode::OBJECT_ALREADY_EXISTS) {
				...
				ASSERT_EQ(clients[rand() % client_num]->Get(key, get_value), ErrorCode::OK);
...
TEST_F(ChaosRandTest, RandomMasterCrashWithLargeValue) {
	...
	static_assert(value_size * kv_range >= segment_size * client_num, "The total value size is too small");
			...
			for (int key_i = 0; key_i < kv_range; ++key_i) {
				...
				if (clients[rand() % client_num]->Get(key, get_value) == ErrorCode::OK) {
					if (get_value != value) {
						...
						ASSERT_EQ(get_value.substr(get_value.size() - 20), value.substr(value.size() - 20));
