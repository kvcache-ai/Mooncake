...
TEST(DfsGlobalAllocatorTest, StaleFreeSizeDoesNotReleaseSameKeyReuse) {
    ...
    ASSERT_NE(current_descriptor->aligned_size, old_descriptor->aligned_size);
...
TEST(DfsGlobalAllocatorTest, RejectsPaddedLegacyShardsWithoutSidecars) {
    EnvGuard env;
    ConfigurePosixDfs(env);
