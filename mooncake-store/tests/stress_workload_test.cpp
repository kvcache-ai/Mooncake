...
DEFINE_string(device_name, "erdma_0", "Device name to use, valid if protocol=rdma");
...
DEFINE_uint64(ram_buffer_size_gb, 15, "RAM buffer size in GB for segment allocation");
DEFINE_uint64(client_buffer_allocator_size_mb, 256, "Client buffer allocator size in MB");

// Network configuration flags
DEFINE_string(local_hostname, "localhost:12345", "Local hostname for client");
DEFINE_string(metadata_connection_string, "P2PHANDSHAKE", "Metadata connection string");
...
bool initialize_segment() {
	...
	if (!g_segment_ptr) {
		LOG(ERROR) << "Failed to allocate segment memory of size " << FLAGS_ram_buffer_size_gb << "GB";
		return false;
	}
	...
	LOG(INFO) << "Segment initialized successfully with " << FLAGS_ram_buffer_size_gb << "GB RAM buffer";
	return true;
}

void cleanup_segment() {
	if (g_segment_ptr && g_client) {
		auto result = g_client->UnmountSegment(g_segment_ptr, g_ram_buffer_size);
		if (!result.has_value()) {
...
bool initialize_client() {
	...
	auto client_opt = Client::Create(FLAGS_local_hostname,             // Local hostname
	...
	auto client_buffer_allocator_size = FLAGS_client_buffer_allocator_size_mb * 1024 * 1024;
	g_client_buffer_allocator = std::make_unique<SimpleAllocator>(client_buffer_allocator_size);

	auto result = g_client->RegisterLocalMemory(g_client_buffer_allocator->getBase(), client_buffer_allocator_size,
	                                            "cpu:0", false, false);
	...
	if (total_required_memory > client_buffer_allocator_size) {
		LOG(ERROR) << "Insufficient buffer allocator memory. Required: " << total_required_memory / (1024 * 1024)
		           << "MB, Available: " << FLAGS_client_buffer_allocator_size_mb << "MB";
		return false;
	}

	LOG(INFO) << "Client initialized successfully with " << FLAGS_client_buffer_allocator_size_mb
	          << "MB buffer allocator";
	return true;
}
...
std::string generate_key(int thread_id, uint64_t operation_id) {
	return "key_" + std::to_string(thread_id) + "_" + std::to_string(operation_id);
}

void worker_thread(int thread_id, std::atomic<bool> &stop_flag, ThreadStats &stats) {
	// Allocate thread-local buffer
	void *write_buffer = g_client_buffer_allocator->allocate(FLAGS_value_size);
	if (!write_buffer) {
		LOG(ERROR) << "Thread " << thread_id << ": Failed to allocate write buffer";
		return;
	}
	...
	slices.emplace_back(Slice {write_buffer, static_cast<size_t>(FLAGS_value_size)});
	...
	for (int i = 0; i < FLAGS_test_operation_nums && !stop_flag.load(); ++i) {
		...
		stats.operations.push_back({static_cast<double>(latency_us), true, success});
	...
	for (int i = 0; i < FLAGS_test_operation_nums && !stop_flag.load() && !stored_keys.empty(); ++i) {
		...
		bool success = result.has_value();
		stats.operations.push_back({static_cast<double>(latency_us), false, success});
...
void calculate_percentiles(std::vector<double> &latencies, double &p50, double &p90, double &p95, double &p99) {
	if (latencies.empty()) {
		p50 = p90 = p95 = p99 = 0.0;
		return;
	}
...
void print_results(const std::vector<ThreadStats> &thread_stats, double duration_s) {
	...
	double put_data_throughput_mb_s = (total_put_ops * FLAGS_value_size) / (duration_s * 1024 * 1024);
	double get_data_throughput_mb_s = (total_get_ops * FLAGS_value_size) / (duration_s * 1024 * 1024);
	double total_data_throughput_mb_s = put_data_throughput_mb_s + get_data_throughput_mb_s;
	...
	LOG(INFO) << "All Operations - P50: " << all_p50 << ", P90: " << all_p90 << ", P95: " << all_p95
	          << ", P99: " << all_p99;

	if (!put_latencies.empty()) {
		LOG(INFO) << "PUT Operations - P50: " << put_p50 << ", P90: " << put_p90 << ", P95: " << put_p95
		          << ", P99: " << put_p99;
	}

	if (!get_latencies.empty()) {
		LOG(INFO) << "GET Operations - P50: " << get_p50 << ", P90: " << get_p90 << ", P95: " << get_p95
		          << ", P99: " << get_p99;
	}
}
...
int main(int argc, char **argv) {
	...
	LOG(INFO) << "Protocol: " << FLAGS_protocol << ", Device: " << FLAGS_device_name;
	...
	LOG(INFO) << "Client buffer allocator size: " << FLAGS_client_buffer_allocator_size_mb << "MB";
	...
	LOG(INFO) << "Starting " << FLAGS_num_threads << " worker threads with " << FLAGS_test_operation_nums
	          << " operations each";
	...
	for (int i = 0; i < FLAGS_num_threads; ++i) {
		workers.emplace_back(worker_thread, i, std::ref(stop_flag), std::ref(thread_stats[i]));
	}
