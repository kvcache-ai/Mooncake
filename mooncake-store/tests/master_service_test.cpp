...
TEST_F(MasterServiceTest, PutStartEndFlow) {
    ...
    EXPECT_EQ(put_end_fail_result.error(), ErrorCode::INVALID_PARAMS);
    ...
    EXPECT_EQ(put_revoke_fail_result.error(), ErrorCode::INVALID_PARAMS);
...
TEST_F(MasterServiceTest, PutStartMonitorTest) {
    ...
    // Wait for a while until key_2 get discarded and released.
