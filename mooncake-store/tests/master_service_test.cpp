...
std::string GenerateKeyForSegment(const std::unique_ptr<MasterService> &service, const std::string &segment_name) {
	...
	while (true) {
		...
		if (code != ErrorCode::OK) {
			throw std::runtime_error("PutStart failed with code: " + std::to_string(static_cast<int>(code)));
		}
...
TEST_F(MasterServiceTest, ConcurrentGarbageCollectionTest) {
	...
	constexpr size_t buffer = 0x300000000;
	constexpr size_t size = 1024 * 1024 * 256; // Larger segment for concurrent use
		...
		for (size_t t = 0; t < num_threads; t++) {
			create_threads.emplace_back([&, t]() {
				for (size_t i = 0; i < objects_per_thread; i++) {
					std::string key = "concurrent_gc_key_" + std::to_string(t) + "_" + std::to_string(i);
					std::vector<uint64_t> slice_lengths = {1024};
					ReplicateConfig config;
					config.replica_num = 1;
					std::vector<Replica::Descriptor> replica_list;
...
TEST_F(MasterServiceTest, UnmountSegmentImmediateCleanup) {
	...
	ASSERT_EQ(replica_list[0].get_memory_descriptor().buffer_descriptors[0].segment_name_, segment2.name);
}
...
TEST_F(MasterServiceTest, UnmountSegmentPerformance) {
	...
	auto unmount_duration = std::chrono::duration_cast<std::chrono::milliseconds>(unmount_end - unmount_start);

	// Unmount operation should be very fast, so we set 1s limit
	EXPECT_LE(unmount_duration.count(), 1000)
	    << "Unmount operation took " << unmount_duration.count() << "ms which exceeds 1 second limit";
	...
	auto total_create_duration = std::chrono::duration_cast<std::chrono::milliseconds>(create_end - start);
	std::cout << "\nPerformance Metrics:\n"
...
TEST_F(MasterServiceTest, RemoveSoftPinObject) {
	...
	std::unique_ptr<MasterService> service_(
	    new MasterService(false, kv_lease_ttl, kv_soft_pin_ttl, allow_evict_soft_pinned_objects));
	// Mount segment and put an object
	constexpr size_t buffer = 0x300000000;
	constexpr size_t size = 1024 * 1024 * 16;
	std::string segment_name = "test_segment";
...
TEST_F(MasterServiceTest, SoftPinObjectsNotEvictedBeforeOtherObjects) {
	...
	const bool allow_evict_soft_pinned_objects = true;
	std::unique_ptr<MasterService> service_(
	    new MasterService(false, kv_lease_ttl, kv_soft_pin_ttl, allow_evict_soft_pinned_objects, eviction_ratio));
	...
	for (int test_i = 0; test_i < 5; test_i++) {
		...
		for (int i = 0; i < 2; i++) {
			...
			ASSERT_TRUE(service_->PutStart(pin_key, slice_lengths, soft_pin_config).has_value());
...
TEST_F(MasterServiceTest, SoftPinExtendedOnGet) {
	...
	for (int test_i = 0; test_i < 3; test_i++) {
		...
		for (int i = 0; i < 2; i++) {
			...
			ASSERT_TRUE(service_->PutStart(pin_key, slice_lengths, soft_pin_config));
			ASSERT_TRUE(service_->PutEnd(pin_key).has_value());
		}
...
TEST_F(MasterServiceTest, BatchExistKeyTest) {
	...
	for (int i = 0; i < test_object_num; ++i) {
		...
		config.replica_num = 1;
		std::vector<uint64_t> slice_lengths = {value_size};
		auto put_start_result = service_->PutStart(test_keys[i], slice_lengths, config);
		ASSERT_TRUE(put_start_result.has_value());
