...
std::string GenerateKeyForSegment(const std::unique_ptr<MasterService>& service,
    ...
    while (true) {
        ...
        auto put_end_result = service->PutEnd(key, {PutResult::SUCCESS});
...
TEST_F(MasterServiceTest, PutStartEndFlow) {
    ...
    // Test PutEnd
    auto put_end_result = service_->PutEnd(key, {PutResult::SUCCESS});
...
TEST_F(MasterServiceTest, RandomPutStartEndFlow) {
    ...
    auto put_end_result = service_->PutEnd(
        key, std::vector<PutResult>(random_number, PutResult::SUCCESS));
    EXPECT_TRUE(put_end_result.has_value());
    // Verify replica list after PutEnd
...
TEST_F(MasterServiceTest, MultiSliceMultiReplicaFlow) {
    ...
    auto put_end_result = service_->PutEnd(
        key, std::vector<PutResult>(replica_list.size(), PutResult::SUCCESS));
    ASSERT_TRUE(put_end_result.has_value());
        ...
        for (size_t t = 0; t < num_threads; t++) {
            create_threads.emplace_back([&, t]() {
                for (size_t i = 0; i < objects_per_thread; i++) {
                    ...
                    ASSERT_TRUE(put_start_result.has_value());
                    auto put_end_result =
                        service_->PutEnd(key, {PutResult::SUCCESS});
                    ASSERT_TRUE(put_end_result.has_value());
...
TEST_F(MasterServiceTest, CleanupStaleHandlesTest) {
    ...
    auto put_end_result2 = service_->PutEnd(key2, {PutResult::SUCCESS});
...
TEST_F(MasterServiceTest, UnmountSegmentImmediateCleanup) {
    ...
    ASSERT_TRUE(put_start_result.has_value());
    replica_list = put_start_result.value();
    auto put_end_result = service_->PutEnd(key1, {PutResult::SUCCESS});
    ASSERT_TRUE(put_end_result.has_value());
...
TEST_F(MasterServiceTest, RemoveLeasedObject) {
    ...
    ASSERT_TRUE(put_start_result2.has_value());
    auto put_end_result2 = service_->PutEnd(key, {PutResult::SUCCESS});
    ASSERT_TRUE(put_end_result2.has_value());
    ...
    auto put_end_result3 = service_->PutEnd(key, {PutResult::SUCCESS});
    ...
    auto put_end_result4 = service_->PutEnd(key, {PutResult::SUCCESS});
...
TEST_F(MasterServiceTest, RemoveSoftPinObject) {
    ...
    ASSERT_TRUE(service_->PutEnd(key, {PutResult::SUCCESS}).has_value());
    ...
    for (int test_i = 0; test_i < 5; test_i++) {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            ASSERT_TRUE(
                service_->PutStart(pin_key, slice_lengths, soft_pin_config)
                    .has_value());
            ASSERT_TRUE(
                service_->PutEnd(pin_key, {PutResult::SUCCESS}).has_value());
        }
        ...
        for (int i = 0; i < 20; i++) {
            ...
            if (service_->PutStart(key, slice_lengths, config).has_value()) {
                ASSERT_TRUE(
                    service_->PutEnd(key, {PutResult::SUCCESS}).has_value());
            } else {
                failed_puts++;
            }
        }
...
TEST_F(MasterServiceTest, BatchExistKeyTest) {
    ...
    for (int i = 0; i < test_object_num; ++i) {
        ...
        ASSERT_TRUE(put_start_result.has_value());
        auto put_end_result =
            service_->PutEnd(test_keys[i], {PutResult::SUCCESS});
        ASSERT_TRUE(put_end_result.has_value());
    }
