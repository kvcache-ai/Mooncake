...
TEST_F(MasterServiceTest, SoftPinObjectsNotEvictedBeforeOtherObjects) {
    ...
    for (int test_i = 0; test_i < 5; test_i++) {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            ASSERT_EQ(ErrorCode::OK,
                      service_->PutStart(pin_key, value_size, slice_lengths,
                                         soft_pin_config, replica_list));
        ...
        for (int i = 0; i < 20; i++) {
            ...
            ReplicateConfig config;
            config.replica_num = 1;
            if (ErrorCode::OK == service_->PutStart(key, value_size,
        ...
        for (int i = 0; i < 2; i++) {
            ...
            std::vector<Replica::Descriptor> replica_list;
            ASSERT_EQ(ErrorCode::OK,
                      service_->GetReplicaList(pin_key, replica_list));
        }
...
TEST_F(MasterServiceTest, BatchExistKeyTest) {
    ...
    for (int i = 0; i < test_object_num; ++i) {
        ...
        std::vector<Replica::Descriptor> replica_list;
        ASSERT_EQ(ErrorCode::OK,
                  service_->PutStart(test_keys[i], value_size, slice_lengths,
                                     config, replica_list));
