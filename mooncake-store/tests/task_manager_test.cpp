...
TEST_F(ClientTaskManagerTest, SubmitAndPopTask) {
    ...
    UUID task_id = manager.get_write_access().submit_task(
...
TEST_F(ClientTaskManagerTest, PruningLogic) {
    ...
    for (size_t i = 0; i < max_tasks + 2; ++i) {
        UUID id = manager.get_write_access().submit_task(
            client_id, TaskType::REPLICA_COPY, "payload");
...
TEST_F(ClientTaskManagerTest, MultipleClients) {
    ...
    UUID id1 = manager.get_write_access().submit_task(
        client1, TaskType::REPLICA_COPY, "p1");
    UUID id2 = manager.get_write_access().submit_task(
        client2, TaskType::REPLICA_COPY, "p2");
