...
TEST_F(ClientTaskManagerTest, SubmitAndPopTask) {
    ...
    UUID task_id = manager.get_access().submit_task(client_id, TaskType::REPLICA_COPY, payload);
    
    auto tasks = manager.get_access().pop_tasks(client_id, 10);
...
TEST_F(ClientTaskManagerTest, TaskStatusLifecycle) {
    ...
    // Initially pending
    auto task_opt = manager.get_access().find_task_by_id(task_id);
    ASSERT_TRUE(task_opt.has_value());
    EXPECT_EQ(task_opt->status, TaskStatus::PENDING);
    
    // Pop to make it processing
    manager.get_access().pop_tasks(client_id, 1);
    task_opt = manager.get_access().find_task_by_id(task_id);
    EXPECT_EQ(task_opt->status, TaskStatus::PROCESSING);
    
    // Mark success
    manager.get_access().mark_success(client_id, task_id);
    task_opt = manager.get_access().find_task_by_id(task_id);
...
TEST_F(ClientTaskManagerTest, PruningLogic) {
    ...
    for (size_t i = 0; i < max_tasks + 2; ++i) {
        UUID id = manager.get_access().submit_task(client_id, TaskType::REPLICA_COPY, "payload");
        task_ids.push_back(id);
        manager.get_access().pop_tasks(client_id, 1);
        manager.get_access().mark_success(client_id, id);
    }
    
    // The first 2 tasks should have been pruned
    EXPECT_FALSE(manager.get_access().find_task_by_id(task_ids[0]).has_value());
    EXPECT_FALSE(manager.get_access().find_task_by_id(task_ids[1]).has_value());
    
    // The last 5 tasks should still exist
    for (size_t i = 2; i < task_ids.size(); ++i) {
        EXPECT_TRUE(manager.get_access().find_task_by_id(task_ids[i]).has_value());
    }
}
...
TEST_F(ClientTaskManagerTest, MultipleClients) {
    ...
    UUID id1 = manager.get_access().submit_task(client1, TaskType::REPLICA_COPY, "p1");
    UUID id2 = manager.get_access().submit_task(client2, TaskType::REPLICA_COPY, "p2");
    
    auto tasks1 = manager.get_access().pop_tasks(client1, 10);
    ASSERT_EQ(tasks1.size(), 1);
    EXPECT_EQ(tasks1[0].id, id1);
    
    auto tasks2 = manager.get_access().pop_tasks(client2, 10);
    ASSERT_EQ(tasks2.size(), 1);
    EXPECT_EQ(tasks2[0].id, id2);
    
    // Cross check: client1 shouldn't get client2's tasks
    auto tasks1_again = manager.get_access().pop_tasks(client1, 10);
...
TEST_F(ClientTaskManagerTest, MarkFailed) {
    ...
    manager.get_access().pop_tasks(client_id, 1);
    manager.get_access().mark_failed(client_id, task_id, "error occurred");
