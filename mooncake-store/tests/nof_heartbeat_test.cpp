   ...
   protected:
    ...
    std::unique_ptr<MasterService> CreateService(int64_t heartbeat_interval_sec,
        ...
        auto config = MasterServiceConfig::builder()
                          ...
                          .set_nof_heartbeat_interval_sec(
                              heartbeat_interval_sec)
                          .set_nof_heartbeat_probe_timeout_ms(
                              probe_timeout_ms)
                          .set_nof_heartbeat_failures_threshold(
                              failure_threshold)
                          .build();
...
TEST_F(NoFHeartbeatTest, FailureCountResetsAfterRecovery) {
    ...
    ASSERT_TRUE(WaitForCondition(std::chrono::milliseconds(5000),
                                 std::chrono::milliseconds(50), [&]() {
                                     auto failure_count = service
                                                              ->GetNoFHeartbeatFailureCountForTesting(
                                                                  segment.id);
                                     return probe_calls.load() >= 4 &&
                                            failure_count.has_value() &&
                                            *failure_count == 0;
                                 }));
    EXPECT_TRUE(service->IsNoFSegmentMountedForTesting(segment.id));
}
...
TEST_F(NoFHeartbeatTest, OnlyFailedSegmentIsUnmounted) {
    ...
    NoFSegment bad_segment = MakeNoFSegment("nof_seg_bad", "nof_bad",
                                           kDefaultNoFSegmentBase +
    ...
    ASSERT_TRUE(WaitForCondition(std::chrono::milliseconds(5000),
                                 std::chrono::milliseconds(50), [&]() {
                                     return service->GetMountedNoFSegmentCountForTesting() ==
                                                1u;
                                 }));
