        ...
        producers.emplace_back([&, producer_id = i]() {
            for (int j = 0; j < items_per_producer; ++j) {
                ...
                if (!queue.push(item, std::chrono::milliseconds(1))) {
                    ...
                    if (!queue.push(item, std::chrono::milliseconds(1))) {
                        LOG(ERROR) << "Push failed after retry for item " << item;
                        break;
                    }
                }
            }
...
TEST_F(ThreadSafeQueueTest, PopBatch_ExactBatchSize) {
    ...
    auto batch_opt = queue.pop_batch(batch_size, std::chrono::milliseconds(100));
            ...
            while (total_popped.load() < total_items) {
                ...
                if (batch_opt.has_value()) {
                    ...
                    total_popped += batch.size();
                    {
                        ...
                        all_popped.insert(all_popped.end(), batch.begin(), batch.end());
                    }
                } else {
...
TEST_F(ThreadSafeQueueTest, BatchOperationsPerformance) {
    ...
    auto fill_time = std::chrono::duration_cast<std::chrono::milliseconds>(end_fill - start_fill);
    LOG(INFO) << "Filled " << num_items << " items in " << fill_time.count() << "ms";
    ...
    while (total_popped < num_items) {
        auto batch_opt = queue.pop_batch(batch_size, std::chrono::milliseconds(1000));
        if (batch_opt.has_value()) {
            auto batch = batch_opt.value();
            total_popped += batch.size();
    ...
    auto pop_time = std::chrono::duration_cast<std::chrono::milliseconds>(end_pop - start_pop);
...
TEST_F(ThreadSafeQueueTest, HighContentionMPSC_PerformanceAndAccuracy) {
    ...
    std::thread consumer([&]() {
        ...
        while (consecutive_empty_cycles < MAX_EMPTY_CYCLES) {
            ...
            if (batch_opt.has_value()) {
                ...
                auto batch_time_ns = std::chrono::duration_cast<std::chrono::nanoseconds>(
                ...
                while (current_size > max_observed && 
                       !stats.max_observed_size.compare_exchange_weak(max_observed, current_size)) {
                }
        ...
        auto consumer_duration_ms = std::chrono::duration_cast<std::chrono::milliseconds>(
            consumer_end - consumer_start
        ).count();
        
        LOG(INFO) << "Consumer thread completed: " << stats.total_consumed.load() 
                  << " events, " << stats.consumer_batches_processed.load() 
    ...
    for (int producer_id = 0; producer_id < NUM_PRODUCERS; ++producer_id) {
        producers.emplace_back([&, producer_id]() {
            ...
            for (int event_id = 0; event_id < NUM_EVENTS_PER_PRODUCER; ++event_id) {
                ...
                int64_t push_duration_ns = std::chrono::duration_cast<std::chrono::nanoseconds>(
                    push_end - push_start
                ).count();
                ...
                if (pushed) {
                    ...
                    LOG(WARNING) << "Producer " << producer_id << " push failed, retrying...";
                    ...
                    if (pushed) {
                        ...
                        local_failures++;
                        LOG(ERROR) << "Producer " << producer_id << " failed to push event " << event_id;
                    }
                }
            ...
            auto producer_duration_ms = std::chrono::duration_cast<std::chrono::milliseconds>(
                producer_end - producer_start
            ).count();
            
            LOG(INFO) << "Producer " << producer_id << " completed: " 
                      << local_events_produced << " events, "
                      << local_block_count << " blocks, "
    ...
    auto producer_only_duration_ms = std::chrono::duration_cast<std::chrono::milliseconds>(
    ...
    double events_per_second_producer = TOTAL_EVENTS / producer_duration_seconds;
    
    int64_t actual_produced = stats.total_produced.load();
    int64_t actual_consumed = stats.total_consumed.load();
    
    double avg_push_time_ns = stats.producer_total_push_time_ns / static_cast<double>(actual_produced);
    double avg_batch_time_ns = stats.consumer_total_time_ns / static_cast<double>(stats.consumer_batches_processed);
    double avg_batch_size = actual_consumed / static_cast<double>(stats.consumer_batches_processed);
    
    double blocking_percentage = stats.producer_block_count * 100.0 / actual_produced;
    double failure_percentage = stats.producer_failures * 100.0 / (actual_produced + stats.producer_failures);
    ...
    EXPECT_EQ(stats.events_out_of_range.load(), 0) << "Events out of range found";
    
    LOG(INFO) << "Performance Metrics:";
    LOG(INFO) << "  Total duration: " << total_duration_ms << "ms";
    LOG(INFO) << "  Producer duration: " << producer_only_duration_ms << "ms";
    LOG(INFO) << "  Total throughput: " << events_per_second_total << " events/sec";
    LOG(INFO) << "  Producer throughput: " << events_per_second_producer << " events/sec";
    LOG(INFO) << "  Average push time: " << avg_push_time_ns << " ns";
    LOG(INFO) << "  Average batch processing time: " << avg_batch_time_ns << " ns";
    ...
    LOG(INFO) << "  Total consumer batches: " << stats.consumer_batches_processed.load();
    ...
    if (blocking_percentage > 5.0) {
        LOG(WARNING) << "High blocking ratio, consider adjusting queue capacity or number of producers";
    }
    
    LOG(INFO) << "Resource Usage Summary:";
    LOG(INFO) << "  Estimated memory usage: " << (max_size * sizeof(int)) << " bytes";
    LOG(INFO) << "  Producer threads: " << NUM_PRODUCERS;
    LOG(INFO) << "  Consumer threads: 1";
    LOG(INFO) << "  Peak queue capacity usage: " << (max_size * 100.0 / QUEUE_CAPACITY) << "%";
    
    EXPECT_LE(blocking_percentage, 20.0) << "Blocking ratio too high";

    if (avg_batch_size < 10) {
        LOG(WARNING) << "Batch size too small(" << avg_batch_size << "< 10), consumer efficiency low";
    }
