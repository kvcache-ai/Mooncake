...
TEST_F(HighAvailabilityTest, EtcdBasicOperations) {
	...
	for (size_t i = 0; i < keys.size(); i++) {
		...
		ASSERT_EQ(ErrorCode::OK,
		          EtcdHelper::CreateWithLease(key.c_str(), key.size(), value.c_str(), value.size(), lease_id, version));
	...
	ASSERT_EQ(ErrorCode::OK, EtcdHelper::CreateWithLease(watch_key.c_str(), watch_key.size(), watch_value.c_str(),
