...
protected:
	...
	void ValidateMountedSegments(const SegmentManager &segment_manager, const std::vector<Segment> &segments,
		...
		for (size_t i = 0; i < client_ids.size(); i++) {
			auto client_it = segment_manager.client_segments_.find(client_ids[i]);
			ASSERT_NE(client_it, segment_manager.client_segments_.end());
			auto segment_it = std::find(client_it->second.begin(), client_it->second.end(), segments[i].id);
			ASSERT_NE(segment_it, client_it->second.end());
			ASSERT_EQ(*segment_it, segments[i].id);

			ASSERT_NE(segment_manager.mounted_segments_.find(segments[i].id), segment_manager.mounted_segments_.end());
			MountedSegment seg = segment_manager.mounted_segments_.at(segments[i].id);
		...
		for (const auto &segment : segments) {
			MountedSegment mounted_segment = segment_manager.mounted_segments_.at(segment.id);
			auto allocator = mounted_segment.buf_allocator;

			// validate allocators_
			ASSERT_NE(std::find(segment_manager.allocators_.begin(), segment_manager.allocators_.end(),
			                    mounted_segment.buf_allocator),
			          segment_manager.allocators_.end());

			// validate allocators_by_name
			auto map_it = segment_manager.allocators_by_name_.find(segment.name);
			ASSERT_NE(map_it, segment_manager.allocators_by_name_.end());
			auto name_allocator_it = map_it->second.begin();
			for (; name_allocator_it != map_it->second.end(); name_allocator_it++) {
	...
	void ValidateMountedSegment(const SegmentManager &segment_manager, const Segment segment, const UUID &client_id) {
...
TEST_F(SegmentTest, MountSegmentDuplicate) {
	...
	ASSERT_EQ(segment_access.MountSegment(segment, client_id), ErrorCode::SEGMENT_ALREADY_EXISTS);
...
TEST_F(SegmentTest, UnmountSegmentSuccess) {
	...
	ASSERT_EQ(segment_access.PrepareUnmountSegment(segment.id, metrics_dec_capacity), ErrorCode::OK);
	ASSERT_EQ(metrics_dec_capacity, segment.size);

	// Commit unmount
	ASSERT_EQ(segment_access.CommitUnmountSegment(segment.id, client_id, metrics_dec_capacity), ErrorCode::OK);

	// Verify segment is unmounted correctly
	std::vector<Segment> empty_segment_vec;
	std::vector<UUID> empty_client_ids_vec;
	ValidateMountedSegments(segment_manager, empty_segment_vec, empty_client_ids_vec);
}
...
TEST_F(SegmentTest, UnmountSegmentDuplicate) {
	...
	ASSERT_EQ(segment_access.PrepareUnmountSegment(segment.id, metrics_dec_capacity), ErrorCode::SEGMENT_NOT_FOUND);
...
TEST_F(SegmentTest, ReMountSegmentSuccess) {
	...
	ASSERT_EQ(segment_access.ReMountSegment(segments_to_remount, client_id), ErrorCode::OK);
...
TEST_F(SegmentTest, ReMountUnmountingSegment) {
	...
	size_t metrics_dec_capacity = 0;
	ASSERT_EQ(segment_access.PrepareUnmountSegment(segment_a.id, metrics_dec_capacity), ErrorCode::OK);

	// Attempt to remount segment A while it's in UNMOUNTING state
	std::vector<Segment> segments_to_remount = {segment_a};
	ASSERT_EQ(segment_access.ReMountSegment(segments_to_remount, client_id), ErrorCode::UNAVAILABLE_IN_CURRENT_STATUS);

	// Complete the unmount process
	ASSERT_EQ(segment_access.CommitUnmountSegment(segment_a.id, client_id, metrics_dec_capacity), ErrorCode::OK);
...
TEST_F(SegmentTest, QuerySegments) {
	...
	for (int i = 0; i < 10; i++) {
		...
		segment.size = 1024 * 1024 * 16;
		segment.base = 0x100000000 + (i * 0x100000000); // Different base addresses
	...
	for (size_t i = 0; i < client_ids.size(); i++) {
		...
		ASSERT_EQ(segment_access.GetClientSegments(client_ids[i], client_segments), ErrorCode::OK);

		// Verify correct number of segments
		ASSERT_EQ(client_segments.size(), 1);

		// Verify all expected segments are present
		ASSERT_EQ(client_segments[0].id, expected_client_segments[client_ids[i]]);
	}
	...
	for (const auto &segment : segments) {
		ASSERT_NE(std::find(all_segments.begin(), all_segments.end(), segment.name), all_segments.end());
	}

	// Test QuerySegments for each segment
	for (const auto &segment : segments) {
		size_t used = 0, capacity = 0;
		ASSERT_EQ(segment_access.QuerySegments(segment.name, used, capacity), ErrorCode::OK);
	...
	size_t used = 0, capacity = 0;
	ASSERT_EQ(segment_access.QuerySegments("non_existent_segment", used, capacity), ErrorCode::SEGMENT_NOT_FOUND);
	ASSERT_EQ(used, 0);
