...
class MasterServiceSnapshotTestBase : public ::testing::Test {
   protected:
    ...
    struct LocalDiskSegmentState {
        ...
        std::map<std::string, int64_t> offloading_objects;  // key -> timestamp (sorted)
    };
    ...
    struct ServiceStateSnapshot {
        ...
        std::unordered_map<std::string, std::pair<size_t, size_t>> segment_stats;
    ...
    ServiceStateSnapshot CaptureServiceState(MasterService* service) const {
        ...
        // === LocalDiskSegment State (via friend access) ===
        {
            ...
            for (const auto& [client_id, segment] : access.getClientLocalDiskSegment()) {
                ...
                for (const auto& [key, timestamp] : segment->offloading_objects) {
    ...
    bool CompareReplicaDescriptor(const Replica::Descriptor& a, const Replica::Descriptor& b) const {
        ...
        if (a.is_memory_replica()) {
            ...
            return ma.buffer_descriptor.size_ == mb.buffer_descriptor.size_ &&
                   ma.buffer_descriptor.buffer_address_ == mb.buffer_descriptor.buffer_address_ &&
                   ma.buffer_descriptor.transport_endpoint_ == mb.buffer_descriptor.transport_endpoint_;
        }
        if (a.is_disk_replica()) {
            const auto& da = a.get_disk_descriptor();
            const auto& db = b.get_disk_descriptor();
            return da.file_path == db.file_path && da.object_size == db.object_size;
        }
    ...
    bool CompareReplicaListResponse(const GetReplicaListResponse& a, const GetReplicaListResponse& b) const {
    ...
    bool CompareLocalDiskSegmentState(const LocalDiskSegmentState& a, const LocalDiskSegmentState& b) const {
    ...
    bool CompareServiceState(const ServiceStateSnapshot& before, const ServiceStateSnapshot& after) const {
        ...
        if (before.local_disk_segments.size() != after.local_disk_segments.size()) {
            ...
            for (const auto& [client_id, seg_state] : before.local_disk_segments) {
        ...
        if (before.all_keys != after.all_keys) {
            LOG(ERROR) << "all_keys mismatch. before.size=" << before.all_keys.size()
                      << ", after.size=" << after.all_keys.size();
            all_passed = false;
        }
        
        if (before.all_segments != after.all_segments) {
            LOG(ERROR) << "all_segments mismatch. before.size=" << before.all_segments.size()
    ...
    // Assert that PutStart results are consistent between original and restored services.
    // The best_segment is pre-selected outside this function (segment with largest remaining space).
    // Logic:
    // - If both services allocate on best_segment: compare replica descriptors
    // - If neither allocates on best_segment: both entered random allocation, considered consistent
    ...
    void AssertPutStartConsistentWithBestSegment(
        ...
        MasterService* restored,
        const std::string& best_segment) const {
        if (best_segment.empty()) {
            LOG(WARNING) << "Skip PutStart consistency check: no best_segment provided";
            return;
        }

        LOG(INFO) << "[PutStart Consistency] Using best_segment: " << best_segment;
        ...
        ASSERT_EQ(before.has_value(), after.has_value())
            << "PutStart has_value mismatch between original and restored service";
        ...
        ASSERT_EQ(reps_before.size(), reps_after.size())
            << "PutStart replica count mismatch: ORIGINAL=" << reps_before.size()
            << ", RESTORED=" << reps_after.size();

        // Helper to get segment name from replica
        auto get_segment = [](const Replica::Descriptor& rep) -> std::string {
            if (rep.is_memory_replica()) {
                return rep.get_memory_descriptor().buffer_descriptor.transport_endpoint_;
            }
            return "";
        };
        ...
        LOG(INFO) << "[PutStart Consistency] ORIGINAL on best_segment: " << orig_on_best
                  << ", RESTORED on best_segment: " << rest_on_best;

        ASSERT_EQ(orig_on_best, rest_on_best)
            << "PutStart state inconsistency: ORIGINAL on best_segment=" << orig_on_best
            << ", RESTORED on best_segment=" << rest_on_best
        ...
        if (!orig_on_best && !rest_on_best) {
            ...
            LOG(INFO) << "[PutStart Consistency] Both services entered random allocation, considered consistent";
            return;
        }
        ...
        LOG(INFO) << "[PutStart Consistency] Check passed: both allocated on best_segment consistently";
    }

    // ==================== File Operation Methods ====================
    
    // Copy msgpack snapshot files to backup directory
    void CopySnapshotToBackup(const std::string& snapshot_id, const std::string& backup_id) const {
        ...
        if (ec) {
            LOG(ERROR) << "Failed to create backup directory: " << dst_dir.string() << ", error: " << ec.message();
            return;
        }
        
        fs::copy(src_dir, dst_dir, fs::copy_options::recursive | fs::copy_options::overwrite_existing, ec);
        if (ec) {
            LOG(ERROR) << "Failed to copy snapshot files from " << src_dir.string() << " to " << dst_dir.string() << ", error: " << ec.message();
        } else {
            LOG(INFO) << "Successfully copied snapshot from " << src_dir.string() << " to " << dst_dir.string();
        }
    }

    // Compare binary contents of two files
    bool CompareBinaryFiles(const std::string& file1, const std::string& file2) const {
        std::ifstream f1(file1, std::ios::binary | std::ios::ate);
        std::ifstream f2(file2, std::ios::binary | std::ios::ate);
        
        if (!f1.is_open() || !f2.is_open()) {
            LOG(ERROR) << "Failed to open files for comparison: " << file1 << " or " << file2;
            return false;
        }
        ...
        auto size2 = f2.tellg();
        if (size1 != size2) {
            LOG(ERROR) << "File sizes differ: " << file1 << " (" << size1 << " bytes) vs " 
                      << file2 << " (" << size2 << " bytes)";
            return false;
        }
    ...
    bool CompareSnapshotDirectories(const std::string& dir1, const std::string& dir2) const {
        LOG(INFO) << "Comparing snapshot directories: " << dir1 << " vs " << dir2;
        ...
        LOG(INFO) << "Compared " << file_count << " msgpack files, all match: " << all_match;
        return all_match;
    }
    ...
    void TestSnapshotAndRestore(std::unique_ptr<MasterService>& service) {
        ...
        ASSERT_TRUE(persist_result1.has_value()) << "Failed to persist state: " 
        ...
        // Inherit key configurations from original service (e.g., root_fs_dir for SSD support)
        ...
        auto persist_result2 = CallPersistState(restored_service.get(), snapshot_id2);
        ASSERT_TRUE(persist_result2.has_value()) << "Failed to persist restored state: " 
                                                  << persist_result2.error().message;
        
        // ========== Phase 5: Compare two metadata snapshots ==========
        bool snapshots_match = CompareSnapshotDirectories(GetBackupDir(backup_id), 
                                                           GetSnapshotDir(snapshot_id2));
        EXPECT_TRUE(snapshots_match);
        
        // ========== Phase 6: Capture state before snapshot ==========
        // Note: Capture state after snapshot to avoid internal state changes affecting comparison
        ...
        if (!best_segment.empty()) {
            ...
            LOG(WARNING) << "[TestSnapshotAndRestore] No valid segment found, skip PutStart consistency check";
        }
    }
    ...
    void TearDown() override {
        // Ensure service_ is properly initialized - if nullptr, likely a local variable shadowing issue
        ASSERT_NE(service_, nullptr) 
            << "service_ is nullptr in TearDown. "
            << "Check if test declared a local 'service_' variable that shadows the base class member. "
            << "Use 'service_.reset(new MasterService(...))' instead of 'std::unique_ptr<MasterService> service_(...)'";
        
        // Test snapshot and restore functionality for all test cases
        TestSnapshotAndRestore(service_);
        service_.reset();
        // Note: Do not call ShutdownGoogleLogging() here - glog only allows single init/shutdown cycle
    }
};
