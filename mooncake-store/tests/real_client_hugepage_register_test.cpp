...
TEST_F(RealClientHugepageRegisterTest, PreTouchPageAlignedBlocksOnLargeSegment) {
...
TEST_F(RealClientHugepageRegisterTest, HotCacheShmRegistersAlignedSegment) {
    ...
    setenv("MC_STORE_LOCAL_HOT_CACHE_SIZE", "34865152", 1);  // 33MB, not 2MB-multiple
    setenv("MC_STORE_LOCAL_HOT_BLOCK_SIZE", "3145728", 1);   // 3MB -> 11 blocks
    setenv("MC_STORE_LOCAL_HOT_CACHE_USE_SHM", "1", 1);

    auto client_opt = Client::Create("localhost:17819", "P2PHANDSHAKE",
                                     protocol_,
...
TEST_F(RealClientHugepageRegisterTest, MisalignedInteriorSubRangeRegistersExactly) {
    const size_t kSegmentSize = 8 * 1024 * 1024;
    void* base = ShmHelper::getInstance()->allocate(kSegmentSize);
    ASSERT_NE(base, nullptr);
