    ...
    if (warp_idx < kNumScaleupWarps) {
                ...
                for (int i = 0; i < kNumScaleupRanksPerLane; ++ i) {
                    if (const auto j = i * 32 + lane_idx; i < (kNumScaleupRanksPerLane - 1) or j < kNumScaleupRanks) {
                        ...
                        if (stored_num_tokens_sent[i] != stored_old_num_tokens_sent[i])
                            ptx::st_release_sys(gin.get_sym_ptr<transport::ScaleupTeam>(tail_ptr, j), stored_num_tokens_sent[i]);
        ...
        while (true) {
            ...
            while (wip_mask) {
                ...
                token_buffer.set_base_ptr(gin.get_sym_ptr<transport::ScaleupTeam>(token_buffer.get_base_ptr(), dst_scaleup_rank_idx));
                    ...
                    for (int k = 0; k < kNumTopk; ++ k) {
                        ...
                        if (ptx::elect_one_sync()) {
                            ...
                            const auto dst_token_buffer = scaleup_buffer
                                ...
                                .get_token_buffer(src_scaleout_rank_idx * kNumMaxTokensPerRank + src_token_idx);
                            ptx::tma_store_1d(
                                gin.get_sym_ptr<transport::ScaleupTeam>(dst_token_buffer.get_base_ptr(), dst_scaleup_rank_idx),
                    ...
                    for (int k = 0; k < kNumTopk; ++ k) {
                        ...
                        if (src_scaleout_rank_idx != scaleout_rank_idx) {
                            gin.put<transport::ScaleoutTeam>(
                                ...
                                topk_valid_mask == 0 and is_token_last_in_chunk ? 0 : 0
                            );
                        }
                    }
                }
