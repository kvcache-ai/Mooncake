...
#define MOONCAKE_EP_COPY_UNROLL 7
#else
#define MOONCAKE_EP_COPY_UNROLL 7
#endif
...
dispatch(void* packed_recv_x, float* packed_recv_x_scales,
    ...
    if (responsible_expert_idx < num_experts) {
        ...
        for (int i = sub_warp_id; i < num_recv_tokens; i += kNumWarpsPerGroup) {
            ...
            UNROLLED_WARP_COPY(MOONCAKE_EP_COPY_UNROLL, lane_id, hidden_int4, dst_data, src_data, mc_ld_nc, mc_st_na);
...
void dispatch(void* packed_recv_x, float* packed_recv_x_scales,
    ...
    // Keep enough data groups per CTA to amortize the count group. The
    // previous SM-derived choice selected three groups on S5000, leaving only
    // two groups for token copies while launching 96 CTAs for 288 experts.
    // Six groups reduce the grid to 48 CTAs and leave five data groups.
    num_warp_groups = 6;
#endif
    ...
    if (responsible_expert_idx < num_experts) {
        ...
        for (int token_idx = offset + sub_warp_id; token_idx < offset + num_tokens_to_send; token_idx += kNumWarpsPerGroup) {
            ...
            if (write_dst != nullptr) {
                // Local or P2P path — warp-cooperative copy
                const auto dst_int4_ptr = reinterpret_cast<int4*>(write_dst);
                    UNROLLED_WARP_COPY(MOONCAKE_EP_COPY_UNROLL, lane_id, hidden_bf16_int4, dst_int4_ptr, x_int4, mc_ld_nc, mc_st_na);
#ifndef MOONCAKE_EP_USE_MUSA
                mc_fence();
#endif
            } else {
                ...
                if (not zero_copy)
                    UNROLLED_WARP_COPY(MOONCAKE_EP_COPY_UNROLL, lane_id, hidden_bf16_int4, buf_int4_ptr, x_int4, mc_ld_nc, mc_st_na);
                __syncwarp();
#if defined(MOONCAKE_EP_USE_MACA) || defined(MOONCAKE_EP_USE_MUSA)
