...
static __device__ void __mlx5gda_device_poll_cq(struct mlx5gda_qp_devctx* ctx,
                                                uint16_t expect) {
...
static __device__ void __mlx5gda_device_post_send_db(
    ...
    struct mlx5gda_wqebb* last_wqe =
        ctx->wq + ((num_posted_wqe - 1) & ctx->wqeid_mask);
    // printf("Last wqe=%lx -> bf=%p\n", *(uint64_t*)last_wqe, ctx->bf +
    // ctx->bf_offset);
    ptx_st64_rel_sys_na((uint64_t*)(ctx->bf + ctx->bf_offset),
                        *(uint64_t*)last_wqe);
    // 3. toggle bf
...
static __device__ void __mlx5gda_device_write_rdma_write_wqe(
    struct mlx5gda_qp_devctx* ctx, uint64_t laddr, __be32 lkey, uint64_t raddr,
    ...
    ctrl_seg.opmod_idx_opcode =
        device_byteswap(((uint32_t)ctx->wq_head << 8) | MLX5_OPCODE_RDMA_WRITE);
...
static __device__ void __mlx5gda_device_write_rdma_atomic_add_wqe(
    ...
    struct mlx5gda_rdma_atomic_wqe* wqe =
        (mlx5gda_rdma_atomic_wqe*)(ctx->wq + (ctx->wq_head & ctx->wqeid_mask));
    struct mlx5_wqe_ctrl_seg& ctrl_seg = wqe->ctrl;
    struct mlx5_wqe_raddr_seg& raddr_seg = wqe->raddr;
    struct mlx5_wqe_data_seg& data_seg = wqe->data;
    ...
    ctrl_seg.fm_ce_se = MLX5_WQE_CTRL_CQ_UPDATE;
    ctrl_seg.opmod_idx_opcode =
        device_byteswap(MLX5_OPCODE_ATOMIC_MASKED_FA |
...
__global__
__launch_bounds__(kNumWarpGroups* kNumWarpsPerGroup * 32, 1) void dispatch(
    void* packed_recv_x, float* packed_recv_x_scales, int* packed_recv_src_info,
    int64_t* packed_recv_layout_range, int* packed_recv_count,
    int32_t* active_ranks, void* mxa_buffer, int* rdma_send_signal_buffer,
    int* rdma_recv_signal_buffer, void* rdma_send_data_buffer,
    void* rdma_recv_data_buffer, void* cuda_counter_buffer,
    void* cuda_data_buffer, void* raddrs, void* rkeys, void* qp_devctxs,
    const int32_t* nvlink_available, void* const* ipc_peer_ptrs, const void* x,
    const int64_t* topk_idx, int* atomic_counter_per_expert,
    int* atomic_finish_counter_per_expert, int* next_clean_buffer,
    int num_tokens, int num_max_dispatch_tokens_per_rank, int num_topk,
    int num_experts, int rank, int num_ranks, int64_t timeout_ticks,
    int phases) {
    ...
    if (warp_id < num_warps - 1) {
        ...
        for (int token_idx = sm_id; token_idx < num_tokens;
            ...
            const auto rdma_x_src_idx = reinterpret_cast<int*>(
                reinterpret_cast<uint8_t*>(rdma_send_data_buffer) +
                token_idx * num_bytes_per_msg);
            ...
            auto dst_expert_idx =
                warp_id < num_topk
                    ? static_cast<int>(
                          __ldg(topk_idx + token_idx * num_topk + warp_id))
                    : -1;
...
#pragma unroll
            for (int i = thread_id; i < hidden_bf16_int4; i += num_threads) {
                ...
                if (kUseFP8) {
                    ...
                    auto bf16_values =
                        reinterpret_cast<nv_bfloat16*>(&int4_value);
...
#pragma unroll
                    for (int j = 0; j < kNumElemsPerRead; j += 2) {
                        ...
                        fp8x2_values[j / 2] = __nv_cvt_float2_to_fp8x2(
            ...
            if (dst_expert_idx >= 0) {
                ...
                const auto dst_ptr =
                    ...
                    rank * num_max_dispatch_tokens_per_rank *
                        num_bytes_per_msg +
                ...
                if (dst_rank != rank) {
                    ...
                    if (use_nvlink) {
                        ...
                        void* peer_dst_ptr =
                            (char*)ipc_peer_ptrs[dst_rank] + offset;
                        // NOTES: only 2 load iterations for 7K hidden with 8
                        // unrolls
                        const auto* src_int4_ptr =
                            reinterpret_cast<const int4*>(src_ptr);
                        const auto* dst_int4_ptr =
                            reinterpret_cast<int4*>(peer_dst_ptr);
                        UNROLLED_WARP_COPY(8, lane_id, num_int4_per_msg,
                                           dst_int4_ptr, src_int4_ptr,
                                           ld_nc_global, st_na_global);
                    } else {
                        if (lane_id == 0) {
                            uint64_t req_rptr_actual =
                                raddr_array[dst_rank] +
                            ...
                            __mlx5gda_device_write_rdma_write_wqe(
                                ctx, src_ptr, device_byteswap(rkey_array[rank]),
                                req_rptr_actual,
...
#pragma unroll
    ...
    if (responsible_expert_idx < num_experts and sub_warp_id == 0 and
        ...
        if (dst_rank != rank) {
            bool use_nvlink = nvlink_available[dst_rank] != 0;
            if (use_nvlink) {
                int* signal_ptr = rdma_recv_signal_buffer +
                                  dst_expert_local_idx * num_ranks + rank;
                size_t offset = (char*)signal_ptr - (char*)(mxa_buffer);
                int* peer_signal_ptr =
                    (int*)((char*)ipc_peer_ptrs[dst_rank] + offset);
                st_na_release(peer_signal_ptr, -num_tokens_sent - 1);
            } else {
                uint64_t laddr =
                    (uint64_t)((char*)(raddr_array[rank]) +
                               ((char*)(rdma_send_signal_buffer +
                                        dst_expert_local_idx * num_ranks +
                                        rank) -
                                (char*)(mxa_buffer)));
                uint64_t rptr_actual =
                    (uint64_t)((char*)(raddr_array[dst_rank]) +
                               ((char*)(rdma_recv_signal_buffer +
                                        dst_expert_local_idx * num_ranks +
                                        rank) -
                                (char*)(mxa_buffer)));
                auto ctx = ctx_array + dst_rank * num_qp_per_rank +
                           dst_expert_local_idx % num_qp_per_rank;
                device_mutex_lock_system(&ctx->mutex);
                __mlx5gda_device_write_rdma_atomic_add_wqe(
                    ctx, -num_tokens_sent - 1, laddr,
                    device_byteswap(rkey_array[rank]), rptr_actual,
                    device_byteswap(rkey_array[dst_rank]));
                __mlx5gda_device_post_send_db(ctx);
                device_mutex_unlock_system(&ctx->mutex);
            }
        } else {
            st_na_release(rdma_recv_signal_buffer +
                              dst_expert_local_idx * num_ranks + rank,
                          -num_tokens_sent - 1);
        }
...
LOW_LATENCY_DISPATCH_RECV:
    ...
    if (phases & LOW_LATENCY_SEND_PHASE) cooperative_groups::this_grid().sync();
    ...
    if (responsible_expert_idx < num_experts) {
        ...
        if (sub_warp_id == 1 and lane_id == 0) {
            ...
            while ((num_recv_tokens = ld_acquire_sys_global(
                        rdma_recv_signal_buffer + local_expert_idx * num_ranks +
                        src_rank)) == 0) {
                unsigned long long end_time = clock64();
                if (timeout_ticks != -1 &&
                    end_time - start_time > timeout_ticks) {
        ...
        for (int i = sub_warp_id; i < num_recv_tokens; i += kNumWarpsPerGroup) {
            ...
            if (kUseFP8) {
                ...
                const auto dst_scales = reinterpret_cast<float*>(
                    recv_x_scales + recv_token_begin_idx + i);
                const auto scale_stride =
                    num_ranks * num_max_dispatch_tokens_per_rank;
                auto scale_0 = lane_id < num_scales
                ...
                auto scale_1 = (lane_id + 32) < num_scales
                                   ...
                                   : 0;
                lane_id < num_scales
                    ? dst_scales[lane_id * scale_stride] = scale_0
                    : 0.0f;
...
#define DISPATCH_LAUNCH_CASE(hidden)                                           \
    {                                                                          \
        ...
        LAUNCH_KERNEL(                                                         \
            ...
            active_ranks, mxa_buffer, rdma_send_signal_buffer,                 \
            rdma_recv_signal_buffer, rdma_send_data_buffer,                    \
            rdma_recv_data_buffer, cuda_counter_buffer, cuda_data_buffer,      \
            raddrs, rkeys, qp_devctxs, nvlink_available, ipc_peer_ptrs, x,     \
            topk_idx, atomic_counter_per_expert,                               \
            atomic_finish_counter_per_expert, next_clean_buffer, num_tokens,   \
            num_max_dispatch_tokens_per_rank, num_topk, num_experts, rank,     \
            num_ranks, timeout_ticks, phases);                                 \
    }                                                                          \
    break
...
__global__ __launch_bounds__(
    ...
    1) void combine(void* combined_x, int32_t* active_ranks, void* mxa_buffer,
                    int* rdma_send_signal_buffer, int* rdma_recv_signal_buffer,
                    void* rdma_send_data_buffer, void* rdma_recv_data_buffer,
                    ...
                    int* atomic_clean_flag, int num_combined_tokens, int hidden,
                    int num_topk, int num_max_dispatch_tokens_per_rank,
...
#pragma unroll
    ...
    if (responsible_expert_idx < num_experts) {
        ...
        for (int token_idx = offset + sub_warp_id;
            ...
            if (dst_rank == rank) {
                const auto dst_int4_ptr = reinterpret_cast<int4*>(dst_ptr);
                UNROLLED_WARP_COPY(7, lane_id, hidden_bf16_int4, dst_int4_ptr,
                                   x_int4, ld_nc_global, st_na_global);
            } else {
                bool use_nvlink = nvlink_available[dst_rank] != 0;
                if (use_nvlink) {
                    size_t offset = (char*)dst_ptr - (char*)(mxa_buffer);
                    ...
                    UNROLLED_WARP_COPY(7, lane_id, hidden_bf16_int4,
                                       dst_int4_ptr, x_int4, ld_nc_global,
                                       st_na_global);
                } else {
                    const auto buf_int4_ptr = reinterpret_cast<int4*>(buf_ptr);
                    if (not zero_copy)
                        UNROLLED_WARP_COPY(7, lane_id, hidden_bf16_int4,
                                           buf_int4_ptr, x_int4, ld_nc_global,
                                           st_na_global);
                    __syncwarp();
                    ...
                    if (lane_id == 0) {
                        ...
                        device_mutex_lock_system(&ctx->mutex);
                        __mlx5gda_device_write_rdma_write_wqe(
                            ctx, (uint64_t)buf_ptr,
                            device_byteswap(rkey_array[rank]), req_rptr_actual,
                            device_byteswap(rkey_array[dst_rank]),
                            num_bytes_per_slot);
                        __mlx5gda_device_post_send_db(ctx);
                        device_mutex_unlock_system(&ctx->mutex);
        ...
        if (sub_warp_id == 1 and lane_id == 0) {
            ...
            if (dst_rank != rank) {
                ...
                if (use_nvlink) {
                    ...
                    device_mutex_lock_system(&ctx->mutex);
                    __mlx5gda_device_write_rdma_atomic_add_wqe(
                        ctx, 1, laddr, device_byteswap(rkey_array[rank]),
...
LOW_LATENCY_COMBINE_RECV:
    ...
    if (responsible_expert_idx < num_experts) {
        ...
        if (sub_warp_id == 0 and lane_id == 0) {
            unsigned long long start_time = clock64();
            while (ld_acquire_sys_global(rdma_recv_signal_buffer +
...
#pragma unroll
            for (int i = 0; i < num_topk; ++i) {
                reg_topk_idx[i] = static_cast<int>(
                    __ldg(topk_idx + token_idx * num_topk + i));
                reg_topk_weights[i] =
                    __ldg(topk_weights + token_idx * num_topk + i);
            }
...
#pragma unroll
            ...
            auto combined_bf16 =
                reinterpret_cast<nv_bfloat16*>(&combined_values);
#pragma unroll
            for (int j = 0; j < kNumElemsPerInt4; ++j)
...
#define COMBINE_LAUNCH_CASE(hidden)                                            \
    {                                                                          \
        ...
        LAUNCH_KERNEL(                                                         \
            &cfg, combine_func, combined_x, active_ranks, mxa_buffer,          \
            ...
            ipc_peer_ptrs, x, topk_idx, topk_weights, src_info, layout_range,  \
            next_clean_buffer, atomic_clean_flag, num_combined_tokens, hidden, \
            num_topk, num_max_dispatch_tokens_per_rank, num_experts, rank,     \
            num_ranks, timeout_ticks, phases, zero_copy);                      \
    }                                                                          \
    break
