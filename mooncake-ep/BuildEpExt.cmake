...
message(STATUS "[EP] CUDA_HOME=$ENV{CUDA_HOME}")
if(EP_USE_MUSA)
...
if("${EP_TORCH_VERSIONS}" STREQUAL "")
  ...
  execute_process(
    COMMAND bash -c "export CUDA_HOME=/usr/local/cuda && ${Python3_EXECUTABLE} setup.py build_ext --build-lib ."
  ...
  foreach(_version IN LISTS EP_TORCH_VERSIONS)
    ...
    execute_process(
      COMMAND bash -c "export CUDA_HOME=/usr/local/cuda && ${Python3_EXECUTABLE} setup.py build_ext --build-lib . --force"
      WORKING_DIRECTORY "${SOURCE_DIR}"
      RESULT_VARIABLE _ret
    )
    if(NOT _ret EQUAL 0)
