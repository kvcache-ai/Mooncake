...
struct BufferPair {
    ...
    BufferPair(void* rdma_buffer, int num_max_dispatch_tokens_per_rank, int hidden, int num_ranks, int num_experts, size_t bytes_reserved) {
        total_bytes = bytes_reserved;
        size_t signaling_buffer_bytes = num_experts * sizeof(int);
        size_t send_recv_buffer_bytes = num_experts * num_max_dispatch_tokens_per_rank * (2 * sizeof(int4) + hidden * sizeof(nv_bfloat16));
        for (int i = 0; i < 2; ++ i) {
            size_t rdma_base_offset = total_bytes + 2 * i * signaling_buffer_bytes + 2 * i * send_recv_buffer_bytes;
            buffers[i] = {
                advance<int*>(rdma_buffer, rdma_base_offset),
                advance<int*>(rdma_buffer, rdma_base_offset + signaling_buffer_bytes),
                advance<int*>(rdma_buffer, rdma_base_offset + 2 * signaling_buffer_bytes),
                advance<int*>(rdma_buffer, rdma_base_offset + 2 * signaling_buffer_bytes + send_recv_buffer_bytes),
                advance<void*>(rdma_buffer, rdma_base_offset + 2 * signaling_buffer_bytes + 2 * send_recv_buffer_bytes),
                advance<void*>(rdma_buffer, rdma_base_offset + 3 * signaling_buffer_bytes + 2 * send_recv_buffer_bytes),
            };
        }
...
public:
    Buffer(int rank, int num_ranks, int64_t num_mxa_bytes, size_t bytes_reserved):
           rank(rank), num_ranks(num_ranks), num_mxa_bytes(num_mxa_bytes), bytes_reserved(bytes_reserved),
        ...
        CUDA_CHECK(cudaMalloc(&qp_devctxs, num_ranks * sizeof(mlx5gda_qp_devctx)));
        init_ibgda();
    ...
    dispatch(const torch::Tensor& x, const torch::Tensor& topk_idx, torch::Tensor& broken_nodes,
        ...
        // Buffer control
        BufferPair layout(gdr_buffer, num_max_dispatch_tokens_per_rank, hidden, num_ranks, num_experts, bytes_reserved);
        ...
        auto packed_recv_count = torch::zeros({num_local_experts}, torch::dtype(torch::kInt32).device(torch::kCUDA));
        ...
        int64_t timeout_ticks = timeout_us == -1 ? -1 : (int64_t) clock_rate_khz * (int64_t) timeout_us / 1000;

        auto launcher = [=](int phases) {
            cudaMemsetAsync(buffer.cuda_counter_buffer, 0, num_experts * sizeof(int), launch_stream);
            mxa_ep::dispatch(packed_recv_x.data_ptr(), packed_recv_x_scales_ptr,
                             packed_recv_src_info.data_ptr<int>(), packed_recv_layout_range.data_ptr<int64_t>(),
                             packed_recv_count.data_ptr<int>(), broken_nodes.data_ptr<int32_t>(),
                             gdr_buffer,
                             buffer.rdma_send_signal_buffer, buffer.rdma_recv_signal_buffer,
                             buffer.rdma_send_data_buffer, buffer.rdma_recv_data_buffer,
                             buffer.cuda_counter_buffer, buffer.cuda_data_buffer,
                             raddrs, rkeys, qp_devctxs,
                             x.data_ptr(), topk_idx.data_ptr<int64_t>(),
                             next_buffer.rdma_recv_signal_buffer,
                             num_tokens, hidden, num_max_dispatch_tokens_per_rank,
                             num_topk, num_experts, rank, num_ranks, use_fp8,
                             workspace, launch_stream, timeout_ticks, phases);
        };
    ...
    combine(const torch::Tensor& x, const torch::Tensor& topk_idx, const torch::Tensor& topk_weights,
            const torch::Tensor& src_info, const torch::Tensor& layout_range, torch::Tensor& gathered_experts,
            int num_max_dispatch_tokens_per_rank, int num_experts, int timeout_us,
        ...
        // Kernel launch
        auto launcher = [=](int phases) {
            mxa_ep::combine(combined_x.data_ptr(), gathered_experts.data_ptr<int32_t>(),
                            gdr_buffer,
                            buffer.rdma_send_signal_buffer, buffer.rdma_recv_signal_buffer,
                            buffer.rdma_send_data_buffer, buffer.rdma_recv_data_buffer,
                            buffer.cuda_counter_buffer, buffer.cuda_data_buffer,
    ...
    torch::Tensor
    get_next_combine_buffer(int num_max_dispatch_tokens_per_rank, int hidden, int num_experts) {
        BufferPair layout(gdr_buffer, num_max_dispatch_tokens_per_rank, hidden, num_ranks, num_experts, bytes_reserved);

        auto buffer = layout.buffers[buffer_idx];
        auto dtype = torch::kBFloat16;
        size_t num_bytes_per_combine_msg = hidden * sizeof(nv_bfloat16);
        auto num_msg_elems = static_cast<int>(num_bytes_per_combine_msg / elementSize(torch::kBFloat16));

        EP_HOST_ASSERT(num_bytes_per_combine_msg % elementSize(torch::kBFloat16) == 0);
        return torch::from_blob(buffer.rdma_send_data_buffer,
    ...
    void all_reduce_without(const torch::Tensor& broken_nodes, torch::Tensor& x) {
        auto mxa_buffer = reinterpret_cast<int*>(gdr_buffer);
        int size = x.numel();
        cudaStream_t stream = at::cuda::getCurrentCUDAStream();
        cudaMemsetAsync(mxa_buffer, 0, (1 + size) * num_ranks * sizeof(int), stream);
        mxa_ep::all_reduce_without(
            broken_nodes.data_ptr<int32_t>(), x.data_ptr<int>(), mxa_buffer,
            raddrs, rkeys, qp_devctxs,
            size, rank, num_ranks, stream);
    }

    void init_ibgda() {
        std::ifstream config("gpu_to_nic.txt");
        if (!config) throw std::runtime_error("Cannot open config file gpu_to_nic.txt");
        ...
        for (size_t lineno = 1; std::getline(config, line); ++lineno) {
            ...
            int gpu, nic;
            if (!(iss >> gpu >> nic)) {
                if (iss.rdbuf()->in_avail() == 0) continue; // blank or comment line
                throw std::runtime_error("Parse error in gpu_to_nic.txt line " + std::to_string(lineno));
            }
            if (!gpu_to_nic.emplace(gpu, nic).second)
                throw std::runtime_error("Duplicate GPU id on line " + std::to_string(lineno));
        }
        ...
        mr = ibv_reg_mr(pd, gdr_buffer, num_mxa_bytes, IBV_ACCESS_LOCAL_WRITE | IBV_ACCESS_REMOTE_READ | IBV_ACCESS_REMOTE_WRITE | IBV_ACCESS_REMOTE_ATOMIC);
        if (!mr) {
        ...
        mlx5dv_devx_umem *ctrl_buf_umem = mlx5dv_devx_umem_reg(ctx, ctrl_buf, ctrl_buf_size, IBV_ACCESS_LOCAL_WRITE);
        if (!ctrl_buf_umem) {
            perror("Failed to register control buffer as umem");
            fprintf(stderr, "If the error is `Bad address`, probably because your GPU does not support GPUDirect RDMA.\n");
            exit(1);
        }
        ...
        for (int i = 0; i < num_ranks; ++i) {
            mlx5gda_qp *qp = mlx5gda_create_rc_qp(mpd, ctrl_buf, ctrl_buf_umem, ctrl_buf_heap, pd, 16384, 1);
            if (!qp) {
            ...
            cudaMemcpy(qp_devctxs + i * sizeof(mlx5gda_qp_devctx), &qp_devctx, sizeof(mlx5gda_qp_devctx), cudaMemcpyHostToDevice);
            qps.push_back(qp);
        }
    }

    void sync(const std::vector<int64_t> &remote_addrs, const std::vector<int32_t> &remote_keys, const std::vector<int32_t> &remote_qpns, const std::vector<int64_t> &subnet_prefixes, const std::vector<int64_t> &interface_ids) {
        for (int i = 0; i < num_ranks; ++i) {
            ibv_gid remote_gid{};
            ...
            if (mlx5gda_modify_rc_qp_init2rtr(qps[i], ah_attr, (uint32_t)remote_qpns[i], IBV_MTU_4096)) {
            ...
            uint64_t raddr = i == rank ? (uint64_t)mr->addr : (uint64_t)remote_addrs[i];
            cudaMemcpy(raddrs + i * sizeof(uint64_t), &raddr, sizeof(uint64_t), cudaMemcpyHostToDevice);
            uint32_t rkey = i == rank ? mr->lkey : (uint32_t)remote_keys[i];
            cudaMemcpy(rkeys + i * sizeof(uint32_t), &rkey, sizeof(uint32_t), cudaMemcpyHostToDevice);
        }
    }
    ...
    std::tuple<int64_t, int64_t> get_gid() {
        return {(int64_t) gid.global.subnet_prefix, (int64_t)gid.global.interface_id};
    }

    std::vector<torch::Tensor> get_local_qpns() {
        std::vector<torch::Tensor> local_qpns;
        for (int i = 0; i < num_ranks; ++i) {
            local_qpns.push_back(torch::full({1}, qps[i]->qpn, torch::dtype(torch::kInt32).device(torch::kCUDA)));
        }
        return local_qpns;
    }
    ...
    std::vector<torch::Tensor> get_local_lids() {
        ...
        for (int i = 0; i < num_ranks; ++i) {
            local_lids.push_back(torch::full({1}, qps[i]->port_attr.lid, torch::dtype(torch::kInt32).device(torch::kCUDA)));
        }
        return local_lids;
    }
};

size_t get_mxa_size_hint(int num_max_dispatch_tokens_per_rank, int hidden, int num_ranks, int num_experts, size_t bytes_reserved) {
    return BufferPair(nullptr, num_max_dispatch_tokens_per_rank, hidden, num_ranks, num_experts, bytes_reserved).total_bytes;
}
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    pybind11::class_<mxa_ep::Buffer>(m, "Buffer")
        ...
        .def("get_next_combine_buffer", &mxa_ep::Buffer::get_next_combine_buffer);
}
