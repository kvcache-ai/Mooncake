...
struct DistributedStorageEnvironmentVariables {
    ...
    MC_DEFINE_ENV_VAR(std::string, MOONCAKE_DFS_ALLOCATOR_TYPE);
    MC_DEFINE_ENV_VAR(uint64_t, MOONCAKE_DFS_BUCKET_CAPACITY);
    MC_DEFINE_ENV_VAR(int, MOONCAKE_DFS_MAX_BUCKET_COUNT);
    MC_DEFINE_ENV_VAR(int, MOONCAKE_DFS_BATCH_READ_THREADS);
    MC_DEFINE_ENV_VAR(bool, MOONCAKE_DFS_BATCH_READ_MERGE_ENABLED);
    MC_DEFINE_ENV_VAR(bool, MOONCAKE_DFS_DIRECT_READ_ENABLED);
};
