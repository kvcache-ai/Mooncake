...
class TestTransferOnCuda(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        ret = cls.engine.initialize(
            cls.local_hostname, "P2PHANDSHAKE", "rdma", "mlx5_bond_0"
        )
        if ret != 0:
            raise unittest.SkipTest(
                f"Failed to initialize engine (code {ret}). Possibly no RDMA device."
            )
        ...
        # Register Memory
        ret1 = cls.engine.register_memory(
            cls.src_tensor.data_ptr(), cls.src_tensor.nbytes
        )
        ret2 = cls.engine.register_memory(
            cls.dst_tensor.data_ptr(), cls.dst_tensor.nbytes
        )
        if ret1 != 0 or ret2 != 0:
    ...
    def test_transfer_write_on_cuda(self):
        ...
        for i in range(10):
            ...
            self.assertTrue(
                torch.all(self.dst_tensor == i),
                f"Write verification failed at iter {i}",
            )
    ...
    def test_transfer_read_on_cuda(self):
        ...
        for i in range(10):
            ...
            torch.cuda.default_stream().wait_event(event)
            self.assertTrue(
                torch.all(self.src_tensor == val),
                f"Read verification failed at iter {i}",
            )
    ...
    def test_batch_transfer_write_on_cuda(self):
        ...
        for i in range(10):
            ...
            torch.cuda.default_stream().wait_event(event)
            self.assertTrue(
                torch.all(self.dst_tensor == val),
                f"Batch write verification failed at iter {i}",
            )
    ...
    def test_batch_transfer_read_on_cuda(self):
        ...
        for i in range(10):
            ...
            self.assertTrue(
                torch.all(self.src_tensor == val),
                f"Batch read verification failed at iter {i}",
            )
