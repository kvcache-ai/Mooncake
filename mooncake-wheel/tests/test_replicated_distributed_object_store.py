...
class TestDistributedObjectStoreReplication(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        for _ in range(
            cls.max_replicate_num - 1
        ):  # -1 because main store is already created
    ...
    def test_put_from_with_config_parameter(self):
        ...
        # Test with default config (backward compatibility)
        result = self.store.put_from(
            key=key, buffer_ptr=buffer_ptr, size=len(test_data)
        )
        self.assertEqual(result, 0)
        ...
        result = self.store.put_from(
            key=key2, buffer_ptr=buffer_ptr, size=len(test_data), config=config
        )
        self.assertEqual(result, 0)
    ...
    def test_batch_put_from_with_config_parameter(self):
        ...
        keys = [
            "test_batch_put_from_config_key1",
            "test_batch_put_from_config_key2",
            "test_batch_put_from_config_key3",
        ]
        ...
        results = self.store.batch_put_from(
            keys=keys, buffer_ptrs=buffer_ptrs, sizes=buffer_sizes
        )
        ...
        results = self.store.batch_put_from(
            keys=keys2, buffer_ptrs=buffer_ptrs, sizes=buffer_sizes, config=config
        )
    ...
    def test_replication_failure_tolerance(self):
        ...
        retrieved_data = self.store.get(key)
        self.assertEqual(
            retrieved_data, test_data, "Data should be accessible after put"
        )
        ...
        self.assertEqual(
            retrieved_data,
            test_data,
            "Data should remain accessible from replica after main store teardown",
        )
        ...
        self.assertEqual(
            retrieved_data,
            test_data,
            "Data should remain accessible after main store reinit",
        )
