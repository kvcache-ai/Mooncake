...
class TestSafetensorFunctions(unittest.TestCase):
    ...
    def test_save_and_load_tensor_with_memory_filesystem(self):
        ...
        key_suffix = uuid.uuid4().hex
        original_key = f"test_memory_tensor_{key_suffix}"
        restored_key = f"test_memory_tensor_restored_{key_suffix}"
        target_url = f"memory://mooncake-tests/{key_suffix}/tensor.pt"

        result = self.store.put_tensor(original_key, tensor)
        self.assertEqual(result, 0)

        fs, path = fsspec.core.url_to_fs(target_url)

        try:
            save_result = self.store.save_tensor_to_file(original_key, target_url)
            self.assertEqual(save_result, 0)
            self.assertTrue(fs.exists(path))

            loaded_tensor = self.store.load_tensor_from_file(restored_key, target_url)
            self.assertIsNotNone(loaded_tensor)
