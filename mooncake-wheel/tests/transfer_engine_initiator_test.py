...
class TestVLLMAdaptorTransfer(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.taget_server_name = os.getenv("TARGET_SERVER_NAME", "127.0.0.1:12345")
    ...
    def test_random_write_circle_times(self):
        ...
        dst_addr = adaptor.getFirstBufferAddress(self.taget_server_name)
        ...
        for i in range(circles):
            ...
            result = adaptor.transferSyncExt(
                self.taget_server_name, src_addr, dst_addr, data_len, adaptor.TransferOpcode.WRITE
            )
            ...
            result = adaptor.transferSyncExt(
                self.taget_server_name, src_addr, dst_addr, data_len, adaptor.TransferOpcode.READ
            )
