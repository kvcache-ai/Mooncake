...
try:
    from mooncake.store import dtype_byte_size as store_dtype_byte_size
except ImportError:  # pragma: no cover - extension may be unavailable in pure Python tests.
    store_dtype_byte_size = None
...
class TensorFieldRef:
    ...
    @classmethod
    def from_tensor(cls, key: str, tensor: Any, data_offset: int = 0) -> "TensorFieldRef":
...
class TensorDimSelection:
    ...
    @staticmethod
    def cat(selection_groups: Sequence[Sequence["TensorDimSelection"]]) -> "TensorDimSelection":
        """Concatenate independently selected views of the same tensor dimension."""
        return TensorDimSelection("cat", tuple(tuple(group) for group in selection_groups))

    def as_tuple(self) -> tuple[str, Any]:
        """Return the pybind-friendly representation accepted by Mooncake store APIs."""
        if self.op == "cat":
            return self.op, tuple(tuple(selection.as_tuple() for selection in group) for group in self.value)
...
class RemoteTensorBatch:
    ...
    def clone(self) -> "RemoteTensorBatch":
        ...
        return replace(self, fields=dict(self.fields), selections=tuple(self.selections))
    ...
    def select_indices(self, index: Any) -> "RemoteTensorBatch":
        ...
        return self._with_selection(selection, _selection_length(self.batch_size, selection))

    def slice(self, start: Optional[int], end: Optional[int], step: Optional[int]) -> "RemoteTensorBatch":
        """Record a slice selection on the batch dimension."""
        selection = TensorDimSelection.slice(start, end, step)
        return self._with_selection(selection, _selection_length(self.batch_size, selection))

    def repeat(self, repeat_times: int = 2, interleave: bool = True) -> "RemoteTensorBatch":
        ...
        return self._with_selection(
            TensorDimSelection.repeat(repeat_times, interleave), self.batch_size * repeat_times
        )

    def union(self, rhs: "RemoteTensorBatch") -> "RemoteTensorBatch":
        """Merge disjoint field sets with identical batch selection state."""
        if self.batch_size != rhs.batch_size or self.selections != rhs.selections or self.dim != rhs.dim:
            raise ValueError("RemoteTensorBatch union requires matching batch metadata")
        overlap = set(self.fields).intersection(rhs.fields)
        if overlap:
            raise ValueError(f"RemoteTensorBatch union has duplicate fields: {sorted(overlap)}")
    ...
    def cat(batches: Sequence["RemoteTensorBatch"]) -> "RemoteTensorBatch":
        ...
        for batch in batches:
            if batch.fields != first.fields or batch.dim != first.dim or batch.device != first.device:
                raise ValueError("RemoteTensorBatch.cat requires matching fields, dim, and device")
        selection = TensorDimSelection.cat([batch.selections for batch in batches])
        return replace(first, batch_size=sum(batch.batch_size for batch in batches), selections=(selection,))

    def read_requests(self, fields: Optional[Sequence[str]] = None) -> list[TensorReadRequest]:
    ...
    def _with_selection(self, selection: TensorDimSelection, batch_size: int) -> "RemoteTensorBatch":
        return replace(self, batch_size=batch_size, selections=(*self.selections, selection))
...
def dtype_byte_size(dtype: Union[str, Any]) -> int:
    ...
    if store_dtype_byte_size is not None:
        return int(store_dtype_byte_size(dtype_name))
    if torch is not None and hasattr(torch, dtype_name):
        return torch.empty((), dtype=getattr(torch, dtype_name)).element_size()
    raise ImportError("mooncake.store.dtype_byte_size is required for dtype-name size lookup")
...
def selection_output_shape(
    ...
    return (*normalized_shape[:normalized_dim], dim_size, *normalized_shape[normalized_dim + 1 :])


def _selection_length(length: int, selection: Union[TensorDimSelection, tuple[str, Any], Mapping[str, Any]]) -> int:
...
def _selection_group_length(
    length: int, selections: Sequence[Union[TensorDimSelection, tuple[str, Any], Mapping[str, Any]]]
) -> int:
    for selection in selections:
...
def _selection_op_value(selection: Union[TensorDimSelection, tuple[str, Any], Mapping[str, Any]]) -> tuple[str, Any]:
