...
class KVBootstrapServer:
    ...
    async def _handle_get(self, key):
        ...
        if value is None:
            return web.Response(text='metadata not found', status=404, content_type='application/json')
    ...
    async def _handle_put(self, key, request):
        ...
        async with self.lock:
            self.store[key] = data
        return web.Response(text='metadata updated', status=200, content_type='application/json')
    ...
    async def _handle_delete(self, key):
        ...
        async with self.lock:
            if key not in self.store:
                return web.Response(text='metadata not found', status=404, content_type='application/json')
            del self.store[key]
        return web.Response(text='metadata deleted', status=200, content_type='application/json')
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="HTTP Metadata Server for Mooncake", formatter_class=argparse.ArgumentDefaultsHelpFormatter
    )
