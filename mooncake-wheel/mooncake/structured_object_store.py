...
try:
    from mooncake import store as _mooncake_store
...
class _StructuredObjectLayer:
    ...
    def _read_torch_tensor_member(
        ...
        payload = bytearray(int(payload_spec["bytes"]))
        self._bundle_store.read_payload_range_into_destination(
            payload_spec,
            memoryview(payload),
            0,
        )
        return _deserialize_tensor_payload(bytes(payload))
...
def _encode_torch_tensor_field(value: Any) -> tuple[dict[str, Any], Any]:
    return {"encoding": "torch_tensor"}, _TensorPayload(
        tensor=value, nbytes=int(value.numel() * value.element_size())
    )


def _tensor_payload_bytes_view(value: _TensorPayload) -> memoryview:
    return memoryview(_tensor_codec_helper("_serialize_tensor")(value.tensor)).cast("B")
...
def _tensor_codec_helper(name: str) -> Any:
    ...
    if not callable(helper):
        raise RuntimeError(
            "mooncake.store tensor serialization helpers are required for "
            "structured tensor raw fallback"
        )
    return helper
