...
std::string JoinParts(const std::vector<std::string_view>& parts, size_t begin,
                      size_t end) {
    std::string result;
...
bool IsNonNegativeDecimal(std::string_view value) {
    // Unsigned target, so a leading sign is rejected; the default options
    // reject surrounding whitespace and require the whole view to be consumed.
    return TryParseInteger<uint64_t>(value).has_value();
}
...
std::string FillProjectedHash(std::string_view hash,
    ...
    for (size_t index = offset; index < offset + 16; ++index) {
        ...
        const uint64_t nibble =
            character <= '9' ? static_cast<uint64_t>(character - '0')
...
std::string ParseSglangObjectKey(const std::string& object_key,
    ...
    *result = {};
    ...
    // The scan above accepted only hex digits, so lowering touches A-F alone.
    result->full_hash = AsciiToLower(
        std::string_view(object_key).substr(hash_start, kSha256HexLength));
    ...
    // Base 16, so std::from_chars is used directly: TryParseInteger is
    // decimal-only and would also reject the trailing hex digits left unread.
...
std::string ParseVllmObjectKey(const std::string& object_key,
    ...
    *result = {};
    ...
    for (size_t index = 1; index < parts.size(); ++index) {
        ...
        if (cursor < hash_index && HasDecimalLabel(parts[cursor], "group:")) {
            ++cursor;
        }
    ...
    for (size_t index = 1; index < parts.size(); ++index) {
        ...
        auto matches_layout = [&](size_t hash_index) {
            if (hash_index <= index + 2 || !IsVllmHashText(parts[hash_index])) {
                return false;
            }
