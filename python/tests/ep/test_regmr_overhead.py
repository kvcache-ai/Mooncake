...
Usage:
    python python/tests/ep/test_regmr_overhead.py
"""
...
def test_ctrl_buf_init_overhead():
    ...
    if (sync_memset_avg - alloc_only_avg) > 0:
        improvement = (
            (sync_memset_avg - async_memset_avg)
            / (sync_memset_avg - alloc_only_avg)
            * 100
        )
...
def test_transfer_engine_reg_mr_overhead():
    ...
    try:
        ...
        if ret != 0:
            ...
            if ret != 0:
                print(
                    f"Skipped: Transfer Engine initialization failed with TCP as well: {ret}"
                )
                return
        ...
        for size in sizes:
            ...
            if times:
                ...
                print(
                    f"  Registration time - avg: {avg_time:.2f} us, min: {min_time:.2f} us, max: {max_time:.2f} us"
                )
...
def test_buffer_init_overhead():
    ...
    try:
        ...
        for _ in range(3):
            try:
                buffer = Buffer(
                    dist.group.WORLD, num_ep_buffer_bytes=num_ep_buffer_bytes
                )
                del buffer
                torch.cuda.empty_cache()
            except Exception:
...
def main():
    ...
    if dist.is_initialized():
        ...
        print(
            f"Using existing distributed environment: rank={dist.get_rank()}, world_size={dist.get_world_size()}"
        )
    else:
        ...
        print(
            "Tests that require distributed environment (Buffer initialization) will be skipped"
        )
        print(
            "To enable distributed tests, initialize the environment before running this script:"
        )
    ...
    try:
        ...
        if dist_initialized:
            ...
            print("\n" + "=" * 60)
            print(
                "Skipping Buffer initialization test (requires distributed environment)"
            )
            print("=" * 60)
