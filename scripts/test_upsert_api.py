...
def setUpModule():
    ...
    segment_size = int(
        os.getenv("MOONCAKE_GLOBAL_SEGMENT_SIZE", str(64 * 1024 * 1024))
    )  # 64 MB default
    buffer_size = int(
        os.getenv("MOONCAKE_LOCAL_BUFFER_SIZE", str(64 * 1024 * 1024))
    )  # 64 MB default

    print(
        f"\n[{os.getpid()}] Connecting to master at {master_addr} " f"({protocol})..."
    )
...
class TestBatchUpsertTensor(UpsertTestBase):
    ...
    def test_basic(self):
        ...
        tensors = [
            torch.randn(20, 20, dtype=torch.float32).contiguous() for _ in range(3)
        ]
...
class TestUpsertPubTensor(UpsertTestBase):
    ...
    def test_batch(self):
        ...
        tensors = [torch.randn(15, 15).contiguous(), torch.randn(15, 15).contiguous()]
