...
cp "${BUILD_DIR_ABS}"/mooncake-integration/engine.*.so mooncake-wheel/mooncake/engine.so

# Copy libasio.so to mooncake directory (runtime dependency of engine.so)
cp "${BUILD_DIR_ABS}/mooncake-common/libasio.so" mooncake-wheel/mooncake/libasio.so

# Copy store.so to mooncake directory
if compgen -G "${BUILD_DIR_ABS}/mooncake-integration/store.*.so" >/dev/null; then
    echo "Copying store.so..."
    cp "${BUILD_DIR_ABS}"/mooncake-integration/store.*.so mooncake-wheel/mooncake/store.so
    echo "Copying master binary..."
    # Copy master binary
    cp "${BUILD_DIR_ABS}/mooncake-store/src/mooncake_master" mooncake-wheel/mooncake/
    # Copy client binary
    cp "${BUILD_DIR_ABS}/mooncake-store/src/mooncake_client" mooncake-wheel/mooncake/
    # Copy async_store.py
...
if [ -f "${BUILD_DIR_ABS}/mooncake-store/src/libmooncake_store.so" ]; then
    echo "Copying libmooncake_store.so..."
    cp "${BUILD_DIR_ABS}/mooncake-store/src/libmooncake_store.so" mooncake-wheel/mooncake/libmooncake_store.so
fi

# Copy libmooncake_common.so to mooncake directory (only when BUILD_SHARED_LIBS is set)
if [ -f "${BUILD_DIR_ABS}/mooncake-common/src/libmooncake_common.so" ]; then
    echo "Copying libmooncake_common.so..."
    cp "${BUILD_DIR_ABS}/mooncake-common/src/libmooncake_common.so" mooncake-wheel/mooncake/libmooncake_common.so
fi

# Copy libtransfer_engine.so to mooncake directory (only when USE_ETCD is set)
if [ -f "${BUILD_DIR_ABS}/mooncake-common/etcd/libetcd_wrapper.so" ]; then
    echo "Copying libetcd_wrapper.so..."
    cp "${BUILD_DIR_ABS}/mooncake-common/etcd/libetcd_wrapper.so" mooncake-wheel/mooncake/libetcd_wrapper.so
fi

# Copy libk8s_lease_wrapper.so to mooncake directory (only when STORE_USE_K8S_LEASE is set)
if [ -f "${BUILD_DIR_ABS}/mooncake-common/k8s-lease/libk8s_lease_wrapper.so" ]; then
    echo "Copying libk8s_lease_wrapper.so..."
    cp "${BUILD_DIR_ABS}/mooncake-common/k8s-lease/libk8s_lease_wrapper.so" mooncake-wheel/mooncake/libk8s_lease_wrapper.so
fi

# Copy libtransfer_engine.so to mooncake directory (only when BUILD_SHARED_LIBS is set)
if [ -f "${BUILD_DIR_ABS}/mooncake-transfer-engine/src/libtransfer_engine.so" ]; then
    echo "Copying libtransfer_engine.so..."
    cp "${BUILD_DIR_ABS}/mooncake-transfer-engine/src/libtransfer_engine.so" mooncake-wheel/mooncake/libtransfer_engine.so
fi

# Copy ascend_transport.so to mooncake directory (only when USE_ASCEND_DIRECT is set)
if [ -f "${BUILD_DIR_ABS}/mooncake-transfer-engine/src/transport/ascend_transport/ascend_transport.so" ]; then
    echo "Copying ascend_transport.so..."
    cp "${BUILD_DIR_ABS}/mooncake-transfer-engine/src/transport/ascend_transport/ascend_transport.so" mooncake-wheel/mooncake/ascend_transport.so
fi

# Copy nvlink-allocator.so to mooncake directory (only if it exists - CUDA builds only)
if [ -f "${BUILD_DIR_ABS}/mooncake-transfer-engine/nvlink-allocator/nvlink_allocator.so" ] \
   || [ -f /usr/lib/libaccl_barex.so ] \
   || [ -f /usr/lib64/libaccl_barex.so ]; then
    if [ -f "${BUILD_DIR_ABS}/mooncake-transfer-engine/nvlink-allocator/nvlink_allocator.so" ]; then
     echo "Copying CUDA nvlink_allocator.so..."
     cp "${BUILD_DIR_ABS}/mooncake-transfer-engine/nvlink-allocator/nvlink_allocator.so" mooncake-wheel/mooncake/nvlink_allocator.so
    fi
...
if [ -f "${BUILD_DIR_ABS}/mooncake-transfer-engine/ubshmem-allocator/ubshmem_fabric_allocator.so" ]; then
    echo "Copying NPU ubshmem_fabric_allocator.so..."
    cp "${BUILD_DIR_ABS}/mooncake-transfer-engine/ubshmem-allocator/ubshmem_fabric_allocator.so" mooncake-wheel/mooncake/ubshmem_fabric_allocator.so
...
cp "${BUILD_DIR_ABS}/mooncake-transfer-engine/example/transfer_engine_bench" mooncake-wheel/mooncake/

if [ -f "${BUILD_DIR_ABS}/mooncake-transfer-engine/src/transport/ascend_transport/hccl_transport/ascend_transport_c/libascend_transport_mem.so" ]; then
    cp "${BUILD_DIR_ABS}/mooncake-transfer-engine/src/transport/ascend_transport/hccl_transport/ascend_transport_c/libascend_transport_mem.so" mooncake-wheel/mooncake/
