...
# Stage the migrated CLI modules and the Reshard package for the temporary
# combined-wheel builder. Their tracked sources remain in authoritative trees.
MIGRATED_CLI_SOURCE_DIR="python/mooncake"
MIGRATED_CLI_STAGING_DIR="$(pwd)/mooncake-wheel/mooncake"
MIGRATED_CLI_MODULES=(
    cli.py
    cli_bench.py
    cli_client.py
...
cleanup_python_staging() {
    for module in "${MIGRATED_CLI_MODULES[@]}"; do
        rm -f "${MIGRATED_CLI_STAGING_DIR}/${module}"
    done
...
cleanup_python_staging
for module in "${MIGRATED_CLI_MODULES[@]}"; do
    cp "${MIGRATED_CLI_SOURCE_DIR}/${module}" \
       "${MIGRATED_CLI_STAGING_DIR}/${module}"
done
