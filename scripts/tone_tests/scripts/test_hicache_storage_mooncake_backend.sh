...
run_test()
{
    ...
    ${docker_exec} "cd /test_run/python && python3 -m pytest test_hicache_storage_mooncake_backend.py -v" | tee "$log_file"
...
parse()
{
    ...
    local log_file="${TONE_TESTS_DIR}/${TEST_CASE_RESULT_PATH}/logs/${test_case_name}.log"
    local result_json="${TONE_TESTS_DIR}/${TEST_CASE_RESULT_PATH}/results.json"

    if [ ! -f "$log_file" ]; then
        echo "ERROR: Test log file not found at $log_file"
        echo "{\"test_case\": \"$test_case_name\", \"status\": \"Fail\", \"timestamp\": \"$(date -u +%Y-%m-%dT%H:%M:%SZ)\", \"error\": \"Log file not found\"}" > "$result_json"
