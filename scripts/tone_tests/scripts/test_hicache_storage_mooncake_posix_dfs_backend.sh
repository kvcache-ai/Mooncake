...
test_case_name="test_hicache_storage_mooncake_posix_dfs_backend"
TEST_TYPE="single"
...
run_test()
{
    ...
    ${docker_exec} "\
        export PYTHONPATH=/test_run/python:/sgl-workspace/sglang/test/registered/hicache:\$PYTHONPATH && \
        cd /test_run/python && \
        python3 -m pytest test_hicache_storage_mooncake_posix_dfs_backend.py -v -s --tb=long" | tee "$log_file"
