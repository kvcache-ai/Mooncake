...
if [ "$CI_ACCELERATOR" = "rocm" ]; then
    # The ROCm cluster uses a root-owned key and pinned host key. Keep the
    # serving/RDMA address separate from the SSH management endpoint.
    REMOTE_SSH_TARGET=${REMOTE_SSH_TARGET:-"mooncake-worker"}
    SSH_CMD=${SSH_CMD:-"ssh -F /etc/mooncake-ci/ssh_config"}
    RSYNC_RSH=${RSYNC_RSH:-"ssh -F /etc/mooncake-ci/ssh_config"}
    SCP_CMD=${SCP_CMD:-"scp -F /etc/mooncake-ci/ssh_config"}
else
...
setup_env_for_test() {
    ...
    case "$framework_type" in
        SGLANG) ARTIFACT_ID=$ARTIFACT_ID_SGLANG ;;
        VLLM) ARTIFACT_ID=$ARTIFACT_ID_VLLM ;;
        *) echo "ERROR: unknown framework type: $framework_type" >&2; return 1 ;;
    esac
    [ -n "$ARTIFACT_ID" ] || {
        echo "ERROR: no wheel artifact configured for $framework_type" >&2
        return 1
    }
    export ARTIFACT_ID
