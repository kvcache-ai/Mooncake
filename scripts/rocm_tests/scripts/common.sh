...
docker_launch(){
    ...
    # The AMD AINIC repository is selected by the container's Ubuntu codename.
...
wait_gpu_idle() {
    ...
    while [ $elapsed -lt $max_seconds ]; do
        ...
        if ! [[ "$max_used" =~ ^[0-9]+$ ]]; then
            echo "ERROR: ROCm GPU memory query failed; cannot verify GPU drain" >&2
            return 1
        fi
...
# Never escalate to host PID killing on the shared MI350X host.
drain_gpu_local() {
    ...
    if ! wait_gpu_idle 60; then
        echo "ERROR: ROCm allocation did not drain after restart; quarantine it without killing host GPU processes" >&2
        return 1
    fi
...
# lightweight container restart (no wheel / RDMA driver reinstall).
drain_gpu_between_tests() {
    # Reset only the dedicated ROCm allocation, never unrelated host processes.
