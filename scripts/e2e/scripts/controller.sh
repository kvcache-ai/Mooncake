...
prepare_single_env(){
    ...
    TEST_RESULT_DIR=$RUN_DIR/logs
    local variable
    local environment_vars=(
        CONTAINER_NAME CI_ACCELERATOR MOONCAKE_CI_TIER MODEL_CACHE
        ...
        MOONCAKE_TRANSFER_DEVICE MOONCAKE_GID_INDEX AINIC_VERSION REMOTE_SSH_TARGET
        SSH_CMD RSYNC_RSH SCP_CMD
    )
    # Quote values once, for both local and remote shells.
    for variable in "${environment_vars[@]}"; do
        printf 'export %s=%q\n' "$variable" "${!variable:-}"
    done > "$RUN_DIR/.shrc"
...
# Isolate test-defined functions and variables from the suite controller.
execute_test() (
    local test_name=$1
    shift
    unset -f run_test parse
    source "./$test_name"
    setup_log_directory "${BASE_DIR}/run/logs/${test_name%.sh}"
    local exit_code=0
    run_test "$@" || exit_code=1
    if declare -f parse >/dev/null; then
...
run_all_tests(){
    ...
    for test_name in "${tests[@]}"; do
        ...
        if [[ ! -f "./$test_name" ]]; then
            echo "ERROR: test case $test_name is not found" >&2
            all_passed=false
            continue
        fi
