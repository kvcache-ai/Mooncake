...
fetches with local_cache=True and verifies cache hits skip
remote transfers.
...
def run_reader(store):
    ...
    # ------------------------------------------
    # Test 2: get_buffer with local_cache=True (miss then hit)
    # ------------------------------------------
    print("--- Test 2: get_buffer cache miss then hit ---")
    expected = make_data(SINGLE_KEY)

    t0 = time.perf_counter()
    buf1 = store.get_buffer(SINGLE_KEY, local_cache=True)
    ...
    t0 = time.perf_counter()
    buf2 = store.get_buffer(SINGLE_KEY, local_cache=True)
    ...
    # ------------------------------------------
    # Test 3: batch_get_buffer with local_cache=True
    # ------------------------------------------
    print("--- Test 3: batch_get_buffer cache miss then hit ---")
    expected_batch = [make_data(k) for k in BATCH_KEYS]

    t0 = time.perf_counter()
    bufs1 = store.batch_get_buffer(BATCH_KEYS, local_cache=True)
    t_miss = time.perf_counter() - t0
    ...
    t0 = time.perf_counter()
    bufs2 = store.batch_get_buffer(BATCH_KEYS, local_cache=True)
    t_hit = time.perf_counter() - t0
    ...
    # ------------------------------------------
    # Test 4: get_into with local_cache=True
    # ------------------------------------------
    ...
    t0 = time.perf_counter()
    length1 = store.get_into(INTO_KEY, buffer_ptr, buf_size, local_cache=True)
    t_miss = time.perf_counter() - t0
    ...
    t0 = time.perf_counter()
    length2 = store.get_into(INTO_KEY, buffer_ptr, buf_size, local_cache=True)
    t_hit = time.perf_counter() - t0
    ...
    # ------------------------------------------
    # Test 5: batch_get_into with local_cache=True
    # ------------------------------------------
    ...
    buf = store.get_buffer(REMOVE_KEY, local_cache=True)
    ...
    for i in range(2):
        buf = store.get_buffer(PARTIAL_KEYS[i], local_cache=True)
        check(f"test_07_pre_{i}", buf is not None, f"pre-cache failed for {i}")

    # Batch get all 4: first 2 hit cache, last 2 miss
    bufs = store.batch_get_buffer(PARTIAL_KEYS, local_cache=True)
