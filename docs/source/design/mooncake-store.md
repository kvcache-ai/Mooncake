...
This is a soft-isolation policy: internal Store paths and external Python leases share the local registered buffer, allowing bursty external usage when memory is available rather than reserving a hard partition. If callers need to cap long-lived external pressure, they should use pool-level controls such as `max_regions` or acquire timeouts.
