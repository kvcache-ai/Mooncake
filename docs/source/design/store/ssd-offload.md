...
5. **Notify master**: On success, the `complete_handler` calls `client_->NotifyOffloadSuccess(keys, metadatas)`. The master adds a `LOCAL_DISK` replica entry carrying the real client's RPC address as `transport_endpoint`. Restart recovery requires this endpoint to remain stable across client UUID rotations and uniquely identify one physical local-disk instance. Standalone real clients use their configured `--host` and `--port`; embedded clients may set `local_rpc_port` to a positive stable port, while the default `0` keeps the old OS-assigned auto-bind behavior.
