...
- On snapshot/standby restore, group state is rebuilt from object metadata (fresh group TTLs). Snapshots produced by a router that placed grouped objects on `hash(group_id)` shards are not re-routed to `hash(tenant, key)` after an upgrade and should be regenerated; current-format snapshots restore correctly.
