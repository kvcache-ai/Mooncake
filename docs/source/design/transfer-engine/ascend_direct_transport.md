...
- **ASCEND_CLIENT_SERVER_MODE**: Enables the HIXL Client/Server communication mode. Default is auto-detected via `GetCapability(CLIENT_SERVER_COMM)`: enabled on supporting libs, disabled on older libs. The env var `ASCEND_CLIENT_SERVER_MODE` (1/0) overrides. When enabled, the transport runs under the client-server topology where servers listen and clients initiate links.
