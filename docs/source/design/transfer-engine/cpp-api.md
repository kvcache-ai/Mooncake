...
- `task_group_id` is an optional field (default: `kNoTaskGroup = 0`). When multiple adjacent requests in the `entries` array share the same non-default `task_group_id` and resolve to the same underlying transport, `submitTransfer` coalesces them into a single logical task. This affects `batch_size` accounting and status tracking: `batch_size` is measured in logical tasks, and `getTransferStatus` uses logical task indices. Ungrouped requests (with `task_group_id == kNoTaskGroup`) each remain a standalone logical task. Grouping is purely a task-aggregation optimization; it does not require the local `source` buffers of grouped requests to reside in the same registered memory region.
