...
- `MC_MUSA_IPC_OPEN_DEVICE` (MUSA only) Set to `metadata` together with `MC_USE_NVLINK_IPC=1` to open CUDA-alike IPC handles on the device ordinal advertised by the remote buffer metadata before restoring the caller's device. This can improve MUSA S5000 peer-copy throughput. The default is `current` for compatibility. Only enable it when every peer uses the same ordered `MUSA_VISIBLE_DEVICES` mapping; a different order or subset can resolve `musa:<ordinal>` to the wrong physical GPU and removes the performance benefit.
