...
- `MC_ENABLE_DEST_DEVICE_AFFINITY` Enable device affinity for RDMA performance optimization. When enabled, Transfer Engine will prioritize communication with remote NICs that have the same name as local NICs to reduce QP count and improve network performance in rail-optimized topologies. If the same-name remote NIC is missing, selection falls back to another device. The default value is false
- `MC_ENABLE_STRICT_DEST_DEVICE_AFFINITY` Opt-in rail isolation for fabrics that are not full-mesh (retry exceeded / `IBV_WC_RETRY_EXC_ERR` when one local NIC talks to a different-name remote NIC; see issues [#35](https://github.com/kvcache-ai/Mooncake/issues/35) and [#1450](https://github.com/kvcache-ai/Mooncake/issues/1450)). When enabled: (1) each slice picks a local HCA instead of pinning one HCA for the whole request; (2) the peer HCA must have the same name, with no fallback; (3) `retry_count==0` round-robins `preferred_hca` and `avail_hca` together so every listed rail can be used. Default off so full-mesh mlx5 and the soft `MC_ENABLE_DEST_DEVICE_AFFINITY` preference stay unchanged. Mutually exclusive with `MC_ENABLE_HCA_PEER_AFFINITY`.
