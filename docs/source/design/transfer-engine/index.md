...
- `MC_RDMA_DEDICATED_POLLER` Controls RDMA worker scheduling when `MC_WORKERS_PER_CTX` is greater than 1. Default value `false` keeps the historical shared model: every worker posts its assigned send shards and polls only its own disjoint CQ shard, so workers do not concurrently poll the same CQ. Set to `true` to dedicate worker 0 to polling all CQs while workers 1..N only post sends; this preserves the dedicated-poller performance behavior introduced by the #2696 RDMA polling change. Leave it disabled when you want the legacy balanced post/poll model, and enable it when polling latency or CQ drain throughput benefits from a dedicated poller on your deployment.
