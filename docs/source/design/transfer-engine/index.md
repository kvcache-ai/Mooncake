...
- `MC_MAX_MR_SIZE` Maximum registration chunk size in bytes, additionally clamped to the selected devices' advertised MR limits. This differs from transfer request slicing (`MC_SLICE_SIZE`).
- `MC_RDMA_MAX_MR_SIZE_OVERRIDE` Optional positive decimal byte count overriding the classic RDMA transport's registration chunk limit, including the device clamp and `MC_MAX_MR_SIZE`. Unset by default; malformed, zero, negative, suffixed, or overflowing values are ignored with a warning. For example, `MC_RDMA_MAX_MR_SIZE_OVERRIDE=4294967296` requests chunks of at most 4 GiB even if a NIC advertises 2 GiB. Applies to all selected RDMA NICs and RDMA memory registrations in the process, including device memory; EFA/CXI limits are unchanged. Use only after validating registration and high-offset READ/WRITE correctness for the exact NIC, driver, firmware, and backing memory. A successful host-hugepage test does not establish GPU-memory support. Buffers above the effective limit are still split into MRs with separate descriptors and keys, and registration failures are propagated without truncation or automatic fallback. This option does not increase NIC translation-table capacity or change RDMA message size. Set it before engine initialization. Unset it to restore normal device-clamped behavior.
