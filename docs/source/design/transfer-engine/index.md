...
- `MC_MUSA_IPC_OPEN_DEVICE` (`musa` transport only) Select the device context used to open imported IPC memory. The default `metadata` uses the logical ordinal advertised by the remote buffer; `current` is a diagnostic rollback. Every peer must use identical ordered physical lists in both `MTHREADS_VISIBLE_DEVICES` and `MUSA_VISIBLE_DEVICES`. Do not combine a physical list such as `MTHREADS_VISIBLE_DEVICES=4,5` with `MUSA_VISIBLE_DEVICES=0,1`, because that can expose a different physical pair.
