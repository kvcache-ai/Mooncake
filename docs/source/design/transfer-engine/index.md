...
- `MC_RDMA_RAIL_PAUSE_SECONDS` Cooldown in seconds before retrying an RDMA peer rail after it reaches the failure threshold (or an error pauses it immediately). The default is 30; valid overrides are 1-3599. While a rail is paused, Transfer Engine selects another available path and automatically makes the rail eligible again after the cooldown.
- `MC_CONN_PAUSE_TTL_MS` Active-connect circuit-breaker duration in milliseconds after an RDMA endpoint is torn down. The default 0 disables the circuit breaker; valid values are 0-600000. Set a positive value in elastic or Kubernetes deployments to avoid blocking a posting worker on repeated handshakes to a removed peer address while unposted slices fail or are redispatched.
...
- `MC_ENABLE_HCA_PEER_AFFINITY` Set to `1` or `true` to enable explicit local-to-peer HCA affinity for classic RDMA Transfer Engine. Use it when paired rails have different HCA names across hosts. It cannot be enabled together with `MC_ENABLE_DEST_DEVICE_AFFINITY`; if both are enabled, Transfer Engine disables both affinity modes and falls back to default peer-device selection.
- `MC_NIC_PEER_AFFINITY` Define the HCA pairs used by `MC_ENABLE_HCA_PEER_AFFINITY`. Separate local-HCA rules with semicolons and multiple allowed peer HCAs with commas: `local_hca=peer_hca[,peer_hca];...`. For example, `mlx5_0=mlx5_4,mlx5_5;mlx5_1=mlx5_6`. A mapping is applied only when the peer HCA is also topology-preferred for the target GPU buffer; invalid or empty rules are ignored with a warning.
- `MC_LOG_RDMA_SLICE_AFFINITY` Set to `1` or `true` to emit verbose per-slice diagnostics with the source and target memory locations, selected local HCA, and selected peer HCA. The default is false. Enable it temporarily when validating topology or `MC_NIC_PEER_AFFINITY`; it adds logging on the transfer path.
...
- `MC_RDMA_BIND_ADDRESS` Advertise a dedicated RDMA-reachable address when the TCP handshake and RDMA data plane use different network interfaces. Transfer Engine keeps the segment name for TCP/P2P routing and uses this address, together with the segment's port, to construct RDMA NIC paths. For example, the segment may use an `eth0` address while this variable points to the IP on the RDMA fabric.
