...
- `MC_MAX_CONCURRENT_REG_MR` Cap on how many buffers `registerLocalMemoryBatch` registers concurrently (EFA transport). The default 0 means unbounded — one thread per buffer, the historical behavior.

  **The cap is per process.** Inference frameworks typically create one `TransferEngine` per TP rank, so on an 8-rank node `MC_MAX_CONCURRENT_REG_MR=16` allows 128 registrations in flight against the same NICs, not 16. Size the value for one process and multiply mentally.

  **Its effect depends on the order you pass buffers in.** Measured on a p5.48xlarge (32 NICs), replaying Kimi-K3's KV registration as SGLang issues it — 8 processes, 182 GPU buffers of 2.5 KB to 391 MB each, all starting together; times are the slowest rank, since nothing serves until every rank has registered:
  ...
  Repeated entries are separate sweeps. Reading it: a cap of 16 cuts registration 2.6× on the order SGLang really uses (112 s → 43 s), but only 1.1× if buffers arrive largest-first, and the order alone moves the capped result by 2.7×. Unbounded is insensitive to order (99–128 s) because every buffer gets its own thread and nothing queues. Largest-first being the *worst* order is the opposite of what longest-processing-time scheduling predicts and is not yet explained, so the transport does not reorder your buffers.

  Practical advice: if you register many large GPU buffers at startup and care about that latency, try 16 first and measure; if you cannot measure, leave it unset, which is never much worse than the best and never the 2× regression that a mismatched cap can be. Host memory (`fi_mr_reg` rather than `fi_mr_regattr`) behaves similarly — 128 × 2 GiB of 4 KB-page memory on that same p5 took 274 s unbounded, 120 s at 64, 89.6 s at 32, 59.1 s at 16 (best), 70.9 s at 8, 257 s at 4. A 2× p6-b300 run with 16 NICs per rank put the optimum lower still (20.7 s at a cap of 8 versus 138.7 s unbounded), which is why there is no compiled-in default: the best value tracks the platform. Do not core-scale it either — where capping helps, the bottleneck is the provider lock, not CPU.
