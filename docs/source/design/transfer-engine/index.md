...
- `MC_HANDSHAKE_WORKERS` The number of worker threads processing inbound handshake connections in P2P mode, default value 8 (valid range 1-256, larger values are capped at 256). Accepted connections are queued to these workers so that a slow peer cannot stall the accept loop; the queue holds up to 4096 pending connections, after which new connections are dropped and the client retries. Increase this when many transfer engines come up at once (e.g. wide expert-parallel deployments) and the default cannot drain the handshake burst fast enough
