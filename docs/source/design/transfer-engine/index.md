...
Requests may optionally carry a `task_group_id` to enable grouped scatter aggregation: adjacent requests that share the same non-default group ID and resolve to the same transport backend are coalesced into a single logical task. This reduces per-request overhead and aligns `batch_size` accounting with logical task count rather than raw request count. Grouping is purely a task-aggregation optimization and does not impose any requirements on the local memory layout of grouped requests — each request retains its own independent `source` buffer.
