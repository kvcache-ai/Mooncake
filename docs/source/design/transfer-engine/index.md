...
- `MC_IB_ENABLE_ECE` Set to `0` or `false` to disable RDMA Enhanced Connection Establishment (ECE) negotiation on RC QPs. Mooncake queries local QP ECE data, exchanges it during RDMA handshake, applies the peer-reduced ECE before RTR, and falls back to non-ECE connections if either peer or QP does not support ECE. Default: enabled. Example: `MC_IB_ENABLE_ECE=0`
