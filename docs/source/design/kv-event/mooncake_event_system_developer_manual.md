...
```mermaid
graph TD
    ApplicationLayer -->|invoke interface<br>publish KVEvent| KVEventProducer
    IR[indexer/router]
...
struct BlockStoreEvent {
    mooncake_key: string;
    replicas: string[];
    store_event_info: StoreEventInfo;
}
```
...
| ------------------ | ---------------------------------- | ---------------------------------- | -------------------------------------------- |
| `mooncake_key`     | `std::string`                      | N/A                                | Primary key when the storage action occurs   |
| `replicas`         | `std::vector<Replica::Descriptor>` | N/A                                | Full replica list after storage is completed |
| `store_event_info` | `StoreEventInfo`                   | N/A                                | Associated storage metadata                  |
...
| Field Name     | Type                               | Default Value | Constraints/Explanation                         |
| -------------- | ---------------------------------- | ------------- | ----------------------------------------------- |
| `mooncake_key` | `std::string`                      | N/A           | Primary key when ObjectMetadata replica changes |
| `replicas`     | `std::vector<Replica::Descriptor>` | N/A           | Full replica list after update is completed     |
...
struct EventBatch {
    ts: number;
    events: Array<BlockStoreEvent | BlockUpdateEvent | RemoveAllEvent>;
}
