...
The default `shard` allocator recovers its metadata after a standalone master
restart and reconciles it with ordinary snapshot restore. The `bucket`
allocator has a separate layout and is not covered by shard metadata recovery;
bucket mode remains incompatible with snapshot restore. Descriptor-based DFS
remains incompatible with HA, OpLog recovery, and standby promotion.
...
In shard mode, the master creates a `.data`, `.meta`, and `.wal` file for every
...
| `MOONCAKE_DFS_ALLOCATOR` | Master | `shard` | Select `shard` or `bucket`. The metadata recovery settings below apply only to `shard`. |
...
| `MOONCAKE_DFS_METADATA_CHECKPOINT_INTERVAL_SECONDS` | Master, shard mode | `300` | Maximum interval between WAL compactions into an allocator checkpoint. |
| `MOONCAKE_DFS_METADATA_WAL_COMPACTION_THRESHOLD_BYTES` | Master, shard mode | `67108864` | WAL size threshold that triggers checkpoint compaction; it is not a hard WAL size limit. |
...
layout. Allocator ownership is recovered from the per-shard checkpoint and WAL;
cached key metadata recovery and HA remain subject to the limitations below.
Do not treat online expansion as a data durability guarantee.
...
- In shard mode, snapshot restore prunes a DFS replica whose descriptor no
  longer matches the allocator; if no other valid replica remains, the object
  is pruned too. Bucket mode cannot use snapshot restore.
