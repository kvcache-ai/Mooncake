...
The client derives the host id from `local_hostname` by removing the port. For example, `host-a:50051` and `host-a:50052` map to the same host id, `host-a`. Empty, loopback, and wildcard values such as `localhost`, `127.0.0.1`, `0.0.0.0`, and `::1` are treated as unknown and do not trigger automatic local-first placement for that client.
