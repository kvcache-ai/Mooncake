...
- On real client restart, `bucket_storage_backend` and `file_per_key_storage_backend` scan existing SSD metadata and report it to the master, so previously offloaded objects remain accessible. The advertised RPC endpoint must stay stable: use the same `--host` and `--port` for a standalone real client, or the same `local_rpc_port` (default `50052`) for embedded clients. `offset_allocator_storage_backend` does not support restart recovery.
