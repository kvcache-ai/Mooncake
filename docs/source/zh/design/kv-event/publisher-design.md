...
| `kv_events_model_name` | 空 | 写入事件 envelope 并用于 `cleared` 的固定模型名。Conductor 要求 `stored` 事件具有非空模型名。 |
...
| `skipped_unparsed_keys` | 因未启用 object key 发布而无法发送的对象事件数量。启用 object key 后，Conductor 会按注册的推理引擎类型解析。 |
...
| `model_name` | 字符串或 `nil` | `kv_events_model_name` 的副本。 |
...
| 字段 | MessagePack 形式 | 来源和用途 |
|---|---|---|
| `group_id` | 字符串或 `nil` | Store 操作传入的 group 字符串。空值会变成 `nil`。 |
| `seq_hashes` | 空数组 | connector hash 由 Conductor 解析，发布端不再解析。 |
...
发布端不会加入 connector 派生字段。Conductor 会按注册的推理引擎类型解析
原始 object key，并在需要时得到以下字段：

| 字段 | 何时存在 |
|---|---|
| `connector_block_hash` | 旧版发布端可能提供的字段；当前 Conductor 在存在时进行一致性核对。 |
| `cache_prefix` | key 中包含 connector 前缀。 |
| `tp_rank` | vLLM key 中解析出张量并行 rank。 |
| `head_or_tp_rank` | vLLM Ascend key 中解析出 head 或张量并行 rank。 |
| `pcp_rank` | key 中解析出 prefill 上下文并行 rank。 |
| `dcp_rank` | key 中解析出 decode 上下文并行 rank。它不是 `dp_rank`。 |
| `pp_rank` | key 中解析出流水线并行 rank。 |
...
|---|---|---|
| `stored` | 该对象可从指定的 `cpu` 或 `disk` 介质读取。重复成功的 Put 或 Upsert 提交可能再次上报相同可用性。 | `backend_id`、非空的 `tenant_id` 和 `model_name`、正数 `block_size`、受支持的介质和 group，以及 `object_key`。connector 派生字段只由 Conductor handler 产生。 |
...
启用 object key 发送后，发布端可以原样发送无法识别的 key。Conductor 会根据
注册的 vLLM 或 SGLang handler 解析它；无法解析的 key 会记录错误且不会写入索引。
...
未知或格式错误的 key。发布端仍可将它们作为 `object_key` 发出，但对应的
Conductor handler 会报告解析错误，无法将该事件加入索引。

对 vLLM Conductor handler 而言，`block_hash_hex` 必须是表示至少八个字节的有效十六进制
文本：去掉可选的 `0x` 前缀后，长度至少为 16 个字符，并且字符数必须为
偶数。handler 保存完整文本，并把最右侧 16 个十六进制字符转成无符号查找值。

旧版事件如果带有完整 connector hash，该哈希仍然有用：Conductor 会将其与对象
key 一起保存，因此两个投影值相同的对象仍可以分别删除。当前发布端不填写这个
connector 派生字段，而是由已注册的 handler 从 `object_key` 投影哈希。查询只使用
64 位值，所以只要其中任一对象存在，就会报告可能命中。可接受的编码和不一致处理方式见
...
发布端不解析 object key。与 Conductor 配合时必须保持
`kv_events_emit_object_key=true`；订阅端使用注册时的引擎类型选择 handler，
而不是猜测 Store key 的格式。发布端只发送固定 envelope 字段、操作传入的
`group_id` 和原始 `object_key`，connector 派生字段保持为空。

SGLang 的 Mooncake key 包含完整 SHA-256 逻辑哈希，后面可能带有 `_0_k`、
`_0_v`、`_0_temporal` 或 `_0_conv_0` 等物理后缀。Conductor 会把这些后缀
规范化为同一个逻辑哈希节点。SGLang 原生事件使用数组形式的 MessagePack
协议，由 SGLang subscriber path 解码；SGLang 经 Mooncake 发布的事件仍使用
Mooncake map 协议。
...
engine-specific object key 解析位于 Conductor。
