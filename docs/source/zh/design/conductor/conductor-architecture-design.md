...
flowchart LR
    ...
    H --> I
    I --> P["每个 rank 单向查询<br/>GPU 到共享 CPU 再到共享 Disk"]
    P --> R["累计 rank_matches<br/>和实例级汇总"]
```
...
所有计数的单位都是 token，而不是块。Conductor 会为每个选中的已注册 vLLM
引擎返回一项结果，并分别计算其中每个已注册 DP rank。

| 结果字段 | 含义 |
|---|---|
| `dp` | 每个已注册 DP rank 完成 GPU 阶段后的累计边界。rank key 是十进制 JSON 字符串。 |
| `rank_matches` | 与 `dp` 使用完全相同的 rank key，包括零命中的 rank。每个值包含该 rank 的累计 `gpu`、`cpu` 和 `disk` 边界。 |
| `gpu` | 该引擎所有 rank 级 `gpu` 边界中的最大值。Conductor 不会把不同 rank 的块拼成更长的 GPU 前缀。 |
| `cpu` | 每个 rank 从 GPU 继续到共享 CPU 后的累计边界，再取其中最大值。 |
| `disk` | 每个 rank 从 GPU 继续到共享 CPU、再继续到共享 Disk 后的累计边界，再取其中最大值。 |
| `longest_matched` | 与实例级 `disk` 完全相同，即某一个已注册 rank 能按顺序实现的最长前缀。 |

Conductor 为每个 rank 使用一个游标，并执行单向状态机：

1. 在该 rank 的 GPU 上连续读取命中块。
2. 第一个 GPU 未命中的块会立即交给共享 CPU 检查，并从这里继续。
3. 第一个 CPU 未命中的块会立即交给共享 Disk 检查，并从这里继续；Disk 第一次
   未命中就结束该 rank 的结果。

进入较低层后，查询不会回到较高层。同一个块即使位于多个层，也只会由当前最早的
阶段计算一次。CPU 阶段可以为空，让 Disk 检查结束 GPU 阶段的同一个块；但进入
Disk 后，如果某个块在 Disk 未命中，即使后续块只存在于 GPU 或 CPU，也不会再计入。
如果较高层已经覆盖全部输入，后续累计边界会继承完整长度，即使较低层没有副本。
...
实例级 `gpu`、`cpu` 和 `disk` 分别是所有 rank 对应边界的最大值。由于同一缓存
共享范围内的 CPU 和 Disk 对所有 rank 可见，GPU 边界最大的 rank 也能实现实例级
CPU 和 Disk 最大值；Conductor 不会拼接不同 rank 的 GPU 块来制造命中。

例如，`block_size=16` 时，前两块只在 GPU、第三块只在 CPU、第四块只在 Disk，
会得到：
...
这些数字是累计边界，不是缓存层物理归属的证明。单个 rank 在各层新增的长度分别为
`gpu`、`cpu - gpu` 和 `disk - cpu`。如果 GPU 独自覆盖全部 64 个 token，
即使 CPU 和 Disk 没有副本，三个 rank 边界仍都会是 `64`。排查物理位置时应检查
索引中的 owner 状态，而不能从累计查询值反推。
