...
- `MC_ENABLE_DEST_DEVICE_AFFINITY` 启用设备亲和性以优化 RDMA 性能。启用后，Transfer Engine 将优先选择和本地网卡同名的远端网卡进行通信，以减少 QP 数量并改善 Rail-optimized 拓扑中的网络性能。同名远端网卡不存在时会回退到其他设备。默认值为 false
- `MC_ENABLE_STRICT_DEST_DEVICE_AFFINITY` 非全互联（rail-isolated）拓扑的可选严格模式：逐片选择本地网卡、远端必须同名且不回退，并把 `preferred_hca` 与 `avail_hca` 一起轮询。默认关闭。不能与 `MC_ENABLE_HCA_PEER_AFFINITY` 同时开启。
