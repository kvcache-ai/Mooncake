...
| NVIDIA NCCL DeviceTransport | Automatic for fresh CUDA EP builds, or `-DUSE_NCCL_DEVICE=ON` | Install NCCL 2.30.4+ with `nccl_device.h`. Requires CUDA. | Set `NCCL_ROOT` when NCCL is outside the standard search paths. Automatic configuration falls back to IPC + IBGDA if NCCL is unavailable; explicit `ON` makes it required. NCCL Device API device code must be rebuilt or re-JITed with headers that exactly match the runtime `libnccl`. |
...
| `-DUSE_NCCL_DEVICE=ON/OFF` | `ON` for fresh CUDA EP builds; otherwise `OFF` | Enable the NCCL DeviceTransport backend. CUDA EP configuration falls back to `OFF` when NCCL 2.30.4+ with `nccl_device.h` is not found; passing `ON` explicitly makes the dependency required. AOT- and JIT-compiled NCCL device code must use headers that exactly match the runtime `libnccl`. |
