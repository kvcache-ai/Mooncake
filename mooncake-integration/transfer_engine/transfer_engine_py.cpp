...
std::string buildConnString(const std::string &metadata_type, const std::string &metadata_server) {
...
int TransferEnginePy::initialize(const char *local_hostname, const char *metadata_server, const char *protocol,
                                 const char *device_name) {
	auto conn_string = parseConnectionString(metadata_server);
	return initializeExt(local_hostname, conn_string.second.c_str(), protocol, device_name, conn_string.first.c_str());
}

int TransferEnginePy::initializeExt(const char *local_hostname, const char *metadata_server, const char *protocol,
	...
	if (getenv("MC_LEGACY_RPC_PORT_BINDING")) {
		...
		int ret = engine_->init(conn_string, local_hostname, hostname_port.first.c_str(), hostname_port.second);
		if (ret)
			return -1;
	} else {
...
int TransferEnginePy::doBuddyAllocate(int class_id) {
	if (class_id == kMaxClassId) {
		...
		for (size_t offset = 0; offset < kDefaultBufferCapacity; offset += 1024ull * kSlabSizeKB[kMaxClassId])
...
uintptr_t TransferEnginePy::allocateManagedBuffer(size_t length) {
	...
	if (class_id < 0) {
		...
		if (buffer)
			large_buffer_list_.insert(buffer);
...
int TransferEnginePy::transferSyncWrite(const char *target_hostname, uintptr_t buffer, uintptr_t peer_buffer_address,
                                        size_t length) {
	return transferSync(target_hostname, buffer, peer_buffer_address, length, TransferOpcode::WRITE);
}

int TransferEnginePy::transferSyncRead(const char *target_hostname, uintptr_t buffer, uintptr_t peer_buffer_address,
                                       size_t length) {
	return transferSync(target_hostname, buffer, peer_buffer_address, length, TransferOpcode::READ);
}

int TransferEnginePy::batchTransferSyncWrite(const char *target_hostname, std::vector<uintptr_t> buffers,
                                             std::vector<uintptr_t> peer_buffer_addresses,
                                             std::vector<size_t> lengths) {
	return batchTransferSync(target_hostname, buffers, peer_buffer_addresses, lengths, TransferOpcode::WRITE);
}

int TransferEnginePy::batchTransferSyncRead(const char *target_hostname, std::vector<uintptr_t> buffers,
                                            std::vector<uintptr_t> peer_buffer_addresses, std::vector<size_t> lengths) {
	return batchTransferSync(target_hostname, buffers, peer_buffer_addresses, lengths, TransferOpcode::READ);
}

batch_id_t TransferEnginePy::batchTransferAsyncWrite(const char *target_hostname, const std::vector<uintptr_t> &buffers,
                                                     const std::vector<uintptr_t> &peer_buffer_addresses,
                                                     const std::vector<size_t> &lengths) {
	return batchTransferAsync(target_hostname, buffers, peer_buffer_addresses, lengths, TransferOpcode::WRITE);
}

batch_id_t TransferEnginePy::batchTransferAsyncRead(const char *target_hostname, const std::vector<uintptr_t> &buffers,
                                                    const std::vector<uintptr_t> &peer_buffer_addresses,
                                                    const std::vector<size_t> &lengths) {
	return batchTransferAsync(target_hostname, buffers, peer_buffer_addresses, lengths, TransferOpcode::READ);
}

int TransferEnginePy::transferSync(const char *target_hostname, uintptr_t buffer, uintptr_t peer_buffer_address,
                                   size_t length, TransferOpcode opcode) {
	...
	for (int retry = 0; retry < max_retry; ++retry) {
		...
		while (!completed) {
			...
			if (current_ts - start_ts > timeout) {
				LOG(INFO) << "Sync data transfer timeout after " << current_ts - start_ts << "ns, local buffer "
				          << (void *)buffer << " remote buffer " << (void *)peer_buffer_address << " length " << length;
				return -1;
			}
		}
	}
...
int TransferEnginePy::batchTransferSync(const char *target_hostname, std::vector<uintptr_t> buffers,
                                        std::vector<uintptr_t> peer_buffer_addresses, std::vector<size_t> lengths,
	...
	for (int retry = 0; retry < max_retry; ++retry) {
		...
		while (!completed) {
			...
			if (current_ts - start_ts > timeout) {
				LOG(INFO) << "Sync batch data transfer timeout after " << current_ts - start_ts << "ns";
...
batch_id_t TransferEnginePy::batchTransferAsync(const char *target_hostname, const std::vector<uintptr_t> &buffers,
                                                const std::vector<uintptr_t> &peer_buffer_addresses,
	...
	while (!timeout_table.empty() && !failed_or_timeout) {
		for (auto &entry : timeout_table) {
			...
			if (current_ts - batch_desc->start_timestamp > entry.second) {
				LOG(INFO) << "Sync batch data transfer timeout after " << current_ts - batch_desc->start_timestamp
				          << "ns";
				failed_or_timeout = true;
			}
		}
...
batch_id_t TransferEnginePy::transferSubmitWrite(const char *target_hostname, uintptr_t buffer,
...
int TransferEnginePy::batchRegisterMemory(std::vector<uintptr_t> buffer_addresses, std::vector<size_t> capacities) {
	pybind11::gil_scoped_release release;
...
uintptr_t TransferEnginePy::getFirstBufferAddress(const std::string &segment_name) {
...
PYBIND11_MODULE(engine, m) {
	py::enum_<TransferEnginePy::TransferOpcode> transfer_opcode(m, "TransferOpcode", py::arithmetic());
	transfer_opcode.value("Read", TransferEnginePy::TransferOpcode::READ)
	    .value("Write", TransferEnginePy::TransferOpcode::WRITE)
	    .export_values();

	auto adaptor_cls = py::class_<TransferEnginePy>(m, "TransferEngine")
	                       .def(py::init<>())
	                       ...
	                       .def("get_first_buffer_address", &TransferEnginePy::getFirstBufferAddress);
