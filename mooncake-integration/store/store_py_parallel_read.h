...
std::optional<TensorIntoPlan> build_full_tensor_into_plan_from_sources(
    ...
    for (int i = static_cast<int>(global_shape.size()) - 2; i >= 0; --i) {
        global_strides[i] =
            global_strides[i + 1] * static_cast<size_t>(global_shape[i + 1]);
    }
    ...
    for (size_t i = 0; i < sources.size(); ++i) {
        ...
        for (int64_t slice_idx = 0; slice_idx < elements_before; ++slice_idx) {
            ...
            const size_t copy_elements = static_cast<size_t>(shard_size) *
...
load_parallelism_full_reconstruction_sources(
    ...
    for (int shard_rank = 0; shard_rank < request_tp_axis->size; ++shard_rank) {
        ...
        auto metadata = get_tensor_metadata(shard_key);
        std::string resolved_key = shard_key;
        if (!metadata.has_value() &&
            uses_legacy_tp_storage_key(*canonical_parallelism)) {
            resolved_key =
                resolve_tp_read_key(key, shard_rank, request_tp_axis->size);
            metadata = get_tensor_metadata(resolved_key);
        }
        if (!metadata.has_value()) {
            LOG(ERROR) << context
        ...
        if (!stored_parallelism.has_value()) {
            if (!uses_legacy_tp_storage_key(*canonical_parallelism) ||
                !is_shard_tensor_metadata(metadata->metadata)) {
                LOG(ERROR) << context
            ...
            const LayoutAxis *stored_tp_axis =
                find_layout_axis(metadata->metadata, LayoutAxisKind::TP);
            if (!stored_tp_axis || stored_tp_axis->shard_rank != shard_rank ||
                stored_tp_axis->shard_count != request_tp_axis->size ||
                stored_tp_axis->split_dim !=
                    request_tp_axis->split_dim.value_or(0)) {
                LOG(ERROR) << context
                           << ": shard metadata mismatch for TP rank "
                           << shard_rank;
                return std::nullopt;
            }
        } else if (!parallelism_specs_equal_by_kind(
        ...
        reconstruction.sources.push_back(
            ReconstructedShardSource{resolved_key, *metadata});
    }
...
pybind11::object get_tensor_with_parallelism(
    ...
    if (parsed_target->mode == ReadTargetMode::SHARD) {
        auto resolved = resolve_parallelism_shard_read(
            ...
            key, *parallelism);
        if (!resolved.has_value() && uses_legacy_tp_storage_key(*parallelism)) {
            const auto *tp_axis =
                find_axis_spec_by_kind(*parallelism, LayoutAxisKind::TP);
            if (tp_axis) {
                return get_tensor(
...
pybind11::object get_tensor_with_parallelism_into(
    ...
    return results.empty() ? py::none() : results[0];
}
