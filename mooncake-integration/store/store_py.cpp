...
//     std::chrono::microseconds query_time{0};
//     std::chrono::microseconds prepare_time{0};
...
//     query_time = std::chrono::duration_cast<std::chrono::microseconds>(query_end - query_start);
...
//     prepare_time = std::chrono::duration_cast<std::chrono::microseconds>(prepare_end - prepare_start);
...
//     transfer_time = std::chrono::duration_cast<std::chrono::microseconds>(transfer_end - transfer_start);
...
//     LOG(INFO) << "batch_get_into time analysis:"
//               << "\n  Total time: " << total_time.count() << "μs"
//               << "\n  Query time: " << query_time.count() << "μs (" 
//               << (query_time.count() * 100 / total_time.count()) << "%)"
//               << "\n  Prepare time: " << prepare_time.count() << "μs (" 
//               << (prepare_time.count() * 100 / total_time.count()) << "%)"
//               << "\n  Transfer time: " << transfer_time.count() << "μs (" 
//               << (transfer_time.count() * 100 / total_time.count()) << "%)"
//               << "\n  Other overhead: " << (total_time - query_time - prepare_time - transfer_time).count() 
//               << "μs (" << ((total_time - query_time - prepare_time - transfer_time).count() * 100 / total_time.count()) << "%)";
