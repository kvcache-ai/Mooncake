...
void ResourceTracker::cleanupAllResources() {
	...
	for (void *instance : instances_) {
		DistributedObjectStore *store = static_cast<DistributedObjectStore *>(instance);
		if (store) {
...
int DistributedObjectStore::setup(const std::string &local_hostname, const std::string &metadata_server,
                                  size_t global_segment_size, size_t local_buffer_size, const std::string &protocol,
	...
	auto client_opt =
	    mooncake::Client::Create(this->local_hostname, metadata_server, protocol, args, master_server_addr);
	if (!client_opt) {
	...
	client_buffer_allocator_ = std::make_unique<SimpleAllocator>(local_buffer_size);
	auto result = client_->RegisterLocalMemory(client_buffer_allocator_->getBase(), local_buffer_size,
	                                           kWildcardLocation, false, true);
	if (!result.has_value()) {
		LOG(ERROR) << "Failed to register local memory: " << toString(result.error());
		return 1;
	}
	...
	auto mount_result = client_->MountSegment(segment_ptr_.get(), global_segment_size);
...
int DistributedObjectStore::initAll(const std::string &protocol_, const std::string &device_name,
	...
	return setup("localhost:12345", "127.0.0.1:2379", mount_segment_size, buffer_allocator_size, protocol_,
	             device_name);
}

int DistributedObjectStore::allocateSlices(std::vector<Slice> &slices, size_t length) {
	uint64_t offset = 0;
	while (offset < length) {
...
int DistributedObjectStore::allocateSlices(std::vector<Slice> &slices, const std::string &value) {
	uint64_t offset = 0;
...
int DistributedObjectStore::allocateSlices(std::vector<Slice> &slices, std::span<const char> value) {
	uint64_t offset = 0;
	while (offset < value.size()) {
		auto chunk_size = std::min(value.size() - offset, kMaxSliceSize);
		auto ptr = client_buffer_allocator_->allocate(chunk_size);
...
int DistributedObjectStore::allocateSlicesPacked(std::vector<mooncake::Slice> &slices,
...
int DistributedObjectStore::allocateSlices(std::vector<mooncake::Slice> &slices,
...
int DistributedObjectStore::allocateBatchedSlices(
    const std::vector<std::string> &keys, std::unordered_map<std::string, std::vector<mooncake::Slice>> &batched_slices,
    const std::vector<std::vector<mooncake::Replica::Descriptor>> &replica_lists,
    std::unordered_map<std::string, uint64_t> &str_length_map) {
	if (replica_lists.empty())
		return -1;
	if (keys.size() != replica_lists.size()) {
		LOG(ERROR) << "Keys size (" << keys.size() << ") doesn't match replica lists size (" << replica_lists.size()
		           << ")";
		return 1;
	}
...
char *DistributedObjectStore::exportSlices(const std::vector<mooncake::Slice> &slices, uint64_t length) {
...
int DistributedObjectStore::freeSlices(const std::vector<mooncake::Slice> &slices) {
	for (auto slice : slices) {
...
int DistributedObjectStore::put(const std::string &key, std::span<const char> value, const ReplicateConfig &config) {
	if (!client_) {
		LOG(ERROR) << "Client is not initialized";
		return 1;
	}
	...
	if (ret) {
		LOG(ERROR) << "Failed to allocate slices for put operation, key: " << key << ", value size: " << value.size();
		return ret;
	}

	auto put_result = client_->Put(key, slices.slices(), config);
	if (!put_result) {
		LOG(ERROR) << "Put operation failed with error: " << toString(put_result.error());
...
int DistributedObjectStore::put_batch(const std::vector<std::string> &keys,
                                      const std::vector<std::span<const char>> &values, const ReplicateConfig &config) {
	if (!client_) {
		LOG(ERROR) << "Client is not initialized";
		return 1;
	}
	...
	for (size_t i = 0; i < keys.size(); ++i) {
		...
		if (ret) {
			LOG(ERROR) << "Failed to allocate slices for put_batch operation, key: " << key
...
int DistributedObjectStore::put_parts(const std::string &key, std::vector<std::span<const char>> values,
	...
	if (ret) {
		LOG(ERROR) << "Failed to allocate slices for put operation, key: " << key << ", values size: " << values.size();
		return ret;
	}
...
pybind11::bytes DistributedObjectStore::get(const std::string &key) {
...

	{
		...
		if (guard.slices().size() == 1 && guard.slices()[0].size == str_length) {
		} else {
	...
	if (use_exported_str) {
		...
		result = pybind11::bytes(static_cast<char *>(guard.slices()[0].ptr), str_length);
	} else {
		result = kNullString;
	}
...
std::vector<pybind11::bytes> DistributedObjectStore::get_batch(const std::vector<std::string> &keys) {
	...
	for (const auto &key : keys) {
		if (!seen.insert(key).second) {
			LOG(ERROR) << "Duplicate key not supported for Batch API, key: " << key;
			py::gil_scoped_acquire acquire_gil;
			return {kNullString};
		}
	}
	...
	std::unordered_map<std::string, std::vector<mooncake::Slice>> batched_slices;
	batched_slices.reserve(keys.size());
	std::unordered_map<std::string, uint64_t> str_length_map;
	{
		py::gil_scoped_release release_gil;
		...
		for (size_t i = 0; i < query_results.size(); ++i) {
			if (!query_results[i]) {
				py::gil_scoped_acquire acquire_gil;
				LOG(ERROR) << "Query failed for key '" << keys[i] << "': " << toString(query_results[i].error());
				return {kNullString};
			}
			replica_lists.emplace_back(query_results[i].value());
		}

		int ret = allocateBatchedSlices(keys, batched_slices, replica_lists, str_length_map);
		if (ret) {
		...
		auto get_results = client_->BatchGet(keys, replica_lists, batched_slices);
		for (size_t i = 0; i < get_results.size(); ++i) {
			if (!get_results[i]) {
				...
				py::gil_scoped_acquire acquire_gil;
				LOG(ERROR) << "BatchGet failed for key '" << keys[i] << "': " << toString(get_results[i].error());
				return {kNullString};
			}
		}
		...
		std::vector<pybind11::bytes> results;
		for (const auto &key : keys) {
			if (batched_slices[key].size() == 1 && batched_slices[key][0].size == str_length_map[key]) {
				results.push_back(
				    pybind11::bytes(static_cast<char *>(batched_slices[key][0].ptr), str_length_map[key]));
			} else {
				char *exported_str_ptr = exportSlices(batched_slices[key], str_length_map[key]);
				if (!exported_str_ptr) {
					for (auto &slice : batched_slices) {
						freeSlices(slice.second);
					}
					...
					results.push_back(pybind11::bytes(exported_str_ptr, str_length_map[key]));
					delete[] exported_str_ptr;
				}
			}
		}
...
int DistributedObjectStore::remove(const std::string &key) {
	...
	if (!remove_result)
		return toInt(remove_result.error());
	return 0;
}
...
SliceBuffer::SliceBuffer(DistributedObjectStore &store, void *buffer, uint64_t size, bool use_allocator_free)
...
std::shared_ptr<SliceBuffer> DistributedObjectStore::get_buffer(const std::string &key) {
	...
	if (!query_result) {
		...
		LOG(ERROR) << "Query failed for key: " << key << " with error: " << toString(query_result.error());
		return nullptr;
	}
	...
	auto get_result = client_->Get(key, replica_list, guard.slices());
	if (!get_result) {
		LOG(ERROR) << "Get failed for key: " << key << " with error: " << toString(get_result.error());
		return nullptr;
	}
	...
	if (guard.slices().size() == 1) {
		...
		result = std::make_shared<SliceBuffer>(*this, contiguous_buffer, total_length, false);
	}
...
int DistributedObjectStore::register_buffer(void *buffer, size_t size) {
	...
	auto register_result = client_->RegisterLocalMemory(buffer, size, kWildcardLocation, false, true);
	if (!register_result) {
		LOG(ERROR) << "Register buffer failed with error: " << toString(register_result.error());
...
int DistributedObjectStore::unregister_buffer(void *buffer) {
	...
	if (!unregister_result) {
		LOG(ERROR) << "Unregister buffer failed with error: " << toString(unregister_result.error());
...
int DistributedObjectStore::get_into(const std::string &key, void *buffer, size_t size) {
	...
	if (size < total_size) {
		LOG(ERROR) << "User buffer too small. Required: " << total_size << ", provided: " << size;
		return -1;
	}
...
std::vector<int> DistributedObjectStore::batch_put_from(const std::vector<std::string> &keys,
	...
	auto batch_put_results = client_->BatchPut(keys, ordered_batched_slices, config);
	...
	for (size_t i = 0; i < batch_put_results.size(); ++i) {
		if (!batch_put_results[i]) {
			LOG(ERROR) << "BatchPut operation failed for key '" << keys[i]
			           << "' with error: " << toString(batch_put_results[i].error());
...
std::vector<int> DistributedObjectStore::batch_get_into(const std::vector<std::string> &keys,
                                                        const std::vector<void *> &buffers,
                                                        const std::vector<size_t> &sizes) {
	...
	if (keys.size() != buffers.size() || keys.size() != sizes.size()) {
		LOG(ERROR) << "Input vector sizes mismatch: keys=" << keys.size() << ", buffers=" << buffers.size()
		           << ", sizes=" << sizes.size();
		return std::vector<int>(keys.size(), -1);
	}
	...
	for (size_t i = 0; i < num_keys; ++i) {
		...
		if (!query_results[i]) {
			...
			results[i] = (error == ErrorCode::OBJECT_NOT_FOUND) ? -toInt(ErrorCode::OBJECT_NOT_FOUND) : -toInt(error);

			if (error != ErrorCode::OBJECT_NOT_FOUND) {
				LOG(ERROR) << "Query failed for key '" << key << "': " << toString(error);
			}
			continue;
		}
		...
		if (sizes[i] < total_size) {
			LOG(ERROR) << "Buffer too small for key '" << key << "': required=" << total_size
			           << ", available=" << sizes[i];
			results[i] = -1;
			continue;
		}
	...
	const auto batch_get_results = client_->BatchGet(batch_keys, batch_replica_lists, batch_slices);
...
int DistributedObjectStore::put_from(const std::string &key, void *buffer, size_t size, const ReplicateConfig &config) {
	// NOTE: The buffer address must be previously registered with
	// register_buffer() for zero-copy RDMA operations to work correctly
	if (!client_) {
		LOG(ERROR) << "Client is not initialized";
...
py::array create_typed_array(char *exported_data, size_t total_length) {
	...
	return py::array_t<T>({static_cast<ssize_t>(total_length / sizeof(T))}, (T *)exported_data, free_when_done);
}
...
PYBIND11_MODULE(store, m) {
	...
	py::class_<SliceBuffer, std::shared_ptr<SliceBuffer>>(m, "SliceBuffer", py::buffer_protocol())
	    ...
	    .def_buffer([](SliceBuffer &self) -> py::buffer_info {
		    ...
		    if (self.size() > 0) {
			    return py::buffer_info(self.ptr(),                            /* Pointer to buffer */
			    ...
			    // Empty buffer
			    return py::buffer_info(nullptr,                               /* Pointer to buffer */
			                           sizeof(char),                          /* Size of one scalar */
			                           py::format_descriptor<char>::format(), /* Python struct-style
			                                                                     format descriptor */
			                           1,                                     /* Number of dimensions */
	...
	py::class_<DistributedObjectStore>(m, "MooncakeDistributedStore")
	    ...
	    .def("get_buffer", &DistributedObjectStore::get_buffer, py::call_guard<py::gil_scoped_release>(),
	         py::return_value_policy::take_ownership)
	    .def("remove", &DistributedObjectStore::remove, py::call_guard<py::gil_scoped_release>())
	    .def("remove_all", &DistributedObjectStore::removeAll, py::call_guard<py::gil_scoped_release>())
	    .def("is_exist", &DistributedObjectStore::isExist, py::call_guard<py::gil_scoped_release>())
	    .def("batch_is_exist", &DistributedObjectStore::batchIsExist, py::call_guard<py::gil_scoped_release>(),
	         py::arg("keys"),
	         "Check if multiple objects exist. Returns list of results: 1 if "
	         "exists, 0 if not exists, -1 if error")
	    .def("close", &DistributedObjectStore::tearDownAll)
	    .def("get_size", &DistributedObjectStore::getSize, py::call_guard<py::gil_scoped_release>())
	    .def("get_tensor", &DistributedObjectStore::get_tensor, py::arg("key"), py::arg("dtype"),
	         "Get a PyTorch tensor from the store")
	    .def("put_tensor", &DistributedObjectStore::put_tensor, py::arg("key"), py::arg("tensor"),
	         "Put a PyTorch tensor into the store")
	    .def(
	        "register_buffer",
	        [](DistributedObjectStore &self, uintptr_t buffer_ptr, size_t size) {
	    ...
	    .def(
	        "get_into",
	        [](DistributedObjectStore &self, const std::string &key, uintptr_t buffer_ptr, size_t size) {
	    ...
	    .def(
	        "batch_get_into",
	        [](DistributedObjectStore &self, const std::vector<std::string> &keys,
	    ...
	    .def(
	        ...
	        py::arg("key"), py::arg("buffer_ptr"), py::arg("size"), py::arg("config") = ReplicateConfig {},
	    ...
	    .def(
	        ...
	        py::arg("keys"), py::arg("buffer_ptrs"), py::arg("sizes"), py::arg("config") = ReplicateConfig {},
	    ...
	    .def(
	        ...
	        [](DistributedObjectStore &self, const std::string &key, py::buffer buf,
	           const ReplicateConfig &config = ReplicateConfig {}) {
		        ...
		        py::gil_scoped_release release;
		        return self.put(
		            key, std::span<const char>(static_cast<char *>(info.ptr), static_cast<size_t>(info.size)), config);
	        },
	        ...
	        [](DistributedObjectStore &self, const std::string &key, py::args parts,
	           const ReplicateConfig &config = ReplicateConfig {}) {
		        ...
		        for (auto &obj : parts) {
			        ...
			        spans.emplace_back(static_cast<const char *>(info.ptr), static_cast<size_t>(info.size));
		        }
	    ...
	    .def(
	        "put_batch",
	        [](DistributedObjectStore &self, const std::vector<std::string> &keys,
	           const std::vector<py::bytes> &py_values, const ReplicateConfig &config = ReplicateConfig {}) {
	        ...
	        py::arg("keys"), py::arg("values"), py::arg("config") = ReplicateConfig {})
