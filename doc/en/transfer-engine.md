...
- `MC_DISABLE_REUSEADDR` Force the TCP daemon not to enable the REUSEADDR option. This avoids rare cases where two processes use the same port. However, it also prevents reusing the port immediately after shutting down previous connections.
