// mooncake_worker_kernels.cu — GPU kernel functions and launch wrappers.
// Compiled by nvcc (CUDA, via mooncake_worker.cu) and mcc (MUSA, via
// mooncake_worker.mu). The __MUSA__ branch avoids torch headers to stay
// compatible with mcc.
